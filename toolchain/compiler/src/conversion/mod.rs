...
fn convert_instruction<F: PrimeField32, EF: ExtensionField<F>>(
    ...
    let instructions = match instruction {
        ...
        AsmInstruction::Bne(label, lhs, rhs) => vec![
            ...
            inst(
                options.opcode_with_offset(BranchEqualOpcode::BNE),
                i32_f(lhs),
                i32_f(rhs),
                labels(label) - pc,
                AS::Memory,
        ...
        AsmInstruction::Beq(label, lhs, rhs) => vec![
            // if mem[lhs] == mem[rhs], pc <- labels[label]
            inst(
                options.opcode_with_offset(BranchEqualOpcode::BEQ),
                i32_f(lhs),
                i32_f(rhs),
                labels(label) - pc,
                AS::Memory,
        ...
        AsmInstruction::BeqE(label, lhs, rhs) => (0..EF::D)
            ...
            // if mem[lhs + i] == mem[rhs + i] for i = 0..4, pc <- labels[label]
            inst(
                if i == 0 { options.opcode_with_offset(BranchEqualOpcode::BEQ) } else { options.opcode_with_offset(BranchEqualOpcode::BNE) },
                i32_f(lhs + (i as i32)),
                i32_f(rhs + (i as i32)),
                if i == 0 {
