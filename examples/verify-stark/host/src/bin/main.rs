...
struct KeygenCmd {
    ...
    /// [output] cached commit used to generate verify-stark guest inputs.
    #[arg(long, required = true)]
    verify_stark_cached_commit: PathBuf,
}

#[derive(Parser)]
struct BuildCmd {
    /// [input] cached SDK proving key from the keygen command.
    #[arg(long, required = true)]
    sdk_pk: PathBuf,

    /// [output] compiled verify-stark VM executable.
    #[arg(long, required = true)]
    vmexe: PathBuf,

    /// [output] baseline metadata for verifying proofs of the verify-stark guest.
    #[arg(long, required = true)]
    baseline: PathBuf,
}
...
struct ProveCmd {
    ...
    /// [input] compiled verify-stark VM executable from the build command.
    #[arg(long, required = true)]
    vmexe: PathBuf,
    ...
    /// [input] baseline metadata for the child proof being verified.
    #[arg(long, required = true)]
    child_baseline: PathBuf,

    /// [input] cached commit used to generate verify-stark guest inputs.
    #[arg(long, required = true)]
    verify_stark_cached_commit: PathBuf,

    /// [input] child STARK proof to verify inside the verify-stark guest.
    #[arg(long, required = true)]
    input_proof: PathBuf,

    /// [output] proof generated by the verify-stark guest.
    #[arg(long, required = true)]
...
fn main() -> Result<()> {
    match Cli::parse().command {
        Command::Keygen(cmd) => {
            ...
            let (cached_pk, vm_config, agg_vk, verify_stark_cached_commit) = keygen(child_agg_vk)?;
            ...
            write_to_file_json(cmd.verify_stark_cached_commit, verify_stark_cached_commit)?;
        ...
        Command::Prove(cmd) => {
            ...
            let verify_stark_cached_commit: CommitBytes =
                read_from_file_json(cmd.verify_stark_cached_commit)?;
