...
//! used when `getrandom` is never called by pulled in as a dependency unavoidably. If no feature is
//! enabled, then no custom implementation is registered, and the user must supply their own as
