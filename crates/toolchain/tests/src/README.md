...
Guest builds in these tests go through `openvm-build`'s `build_guest_package`, which invokes `cargo build` under the prebuilt `openvm-1.97.1` toolchain for the `riscv64im-unknown-openvm-elf` target. Install the toolchain first:
