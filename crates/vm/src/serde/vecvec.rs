...
pub fn load(data: &[u8]) -> Result<Vec<Vec<u8>>, SerdeError> {
    ...
    let mut result = Vec::with_capacity(num_vecs);
    let mut internal_vecs: Vec<u8> = vec![0; sum_len];
    ...
    unsafe {
        for i in 0..num_vecs {
            ...
            offset += 8;
            println!(
                "in loop i = {}, internal_vec_count = {}, len = {}",
                i, internal_vec_count, len
            );

            // let inner_vec = data[offset..offset + len].to_vec();
            println!(
                "vec at ptr: {:p}",
                internal_vecs.as_mut_ptr().add(internal_vec_count)
            );
            let mut inner_vec =
