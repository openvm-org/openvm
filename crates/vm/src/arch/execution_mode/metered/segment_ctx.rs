...
impl SegmentationCtx {
    ...
    fn emit_metered_bus_metrics(&self, segment: &str, trace_heights: &[u32]) {
        ...
        for cells in self.calculate_bus_interaction_cells(trace_heights) {
            ...
            // This is the same interaction-buffer estimate used by AIR metering. The shared fixed
            // overhead remains in `metered_interaction_memory_overhead_bytes` at segment scope.
            let unpadded_memory =
                memory_config.interaction_memory_bytes_without_overhead(cells.unpadded);
            let total_memory = memory_config.interaction_memory_bytes_without_overhead(total_cells);
