    ...
    [u8]: CustomBorrow<'a, T, I>,
{
    ...
    let record_buffer = &mut *slice_from_raw_parts_mut(
...
impl<AS, I> Default for AdapterCoreLayout<AS, I>
where
    I: Default,
{
    fn default() -> Self {
...
where
    [u8]: BorrowMut<Record>,
{
    fn width_bytes(&self, _layout: &()) -> usize {
        size_of::<Record>()
    }
}
...
{
    fn width_bytes(&self, _layout: &AdapterCoreLayout<AS, I>) -> usize {
        AS::WIDTH * size_of::<F>()
    }
}
    ...
    AIR: BaseAir<F>,
{
    pub fn new(air: AIR, step: STEP, height: usize, mem_helper: SharedMemoryHelper<F>) -> Self {
        ...
        let arena = MatrixRecordArena::with_capacity(height, width);
        Self {
            air,
            step,
            arena,
