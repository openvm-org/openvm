...
use rvr_openvm::{
    ...
    G2DsoManifestConfigV1, InlineRecordShape, SuspendPolicy, TracerMode,
};
...
/// version. Inline-record metadata is not recoverable from a bare library, so
...
fn compile_impl<F: PrimeField32>(
    ...
    let lto_env = match opts.tracer_mode {
        TracerMode::Metered => Some("OPENVM_RVR_METERED_LTO"),
        TracerMode::Preflight => Some("OPENVM_RVR_PREFLIGHT_LTO"),
    ...
    let suspend_policy = opts
        .suspend_policy
        .unwrap_or_else(|| opts.tracer_mode.default_suspend_policy());
    match (opts.tracer_mode, suspend_policy) {
    ...
    let inline_records =
        opts.tracer_mode == TracerMode::Preflight && !env_flag_is_off("OPENVM_RVR_INLINE_RECORDS");
    if opts.tracer_mode == TracerMode::Preflight
        && matches!(
    ...
    let chips = match opts.tracer_mode {
        TracerMode::Pure => None,
...
fn generated_project_input_cache_key<F: PrimeField32>(
    ...
    let mut hasher = Sha256::new();
    hasher.update(b"openvm-rvr-generated-project-input-v2\0");
    ...
    hasher.update([match opts.tracer_mode {
        TracerMode::Pure => 0,
        TracerMode::MeteredCost => 1,
        TracerMode::Metered => 2,
        TracerMode::Preflight => 3,
    }]);
    hasher.update([match suspend_policy {
        SuspendPolicy::Disabled => 0,
        SuspendPolicy::InstretLimit => 1,
        SuspendPolicy::SegmentBoundary => 2,
    }]);
    hasher.update([
        disable_lto as u8,
