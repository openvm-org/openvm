...
fn run_preflight_and_finalize<F>(
    compiled: &RvrCompiled,
    extensions: &ExtensionRegistry<F>,
    vm_state: &mut VmState<F, GuestMemory>,
...
pub fn execute_preflight<F: PrimeField32>(
    compiled: &RvrCompiled,
    extensions: &ExtensionRegistry<F>,
    vm_state: &mut VmState<F, GuestMemory>,
...
#[cfg(test)]
pub(crate) fn execute_preflight_for_test<F: PrimeField32>(
    compiled: &RvrCompiled,
    vm_state: &mut VmState<F, GuestMemory>,
    tracer_data: &mut PreflightTracerData,
) -> Result<PreflightState, ExecuteError> {
    let extensions = ExtensionRegistry::new();
    execute_preflight(compiled, &extensions, vm_state, tracer_data, None).map(|result| result.state)
}
