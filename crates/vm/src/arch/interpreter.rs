    ...
    ($span:literal, $interpreter:ident, $exec_state:ident, $ctx:ident) => {{
        ...
        info_span!($span).in_scope(|| -> Result<(), ExecutionError> {
            ...
            #[cfg(feature = "tco")]
            {
                ...
                //   valid VM state
                // - Handler is fn pointer type which can be transmuted to *const ()
                unsafe {
                    handler(
                        $interpreter.pre_compute_ptrs.as_ptr(),
                        std::mem::transmute::<&[Handler<_, _>], &[*const ()]>(
    ...
    Ctx: ExecutionCtxTrait,
{
        ...
        E: Executor<F>,
    {
        ...
        #[cfg(feature = "tco")]
        let pre_compute_ptrs = pre_compute_insns
            .into_iter()
            .map(|insn| insn.pre_compute)
            .collect::<Vec<_>>();
...
unsafe fn terminate_execute_e12_tco_handler<F: PrimeField32, CTX: ExecutionCtxTrait>(
    ...
    let pc_idx = get_pc_index(vm_state.pc);
