...
{    
    ...
    pub fn create_asm(exe: &VmExe<F>) -> String {
        ...
        for (pc, instruction, _) in exe.program.enumerate_by_pc() {
            if instruction.opcode == LessThanOpcode::SLT.global_opcode() || instruction.opcode == LessThanOpcode::SLTU.global_opcode() {
                /*
                SLT_RV32: Set Less Than (signed comparison)
                [a:4]_1 = i32([b:4]_1) < i32([c:4]_e) ? 1 : 0
                */
                ...
                if instruction.opcode == LessThanOpcode::SLT.global_opcode() {
                    asm_opcode += "setl";
                } else if instruction.opcode == LessThanOpcode::SLTU.global_opcode() {
                    asm_opcode += "setb";
                }
