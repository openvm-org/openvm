...
use crate::{
    arch::{
        ...
        AdapterTraceStep, BasicAdapterInterface, EmptyAdapterCoreLayout, ExecutionState,
        InsExecutor, InsExecutorE1, InstructionExecutor, MinimalInstruction, RecordArena, Result,
        Streams, VmAirWrapper, VmCoreAir, VmStateMut,
    },
    system::{
        memory::{
...
pub struct PublicValuesCoreStep<F, AdapterAir, AdapterStep> {
...
impl<F, AdapterAir, AdapterStep> PublicValuesCoreStep<F, AdapterAir, AdapterStep>
where
    F: PrimeField32,
{
...
impl<F, AdapterAir, AdapterStep> InstructionExecutor<F>
    for PublicValuesCoreStep<F, AdapterAir, AdapterStep>
where
    F: PrimeField32,
{
...
where
    ...
    for<'a> RA: RecordArena<
        ...
        (AdapterStep::RecordMut<'a>, &'a mut PublicValuesRecord<F>),
    >,
{
    fn execute_tracegen(
        &mut self,
...
impl<F, AdapterAir, AdapterStep> InsExecutorE1<F>
    for PublicValuesCoreStep<F, AdapterAir, AdapterStep>
where
    F: PrimeField32,
...
impl<F, AdapterAir, AdapterStep> ChipUsageGetter
    for PublicValuesCoreStep<F, AdapterAir, AdapterStep>
where
    F: Field,
...
impl<SC, AdapterAir, AdapterStep> Chip<SC>
    for PublicValuesCoreStep<Val<SC>, AdapterAir, AdapterStep>
where
