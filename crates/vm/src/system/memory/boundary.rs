...
pub struct MemoryBoundaryCols<T, const CHUNK: usize> {
...
///   `merkle_bus`.
#[derive(Clone, Debug)]
pub struct MemoryBoundaryAir<const CHUNK: usize> {
...
impl<const CHUNK: usize, F> BaseAir<F> for MemoryBoundaryAir<CHUNK> {
    fn width(&self) -> usize {
        MemoryBoundaryCols::<F, CHUNK>::width()
    }
}

impl<const CHUNK: usize, F> BaseAirWithPublicValues<F> for MemoryBoundaryAir<CHUNK> {}
impl<const CHUNK: usize, F> PartitionedBaseAir<F> for MemoryBoundaryAir<CHUNK> {}

impl<const CHUNK: usize, AB: InteractionBuilder> Air<AB> for MemoryBoundaryAir<CHUNK> {
    fn eval(&self, builder: &mut AB) {
        let main = builder.main();
        let local = main.row_slice(0).expect("window should have two elements");
        let local: &MemoryBoundaryCols<AB::Var, CHUNK> = (*local).borrow();
...
pub struct MemoryBoundaryChip<F, const CHUNK: usize> {
    pub air: MemoryBoundaryAir<CHUNK>,
...
impl<const CHUNK: usize, F: PrimeField32> MemoryBoundaryChip<F, CHUNK> {
    pub fn new(
...
impl<const CHUNK: usize, RA, SC> Chip<RA, CpuBackend<SC>> for MemoryBoundaryChip<Val<SC>, CHUNK>
where
    ...
    Val<SC>: PrimeField32,
{
    fn generate_proving_ctx(&self, _: RA) -> AirProvingContext<CpuBackend<SC>> {
        let trace = {
            let width = MemoryBoundaryCols::<Val<SC>, CHUNK>::width();
            ...
            rows.par_chunks_mut(2 * width)
                ...
                .for_each(|(row, touched_label)| {
                    ...
                    *initial_row.borrow_mut() = MemoryBoundaryCols {
                    ...
                    *final_row.borrow_mut() = MemoryBoundaryCols {
