...
mod tests {
    ...
    #[test]
    fn sparse_clone_keeps_default_rv64_backing_sparse_at_high_addresses() {
        ...
        let mut sparse = SparseMemoryImage::new();
        sparse.insert((RV64_MEMORY_AS, 0), 11);
        sparse.insert((RV64_MEMORY_AS, u32::MAX), 22);
        memory.set_from_sparse(&sparse);

        let snapshot = memory.sparse_clone();
        let rv64_memory = &snapshot.mem[RV64_MEMORY_AS as usize];
        assert_eq!(rv64_memory.as_slice()[0], 11);
        assert_eq!(rv64_memory.as_slice()[u32::MAX as usize], 22);
        ...
        assert!(
            resident_pages(rv64_memory) < 1024,
            "a sparse two-page snapshot should not materialize the 4-GiB mapping"
        );
    }
