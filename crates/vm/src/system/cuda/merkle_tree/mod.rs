...
mod tests {
    ...
    fn test_cuda_merkle_tree_sparse_pages_cpu_gpu_equivalence() {
        ...
        // Fill RV64_REGISTER_AS fully, plus a low prefix and the final cell of each large address
        // space. The distant page forces selection of SparsePages (asserted below).
        let prefix_cells = 1 << 10;
        let mut initial_memory = GuestMemory::new(AddressMap::from_mem_config(&mem_config));
        for (idx, space) in mem_config.addr_spaces.iter().enumerate() {
            let num_filled = if idx == RV64_REGISTER_AS as usize {
                space.num_cells
            } else {
            ...
            unsafe {
                ...
                if (idx == RV64_MEMORY_AS as usize || idx == DEFERRAL_AS as usize)
                    && space.num_cells != 0
                {
        ...
        for addr_space in [RV64_MEMORY_AS as usize, DEFERRAL_AS as usize] {
            ...
            .flat_map(|(i, cnf)| {
                ...
                for j in 0..num_leaves {
                    let is_last_large = j + 1 == num_leaves
                        && (i == RV64_MEMORY_AS as usize || i == DEFERRAL_AS as usize);
