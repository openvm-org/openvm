...
pub struct VmMetrics {
    ...
    /// Maps (dsl_ir, opcode) to the total amount of time opcode took to execute (nanosecs)
    pub execution_times: BTreeMap<(Option<String>, String), u128>,
    ...
    Executor: InstructionExecutor<F, RA>,
{
    ...
    #[cfg(feature = "perf-metrics")]
    {
        ...
        metrics.update_trace_cells(now_trace_cells, opcode_name, dsl_instr, execution_time);
...
impl VmMetrics {
    ...
    pub fn update_trace_cells(
        ...
        *self.execution_times.entry(key.clone()).or_insert(0) += execution_time;
    ...
    pub fn emit(&self) {
        let mut opcode_stats = BTreeMap::new();
        for ((dsl_ir, opcode), value) in self.counts.iter() {
            let labels = [
                ("dsl_ir", dsl_ir.clone().unwrap_or_else(String::new)),
                ("opcode", opcode.clone()),
            ];
            opcode_stats.entry(opcode.clone()).or_insert((0, 0)).0 += *value as u64;

            counter!("frequency", &labels).absolute(*value as u64);
        }

        for ((dsl_ir, opcode), value) in self.execution_times.iter() {
            let labels = [
                ("dsl_ir", dsl_ir.clone().unwrap_or_else(String::new)),
                ("opcode", opcode.clone()),
            ];
            opcode_stats.entry(opcode.clone()).or_insert((0, 0)).1 += *value as u64;
            counter!("execution_time", &labels).absolute(*value as u64);
        }
        for (opcode, (value, value2)) in opcode_stats {
            println!("{opcode}\t{value}\t{value2}");
        }
