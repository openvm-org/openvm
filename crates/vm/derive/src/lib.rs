...
pub fn ins_executor_derive(input: TokenStream) -> TokenStream {
    ...
    match &ast.data {
        Data::Struct(inner) => {
            ...
            quote! {
                impl<RA> #impl_generics ::openvm_circuit::arch::InsExecutor<F, RA> for #name #ty_generics #where_clause {
                    fn execute_tracegen(
                        &mut self,
        ...
        Data::Enum(e) => {
            ...
            // Add where clause for each variant field type
            for (_, field) in &variants {
                let field_ty = &field.ty;
                where_clause
                    .predicates
                    .push(syn::parse_quote! { #field_ty: ::openvm_circuit::arch::InsExecutor<#first_ty_generic, RA> });
            }
            ...
            quote! {
                impl #new_impl_generics ::openvm_circuit::arch::InsExecutor<#first_ty_generic, RA> for #name #ty_generics #new_where_clause {
                    fn execute_tracegen(
                        &mut self,
    ...
    match &ast.data {
        syn::Data::Struct(inner) => {
            ...
            TokenStream::from(quote! {
                #[derive(::openvm_circuit::circuit_derive::ChipUsageGetter, ::openvm_circuit::circuit_derive::Chip, ::openvm_circuit::derive::InstructionExecutor, ::openvm_circuit::derive::InsExecutorE1, ::openvm_circuit::derive::InsExecutor, ::derive_more::derive::From, ::openvm_circuit::derive::AnyEnum)]
