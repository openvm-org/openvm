...
impl<F: VmField> VmFieldExecutionConfig<F> for SdkVmConfig
where
    SdkVmConfigInner: VmFieldExecutionConfig<F>,
{
    type Executor = <SdkVmConfigInner as VmFieldExecutionConfig<F>>::Executor;

    fn create_field_executors(
        &self,
    ) -> Result<ExecutorInventory<Self::Executor>, ExecutorInventoryError> {
        self.to_inner().create_field_executors()
    }

    #[cfg(feature = "rvr")]
    fn create_field_rvr_extensions(&self, air_idx: Option<&[usize]>) -> RvrExtensions {
        self.to_inner().create_field_rvr_extensions(air_idx)
    }
}
