...
namespace program {
// Byte program counters are represented in-circuit as two little-endian u16 limbs.
inline constexpr size_t PC_BITS = 32;
inline constexpr size_t PC_LIMB_BITS = openvm::U16_BITS;
inline constexpr size_t PC_LIMBS = 2;
...
__device__ __host__ inline constexpr uint32_t pc_lo(uint32_t pc) { return pc & UINT16_MAX; }
__device__ __host__ inline constexpr uint32_t pc_hi(uint32_t pc) { return pc >> PC_LIMB_BITS; }
} // namespace program
