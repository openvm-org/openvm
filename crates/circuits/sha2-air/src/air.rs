...
impl<C: Sha2BlockHasherSubairConfig> Sha2BlockHasherSubAir<C> {
    ...
    fn eval_work_vars<'a, AB: InteractionBuilder>(
        ...
        for i in 0..C::ROUNDS_PER_ROW {
            ...
            let row_idx = self.row_idx_encoder.flag_with_val::<AB>(
                next.flags.row_idx.to_slice().unwrap(),
                &(0..=C::ROWS_PER_BLOCK).map(|i| (i, i)).collect::<Vec<_>>(),
            );
            let row_idx = format!("{:?}", row_idx);
            if row_idx == "0" {
                println!("row {} w_limbs: {:?}", row_idx, w_limbs);
                println!("row {} k_limbs: {:?}", row_idx, k_limbs);
            }
            ...
            constraint_word_addition::<_, C>(
                ...
                &[
                    ...
                    &big_sig0_field::<AB::Expr, C>(a.row(i + 3).as_slice().unwrap()), /* sig_0 of
                                                                                      previous `a` */
                    &maj_field::<AB::Expr>(
