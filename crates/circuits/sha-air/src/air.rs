...
impl<C: ShaConfig + ShaPrecomputedValues<C::Word>> ShaAir<C> {
    ...
    fn eval_message_schedule<'a, AB: InteractionBuilder>(
        ...
        for i in 0..C::ROUNDS_PER_ROW {
            ...
            // We would like to constrain this only on rows 4..C::ROUND_ROWS, but we can't do a conditional check because the degree of sum is already 3
            // So we must fill in `intermed_12` with dummy values on rows 0..3 and C::ROUND_ROWS-1 and C::ROUND_ROWS to ensure the constraint holds on rows
            // 0..4 and C::ROUND_ROWS. Note that the dummy value goes in the previous row to make the current row's constraint hold.
