...
impl<C: ShaConfig + ShaPrecomputedValues<C::Word>> ShaAir<C> {
    ...
    pub fn generate_missing_cells<F: PrimeField32>(
        ...
        let (last_message_row, rows) = rows.split_at_mut(trace_width);
        let (padding_row, next_block_first_row) = rows.split_at_mut(trace_width);
        let mut last_message_row: ShaRoundColsRefMut<F> = ShaRoundColsRefMut::from::<C>(
            &mut last_message_row[trace_start_col..trace_start_col + C::ROUND_WIDTH],
        );
        let mut padding_row: ShaRoundColsRefMut<F> = ShaRoundColsRefMut::from::<C>(
            &mut padding_row[trace_start_col..trace_start_col + C::ROUND_WIDTH],
        );
        let mut next_block_first_row: ShaRoundColsRefMut<F> = ShaRoundColsRefMut::from::<C>(
            &mut next_block_first_row[trace_start_col..trace_start_col + C::ROUND_WIDTH],
        );
        Self::generate_intermed_12(
            &mut last_message_row,
            ShaRoundColsRef::from_mut::<C>(&padding_row),
        );
        Self::generate_intermed_12(
            &mut padding_row,
            ShaRoundColsRef::from_mut::<C>(&next_block_first_row),
        );
        Self::generate_intermed_4(
            ShaRoundColsRef::from_mut::<C>(&padding_row),
            &mut next_block_first_row,
        );
    }
