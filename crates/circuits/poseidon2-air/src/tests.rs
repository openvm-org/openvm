...
fn run_poseidon2_subchip_test(
    ...
    let num_rows = 1 << 4;
    let states: Vec<[KoalaBear; 16]> = (0..num_rows)
        .map(|_| {
            let vec: Vec<KoalaBear> = (0..16)
                .map(|_| KoalaBear::from_canonical_u32(rng.next_u32() % (1 << 30)))
                .collect();
            vec.try_into().unwrap()
        })
        .collect();
    ...
    for _ in 0..10 {
        ...
        let rand_inc = KoalaBear::from_canonical_u32(rng.gen_range(1..=1 << 27));
...
fn test_poseidon2_default() {
    ...
    let poseidon2_config = Poseidon2Config::new(default_koalabear_rc());
...
fn test_poseidon2_random_constants() {
    ...
    let partial_round_constants = (0..KOALABEAR_POSEIDON2_PARTIAL_ROUNDS)
