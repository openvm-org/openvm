...
use crate::{test_utils::*, ExprBuilder, FieldExpr, FieldExprCols, FieldVariable, SymbolicExpr, FieldExpressionCoreRecord};
...
fn test_e3_e4_mathematical_equivalence() {
    ...
    for _ in 0..10 {
        ...
        expr.generate_subrow((&range_checker, reconstructed_inputs, reconstructed_flags), &mut e4_row);
        
        // CRITICAL: E3 and E4 must produce bit-identical traces
        assert_eq!(e3_row, e4_row, "E3 and E4 traces must be mathematically equivalent for inputs: {:?}", inputs);
        ...
        assert_eq!(generated, expected, "Mathematical result incorrect for inputs: {:?}", inputs);
    }
}
...
fn test_record_arena_allocation_patterns() {
    ...
    let max_record = FieldExpressionCoreRecord::from_execution_data(
        0, 0, 0, &max_inputs, vec![], 8, 4
    );
    assert_eq!(max_record.num_inputs, 32);
    
    let max_flags = vec![true; 16];
    let flag_record = FieldExpressionCoreRecord::from_execution_data(
        0, 0, 0, &inputs, max_flags, 8, 32
    );
...
fn test_tracestep_tracefiller_roundtrip() {
    ...
    for (direct, reconstructed) in vars_direct.iter().zip(vars_reconstructed.iter()) {
        assert_eq!(direct, reconstructed, "Variable preservation failed in roundtrip");
    }
    ...
    if builder_data.num_flags > 0 {
        ...
        let record_with_flags = FieldExpressionCoreRecord::from_execution_data(
            42, 100, 1, &inputs, flags.clone(),
            expr.canonical_limb_bits(), expr.canonical_num_limbs(),
        );
        
        let reconstructed_flags = record_with_flags.reconstruct_flags();
        assert_eq!(flags, reconstructed_flags);
        
        let reconstructed_inputs_flags = record_with_flags.reconstruct_inputs(expr.canonical_limb_bits());
        let vars_reconstructed_flags = expr.execute(reconstructed_inputs_flags, reconstructed_flags);
        
        assert_eq!(vars_with_flags, vars_reconstructed_flags, "Flag preservation failed");
    }
}
...
fn test_e3_e4_with_complex_operations() {
    ...
    let test_cases = vec![
        ...
        (BigUint::from(100u32), BigUint::from(200u32), BigUint::from(300u32)),
        (generate_random_biguint(&prime), generate_random_biguint(&prime), generate_random_biguint(&prime)),
    ];
    ...
    for (x, y, z) in test_cases {
        ...
        assert_eq!(e3_row, e4_row, "Complex expression E3/E4 mismatch for inputs: {:?}", inputs);
...
fn test_error_handling_and_edge_cases() {
    ...
    let basic_record = FieldExpressionCoreRecord::from_execution_data(
        0, 0, 0, &simple_inputs, vec![], 8, 4
    );
    ...
    let flag_boundary_record = FieldExpressionCoreRecord::from_execution_data(
        0, 0, 0, &small_input, large_flags, 8, 4
    );
    assert_eq!(flag_boundary_record.num_flags as usize, max_flags);
    
    // Test zero inputs edge case
    let zero_input = vec![BigUint::zero()];
    let zero_record = FieldExpressionCoreRecord::from_execution_data(
        0, 0, 0, &zero_input, vec![], 8, 4
    );
    ...
    let small_record = FieldExpressionCoreRecord::from_execution_data(
        0, 0, 0, &small_input, vec![], 8, 4
    );
    ...
    let test_inputs = vec![BigUint::from(42u32), BigUint::from(123u32), BigUint::from(255u32)];
    let record = FieldExpressionCoreRecord::from_execution_data(
        0, 0, 0, &test_inputs, vec![], 8, 4
    );
    ...
    for &pc in &pc_values {
        for &timestamp in &timestamp_values {
            let record = FieldExpressionCoreRecord::from_execution_data(
                pc, timestamp, 0, &simple_input, vec![], 8, 4
            );
    ...
    for opcode in 0..=255u8 {
        let record = FieldExpressionCoreRecord::from_execution_data(
            0, 0, opcode, &simple_input, vec![], 8, 4
        );
    ...
    for flags in flag_test_cases {
        let record = FieldExpressionCoreRecord::from_execution_data(
            0, 0, 0, &simple_input, flags.clone(), 8, 4
        );
        let reconstructed_flags = record.reconstruct_flags();
        assert_eq!(flags, reconstructed_flags);
    }
...
fn test_concurrent_e3_e4_simulation() {
    ...
    for (name, inputs, trace) in &all_traces {
        ...
        assert_eq!(generated, expected, "Concurrent execution {} failed verification", name);
    }
    ...
    assert_eq!(e3_trace, e4_trace, "Concurrent E3/E4 with same inputs must produce identical traces");
}