...
// expressions. Adapter columns use the shared Rv32VecHeapAdapter device fill.
...
__device__ __forceinline__ FieldExprProg load_prog(const uint32_t *blob) {
    FieldExprProg s;
...
__device__ void field_expr_fill_core_row(
    ...
    RowSlice core_row,
    const uint8_t *rec, // opcode byte + input limbs; may be null when is_dummy
...
#define MB_PUT(v) do { core_row[col] = Fp((uint32_t)(v)); col++; } while (0)
    ...
    for (int i = 0; i < s.num_input * nl; i++)
        MB_PUT(is_dummy ? 0u : (uint32_t)in_limbs[i]);
...
// RV32 heap blocks are DEFAULT_BLOCK_SIZE = 4 bytes.
constexpr size_t FE_BLOCK_SIZE = 4;
...
__global__ void field_expr_tracegen_kernel(
    ...
    uint32_t *__restrict__ rc_ptr, size_t rc_bins,
    uint32_t *__restrict__ bitwise_ptr, size_t bitwise_num_bits,
    uint32_t *__restrict__ aux, size_t aux_words,
    uint32_t pointer_max_bits, uint32_t timestamp_max_bits,
    int should_finalize, uint32_t *__restrict__ err) {
    const FieldExprProg s = load_prog(blob);
    constexpr size_t ADAPTER_WIDTH = sizeof(
        Rv32VecHeapAdapterCols<uint8_t, NUM_READS, BLOCKS, BLOCKS, FE_BLOCK_SIZE, FE_BLOCK_SIZE>);
    ...
    BitwiseOperationLookup bitwise(bitwise_ptr, bitwise_num_bits);
    ...
    for (size_t row = tid; row < height; row += nthreads) {
        ...
        if (row < rows_used) {
            ...
            Rv32VecHeapAdapter<NUM_READS, BLOCKS, BLOCKS, FE_BLOCK_SIZE, FE_BLOCK_SIZE> adapter(
                pointer_max_bits, rc, bitwise, timestamp_max_bits);
            adapter.fill_trace_row(
                row_slice,
                *(const Rv32VecHeapAdapterRecord<NUM_READS, BLOCKS, BLOCKS, FE_BLOCK_SIZE,
                                                 FE_BLOCK_SIZE> *)rec);
            field_expr_fill_core_row(s, row_slice.slice_from(ADAPTER_WIDTH),
                                     rec + rec_core_offset, rc, my_aux, false, err);
        } else {
...
#define MB_LAUNCHER(R, B)                                                                     \
    extern "C" int _field_expr_tracegen_r##R##_b##B(                                          \
        ...
        size_t bitwise_num_bits, uint32_t *d_aux, size_t aux_words,                           \
        ...
        field_expr_tracegen_kernel<R, B><<<blocks, threads, 0, stream>>>(                     \
            ...
            d_range_checker, rc_bins, d_bitwise_lookup, bitwise_num_bits, d_aux, aux_words,   \
            pointer_max_bits, timestamp_max_bits, should_finalize, d_err);                    \
        return CHECK_KERNEL();                                                                \
    }

MB_LAUNCHER(2, 8)   // modular 32-limb (addsub, muldiv)
MB_LAUNCHER(2, 12)  // modular 48-limb
MB_LAUNCHER(2, 16)  // fp2 32-limb, EcAddNe 32-limb
MB_LAUNCHER(2, 24)  // fp2 48-limb, EcAddNe 48-limb
MB_LAUNCHER(1, 16)  // EcDouble 32-limb
MB_LAUNCHER(1, 24)  // EcDouble 48-limb
