...
impl TLFunction_merkle_verify {
    ...
    pub fn stage_0(&mut self, tracker: &mut Tracker) {
        ...
        if self.scope_0_True {
            self.commit = tracker.memory_ConstArray8_F.dereference(self.leaf);
        }
...
pub struct TLFunction_main {
    ...
    pub appended_array_13: TLArray,
    pub appended_index_13: usize,
    pub callee_0: Box<Option<TLFunction_merkle_verify>>,
...
impl TLFunction_main {
    ...
    pub fn stage_0(&mut self, tracker: &mut Tracker) {
        ...
        self.ref_1 = tracker
            .memory_ConstArray8_F
            .create_ref(self.leaf, self.calc_zk_identifier(1usize));
        self.leaf_ref = self.ref_1;
        ...
        self.bits0 = tracker
            .memory_Bool
            .create_empty_under_construction_array(self.calc_zk_identifier(10usize));
        self.appended_array_11 = self.bits0;
        let (temp_8, temp_7) = tracker
            .memory_Bool
            .append_under_construction_array(self.appended_array_11, false);
        self.appended_index_11 = temp_8;
        self.bits1 = temp_7;
        self.appended_array_12 = self.bits1;
        let (temp_10, temp_9) = tracker
            .memory_Bool
            .append_under_construction_array(self.appended_array_12, true);
        self.appended_index_12 = temp_10;
        self.bits2 = temp_9;
        self.appended_array_13 = self.bits2;
        let (temp_12, temp_11) = tracker
            .memory_Bool
            .append_under_construction_array(self.appended_array_13, false);
        self.appended_index_13 = temp_12;
        self.bits3 = temp_11;
        ...
        if self.scope_0_False {
            self.callee_0 = Box::new(Some(TLFunction_merkle_verify::default()));
            self.callee_0.as_mut().as_mut().unwrap().materialized = self.materialized;
            self.callee_0.as_mut().as_mut().unwrap().leaf = self.leaf_ref;
        ...
        if self.scope_0_True {
            ...
            self.callee_1.as_mut().as_mut().unwrap().leaf = self.leaf_ref;
...
impl TLFunction_merkle_verify {
    pub const TRACE_WIDTH: usize = 30usize;
    ...
    pub fn generate_trace(&self, tracker: &Tracker, trace_set: &mut TraceSet) {
        ...
        row[22usize] = as_cells[0usize];
        row[23usize] = as_cells[1usize];
        row[24usize] = as_cells[2usize];
        row[25usize] = as_cells[3usize];
        row[26usize] = as_cells[4usize];
        row[27usize] = as_cells[5usize];
        ...
        row[8usize] = as_cells[2usize];
        row[9usize] = as_cells[3usize];
        row[10usize] = as_cells[4usize];
...
impl TLFunction_main {
    pub const TRACE_WIDTH: usize = 8usize;
    pub const NUM_REFERENCES: usize = 2usize;
    pub fn generate_trace(&self, tracker: &Tracker, trace_set: &mut TraceSet) {
        if self.scope_0_False {
            self.callee_0
                .as_ref()
...
fn to_cells_Ref_ConstArray8_F(value: TLRef, result: &mut [F]) {
    result[0] = F::from_canonical_usize(value.zk_identifier);
}
...
impl<AB: InteractionBuilder> Air<AB> for Air_merkle_verify {
    fn eval(&self, builder: &mut AB) {
        ...
        builder
            .when(cell(0))
            .assert_eq(cell(4), AB::Expr::from_canonical_usize(0));
        builder
            .when(cell(0))
        ...
        builder
            .when(cell(0))
            .assert_eq(cell(29), AB::Expr::from_canonical_usize(0));
        ...
        builder.when(cell(1)).assert_eq(cell(29), cell(29));
        ...
        builder.when(cell(2)).assert_eq(cell(21), cell(21));
        builder
            .when(cell(1) + cell(2))
            .assert_eq(cell(6), cell(14) + cell(22));
        builder
            .when(cell(1) + cell(2))
            .assert_eq(cell(7), cell(15) * cell(23));
        builder.when(cell(1) + cell(2)).assert_eq(
            cell(8),
            cell(16) + (cell(24) * -AB::F::from_canonical_usize(1)),
        );
        builder.when(cell(1) + cell(2)).assert_eq(cell(9), cell(17));
        builder
            .when(cell(1) + cell(2))
            .assert_eq(cell(10), cell(26));
        builder
            .when(cell(1) + cell(2))
            .assert_eq(cell(11), AB::Expr::from_canonical_usize(115));
        builder
            .when(cell(1) + cell(2))
            .assert_eq(cell(12), cell(20) * cell(21));
        builder
            .when(cell(1) + cell(2))
            .assert_eq(cell(13), cell(28) * cell(29));
        builder.push_interaction(
            1u16,
            [
                cell(3),
        ...
        builder.push_interaction(
            ...
            [
                cell(5),
                cell(4) + (AB::Expr::from_canonical_usize(1) * -AB::F::from_canonical_usize(1)),
