...
/// `program_commit`, `initial_state`, and `initial_pc`, all derived from the VM config and the
/// aggregation parameters. Two proofs verified against baselines with equal [VmBaseline]s were
/// generated by the same VM and aggregation pipeline.
...
impl From<&VerificationBaseline> for VmBaseline {
    /// Extracts the VM-specific subset of a baseline, dropping the executable-specific fields.
...
pub struct VerificationBaselineJson {
    pub program_commit: CommitBytes,
    pub initial_state: CommitBytes,
    pub initial_pc: F,
    pub memory_dimensions: MemoryDimensions,
    pub num_user_pvs: usize,
...
impl From<VerificationBaseline> for VerificationBaselineJson {
    fn from(b: VerificationBaseline) -> Self {
        ...
        Self {
            program_commit: CommitBytes::from(b.program_commit),
            initial_state: CommitBytes::from(b.initial_state),
            initial_pc: b.initial_pc,
...
impl From<VerificationBaselineJson> for VerificationBaseline {
    fn from(b: VerificationBaselineJson) -> Self {
        ...
        Self {
            program_commit: b.program_commit.into(),
            initial_state: b.initial_state.into(),
            initial_pc: b.initial_pc,
            memory_dimensions: b.memory_dimensions,
            num_user_pvs: b.num_user_pvs,
            app_vk_commit: vk(b.app_vk_commit),
