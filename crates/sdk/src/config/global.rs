...
pub struct SdkVmConfig {
    ...
    pub edwards: Option<EdwardsExtension>,
}
...
impl SdkVmConfig {
    ...
    pub fn standard() -> SdkVmConfig {
        ...
        SdkVmConfig::builder()
            ...
            .edwards(EdwardsExtension::new(vec![ED25519_CONFIG.clone()]))
...
impl TranspilerConfig<F> for SdkVmConfig {
    fn transpiler(&self) -> Transpiler<F> {
        ...
        if self.edwards.is_some() {
            transpiler = transpiler.with_extension(EdwardsTranspilerExtension);
        }
        transpiler
    }
}
...
impl SdkVmConfig {
    ...
    pub fn to_inner(&self) -> SdkVmConfigInner {
        ...
        let edwards = config.edwards.clone();
...
pub struct SdkVmConfigInner {
    ...
    pub ecc: Option<WeierstrassExtension>,
    #[extension(executor = "EdwardsExtensionExecutor")]
    pub edwards: Option<EdwardsExtension>,
}
    ...
    Val<SC>: PrimeField32,
{
    ...
    > {
        ...
        if let Some(edwards) = &config.edwards {
            VmProverExtension::<E, _, _>::extend_prover(&EdwardsCpuProverExt, edwards, inventory)?;
        }
        Ok(chip_complex)
    }
}
...
impl VmBuilder<GpuBabyBearPoseidon2Engine> for SdkVmGpuBuilder {
    ...
    > {
        ...
        if let Some(edwards) = &config.edwards {
            VmProverExtension::<E, _, _>::extend_prover(&EdwardsProverExt, edwards, inventory)?;
        }
        Ok(chip_complex)
    }
}
...
impl InitFileGenerator for SdkVmConfigInner {
    fn generate_init_file_contents(&self) -> Option<String> {
            ...
            || self.edwards.is_some()
        {
            ...
            if let Some(edwards_config) = &self.edwards {
                contents.push_str(&edwards_config.generate_te_init());
                contents.push('\n');
            }
