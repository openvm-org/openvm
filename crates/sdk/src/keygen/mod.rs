...
#[derive(Clone, Serialize, Deserialize)]
pub struct DeferralProvingKey {
    pub circuits: Vec<DeferralCircuitProvingKey>,
    pub def_internal_recursive_pk: Arc<MultiStarkProvingKey<SC>>,
    pub def_hook_pk: Arc<MultiStarkProvingKey<SC>>,
}
...
/// Proving keys that have already been generated by an SDK.
///
/// `None` means the SDK has not generated or been seeded with that key yet. Constructing this type
/// should not trigger key generation.
///
/// Halo2 proving keys use a separate codec path; use `write_halo2_pk_to_file` and
/// `read_halo2_pk_from_file` for those.
#[derive(Clone, Default, Serialize, Deserialize)]
pub struct SdkCachedProvingKey<VC> {
    pub app_pk: Option<AppProvingKey<VC>>,
    pub agg_pk: Option<AggProvingKey>,
    pub deferral_pk: Option<DeferralProvingKey>,
    pub deferral_agg_pk: Option<AggProvingKey>,
    #[cfg(feature = "root-prover")]
    pub root_pk: Option<RootProvingKey>,
}
