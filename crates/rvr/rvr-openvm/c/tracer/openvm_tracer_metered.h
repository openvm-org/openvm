...
_Static_assert(
    TRACER_MEM_PAGE_BUF_CAP >=
        2 * TRACER_SEGMENT_CHECK_INSNS * MAX_MEM_PAGES_PER_INSN,
    "MEM_PAGE_BUF_CAP too small for worst-case pages per flush interval");
_Static_assert(
    TRACER_PV_PAGE_BUF_CAP >=
        2 * TRACER_SEGMENT_CHECK_INSNS * MAX_PV_PAGES_PER_INSN,
...
static __attribute__((always_inline)) inline void trace_memory_access(
  ...
  trace_memory_access_page(memory, addr_to_local_page(addr));
}
...
static __attribute__((always_inline)) inline void trace_rd_mem_u32_range(
  ...
  record_mem_page_range(state->tracer, addr_to_local_page(base_addr),
                        addr_to_local_page(last_addr));
}
