...
/// R4: baked operands for an arena-native wr1 record (JalLui / Auipc).
#[derive(Clone, Copy, Debug)]
pub(crate) struct ArenaWr1Baked {
    /// `u32::MAX` for a suppressed link write.
    pub rd_ptr: u32,
    pub core_imm: u32,
    /// JalLui only: the written block value (link pc+4 or sign-extended LUI).
    pub rd_data: u64,
    pub is_jal: u8,
    /// Auipc only.
...
/// R4: baked operands for an arena-native rw1 record (Jalr).
#[derive(Clone, Copy, Debug)]
pub(crate) struct ArenaRw1Baked {
    pub rs1_ptr: u32,
    /// `u32::MAX` for a suppressed link write.
    pub rd_ptr: u32,
    pub core_imm: u16,
...
#[derive(Clone, Copy, Debug)]
pub(crate) struct ArenaAlu3Baked {
    /// `rs2` record field: register byte pointer (reg-form) or immediate.
    pub rs2_field: u32,
    /// 1 = rs2 was a register read, 0 = immediate.
    pub rs2_as: u8,
    /// Immediate sign bit (imm-form only).
    pub rs2_imm_sign: u8,
    /// The family's `local_opcode` ordinal as the core filler consumes it.
    pub local_opcode: u8,
}
...
impl EmitContext {
    ...
    pub(crate) fn emit_store_inline(&mut self, width: u8, rs1: u8, rs2: u8, offset: i16) {
        ...
        let (wr_func, cast_ty) = Self::write_mem_helper(width, false);
    ...
    /// REVEAL is the current user: its two register reads and public-values
    /// write have the same three-tick shape as STORED, so it can share the
    /// compact/arena-native LoadStore path instead of forcing the entire AIR
    /// through verbose logs and host assembly.
    ...
    fn trace_reveal_compact_inline(
        ...
        let offset = if offset < 0 {
            ...
            format!("0x{offset:08x}u")
        };
        self.extern_call_without_page_flush(
            "openvm_reveal",
            &[&src, &ptr, &offset, &width.to_string()],
        );
        self.indent -= 1;
        self.write_line("}");
        true
...
impl rvr_openvm_ir::ExtEmitCtx for EmitContext {
    ...
    fn trace_store_u64_as(
        ...
        self.trace_store_u64_as_inline(src_reg, ptr_reg, offset, addr_space, local_opcode)
    }
...
mod tests {
    ...
    fn aligned_memory_access_uses_single_leaf_trace() {
        ...
        assert!(ctx
            .buf()
            .contains("trace_memory_access(&trace_memory, addr);"));
    }
}
