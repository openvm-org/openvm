...
The OpenVM framework supports transpilation of a RISC-V ELF consisting of the RV32IM/RV64IM instruction set as well as [custom RISC-V instructions](/specs/reference/riscv-custom-code) specified by VM extensions into an OpenVM executable.

## Notation

We use the following notation through this specification:
...
- Let `ind(rd)` denote `LIMBS * (register index)`, which is in `0..32*LIMBS`. In particular, it fits in one field element.
...
- We use `sign_extend_24` to convert a 12-bit or 20-bit integer into a 24-bit integer via sign extension. We use this in conjunction with `utof`, which converts 24 bits into an unsigned integer and then maps it to the corresponding field element. Note that each 24-bit unsigned integer fits in one field element.
...
- The notation `imm[0:5]` means the lowest 6 bits of the immediate.
...
- A RISC-V XLEN bit register `x{i}` read or write access corresponds to an OpenVM memory access at `[LIMBS * i: LIMBS]_1` **except** for writes to `x0`, see [below](#register-x0-handling). The XLEN bits of `x{i}` are represented as LIMBS little-endian bytes in OpenVM memory.
  - A RISC-V code block must **never** map to any OpenVM instruction that changes the value of `[0:LIMBS]_1` in OpenVM memory.
- A RISC-V XLEN bit user memory access of the `j`th byte in word `i` corresponds to an OpenVM memory access at `[LIMBS * i + j]_2`.
...
The OpenVM ISA treats `[0:LIMBS]_1` as normal read/write memory and makes no guarantees on memory accesses to this location. The transpiler must **never** transpile a RISC-V code block to any OpenVM instruction that changes the value of `[0:LIMBS]_1` in OpenVM memory. For compatibility with the RISC-V ISA, the transpiler must always transpile a RISC-V instruction to an OpenVM instruction that matches the RISC-V specification. In particular, any RISC-V instruction that has `rd=x0` must be transpiled to either the `NOP` OpenVM instruction if it has no side effects or to an OpenVM instruction that executes the expected side effect and does not change the value of `[0:LIMBS]_1`.

## Transpiler Specification for Default VM Extensions

This section specifies the behavior of the transpiler for the default VM extensions with the custom RISC-V instructions specified [here](/specs/reference/riscv-custom-code).
...
## RV32IM/RV64IM Extension

Transpilation from RV32IM/RV64IM to OpenVM assembly follows the mapping below, which is generally
a 1-1 translation between RV32IM/RV64IM instructions and OpenVM instructions. The main exception relates
...
Because `[0:LIMBS]_1` is initialized to `0` and never written to, this guarantees that reads from `x0` yield `0` and enforces that any OpenVM program transpiled from RV32IM/RV64IM conforms to the RV32IM/RV64IM specification for `x0`.

### System Level Extensions to RV32IM/RV64IM

| RISC-V Inst        | OpenVM Instruction                                                                                             |
| ------------------ | -------------------------------------------------------------------------------------------------------------- |
| hintstorew/hintstored  | HINT_STOREW_RV32/HINT_STORED_RV64 `0, ind(rd), _, 1, 2`                                                        |
| hintbuffer         | HINT_BUFFER_RV32/HINT_BUFFER_RV64 `ind(rs1), ind(rd), _, 1, 2`                                                 |
| reveal             | STOREW_RV32/STORED_RV64 `ind(rs1), ind(rd), utof(sign_extend_16(imm)), 1, 3, 1, sign_of(imm)`                  |
| hintinput          | PHANTOM `_, _, disc(Rv32HintInput)`/PHANTOM `_, _, disc(Rv64HintInput)`                                        |
| printstr           | PHANTOM `ind(rd), ind(rs1), disc(Rv32PrintStr)`/PHANTOM `ind(rd), ind(rs1), disc(Rv64PrintStr)`                |
| hintrandom         | PHANTOM `ind(rd), _, disc(Rv32HintRandom)`/PHANTOM `ind(rd), _, disc(Rv64HintRandom)`                          |

### RV32IM/RV64IM Instructions

| RISC-V Inst | OpenVM Instruction                                                                                            |
| ----------- | ------------------------------------------------------------------------------------------------------------- |
| add         | ADD_RV32/ADD_RV64 `ind(rd), ind(rs1), ind(rs2), 1, 1` if `rd != x0`, otherwise PHANTOM `_, _, disc(Nop)`      |
| addw        | ADDW_RV64 `ind(rd), ind(rs1), ind(rs2), 1, 1` if `rd != x0`, otherwise PHANTOM `_, _, disc(Nop)`              |
| sub         | SUB_RV32/SUB_RV64 `ind(rd), ind(rs1), ind(rs2), 1, 1` if `rd != x0`, otherwise PHANTOM `_, _, disc(Nop)`      |
| subw        | SUBW_RV64 `ind(rd), ind(rs1), ind(rs2), 1, 1` if `rd != x0`, otherwise PHANTOM `_, _, disc(Nop)`              |
| xor         | XOR_RV32/XOR_RV64 `ind(rd), ind(rs1), ind(rs2), 1, 1` if `rd != x0`, otherwise PHANTOM `_, _, disc(Nop)`      |
| or          | OR_RV32/OR_RV64 `ind(rd), ind(rs1), ind(rs2), 1, 1` if `rd != x0`, otherwise PHANTOM `_, _, disc(Nop)`        |
| and         | AND_RV32/AND_RV64 `ind(rd), ind(rs1), ind(rs2), 1, 1` if `rd != x0`, otherwise PHANTOM `_, _, disc(Nop)`      |
| sll         | SLL_RV32/SLL_RV64 `ind(rd), ind(rs1), ind(rs2), 1, 1` if `rd != x0`, otherwise PHANTOM `_, _, disc(Nop)`      |
| sllw        | SLLW_RV64 `ind(rd), ind(rs1), ind(rs2), 1, 1` if `rd != x0`, otherwise PHANTOM `_, _, disc(Nop)`              |
| srl         | SRL_RV32/SRL_RV64 `ind(rd), ind(rs1), ind(rs2), 1, 1` if `rd != x0`, otherwise PHANTOM `_, _, disc(Nop)`      |
| srlw        | SRLW_RV64 `ind(rd), ind(rs1), ind(rs2), 1, 1` if `rd != x0`, otherwise PHANTOM `_, _, disc(Nop)`              |
| sra         | SRA_RV32/SRA_RV64 `ind(rd), ind(rs1), ind(rs2), 1, 1` if `rd != x0`, otherwise PHANTOM `_, _, disc(Nop)`      |
| sraw        | SRAW_RV64 `ind(rd), ind(rs1), ind(rs2), 1, 1` if `rd != x0`, otherwise PHANTOM `_, _, disc(Nop)`              |
| slt         | SLT_RV32/SLT_RV64 `ind(rd), ind(rs1), ind(rs2), 1, 1` if `rd != x0`, otherwise PHANTOM `_, _, disc(Nop)`      |
| sltu        | SLTU_RV32/SLTU_RV64 `ind(rd), ind(rs1), ind(rs2), 1, 1` if `rd != x0`, otherwise PHANTOM `_, _, disc(Nop)`    |
| addi        | ADD_RV32/ADD_RV64 `ind(rd), ind(rs1), utof(sign_extend_24(imm)), 1, 0` if `rd != x0`, else PHANTOM `_, _, disc(Nop)` |
| addiw       | ADDW_RV64 `ind(rd), ind(rs1), utof(sign_extend_24(imm)), 1, 0` if `rd != x0`, else PHANTOM `_, _, disc(Nop)`  |
| xori        | XOR_RV32/XOR_RV64 `ind(rd), ind(rs1), utof(sign_extend_24(imm)), 1, 0` if `rd != x0`, else PHANTOM `_, _, disc(Nop)` |
| ori         | OR_RV32/OR_RV64 `ind(rd), ind(rs1), utof(sign_extend_24(imm)), 1, 0` if `rd != x0`, else PHANTOM `_, _, disc(Nop)` |
| andi        | AND_RV32/AND_RV64 `ind(rd), ind(rs1), utof(sign_extend_24(imm)), 1, 0` if `rd != x0`, else PHANTOM `_, _, disc(Nop)` |
| slli        | SLL_RV32/SLL_RV64 `ind(rd), ind(rs1), utof(zero_extend_24(imm[0:4]/imm[0:5])), 1, 0` if `rd != x0`, else PHANTOM `_, _, disc(Nop)` |
| slliw       | SLLW_RV64 `ind(rd), ind(rs1), utof(zero_extend_24(imm[0:4])), 1, 0` if `rd != x0`, else PHANTOM `_, _, disc(Nop)` |
| srli        | SRL_RV32/SRL_RV64 `ind(rd), ind(rs1), utof(zero_extend_24(imm[0:4]/imm[0:5])), 1, 0` if `rd != x0`, else PHANTOM `_, _, disc(Nop)` |
| srliw       | SRLW_RV64 `ind(rd), ind(rs1), utof(zero_extend_24(imm[0:4])), 1, 0` if `rd != x0`, else PHANTOM `_, _, disc(Nop)` |
| srai        | SRA_RV32/SRA_RV64 `ind(rd), ind(rs1), utof(zero_extend_24(imm[0:4]/imm[0:5])), 1, 0` if `rd != x0`, else PHANTOM `_, _, disc(Nop)` |
| sraiw       | SRAW_RV64 `ind(rd), ind(rs1), utof(zero_extend_24(imm[0:4])), 1, 0` if `rd != x0`, else PHANTOM `_, _, disc(Nop)` |
| slti        | SLT_RV32/SLT_RV64 `ind(rd), ind(rs1), utof(sign_extend_24(imm)), 1, 0` if `rd != x0`, else PHANTOM `_, _, disc(Nop)` |
| sltiu       | SLTU_RV32/SLTU_RV64 `ind(rd), ind(rs1), utof(sign_extend_24(imm)), 1, 0` if `rd != x0`, else PHANTOM `_, _, disc(Nop)` |
| lb          | LOADB_RV32/LOADB_RV64 `ind(rd), ind(rs1), utof(sign_extend_16(imm)), 1, 2, (rd != x0), sign_of(imm)`          |
| lh          | LOADH_RV32/LOADH_RV64 `ind(rd), ind(rs1), utof(sign_extend_16(imm)), 1, 2, (rd != x0), sign_of(imm)`          |
| lw          | LOADW_RV32/LOADW_RV64 `ind(rd), ind(rs1), utof(sign_extend_16(imm)), 1, 2, (rd != x0), sign_of(imm)`          |
| ld          | LOADD_RV64 `ind(rd), ind(rs1), utof(sign_extend_16(imm)), 1, 2, (rd != x0), sign_of(imm)`                     |
| lbu         | LOADBU_RV32/LOADBU_RV64 `ind(rd), ind(rs1), utof(sign_extend_16(imm)), 1, 2, (rd != x0), sign_of(imm)`        |
| lhu         | LOADHU_RV32/LOADHU_RV64 `ind(rd), ind(rs1), utof(sign_extend_16(imm)), 1, 2, (rd != x0), sign_of(imm)`        |
| lwu         | LOADWU_RV64 `ind(rd), ind(rs1), utof(sign_extend_16(imm)), 1, 2, (rd != x0), sign_of(imm)`                    |
| sb          | STOREB_RV32/STOREB_RV64 `ind(rs2), ind(rs1), utof(sign_extend_16(imm)), 1, 2, 1, sign_of(imm)`                |
| sh          | STOREH_RV32/STOREH_RV64 `ind(rs2), ind(rs1), utof(sign_extend_16(imm)), 1, 2, 1, sign_of(imm)`                |
| sw          | STOREW_RV32/STOREW_RV64 `ind(rs2), ind(rs1), utof(sign_extend_16(imm)), 1, 2, 1, sign_of(imm)`                |
| sd          | STORED_RV64 `ind(rs2), ind(rs1), utof(sign_extend_16(imm)), 1, 2, 1, sign_of(imm)`                            |
| beq         | BEQ_RV32/BEQ_RV64 `ind(rs1), ind(rs2), itof(imm), 1, 1`                                                       |
| bne         | BNE_RV32/BNE_RV64 `ind(rs1), ind(rs2), itof(imm), 1, 1`                                                       |
| blt         | BLT_RV32/BLT_RV64 `ind(rs1), ind(rs2), itof(imm), 1, 1`                                                       |
| bge         | BGE_RV32/BGE_RV64 `ind(rs1), ind(rs2), itof(imm), 1, 1`                                                       |
| bltu        | BLTU_RV32/BLTU_RV64 `ind(rs1), ind(rs2), itof(imm), 1, 1`                                                     |
| bgeu        | BGEU_RV32/BGEU_RV64 `ind(rs1), ind(rs2), itof(imm), 1, 1`                                                     |
| jal         | JAL_RV32/JAL_RV64 `ind(rd), 0, itof(imm), 1, 0, (rd != x0)`                                                   |
| jalr        | JALR_RV32/JALR_RV64 `ind(rd), ind(rs1), utof(sign_extend_16(imm)), 1, 0, (rd != x0), sign_of(imm)`            |
| lui         | LUI_RV32/LUI_RV64 `ind(rd), 0, utof(sign_extend_24(imm[12:31])), 1, 0, 1` if `rd != x0`, else PHANTOM `_, _, disc(Nop)` |
| auipc       | AUIPC_RV32/AUIPC_RV64 `ind(rd), 0, utof(sign_extend_24(imm[12:31]) << 4), 1` if `rd != x0`, else PHANTOM `_, _, disc(Nop)`. |
| mul         | MUL_RV32/MUL_RV64 `ind(rd), ind(rs1), ind(rs2), 1` if `rd != x0`, otherwise PHANTOM `_, _, disc(Nop)`         |
| mulw        | MULW_RV64 `ind(rd), ind(rs1), ind(rs2), 1` if `rd != x0`, otherwise PHANTOM `_, _, disc(Nop)`                 |
| mulh        | MULH_RV32/MULH_RV64 `ind(rd), ind(rs1), ind(rs2), 1` if `rd != x0`, otherwise PHANTOM `_, _, disc(Nop)`       |
| mulhsu      | MULHSU_RV32/MULHSU_RV64 `ind(rd), ind(rs1), ind(rs2), 1` if `rd != x0`, otherwise PHANTOM `_, _, disc(Nop)`   |
| mulhu       | MULHU_RV32/MULHU_RV64 `ind(rd), ind(rs1), ind(rs2), 1` if `rd != x0`, otherwise PHANTOM `_, _, disc(Nop)`     |
| div         | DIV_RV32/DIV_RV64 `ind(rd), ind(rs1), ind(rs2), 1` if `rd != x0`, otherwise PHANTOM `_, _, disc(Nop)`         |
| divw        | DIVW_RV64 `ind(rd), ind(rs1), ind(rs2), 1` if `rd != x0`, otherwise PHANTOM `_, _, disc(Nop)`                 |
| divu        | DIVU_RV32/DIVU_RV64 `ind(rd), ind(rs1), ind(rs2), 1` if `rd != x0`, otherwise PHANTOM `_, _, disc(Nop)`       |
| divuw       | DIVUW_RV64 `ind(rd), ind(rs1), ind(rs2), 1` if `rd != x0`, otherwise PHANTOM `_, _, disc(Nop)`                |
| rem         | REM_RV32/REM_RV64 `ind(rd), ind(rs1), ind(rs2), 1` if `rd != x0`, otherwise PHANTOM `_, _, disc(Nop)`         |
| remw        | REMW_RV64 `ind(rd), ind(rs1), ind(rs2), 1` if `rd != x0`, otherwise PHANTOM `_, _, disc(Nop)`                 |
| remu        | REMU_RV32/REMU_RV64 `ind(rd), ind(rs1), ind(rs2), 1` if `rd != x0`, otherwise PHANTOM `_, _, disc(Nop)`       |
...
- Transpile to an OpenVM assembly instruction that does not write to `[0:LIMBS]_1` and processes side effects if the register is `x0` and there are side effects.
...
- Kernel code must not write to `[0:LIMBS]_1` in OpenVM memory
