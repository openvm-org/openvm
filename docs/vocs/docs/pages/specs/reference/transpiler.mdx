...
| lui         | LUI_RV32/LUI_RV64 `ind(rd), 0, utof(zero_extend_24(imm[12:31])), 1, 0, 1` if `rd != x0`, else PHANTOM `_, _, disc(Nop)` |
| auipc       | AUIPC_RV32/AUIPC_RV64 `ind(rd), 0, utof(zero_extend_24(imm[12:31]) << 4), 1` if `rd != x0`, else PHANTOM `_, _, disc(Nop)` |
