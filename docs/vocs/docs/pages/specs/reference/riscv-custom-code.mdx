...
| hintinput   | I   | 0001011     | 011    | 0x0       | Pop next vector from input stream and reset hint stream to `[(len as u32).to_le_bytes(), vector, zero padding to 8-byte alignment]`, where `len` is the vector length in bytes. |
