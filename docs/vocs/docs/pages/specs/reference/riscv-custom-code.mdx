...
- user public outputs: a bounded append-only stream of 64-bit values. Its capacity is determined
  by the `num_public_value_cells` execution-environment parameter. Each reveal appends one value.
...
| reveal      | I   | 0001011     | 010    | 0x0       | Appends the 8-byte dword `rs1` to the public-output stream. `rd` must be `x0`. |
...
| RISC-V Inst     | FMT | opcode[6:0] | funct3 | funct7    | RISC-V description and notes                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| --------------- | --- | ----------- | ------ | --------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| sw_add_ne\<C\>  | R   | 0101011     | 001    | `idx*8`   | `EcPoint([rd:2*C::Fp::NUM_LIMBS]_2) = EcPoint([rs1:2*C::Fp::NUM_LIMBS]_2) + EcPoint([rs2:2*C::Fp::NUM_LIMBS]_2)`. Assumes that input affine points are not identity and do not have same x-coordinate.                                                                                                                                                                                                                                                                                                                                                                  |
| sw_double\<C\>  | R   | 0101011     | 001    | `idx*8+1` | `EcPoint([rd:2*C::Fp::NUM_LIMBS]_2) = 2 * EcPoint([rs1:2*C::Fp::NUM_LIMBS]_2)`. Assumes that input affine point is not identity. `rs2` is unused and must be set to `x0`.                                                                                                                                                                                                                                                                                                                                                                                            |
| setup\<C\>      | R   | 0101011     | 001    | `idx*8+2` | If `ind(rs2) != 0`, then this instruction is setup for `sw_add_ne`. Otherwise it is setup for `sw_double`. If setup for `sw_add_ne`, it checks `assert([rs1: C::Fp::NUM_LIMBS]_2 == C::Fp::MODULUS)`, and if setup for `sw_double`, checks `assert([rs1: 2*C::Fp::NUM_LIMBS]_2 == [C::Fp::MODULUS, C::CURVE_A])`. For the sake of implementation convenience it also writes an unconstrained value into `[rd: 2*C::Fp::NUM_LIMBS]_2`. When `ind(rs2) != 0` (add_ne), it is required for proper functionality that `[rs2: C::Fp::NUM_LIMBS]_2 != [rs1: C::Fp::NUM_LIMBS]_2`; otherwise (double), it is required that `[rs1 + C::Fp::NUM_LIMBS: C::Fp::NUM_LIMBS]_2 != C::Fp::ZERO` |
