...
Deferral instructions use the _custom-1_ opcode prefix (`0101011`) and `funct3 = 0b111`. The 12-bit immediate encodes a deferral index and sub-opcode as `[def_idx(10 bits) | opcode(2 bits)]`; `def_idx` is currently constrained to be less than 512 by the continuations deferral aggregation circuit.
