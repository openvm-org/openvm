...
- [SHA2](#sha-2-extension): An extension implementing the SHA-256 and SHA-512 compression functions compatibly with RISC-V memory. These are sufficient to implement SHA-256, SHA-384, and SHA-512.
...
The RV64IM extension supports the RV64I Base Integer Instruction Set with `XLEN=64`
and the "M" Extension for Integer Multiplication and Division with `XLEN=64`, following
...
### IO
In addition to the standard RV64IM opcodes, we support the following additional intrinsic instructions to handle interactions between the guest and host environments.
...
| RISC-V Inst | FMT | opcode[6:0] | funct3 | imm[0:11] | RISC-V description and notes                                                                                                                                               |
| ----------- | --- | ----------- | ------ | --------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| hintstored  | I   | 0001011     | 001    | 0x0       | Stores next 8-byte dword from hint stream in user memory at `[rd]_2`. The address `rd` does not have any alignment requirements. |
| hintbuffer  | I   | 0001011     | 001    | 0x1       | Stores next `8 * rs1` bytes from hint stream in user memory at `[rd..rd + 8 * rs1]_2`. Only valid if next `8 * rs1` values in hint stream are bytes and `rs1` is non-zero and at most `1023`. The address `rd` does not have any alignment requirements. |
| reveal      | I   | 0001011     | 010    |           | Stores the 8-byte dword `rs1` at address `rd + imm` in user IO space. The address `rd + imm` must be aligned to an 8-byte boundary. |
| hintinput   | I   | 0001011     | 011    | 0x0       | Pop next vector from input stream and reset hint stream to `[(len as u64).to_le_bytes(), vector, zero padding to 8-byte alignment]`, where `len` is the vector length in bytes. |
...
| hintrandom  | I   | 0001011     | 011    | 0x2       | Resets the hint stream to `8 * rd` random bytes from a fixed-seed RNG (`rand::rngs::StdRng`) on the host.                                                                                      |

## Deferral Extension

Deferral instructions use the _custom-1_ opcode prefix (`0101011`) and `funct3 = 0b111`. The 12-bit immediate encodes a deferral index and sub-opcode as `[def_idx(10 bits) | opcode(2 bits)]`; `def_idx` is currently constrained to be less than `MAX_DEF_CIRCUITS` by the continuations deferral aggregation circuit.

| RISC-V Inst | FMT | opcode[6:0] | funct3 | imm[1:0] | RISC-V description and notes |
| ----------- | --- | ----------- | ------ | -------- | ---------------------------------------------------------------- |
| def_call    | I   | 0101011     | 111    | 0x0      | Deferred call: reads 32-byte input commitment from `[rs1]`, writes 40-byte `OutputKey = [output_commit:32 bytes || output_len_le:8 bytes]` to `[rd]`. The `output_len` field is encoded as 8-byte little-endian but is currently constrained to a zero-extended 32-bit value, and deferred outputs must have byte length divisible by `DIGEST_SIZE`. `imm[11:2]` = deferral index. |
| def_output  | I   | 0101011     | 111    | 0x1      | Retrieve deferred output: reads 40-byte `OutputKey` from `[rs1]`, writes `output_len` bytes to `[rd]`. The instruction is only valid for deferred outputs whose byte length is divisible by `DIGEST_SIZE`. `imm[11:2]` = deferral index. |
...
| xorin       | R   | 0001011     | 100    | 0x1    | XORs `rs2` input bytes into state buffer: for `i` in `0..rs2`, `[rd+i]_2 ^= [rs1+i]_2`. `rs2` must be a multiple of 4 and at most 136; `rs2 = 0` is a no-op. |
...
| mul256      | R   | 0001011     | 101    | 0x0A   | `[rd:32]_2 = ([rs1:32]_2 * [rs2:32]_2)[0:255]`            |
...
The elliptic curve extension supports arithmetic over short Weierstrass curves using projective coordinates, which requires specification of the elliptic curve `C`. The extension must be configured to support a fixed ordered list of supported curves. We use `config.curve_idx(C)` to denote the index of `C` in this list. In the list below, `idx` denotes `config.curve_idx(C)`.

Points are represented in projective coordinates `(X, Y, Z)` where the corresponding affine point is `(X/Z, Y/Z)`. The identity point is represented by `Z = 0`. Operations use complete formulas from [ePrint 2015/1060](https://eprint.iacr.org/2015/1060.pdf) that handle all edge cases without branching.

| RISC-V Inst     | FMT | opcode[6:0] | funct3 | funct7    | RISC-V description and notes                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| --------------- | --- | ----------- | ------ | --------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| sw_add_proj\<C\>  | R   | 0101011     | 001    | `idx*8`   | `EcPoint([rd:3*C::Fp::NUM_LIMBS]_2) = EcPoint([rs1:3*C::Fp::NUM_LIMBS]_2) + EcPoint([rs2:3*C::Fp::NUM_LIMBS]_2)`. Uses complete formula that handles all cases including identity points (Z=0) and equal points.                                                                                                                                                                                                                                                                                                                                                                  |
| sw_double_proj\<C\>  | R   | 0101011     | 001    | `idx*8+1` | `EcPoint([rd:3*C::Fp::NUM_LIMBS]_2) = 2 * EcPoint([rs1:3*C::Fp::NUM_LIMBS]_2)`. Uses complete formula that handles identity point (Z=0) and order-2 points (Y=0). `rs2` is unused and must be set to `x0`.                                                                                                                                                                                                                                                                                                                                                                                            |
| sw_setup\<C\>      | R   | 0101011     | 001    | `idx*8+2` | If `ind(rs2) != 0`, then this instruction is setup for `sw_add_proj`. Otherwise it is setup for `sw_double_proj`. Checks `assert([rs1: 3*C::Fp::NUM_LIMBS]_2 == [C::Fp::MODULUS, C::A, C::B])`. For the sake of implementation convenience it also writes an unconstrained value into `[rd: 3*C::Fp::NUM_LIMBS]_2`. |
...
| RISC-V Inst                | FMT | opcode[6:0] | funct3 | funct7       | RISC-V description and notes                                                                                                                                                                                                                                   |
| -------------------------- | --- | ----------- | ------ | ------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| hint_final_exp             | R   | 0101011     | 011    | `idx*16`     | `rs1` points to `(p_ptr, p_len): (u32, u32)` and `rs2` points to `(q_ptr, q_len): (u32, u32)` in memory. Reads `P` as `p_len` `G1Affine` elements from `[p_ptr..]_2` and `Q` as `q_len` `G2Affine` elements from `[q_ptr..]_2`. Computes `f: Fp12 = multi_miller_loop(P, Q)` and resets the hint stream to `(residue_witness, scaling_factor): (Fp12, Fp12)` flattened into bytes. `rd` should be `x0`. Requires `p_len = q_len`. |
