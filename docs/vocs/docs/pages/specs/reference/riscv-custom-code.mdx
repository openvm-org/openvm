...
| RISC-V Inst | FMT | opcode[6:0] | funct3 | imm[0:11] | RISC-V description and notes                                                                                                                                               |
| ----------- | --- | ----------- | ------ | --------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| hintstored  | I   | 0001011     | 001    | 0x0       | Stores next 8-byte dword from hint stream in user memory at `[rd]_2`. The address `rd` must be aligned to an 8-byte boundary. |
| hintbuffer  | I   | 0001011     | 001    | 0x1       | Stores next `8 * rs1` bytes from hint stream in user memory at `[rd..rd + 8 * rs1]_2`. Only valid if next `8 * rs1` values in hint stream are bytes, `rs1` is non-zero and at most `1023`, and `rd` is aligned to an 8-byte boundary. |
