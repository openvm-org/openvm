...
The following are _not_ intrinsic instructions, but custom RISC-V instructions used to frame the insertion of custom kernel code. They are not meant to be used alone. See the [transpiler](../reference/transpiler#openvm-kernel-code-transpilation) for more details.
...
**Note:** The output for the first 4 instructions is not guaranteed to be less than `N`. See the [ISA specification](../openvm/isa#algebra-extension) for more details.
