...
| hintbuffer  | I   | 0001011     | 001    | 0x1       | Stores next `4 * rs1` bytes from hint stream in user memory at `[rd..rd + 4 * rs1]_2`. Only valid if next `4 * rs1` values in hint stream are bytes and `rs1` is non-zero and at most `1023`. The address `rd` does not have any alignment requirements. |
...
| hintinput   | I   | 0001011     | 011    | 0x0       | Pop next vector from input stream and reset hint stream to `[(len as u32).to_le_bytes(), vector, zero padding to 4-byte alignment]`, where `len` is the vector length in bytes. |
...
| RISC-V Inst     | FMT | opcode[6:0] | funct3 | funct7    | RISC-V description and notes                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| --------------- | --- | ----------- | ------ | --------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| sw_add_proj\<C\>  | R   | 0101011     | 001    | `idx*8`   | `EcPoint([rd:3*C::COORD_SIZE]_2) = EcPoint([rs1:3*C::COORD_SIZE]_2) + EcPoint([rs2:3*C::COORD_SIZE]_2)`. Uses complete formula that handles all cases including identity points (Z=0) and equal points.                                                                                                                                                                                                                                                                                                                                                                  |
| sw_double_proj\<C\>  | R   | 0101011     | 001    | `idx*8+1` | `EcPoint([rd:3*C::COORD_SIZE]_2) = 2 * EcPoint([rs1:3*C::COORD_SIZE]_2)`. Uses complete formula that handles identity point (Z=0) and order-2 points (Y=0). `rs2` is unused and must be set to `x0`.                                                                                                                                                                                                                                                                                                                                                                                            |
| sw_setup\<C\>      | R   | 0101011     | 001    | `idx*8+2` | If `ind(rs2) != 0`, then this instruction is setup for `sw_add_proj`. Otherwise it is setup for `sw_double_proj`. Checks `assert([rs1: 3*C::COORD_SIZE]_2 == [C::MODULUS, C::A, C::B])`. For the sake of implementation convenience it also writes an unconstrained value into `[rd: 3*C::COORD_SIZE]_2`. |
