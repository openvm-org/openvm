...
:::info
The guest target is a [Tier 3](https://doc.rust-lang.org/beta/rustc/target-tier-policy.html#tier-3-target-policy) target built into the openvm-org/rust fork. The toolchain ships prebuilt `std`/`core`/`alloc`/`panic_abort` rlibs for the target, so guest builds do not need `-Z build-std`. Install it via `cargo openvm toolchain install`.
:::
