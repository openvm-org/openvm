...
OpenVM supports a Rust frontend via compilation to a 64-bit RISC-V target which is
then transpiled into an OpenVM executable with instructions from the RV64IM VM extension.
...
- **Host**: We support `aarch64-apple-darwin` and `x86_64-unknown-linux-gnu` with Rust 1.91.1.
For reproducible builds, we recommend using the `x86_64-unknown-linux-gnu` platform.
- **Guest**: `riscv64im-unknown-openvm-elf` with the prebuilt `openvm-1.97.1` (stable) toolchain from the [openvm-org/rust](https://github.com/openvm-org/rust) fork. Published nightly tarballs (`openvm-nightly-YYYY-MM-DD`) are available for guest code that requires nightly Rust features.

:::info
The guest target is a [Tier 3](https://doc.rust-lang.org/beta/rustc/target-tier-policy.html#tier-3-target-policy) target built into the openvm-org/rust fork. The toolchain ships prebuilt `std`/`core`/`alloc`/`panic_abort` rlibs for the target. Install it via `cargo openvm toolchain install`.
:::
...
```
This function always returns `0`, which is equivalent to `std::env::var_os` always returning `Some("")`.
