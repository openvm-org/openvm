...
- **Guest**: `riscv64im-unknown-openvm-elf` with the prebuilt `openvm-1.94.0` (stable) toolchain from the [openvm-org/rust](https://github.com/openvm-org/rust) fork. Published nightly tarballs (`openvm-nightly-YYYY-MM-DD`) are available for guest code that requires nightly Rust features.
