...
Public-values writes use the separate [`REVEAL_RV64`](#rv64-io) instruction.
...
| REVEAL_RV64      | `a,b,c,1,3,1,g` | Dedicated instruction that writes 8 bytes from register `a` to the 8-byte-aligned address `b + sign_extend(c)` in the user IO address space `3`.                                |
