...
| `PC_BITS`                    | The number of bits in the byte program counter.                    | Fixed to 32.                                                         |
...
Program code is a map from 4-byte-aligned addresses in `[0, 2^PC_BITS)` to the space of instructions
...
There is a single special purpose register `pc` for the 32-bit byte program counter. In the proof
system it is represented by two little-endian 16-bit limbs rather than one element of `F`. During
execution, the program counter must be a 4-byte-aligned address where program code is defined.
...
- `hint_stream`: a queue of bytes populated during runtime execution
  via [phantom sub-instructions](#phantom-sub-instructions) such as `HintInput`.
...
| STOREW_RV64  | `a,b,c,1,e,1,g` | `[r64{c,g}(b):4]_e <- [a:4]_1`                                                                                                                                                      |
...
| JALR_RV64  | `a,b,c,1,_,f,g` | `if(f!=0) [a:8]_1 = zero_extend(decompose(pc+4)); pc = (u32([b:8]_1) + sign_extend(decompose(c)[0:2], g) as u32) & !1`. The resulting target must be 4-byte aligned. Here `u32(c)` must be in `[0, 2^16)`. The operands `f` and `g` are assumed to be boolean. The `pc` assignment is always done regardless of `f`'s value. |
...
For branch and JAL_RV64 instructions, the instructions assume that operand `i32(c)` is in
`[-2^24,2^24)`. The byte-address result must remain in the 32-bit address space, be 4-byte
aligned, and identify an instruction in program ROM. The circuit obtains that 32-bit identity
from the two-limb program lookup rather than requiring the byte pc to fit in one base-field
element.

For JALR_RV64, we treat `c` in `[0, 2^16)` as a raw encoding of 16-bits.
The operand `g` must be a boolean. We let `sign_extend(decompose(c)[0:2], g)` denote the `i32` defined by first taking
the unsigned integer encoding of `c` as 16 bits, then sign extending it to 32 bits using the sign bit `g`, and considering the 32 bits as the 2's complement of an `i32`. Then it is added to the register value `u32([b:8]_1)` (the upper 4 bytes of the 64-bit register must be zero), where 32-bit overflow is ignored. RISC-V clears bit 0 of the result, and OpenVM additionally requires the resulting instruction address to be 4-byte aligned.
...
[Word-Width ALU](#word-width-alu) section.
...
| Name             | Operands        | Description                                                                                                                                                                       |
| ---------------- | --------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| HINT_STORED_RV64 | `_,b,_,1,2`     | `[r64{0}(b):8]_2 = next 8 bytes from hint stream`. Only valid if next 8 values in hint stream are bytes and `r64{0}(b)` is aligned to an 8-byte boundary.                                                                          |
| HINT_BUFFER_RV64 | `a,b,_,1,2`     | `[r64{0}(b):8 * l]_2 = next 8 * l bytes from hint stream` where `l = r64{0}(a)`. Only valid if next `8 * l` values in hint stream are bytes, `l` is non-zero and `<=` `MAX_HINT_BUFFER_DWORDS` (1,023 dwords ≈ 8KB), and `r64{0}(b)` is aligned to an 8-byte boundary. |
| REVEAL_RV64      | `a,b,c,1,3,1,g` | `[r64{c,g}(b):8]_3 = [a:8]_1`. The effective address must be 8-byte aligned.                                                                                                        |
...
| Name              | Discriminant | Operands | Description                                                                                                                                                                                                                                                    |
|-------------------| ------------ | -------- |----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| HintInput     | 0x20         | `_`      | Pops a byte vector `hint` from the input stream and resets the hint stream to equal `[(hint.len() as u64).to_le_bytes(), hint, zero padding to 8-byte alignment].concat()`.                                                                       |
| PrintStr      | 0x21         | `a,b,_`  | Peeks at `[r64{0}(a)..r64{0}(a) + r64{0}(b)]_2`, tries to convert to byte array and then UTF-8 string and prints to host stdout. Prints error message if conversion fails. Does not change any VM state.                                                       |
| HintRandom    | 0x22         | `a,_,_`  | Resets the hint stream to `8 * r64{0}(a)` random bytes. The source of randomness is deterministic using a fixed-seed RNG (`rand::rngs::StdRng`). Its result is not constrained in any way.                                                                     |
...
Each deferral instruction encodes a **deferral index** (`def_idx`) identifying which deferred function to invoke. The index and sub-opcode are packed into a 12-bit immediate as `[def_idx(10 bits) | opcode(2 bits)]`. The continuations deferral aggregation circuit currently constrains the semantic range to `def_idx < MAX_DEF_CIRCUITS`, so a VM configuration can support up to `MAX_DEF_CIRCUITS` deferral circuits.
...
| HintSqrt\<N\>   | 0x51         | `a,_,c_upper` | Use `c_upper` to determine the index of the modulus from the list of supported moduli. Read from memory `x = [r64{0}(a): N::NUM_LIMBS]_2`.  If `x` is a quadratic residue modulo `N`, reset the hint stream to `[1u8, 0u8, 0u8, 0u8, 0u8, 0u8, 0u8, 0u8]` followed by a square root of `x`.  If `x` is not a quadratic residue, reset the hint stream to `[0u8; 8]` followed by a square root of `x * non_qr`, where `non_qr` is the quadratic nonresidue returned by `HintNonQr<N>`. |
