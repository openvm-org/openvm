...
| Name                 | Operands    | Description                                                                                                                                                                                                                                                                                    |
| -------------------- | ----------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| TE_ADD\<C\>       | `a,b,c,1,2` | Set `r32_te_point(a) = r32_te_point(b) + r32_te_point(c)` (curve addition). Assumes that `r32_te_point(b), r32_te_point(c)` both lie on the curve. 
| SETUP_TE_ADD\<C\> | `a,b,c,1,2` | `assert(r32_te_point(b).x == C::MODULUS && r32_te_point(b).y == C::A && r32_te_point(c).x == C::D)` in the chip for TE ADD. For the sake of implementation convenience it also writes something (can be anything) into `[r32{0}(a): 2*C::COORD_SIZE]_2`.   |
