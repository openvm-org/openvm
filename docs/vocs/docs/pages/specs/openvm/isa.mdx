...
A projective curve point `EcPoint(X, Y, Z)` is a triple of `X, Y, Z` where each element is an array of `C::COORD_SIZE` elements each
...
represented as `3 * C::COORD_SIZE` contiguous cells in memory.

We use the following notation below:

```
r32_projective_point(a) -> EcPoint {
    let X = [r32{0}(a): C::COORD_SIZE]_2;
    let Y = [r32{0}(a) + C::COORD_SIZE: C::COORD_SIZE]_2;
    let Z = [r32{0}(a) + 2*C::COORD_SIZE: C::COORD_SIZE]_2;
...
| Name                         | Operands    | Description                                                                                                                                                                                                                                                                                    |
| ---------------------------- | ----------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| SW_EC_ADD_PROJ\<C\>          | `a,b,c,1,2` | Set `r32_projective_point(a) = r32_projective_point(b) + r32_projective_point(c)` (curve addition). Assumes that inputs lie on the curve. The complete formula handles all cases including identity points (Z=0) and equal points.           |
| SETUP_SW_EC_ADD_PROJ\<C\>    | `a,b,c,1,2` | `assert(r32_projective_point(b).X == C::MODULUS && r32_projective_point(b).Y == C::A && r32_projective_point(b).Z == C::B)` in the chip for EC ADD. For the sake of implementation convenience it also writes something (can be anything) into `[r32{0}(a): 3*C::COORD_SIZE]_2`.   |
| SW_EC_DOUBLE_PROJ\<C\>       | `a,b,_,1,2` | Set `r32_projective_point(a) = 2 * r32_projective_point(b)`. This doubles the input point. Assumes that input lies on the curve. The formula handles identity point (Z=0) and order-2 points (Y=0).                                                                                                                                     |
| SETUP_SW_EC_DOUBLE_PROJ\<C\> | `a,b,_,1,2` | `assert(r32_projective_point(b).X == C::MODULUS && r32_projective_point(b).Y == C::A && r32_projective_point(b).Z == C::B)` in the chip for EC DOUBLE. For the sake of implementation convenience it also writes something (can be anything) into `[r32{0}(a): 3*C::COORD_SIZE]_2`. |
