...
| NativeHintInput | 0x11         | `_`           | Pops a vector `hint` of field elements from `input_stream` and sets `hint_stream` to equal the vector `[[F::from_usize(hint.len())], hint].concat()`. The hint stream must previously be empty.                            |
...
All memory accesses use block size `4`.
