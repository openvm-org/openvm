...
| `MAX_HINT_BUFFER_BITS` | The maximum number of bits for hint buffer word count.          | Fixed to 18.                                                                                        |
| `MAX_HINT_BUFFER_WORDS` | The maximum number of words per `HINT_BUFFER_RV32` instruction. | Fixed to `2^MAX_HINT_BUFFER_BITS - 1` = 262,143 words (≈1MB).                                       |
...
| HINT_BUFFER_RV32 | `a,b,_,1,2`     | `[r32{0}(b):4 * l]_2 = next 4 * l bytes from hint stream` where `l = r32{0}(a)`. Only valid if next `4 * l` values in hint stream are bytes. **Constraints**: `l` must be non-zero and <= `MAX_HINT_BUFFER_WORDS` (262,143 words ≈ 1MB). This bound is enforced by both the executor and AIR constraints. The SDK's `hint_buffer_chunked` function automatically splits larger reads into multiple `HINT_BUFFER_RV32` instructions. The pointer address `r32{0}(b)` does not need to be a multiple of `4`. |
