...
| JALR_RV32  | `a,b,c,1,_,f,g` | `if(f!=0) [a:4]_1 = decompose(pc+4); pc = F::from_u32((i32([b:4]_1) + sign_extend(decompose(c)[0:2], g) as u32) & !1)`. Constrains that `i32([b:4]_1) + sign_extend(decompose(c)[0:2], g) is in [0, 2^PC_BITS)`. Here `u32(c)` must be in `[0, 2^16)`. The operands `f` and `g` are assumed to be boolean. The `pc` assignment is always done regardless of `f`'s value. |
...
result is then cast to `u32`, has its least significant bit set to `0`, and is then cast to `F` and assigned to `pc`.
