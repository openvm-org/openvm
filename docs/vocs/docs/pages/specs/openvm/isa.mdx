...
- [RV32IM/RV64IM](#rv32imrv64im-extensions): Extensions supporting the 32-bit and 64-bit RISC-V ISA with multiplication.
...
| `XLEN`              | The width of an integer register in bits.                          | 32 for RV32IM, 64 for RV64IM.                                                                       |
| `LIMBS`             | The number of 8-bit limbs per integer register (`XLEN / 8`).       | 4 for RV32IM, 8 for RV64IM.                                                                         |
...
- Valid pointers are field elements that lie in `[0, 2^pointer_max_bits)`, for
  ...
  panic.
- For the register address space (address space `1`), valid pointers lie in `[0, 32*LIMBS)`, corresponding to 32 registers with `LIMBS` bytes each.
...
- `uXLEN(x)` where `x: [F; LIMBS]` consists of LIMBS bytes will mean the casting from little-endian bytes in 2's complement to
  unsigned XLEN-bit integer.
- `iXLEN(x)` where `x: [F; LIMBS]` consists of LIMBS bytes will mean the casting from little-endian bytes in 2's complement to
  signed XLEN-bit integer.
- `sign_extend` means sign extension of bits in 2's complement.
- `iXLEN(c)` where `c` is a field element will mean interpreting `c` as a signed integer in `F`, i.e.,
  if `c.as_canonical_u32() < F::modulus() / 2` then `iXLEN(c) = c.as_canonical_u32() as iXLEN` else
  `iXLEN(c) = c.as_canonical_u32() - F::modulus() as iXLEN`.
- `decompose(c)` where `c` is a field element means `(c.as_canonical_u32() as uXLEN).to_le_bytes()`, i.e., LIMBS little-endian bytes (zero-extended).
- `reg{0}(a) := iXLEN([a:LIMBS]_1)` means casting the value at `[a:LIMBS]_1` to `iXLEN`.
...
### RV32IM/RV64IM Extensions

Due to the overlap between RV32IM and RV64IM instruction sets, the RV32IM and RV64IM extensions will be described together.

The RV32IM/RV64IM extensions introduce OpenVM opcodes which support 32-bit/64-bit RISC-V via transpilation from a standard RV32IM/RV64IM ELF
binary, specified [here](../reference/riscv-custom-code). These consist of opcodes corresponding 1-1 with RV32IM/RV64IM opcodes, as well as
additional user IO opcodes and phantom sub-instructions to support input and debug printing on the host. 
We denote the OpenVM opcode corresponding to a RV32IM opcode by appending `_RV32`.
We denote the OpenVM opcode corresponding to a RV64IM opcode by appending `_RV64`.

The RV32IM/RV64IM extensions use address space `0` for immediates, address space `1` for registers, and address space `2` for
...
The `i`th RISC-V register is represented by the block `[LIMBS * i:LIMBS]_1` of `LIMBS` limbs in address space `1`. 
All memory addresses in address space `1` behave uniformly, and in particular writes to the block `[0:LIMBS]_1` corresponding to the
RISC-V register `x0` are allowed in the RV32IM/RV64IM extensions. However, as detailed in 
[RV32IM/RV64IM Transpilation](/specs/reference/transpiler#rv32imrv64im-extension), any OpenVM program transpiled from a RV32IM/RV64IM ELF will
never contain such a write and conforms to the RV32IM/RV64IM specification.
...
unsigned integer, and convert to field element. In the instruction descriptions below, `[c:LIMBS]_0` should be interpreted 
as `c_i16 as iXLEN` sign extended to XLEN bits.

| Name                | Operands    | Description                                                                                                |
| ------------------- | ----------- | ---------------------------------------------------------------------------------------------------------- |
| ADD_RV32/ADD_RV64   | `a,b,c,1,e` | `[a:LIMBS]_1 = [b:LIMBS]_1 + [c:LIMBS]_e`. Overflow is ignored and the lower XLEN bits are written to the destination. |
| ADDW_RV64           | `a,b,c,1,e` | `[a:8]_1 = sign_extend([b:4]_1 + [c:4]_e)`. RV64 only. Adds lower 32 bits, sign-extends result to 64 bits. |
| SUB_RV32/SUB_RV64   | `a,b,c,1,e` | `[a:LIMBS]_1 = [b:LIMBS]_1 - [c:LIMBS]_e`. Overflow is ignored and the lower XLEN bits are written to the destination. |
| SUBW_RV64           | `a,b,c,1,e` | `[a:8]_1 = sign_extend([b:4]_1 - [c:4]_e)`. RV64 only. Subtracts lower 32 bits, sign-extends result to 64 bits. |
| XOR_RV32/XOR_RV64   | `a,b,c,1,e` | `[a:LIMBS]_1 = [b:LIMBS]_1 ^ [c:LIMBS]_e`                                                                  |
| OR_RV32/OR_RV64     | `a,b,c,1,e` | `[a:LIMBS]_1 = [b:LIMBS]_1 \| [c:LIMBS]_e`                                                                 |
| AND_RV32/AND_RV64   | `a,b,c,1,e` | `[a:LIMBS]_1 = [b:LIMBS]_1 & [c:LIMBS]_e`                                                                  |
| SLL_RV32/SLL_RV64   | `a,b,c,1,e` | `[a:LIMBS]_1 = [b:LIMBS]_1 << [c:LIMBS]_e`                                                                 |
| SLLW_RV64           | `a,b,c,1,e` | `[a:8]_1 = sign_extend([b:4]_1 << [c:4]_e)`. RV64 only. Shifts lower 32 bits left, sign-extends to 64 bits. |
| SRL_RV32/SRL_RV64   | `a,b,c,1,e` | `[a:LIMBS]_1 = [b:LIMBS]_1 >> [c:LIMBS]_e`. Shifts XLEN bits right, sign-extends to XLEN bits.  |
| SRLW_RV64           | `a,b,c,1,e` | `[a:8]_1 = sign_extend([b:4]_1 >> [c:4]_e)`. RV64 only. Shifts lower 32 bits right, sign-extends to 64 bits. |
| SRA_RV32/SRA_RV64   | `a,b,c,1,e` | `[a:LIMBS]_1 = [b:LIMBS]_1 >> [c:LIMBS]_e`. Shifts XLEN bits right, most significant bit extends.  |                                                    |
| SRAW_RV64           | `a,b,c,1,e` | `[a:8]_1 = sign_extend([b:4]_1 >> [c:4]_e)`. RV64 only. Shifts lower 32 bits right, most significant bit extends to 64 bits. |
| SLT_RV32/SLT_RV64   | `a,b,c,1,e` | `[a:LIMBS]_1 = iXLEN([b:LIMBS]_1) < iXLEN([c:LIMBS]_e) ? 1 : 0`                                            |
| SLTU_RV32/SLTU_RV64 | `a,b,c,1,e` | `[a:LIMBS]_1 = uXLEN([b:LIMBS]_1) < uXLEN([c:LIMBS]_e) ? 1 : 0`                                            |
...
The operand `g` must be a boolean. We let `sign_extend(decompose(c)[0:2], g)` denote the `iXLEN` defined by first taking
the unsigned integer encoding of `c` as 16 bits, then sign extending it to XLEN bits using the sign bit `g`, and considering
the XLEN bits as the 2's complement of an `iXLEN`.
We will use shorthand `reg{c,g}(b) := iXLEN([b:4]_1) + sign_extend(decompose(c)[0:2], g)` as `iXLEN`. This means performing
signed addition with the value of the register `[b:4]_1`. For consistency with other notation,
we define the shorthand `reg{c}(b)` to mean `reg{c,g}(b)` where `g` is set to the most significant bit of `c`.
...
The data size is `1` for LOADB/STOREB, `2` for LOADH/STOREH, `4` for LOADW/STOREW, and `8` for LOADD_RV64/STORED_RV64.

All load/store instructions always do block accesses of block size `LIMBS`.

| Name                        | Operands        | Description                                                                                                                                                                         |
| --------------------------- | --------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| LOADB_RV32/LOADB_RV64       | `a,b,c,1,e,f,g` | `if(f!=0) [a:LIMBS]_1 = sign_extend([reg{c,g}(b):1]_e)` The operand `f` is assumed to be boolean. Must sign-extend the byte read from memory, which is represented in 2's complement.   |
| LOADH_RV32/LOADH_RV64       | `a,b,c,1,e,f,g` | `if(f!=0) [a:LIMBS]_1 = sign_extend([reg{c,g}(b):2]_e)` The operand `f` is assumed to be boolean. Must sign-extend the number read from memory, which is represented in 2's complement. |
| LOADW_RV32/LOADW_RV64       | `a,b,c,1,e,f,g` | `if(f!=0) [a:LIMBS]_1 = sign_extend([reg{c,g}(b):4]_e)` The operand `f` is assumed to be boolean. For RV64, sign-extends the 32-bit value to 64 bits.                               |
| LOADD_RV64                  | `a,b,c,1,e,f,g` | `if(f!=0) [a:8]_1 = [reg{c,g}(b):8]_e` RV64 only. The operand `f` is assumed to be boolean.                                                                                         |
| LOADBU_RV32/LOADBU_RV64     | `a,b,c,1,e,f,g` | `if(f!=0) [a:LIMBS]_1 = zero_extend([reg{c,g}(b):1]_e)` The operand `f` is assumed to be boolean. Must zero-extend the byte read from memory.                                     |
| LOADHU_RV32/LOADHU_RV64     | `a,b,c,1,e,f,g` | `if(f!=0) [a:LIMBS]_1 = zero_extend([reg{c,g}(b):2]_e)` The operand `f` is assumed to be boolean. Must zero-extend the number read from memory.                                     |
| LOADWU_RV64                 | `a,b,c,1,e,f,g` | `if(f!=0) [a:8]_1 = zero_extend([reg{c,g}(b):4]_e)` RV64 only. The operand `f` is assumed to be boolean. Zero-extends the 32-bit value to 64 bits.                                  |
| STOREB_RV32/STOREB_RV64     | `a,b,c,1,e,1,g` | `[reg{c,g}(b):1]_e <- [a:1]_1`                                                                                                                                                      |
| STOREH_RV32/STOREH_RV64     | `a,b,c,1,e,1,g` | `[reg{c,g}(b):2]_e <- [a:2]_1`                                                                                                                                                      |
| STOREW_RV32/STOREW_RV64     | `a,b,c,1,e,1,g` | `[reg{c,g}(b):4]_e <- [a:4]_1`                                                                                                                                                      |
| STORED_RV64                 | `a,b,c,1,e,1,g` | `[reg{c,g}(b):8]_e <- [a:8]_1` RV64 only.                                                                                                                                           |
...
| Name                | Operands         | Description                                                                                                                                                                                                                                                                                                                                                                 |
| ------------------- | ---------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| BEQ_RV32/BEQ_RV64   | `a,b,c,1,1`      | `if([a:LIMBS]_1 == [b:LIMBS]_1) pc += c`                                                                                                                                                                                                                                                                                                                                    |
| BNE_RV32/BNE_RV64   | `a,b,c,1,1`      | `if([a:LIMBS]_1 != [b:LIMBS]_1) pc += c`                                                                                                                                                                                                                                                                                                                                    |
| BLT_RV32/BLT_RV64   | `a,b,c,1,1`      | `if(iXLEN([a:LIMBS]_1) < iXLEN([b:LIMBS]_1)) pc += c`                                                                                                                                                                                                                                                                                                                       |
| BGE_RV32/BGE_RV64   | `a,b,c,1,1`      | `if(iXLEN([a:LIMBS]_1) >= iXLEN([b:LIMBS]_1)) pc += c`                                                                                                                                                                                                                                                                                                                      |
| BLTU_RV32/BLTU_RV64 | `a,b,c,1,1`      | `if(uXLEN([a:LIMBS]_1) < uXLEN([b:LIMBS]_1)) pc += c`                                                                                                                                                                                                                                                                                                                       |
| BGEU_RV32/BGEU_RV64 | `a,b,c,1,1`      | `if(uXLEN([a:LIMBS]_1) >= uXLEN([b:LIMBS]_1)) pc += c`                                                                                                                                                                                                                                                                                                                      |
| JAL_RV32/JAL_RV64   | `a,_,c,1,_,f`    | `if(f!=0) [a:LIMBS]_1 = decompose(pc+DEFAULT_PC_STEP); pc += c`. The operand `f` is assumed to be boolean. The `pc` increment is always done regardless of `f`'s value. Here `iXLEN(c)` must be in `[-2^24, 2^24)`.                                                                                                                                                         |
| JALR_RV32/JALR_RV64 | `a,b,c,1,_,f,g`  | `if(f!=0) [a:LIMBS]_1 = decompose(pc+DEFAULT_PC_STEP); pc = F::from_canonical_u32(iXLEN([b:LIMBS]_1 as u32) + sign_extend(decompose(c)[0:2], g) as uXLEN)`. Constrains that `iXLEN([b:LIMBS]_1) + sign_extend(decompose(c)[0:2], g)` is in `[0, 2^PC_BITS)`. Here `uXLEN(c)` must be in `[0, 2^16)`. The operands `f` and `g` are assumed to be boolean. The `pc` assignment is always done regardless of `f`'s value. |
| LUI_RV32/LUI_RV64   | `a,_,c,1,_,1`    | `[a:LIMBS]_1 = uXLEN(c) << 12`. Here `uXLEN(c)` must be in `[0, 2^20)`. The immediate is sign-extended.                                                                                                                                                                                                                                                                     |
| AUIPC_RV32/AUIPC_RV64 | `a,_,c,1,_,_`  | `[a:LIMBS]_1 = decompose(pc) + (decompose(iXLEN(c)) << 8)`. Here `iXLEN(c)` must be in `[-2^23, 2^23)`. The immediate is sign-extended, and the addition is performed as wrapping `iXLEN` arithmetic.                                                                                                                                                                                  |

For branch and JAL instructions, the instructions assume that the operand `iXLEN(c)` is in `[-2^24,2^24)`. The
...
`2^PC_BITS + 2*2^24 < F::modulus()` so `to_pc = from_pc + c` is only valid if `iXLEN(from_pc) + iXLEN(c)` is in
`[0, 2^PC_BITS)`.

For JALR, we treat `c` in `[0, 2^16)` as a raw encoding of 16-bits.
The operand `g` must be a boolean. We let `sign_extend(decompose(c)[0:2], g)` denote the `iXLEN` defined by first taking
the unsigned integer encoding of `c` as 16 bits, then sign extending it to XLEN bits using the sign bit `g`, and considering the XLEN bits as the 2's complement of an `iXLEN`. 
Then it is added to the register value `iXLEN([b:LIMBS]_1)`, where XLEN-bit overflow is ignored. 
The instruction is only valid if the resulting `iXLEN` is in range `[0, 2^PC_BITS)`.
The result is then cast to `uXLEN` and then to `F` and assigned to `pc`.

For LUI, we are treating `c` in `[0, 2^20)` as a raw encoding of 20-bits.
For AUIPC, we are treating `c` as a signed 24-bit value in `[-2^23, 2^23)`. The shift `iXLEN(c) << 8`
places the upper 20 bits of the immediate at bits 12-31, and the immediate is sign-extended before addition. The
addition with `pc` is treated as unchecked `iXLEN` addition.

Note that AUIPC does not have any condition for the register write.
...
destination cells. MULH, MULHU, and MULHSU perform the same multiplication but return
...
and REMU provide the remainder of the corresponding division operation. Integer division is defined by
`dividend = q * divisor + r` where `0 <= |r| < |divisor|` and either `sign(r) = sign(dividend)` or `r = 0`.

RV64M also includes word variants (MULW, DIVW, DIVUW, REMW, REMUW) that operate on the lower 32 bits and sign-extend the result to 64 bits.

Below `x[n:m]` denotes the bits from `n` to `m` inclusive of `x`.

| Name                  | Operands  | Description                                                                                                                                                                                                              |
| --------------------- | --------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| MUL_RV32/MUL_RV64     | `a,b,c,1` | `[a:LIMBS]_1 = ([b:LIMBS]_1 * [c:LIMBS]_1)[0:LIMBS-1]`                                                                                                                                                                   |
| MULH_RV32/MULH_RV64   | `a,b,c,1` | `[a:LIMBS]_1 = (sign_extend([b:LIMBS]_1) * sign_extend([c:LIMBS]_1))[LIMBS:2*LIMBS-1]`. We sign extend `b` and `c` into 2×LIMBS-limb (i.e. 2×XLEN-bit) integers                                                          |
| MULHSU_RV32/MULHSU_RV64 | `a,b,c,1` | `[a:LIMBS]_1 = (sign_extend([b:LIMBS]_1) * zero_extend([c:LIMBS]_1))[LIMBS:2*LIMBS-1]`                                                                                                                                  |
| MULHU_RV32/MULHU_RV64 | `a,b,c,1` | `[a:LIMBS]_1 = (zero_extend([b:LIMBS]_1) * zero_extend([c:LIMBS]_1))[LIMBS:2*LIMBS-1]`                                                                                                                                   |
| MULW_RV64             | `a,b,c,1` | `[a:LIMBS]_1 = sign_extend(([b:4]_1 * [c:4]_1)[0:3])`. RV64 only. Multiplies lower 32 bits and sign-extends result to 64 bits.                                                                                           |
| DIV_RV32/DIV_RV64     | `a,b,c,1` | `[a:LIMBS]_1 = [b:LIMBS]_1 / [c:LIMBS]_1` integer division (signed). Division by zero: if `iXLEN([c:LIMBS]_1) = 0`, set `iXLEN([a:LIMBS]_1) = -1`. Overflow: if `iXLEN([b:LIMBS]_1) = -2^(XLEN-1)` and `iXLEN([c:LIMBS]_1) = -1`, set `iXLEN([a:LIMBS]_1) = -2^(XLEN-1)`. |
| DIVU_RV32/DIVU_RV64   | `a,b,c,1` | `[a:LIMBS]_1 = [b:LIMBS]_1 / [c:LIMBS]_1` integer division (unsigned). Division by zero: if `uXLEN([c:LIMBS]_1) = 0`, set `uXLEN([a:LIMBS]_1) = 2^XLEN - 1`.                                                                        |
| DIVW_RV64             | `a,b,c,1` | `[a:LIMBS]_1 = sign_extend([b:4]_1 / [c:4]_1)`. RV64 only. Divides lower 32 bits (signed) and sign-extends result to 64 bits.                                                                                            |
| DIVUW_RV64            | `a,b,c,1` | `[a:LIMBS]_1 = sign_extend([b:4]_1 / [c:4]_1)`. RV64 only. Divides lower 32 bits (unsigned) and sign-extends result to 64 bits.                                                                                          |
| REM_RV32/REM_RV64     | `a,b,c,1` | `[a:LIMBS]_1 = [b:LIMBS]_1 % [c:LIMBS]_1` integer remainder (signed). Division by zero: if `iXLEN([c:LIMBS]_1) = 0`, set `[a:LIMBS]_1 = [b:LIMBS]_1`. Overflow: if `iXLEN([b:LIMBS]_1) = -2^(XLEN-1)` and `iXLEN([c:LIMBS]_1) = -1`, set `[a:LIMBS]_1 = 0`. |
| REMU_RV32/REMU_RV64   | `a,b,c,1` | `[a:LIMBS]_1 = [b:LIMBS]_1 % [c:LIMBS]_1` integer remainder (unsigned). Division by zero: if `uXLEN([c:LIMBS]_1) = 0`, set `[a:LIMBS]_1 = [b:LIMBS]_1`.                                                                             |
| REMW_RV64             | `a,b,c,1` | `[a:LIMBS]_1 = sign_extend([b:4]_1 % [c:4]_1)`. RV64 only. Remainder of lower 32 bits (signed) and sign-extends result to 64 bits.                                                                                       |
| REMUW_RV64            | `a,b,c,1` | `[a:LIMBS]_1 = sign_extend([b:4]_1 % [c:4]_1)`. RV64 only. Remainder of lower 32 bits (unsigned) and sign-extends result to 64 bits.                                                                                     |

#### User IO

In addition to opcodes which match 1-1 with the RV32IM/RV64IM opcodes, the following additional
opcodes interact with address spaces outside of 1 and 2 to enable verification of programs
with user input-output.

| Name                               | Operands        | Description                                                                                                                                                                       |
| ---------------------------------- | --------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| HINT_STOREW_RV32/HINT_STORED_RV64  | `_,b,_,1,2`     | `[reg{0}(b):LIMBS]_2 = next LIMBS bytes from hint stream`. Only valid if next LIMBS values in hint stream are bytes.                                                            |
| HINT_BUFFER_RV32/HINT_BUFFER_RV64  | `a,b,_,1,2`     | `[reg{0}(b):LIMBS * l]_2 = next LIMBS * l bytes from hint stream` where `l = reg{0}(a)`. Only valid if next `LIMBS * l` values in hint stream are bytes. `l` must be non-zero and <= `MAX_HINT_BUFFER_WORDS` (1,023 words ≈ 4KB). The pointer address `reg{0}(b)` does not need to be a multiple of `LIMBS`. |
| REVEAL_RV32/REVEAL_RV64            | `a,b,c,1,3,_,g` | Pseudo-instruction for `STOREW_RV32/STORED_RV64 a,b,c,1,3,_,g` writing to the user IO address space `3`. Only valid when continuations are enabled.                               |
...
The RV32IM/RV64IM extension defines the following phantom sub-instructions.

| Name                                | Discriminant | Operands | Description                                                                                                                                            |
| ----------------------------------- | ------------ | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Rv32HintInput/Rv64HintInput         | 0x20         | `_`      | Pops a vector `hint` of field elements from the input stream and resets the hint stream to equal the vector `[(hint.len() as u32).to_le_bytes()), hint].concat()`. |
| Rv32PrintStr/Rv64PrintStr           | 0x21         | `a,b,_`  | Peeks at `[reg{0}(a):reg{0}(b)]_2`, tries to convert to byte array and then UTF-8 string and prints to host stdout. Prints error message if conversion fails. Does not change any VM state. |
| Rv32HintRandom/Rv64HintRandom       | 0x22         | `a,_,_`  | Resets the hint stream to `LIMBS * reg{0}(a)` random bytes. The source of randomness is deterministic using a fixed-seed RNG (`rand::rngs::StdRng`). Its result is not constrained in any way. |
| Rv32HintLoadByKey/Rv64HintLoadByKey | 0x23         | `a,b,_`  | Look up the value by key `[reg{0}(a):reg{0}(b)]_2` and prepend the value into `input_stream`. The logical value is `Vec<Vec<F>>`. The serialization of `Vec` follows the format `[length, <content>]`. Both length and content encoded as little-endian bytes. |
...
| Name           | Operands    | Description                                                                                                       |
| -------------- | ----------- | ----------------------------------------------------------------------------------------------------------------- |
| KECCAK256_RV32 | `a,b,c,1,2` | `[reg{0}(a):32]_2 = keccak256([reg{0}(b)..reg{0}(b)+reg{0}(c)]_2)`. Performs memory accesses with block size `4`. |
...
| Name        | Operands    | Description                                                                                                                                                              |
| ----------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| SHA256_UPDATE_RV32 | `a,b,c,1,2` | `[reg{0}(a):32]_2 = compress256([reg{0}(b):32]_2, [reg{0}(c):64]_2)`. where `compress256(state, input)` is the SHA-256 block compression function which returns the updated state. This instruction performs memory reads and writes with block size `4`. |
| SHA512_UPDATE_RV32 | `a,b,c,1,2` | `[reg{0}(a):64]_2 = compress512([reg{0}(b):64]_2, [reg{0}(c):128]_2)`. where `compress512(state, input)` is the SHA-512 block compression function which returns the updated state. This instruction performs memory reads and writes with block size `4`. |
...
| Name         | Operands    | Description                                                                                                                          |
| ------------ | ----------- | ------------------------------------------------------------------------------------------------------------------------------------ |
| ADD256_RV32  | `a,b,c,1,2` | `[reg{0}(a):32]_2 = [reg{0}(b):32]_2 + [reg{0}(c):32]_2`. Overflow is ignored and the lower 256-bits are written to the destination. |
| SUB256_RV32  | `a,b,c,1,2` | `[reg{0}(a):32]_2 = [reg{0}(b):32]_2 - [reg{0}(c):32]_2`. Overflow is ignored and the lower 256-bits are written to the destination. |
| XOR256_RV32  | `a,b,c,1,2` | `[reg{0}(a):32]_2 = [reg{0}(b):32]_2 ^ [reg{0}(c):32]_2`                                                                             |
| OR256_RV32   | `a,b,c,1,2` | `[reg{0}(a):32]_2 = [reg{0}(b):32]_2 \| [reg{0}(c):32]_2`                                                                            |
| AND256_RV32  | `a,b,c,1,2` | `[reg{0}(a):32]_2 = [reg{0}(b):32]_2 & [reg{0}(c):32]_2`                                                                             |
| SLL256_RV32  | `a,b,c,1,2` | `[reg{0}(a):32]_2 = [reg{0}(b):32]_2 << [reg{0}(c):32]_2`                                                                            |
| SRL256_RV32  | `a,b,c,1,2` | `[reg{0}(a):32]_2 = [reg{0}(b):32]_2 >> [reg{0}(c):32]_2`                                                                            |
| SRA256_RV32  | `a,b,c,1,2` | `[reg{0}(a):32]_2 = [reg{0}(b):32]_2 >> [reg{0}(c):32]_2` MSB extends                                                                |
| SLT256_RV32  | `a,b,c,1,2` | `[reg{0}(a):32]_2 = i256([reg{0}(b):32]_2) < i256([reg{0}(c):32]_2) ? 1 : 0`                                                         |
| SLTU256_RV32 | `a,b,c,1,2` | `[reg{0}(a):32]_2 = u256([reg{0}(b):32]_2) < u256([reg{0}(c):32]_2) ? 1 : 0`                                                         |

#### 256-bit Branch

| Name         | Operands    | Description                                                    |
| ------------ | ----------- | -------------------------------------------------------------- |
| BEQ256_RV32  | `a,b,c,1,2` | `if([reg{0}(a):32]_2 == [reg{0}(b):32]_2) pc += c`             |
| BNE256_RV32  | `a,b,c,1,2` | `if([reg{0}(a):32]_2 != [reg{0}(b):32]_2) pc += c`             |
| BLT256_RV32  | `a,b,c,1,2` | `if(i256([reg{0}(a):32]_2) < i256([reg{0}(b):32]_2)) pc += c`  |
| BGE256_RV32  | `a,b,c,1,2` | `if(i256([reg{0}(a):32]_2) >= i256([reg{0}(b):32]_2)) pc += c` |
| BLTU256_RV32 | `a,b,c,1,2` | `if(u256([reg{0}(a):32]_2) < u256([reg{0}(b):32]_2)) pc += c`  |
| BGEU256_RV32 | `a,b,c,1,2` | `if(u256([reg{0}(a):32]_2) >= u256([reg{0}(b):32]_2)) pc += c` |
...
| Name        | Operands    | Description                                                       |
| ----------- | ----------- | ----------------------------------------------------------------- |
| MUL256_RV32 | `a,b,c,1,2` | `[reg{0}(a):32]_2 = ([reg{0}(b):32]_2 * [reg{0}(c):32]_2)[0:255]` |
...
`[reg{0}(b): N::NUM_LIMBS]_2, [reg{0}(c):N::NUM_LIMBS]_2` are assumed to be unsigned big integers in little-endian
format with each limb having `LIMB_BITS` bits. However, the big integers are **not** required to be less than `N`. Under
these conditions, the output element `[reg{0}(a): N::NUM_LIMBS]_2` written to memory will be an unsigned big integer of
...
| Name                      | Operands    | Description                                                                                                                                                                                                |
| ------------------------- | ----------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ADDMOD_RV32\<N\>          | `a,b,c,1,2` | `[reg{0}(a): N::NUM_LIMBS]_2 = [reg{0}(b): N::NUM_LIMBS]_2 + [reg{0}(c): N::NUM_LIMBS]_2 (mod N)`                                                                                                          |
| SUBMOD_RV32\<N\>          | `a,b,c,1,2` | `[reg{0}(a): N::NUM_LIMBS]_2 = [reg{0}(b): N::NUM_LIMBS]_2 - [reg{0}(c): N::NUM_LIMBS]_2 (mod N)`                                                                                                          |
| SETUP_ADDSUBMOD_RV32\<N\> | `a,b,c,1,2` | `assert([reg{0}(b): N::NUM_LIMBS]_2 == N)` for the chip that handles add and sub. For the sake of implementation convenience it also writes something (can be anything) into `[reg{0}(a): N::NUM_LIMBS]_2` |
| MULMOD_RV32\<N\>          | `a,b,c,1,2` | `[reg{0}(a): N::NUM_LIMBS]_2 = [reg{0}(b): N::NUM_LIMBS]_2 * [reg{0}(c): N::NUM_LIMBS]_2 (mod N)`                                                                                                          |
| DIVMOD_RV32\<N\>          | `a,b,c,1,2` | `[reg{0}(a): N::NUM_LIMBS]_2 = [reg{0}(b): N::NUM_LIMBS]_2 / [reg{0}(c): N::NUM_LIMBS]_2 (mod N)`. Undefined behavior if `gcd([reg{0}(c): N::NUM_LIMBS]_2, N) != 1`.                                       |
| SETUP_MULDIVMOD_RV32\<N\> | `a,b,c,1,2` | `assert([reg{0}(b): N::NUM_LIMBS]_2 == N)` for the chip that handles mul and div. For the sake of implementation convenience it also writes something (can be anything) into `[reg{0}(a): N::NUM_LIMBS]_2` |
...
| Name                    | Operands    | Description                                                                                                                                                                                                                                                                                         |
| ----------------------- | ----------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ISEQMOD_RV32\<N\>       | `a,b,c,1,2` | `[a:4]_1 = [reg{0}(b): N::NUM_LIMBS]_2 == [reg{0}(c): N::NUM_LIMBS]_2 (mod N) ? 1 : 0`. Enforces that `[reg{0}(b): N::NUM_LIMBS]_2, [reg{0}(c): N::NUM_LIMBS]_2` are less than `N` and then sets the register value of `[a:4]_1` to `1` or `0` depending on whether the two big integers are equal. |
| SETUP_ISEQMOD_RV32\<N\> | `a,b,c,1,2` | `assert([reg{0}(b): N::NUM_LIMBS]_2 == N)` in the chip that handles modular equality. For the sake of implementation convenience it also writes something (can be anything) into register value of `[a:4]_1`                                                                                        |
...
```
reg_fp2(a) -> Fp2 {
    let c0 = [reg{0}(a): Fp::NUM_LIMBS]_2;
    let c1 = [reg{0}(a) + Fp::NUM_LIMBS: Fp::NUM_LIMBS]_2;
    return Fp2 { c0, c1 };
}
```
...
| Name                | Operands    | Description                                                                                                                                                                  |
| ------------------- | ----------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ADD\<Fp2\>          | `a,b,c,1,2` | Set `reg_fp2(a) = reg_fp2(b) + reg_fp2(c)`                                                                                                                                   |
| SUB\<Fp2\>          | `a,b,c,1,2` | Set `reg_fp2(a) = reg_fp2(b) - reg_fp2(c)`                                                                                                                                   |
| SETUP_ADDSUB\<Fp2\> | `a,b,c,1,2` | `assert([reg_fp2(b).c0 == N)` for the chip that handles add and sub. For the sake of implementation convenience it also writes something (can be anything) into `reg_fp2(a)` |
| MUL\<Fp2\>          | `a,b,c,1,2` | Set `reg_fp2(a) = reg_fp2(b) * reg_fp2(c)`                                                                                                                                   |
| DIV\<Fp2\>          | `a,b,c,1,2` | Set `reg_fp2(a) = reg_fp2(b) / reg_fp2(c)`                                                                                                                                   |
| SETUP_MULDIV\<Fp2\> | `a,b,c,1,2` | `assert([reg_fp2(b).c0 == N)` for the chip that handles mul and div. For the sake of implementation convenience it also writes something (can be anything) into `reg_fp2(a)` |
...
```
reg_ec_point(a) -> EcPoint {
    let x = [reg{0}(a): C::COORD_SIZE]_2;
    let y = [reg{0}(a) + C::COORD_SIZE: C::COORD_SIZE]_2;
    return EcPoint(x, y);
}
```
...
| Name                 | Operands    | Description                                                                                                                                                                                                                                                                                    |
| -------------------- | ----------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| EC_ADD_NE\<C\>       | `a,b,c,1,2` | Set `reg_ec_point(a) = reg_ec_point(b) + reg_ec_point(c)` (curve addition). Assumes that `reg_ec_point(b), reg_ec_point(c)` both lie on the curve and are not the identity point. Further assumes that `reg_ec_point(b).x, reg_ec_point(c).x` are not equal in the coordinate field.           |
| SETUP_EC_ADD_NE\<C\> | `a,b,c,1,2` | `assert(reg_ec_point(b).x == C::MODULUS)` in the chip for EC ADD. For the sake of implementation convenience it also writes something (can be anything) into `[reg{0}(a): 2*C::COORD_SIZE]_2`. It is required for proper functionality that `assert(reg_ec_point(b).x != reg_ec_point(c).x)`   |
| EC_DOUBLE\<C\>       | `a,b,_,1,2` | Set `reg_ec_point(a) = 2 * reg_ec_point(b)`. This doubles the input point. Assumes that `reg_ec_point(b)` lies on the curve and is not the identity point.                                                                                                                                     |
| SETUP_EC_DOUBLE\<C\> | `a,b,_,1,2` | `assert(reg_ec_point(b).x == C::MODULUS && reg_ec_point(b).y == C::A)` in the chip for EC DOUBLE. For the sake of implementation convenience it also writes something (can be anything) into `[reg{0}(a): 2*C::COORD_SIZE]_2`. It is required for proper functionality that `assert(reg_ec_point(b).y != 0 mod C::MODULUS)` |
...
| Name         | Discriminant | Operands      | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| ------------ | ------------ | ------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| HintFinalExp | 0x30         | `a,b,c_upper` | Uses `c_upper = PAIRING_IDX` to determine the curve: `BN254 = 0, BLS12-381 = 1`. `a` is a pointer to `(p_ptr, p_len): (u32, u32)` in memory, and `b` is a pointer to `(q_ptr, q_len): (u32, u32)` in memory (e.g., `p_ptr = [reg{0}(a)..reg{0}(a) + 4]_2`). The sub-instruction peeks at `P = [p_ptr..p_ptr + p_len * size_of<Fp>() * 2]_2` and `Q = [q_ptr..q_ptr + q_len * size_of<Fp2>() * 2]_2` and views `P` as a list of `G1Affine` elements and `Q` as a list of `G2Affine` elements. It computes the multi-Miller loop on `(P, Q)` and then the final exponentiation hint `(residue_witness, scaling_factor): (Fp12, Fp12)`. It resets the hint stream to equal `(residue_witness, scaling_factor)` as `NUM_LIMBS * 12 * 2` bytes. |
