...
- [Short Weierstrass Elliptic curve](#short-weierstrass-elliptic-curve-extension): An extension for elliptic curve operations over short Weierstrass curves,
...
The address space `e` is `2` for load instructions. Store instructions use `e = 2` for user memory and
`e = 3` for public values.
...
### Short Weierstrass Elliptic Curve Extension

The elliptic curve extension supports arithmetic over elliptic curves `C` in Weierstrass form given by
equation `C: y^2 = x^3 + C::A * x + C::B` where `C::A` and `C::B` are constants in the coordinate field. The VM configuration will specify a list of supported curves. For
...
Points are represented in projective coordinates `(X, Y, Z)` where the corresponding affine point is `(X/Z, Y/Z)`.
The identity point (point at infinity) is represented by `Z = 0`.

A projective curve point `EcPoint(X, Y, Z)` is a triple of `X, Y, Z` where each element is an array of `C::Fp::NUM_LIMBS` elements each
with `LIMB_BITS = 8` bits. When the coordinate field `C::Fp` of `C` is prime, the format of `X, Y, Z` is guaranteed to be
the same as the format used in the [modular arithmetic instructions](#modular-arithmetic). A curve point will be
represented as `3 * C::Fp::NUM_LIMBS` contiguous cells in memory.

We use the following notation below:

```
r64_projective_point(a) -> EcPoint {
    let ptr = r64{0}(a);
    let X = [ptr: C::Fp::NUM_LIMBS]_2;
    let Y = [ptr + C::Fp::NUM_LIMBS: C::Fp::NUM_LIMBS]_2;
    let Z = [ptr + 2*C::Fp::NUM_LIMBS: C::Fp::NUM_LIMBS]_2;
    return EcPoint(X, Y, Z);
}
```

The addition and doubling operations use complete formulas from [ePrint 2015/1060](https://eprint.iacr.org/2015/1060.pdf) that handle all edge cases (identity points, equal points, inverse points) without branching.

| Name                    | Operands    | Description                                                                                                                                                                                                                                                                                    |
| ----------------------- | ----------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| SW_EC_ADD_PROJ_RV64\<C\>       | `a,b,c,1,2` | Set `r64_projective_point(a) = r64_projective_point(b) + r64_projective_point(c)` (curve addition). Assumes that inputs lie on the curve. The complete formula handles all cases including identity points (Z=0) and equal points.           |
| SETUP_SW_EC_ADD_PROJ_RV64\<C\> | `a,b,c,1,2` | `assert(r64_projective_point(b).X == C::Fp::MODULUS && r64_projective_point(b).Y == C::A && r64_projective_point(b).Z == C::B)` in the chip for EC ADD. For the sake of implementation convenience it also writes something (can be anything) into `[r64{0}(a): 3*C::Fp::NUM_LIMBS]_2`.   |
| SW_EC_DOUBLE_PROJ_RV64\<C\>       | `a,b,_,1,2` | Set `r64_projective_point(a) = 2 * r64_projective_point(b)`. This doubles the input point. Assumes that input lies on the curve. The formula handles identity point (Z=0) and order-2 points (Y=0).                                                                                                                                     |
| SETUP_SW_EC_DOUBLE_PROJ_RV64\<C\> | `a,b,_,1,2` | `assert(r64_projective_point(b).X == C::Fp::MODULUS && r64_projective_point(b).Y == C::A && r64_projective_point(b).Z == C::B)` in the chip for EC DOUBLE. For the sake of implementation convenience it also writes something (can be anything) into `[r64{0}(a): 3*C::Fp::NUM_LIMBS]_2`. |
