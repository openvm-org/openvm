...
- [RV32IM/RV64IM](#rv32imrv64im-extensions): An extension supporting the 32-bit and 64-bit RISC-V ISA with multiplication.
...
- `rXLEN{0}(a) := iXLEN([a:LIMBS]_1)` means casting the register value at `[a:LIMBS]_1` to `iXLEN`.
...
unsigned integer, and convert to field element. In the instructions below, `[c:LIMBS]_0` should be interpreted as
`c_i16 as iXLEN` sign extended to XLEN bits.
...
| SLLW_RV64           | `a,b,c,1,e` | `[a:8]_1 = sign_extend([b:4]_1 << [c:4]_e[0:4])`. RV64 only. Shifts lower 32 bits left, sign-extends to 64 bits. |
| SRL_RV32/SRL_RV64   | `a,b,c,1,e` | `[a:LIMBS]_1 = [b:LIMBS]_1 >> [c:LIMBS]_e`                                                                 |
| SRLW_RV64           | `a,b,c,1,e` | `[a:8]_1 = sign_extend([b:4]_1 >> [c:4]_e[0:4])`. RV64 only. Shifts lower 32 bits right (logical), sign-extends to 64 bits. |
| SRA_RV32/SRA_RV64   | `a,b,c,1,e` | `[a:LIMBS]_1 = [b:LIMBS]_1 >> [c:LIMBS]_e` MSB extends                                                     |
| SRAW_RV64           | `a,b,c,1,e` | `[a:8]_1 = sign_extend(i32([b:4]_1) >> [c:4]_e[0:4])`. RV64 only. Shifts lower 32 bits right (arithmetic), sign-extends to 64 bits. |
...
For shift instructions, the shift amount is the low 5 bits of the source for RV32, the low 6 bits for RV64, and the low 5 bits for RV64 W variants.
...
The data size is `1` for LOADB_RV32/LOADB_RV64, LOADBU_RV32/LOADBU_RV64, STOREB_RV32/STOREB_RV64, `2` for LOADH_RV32/LOADH_RV64, LOADHU_RV32/LOADHU_RV64, STOREH_RV32/STOREH_RV64, `4` for LOADW_RV32/LOADW_RV64, LOADWU_RV64, STOREW_RV32/STOREW_RV64, and `8` for LOADD_RV64, STORED_RV64.

All load/store instructions always do block accesses of block size `LIMBS`, even for LOADB/STOREB.

| Name                        | Operands        | Description                                                                                                                                                                         |
| --------------------------- | --------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| LOADB_RV32/LOADB_RV64       | `a,b,c,1,e,f,g` | `if(f!=0) [a:LIMBS]_1 = sign_extend([r32{c,g}(b):1]_e)` The operand `f` is assumed to be boolean. Must sign-extend the byte read from memory, which is represented in 2's complement.   |
| LOADH_RV32/LOADH_RV64       | `a,b,c,1,e,f,g` | `if(f!=0) [a:LIMBS]_1 = sign_extend([r32{c,g}(b):2]_e)` The operand `f` is assumed to be boolean. Must sign-extend the number read from memory, which is represented in 2's complement. |
| LOADW_RV32/LOADW_RV64       | `a,b,c,1,e,f,g` | `if(f!=0) [a:LIMBS]_1 = sign_extend([r32{c,g}(b):4]_e)` The operand `f` is assumed to be boolean. For RV64, sign-extends the 32-bit value to 64 bits.                               |
| LOADD_RV64                  | `a,b,c,1,e,f,g` | `if(f!=0) [a:8]_1 = [r32{c,g}(b):8]_e` RV64 only. The operand `f` is assumed to be boolean.                                                                                         |
| LOADBU_RV32/LOADBU_RV64     | `a,b,c,1,e,f,g` | `if(f!=0) [a:LIMBS]_1 = zero_extend([r32{c,g}(b):1]_e)` The operand `f` is assumed to be boolean. Must zero-extend the number read from memory.                                     |
| LOADHU_RV32/LOADHU_RV64     | `a,b,c,1,e,f,g` | `if(f!=0) [a:LIMBS]_1 = zero_extend([r32{c,g}(b):2]_e)` The operand `f` is assumed to be boolean. Must zero-extend the number read from memory.                                     |
| LOADWU_RV64                 | `a,b,c,1,e,f,g` | `if(f!=0) [a:8]_1 = zero_extend([r32{c,g}(b):4]_e)` RV64 only. The operand `f` is assumed to be boolean. Zero-extends the 32-bit value to 64 bits.                                  |
| STOREB_RV32/STOREB_RV64     | `a,b,c,1,e,1,g` | `[r32{c,g}(b):1]_e <- [a:1]_1`                                                                                                                                                      |
| STOREH_RV32/STOREH_RV64     | `a,b,c,1,e,1,g` | `[r32{c,g}(b):2]_e <- [a:2]_1`                                                                                                                                                      |
| STOREW_RV32/STOREW_RV64     | `a,b,c,1,e,1,g` | `[r32{c,g}(b):4]_e <- [a:4]_1`                                                                                                                                                      |
| STORED_RV64                 | `a,b,c,1,e,1,g` | `[r32{c,g}(b):8]_e <- [a:8]_1` RV64 only.                                                                                                                                           |
...
| JALR_RV32/JALR_RV64 | `a,b,c,1,_,f,g`  | `if(f!=0) [a:LIMBS]_1 = decompose(pc+DEFAULT_PC_STEP); pc = F::from_canonical_u64(iXLEN([b:LIMBS]_1) + sign_extend(decompose(c)[0:2], g) as uXLEN)`. Constrains that `iXLEN([b:LIMBS]_1) + sign_extend(decompose(c)[0:2], g)` is in `[0, 2^PC_BITS)`. Here `uXLEN(c)` must be in `[0, 2^16)`. The operands `f` and `g` are assumed to be boolean. The `pc` assignment is always done regardless of `f`'s value. |
| LUI_RV32/LUI_RV64   | `a,_,c,1,_,1`    | `[a:LIMBS]_1 = uXLEN(c) << 12`. Here `iXLEN(c)` must be in `[0, 2^20)`. The immediate is sign-extended.                                                                                                                                                                                                                                                                     |
| AUIPC_RV32/AUIPC_RV64 | `a,_,c,1,_,_`  | `[a:LIMBS]_1 = decompose(pc + (iXLEN(c) << 8))`. Here `iXLEN(c)` must be in `[-2^23, 2^23)`. The immediate is sign-extended, and the addition is performed as wrapping `iXLEN` arithmetic.                                                                                                                                                                                  |
...
the unsigned integer encoding of `c` as 16 bits, then sign extending it to XLEN bits using the sign bit `g`, and considering the XLEN bits as the 2's complement of an `iXLEN`. Then it is added to the register value `iXLEN([b:LIMBS]_1)`, where XLEN-bit overflow is ignored. The instruction is only valid if the resulting `iXLEN` is in range `[0, 2^PC_BITS)`. The
result is then cast to `uXLEN` and then to `F` and assigned to `pc`.
...
| DIV_RV32/DIV_RV64     | `a,b,c,1` | `[a:LIMBS]_1 = [b:LIMBS]_1 / [c:LIMBS]_1` integer division. Division by zero: if `iXLEN([c:LIMBS]_1) = 0`, set `iXLEN([a:LIMBS]_1) = -1`. Overflow: if `iXLEN([b:LIMBS]_1) = -2^(XLEN-1)` and `iXLEN([c:LIMBS]_1) = -1`, set `iXLEN([a:LIMBS]_1) = -2^(XLEN-1)`. |
| DIVU_RV32/DIVU_RV64   | `a,b,c,1` | `[a:LIMBS]_1 = [b:LIMBS]_1 / [c:LIMBS]_1` integer division. Division by zero: if `uXLEN([c:LIMBS]_1) = 0`, set `uXLEN([a:LIMBS]_1) = 2^XLEN - 1`.                                                                        |
...
| REM_RV32/REM_RV64     | `a,b,c,1` | `[a:LIMBS]_1 = [b:LIMBS]_1 % [c:LIMBS]_1` integer remainder. Division by zero: if `iXLEN([c:LIMBS]_1) = 0`, set `[a:LIMBS]_1 = [b:LIMBS]_1`. Overflow: if `iXLEN([b:LIMBS]_1) = -2^(XLEN-1)` and `iXLEN([c:LIMBS]_1) = -1`, set `[a:LIMBS]_1 = 0`. |
| REMU_RV32/REMU_RV64   | `a,b,c,1` | `[a:LIMBS]_1 = [b:LIMBS]_1 % [c:LIMBS]_1` integer remainder. Division by zero: if `uXLEN([c:LIMBS]_1) = 0`, set `[a:LIMBS]_1 = [b:LIMBS]_1`.                                                                             |
...
| Name                               | Operands        | Description                                                                                                                                                                       |
| ---------------------------------- | --------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| HINT_STOREW_RV32/HINT_STORED_RV64  | `_,b,_,1,2`     | `[rXLEN{0}(b):LIMBS]_2 = next LIMBS bytes from hint stream`. Only valid if next LIMBS values in hint stream are bytes.                                                            |
| HINT_BUFFER_RV32/HINT_BUFFER_RV64  | `a,b,_,1,2`     | `[rXLEN{0}(b):LIMBS * l]_2 = next LIMBS * l bytes from hint stream` where `l = rXLEN{0}(a)`. Only valid if next `LIMBS * l` values in hint stream are bytes. `l` must be non-zero and <= `MAX_HINT_BUFFER_WORDS` (1,023 words ≈ 4KB). The pointer address `rXLEN{0}(b)` does not need to be a multiple of `LIMBS`. |
...
| Rv32PrintStr/Rv64PrintStr           | 0x21         | `a,b,_`  | Peeks at `[rXLEN{0}(a):rXLEN{0}(b)]_2`, tries to convert to byte array and then UTF-8 string and prints to host stdout. Prints error message if conversion fails. Does not change any VM state. |
| Rv32HintRandom/Rv64HintRandom       | 0x22         | `a,_,_`  | Resets the hint stream to `LIMBS * rXLEN{0}(a)` random bytes. The source of randomness is deterministic using a fixed-seed RNG (`rand::rngs::StdRng`). Its result is not constrained in any way. |
| Rv32HintLoadByKey/Rv64HintLoadByKey | 0x23         | `a,b,_`  | Look up the value by key `[rXLEN{0}(a):rXLEN{0}(b)]_2` and prepend the value into `input_stream`. The logical value is `Vec<Vec<F>>`. The serialization of `Vec` follows the format `[length, <content>]`. Both length and content encoded as little-endian bytes. |
