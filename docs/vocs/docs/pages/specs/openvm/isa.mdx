...
| LUI_RV32/LUI_RV64   | `a,_,c,1,_,1`    | `[a:LIMBS]_1 = uXLEN(c) << 12`. Here `iXLEN(c)` must be in `[0, 2^20)`.                                                                                                                                                                                                                                                                                                     |
| AUIPC_RV32/AUIPC_RV64 | `a,_,c,1,_,_`  | `[a:LIMBS]_1 = decompose(pc) + (decompose(c) << 8)`. Here `iXLEN(c)` must be in `[0, 2^24)`.                                                                                                                                                                                                                                                                                |
...
For AUIPC, we are treating `c` in `[0, 2^24)` as a raw encoding of 24-bits.
The instruction does not need to interpret whether the register is signed or unsigned.
For AUIPC, the addition is treated as unchecked `uXLEN` addition since that is the same as `iXLEN` addition at the bit
level.
