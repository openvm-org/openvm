...
| `num_public_value_cells`     | The number of `u16` cells in user public output.                    | Configurable, must equal `4` times a nonzero power of two.           |
...
To make program outputs public, OpenVM maintains a bounded append-only output stream. Programs
append to this stream with `REVEAL_RV64` from the RV64 IO extension. Public outputs are committed
in execution order, independently from random-access memory.

The public output contains `num_public_value_cells` `u16` field elements, where
`num_public_value_cells` is a
VM configuration parameter. Each revealed `u64` occupies four consecutive elements. The terminal
output is padded with zeros to this fixed capacity and carries an explicit length, so a revealed
zero remains distinct from padding.
...
- User public outputs can only be appended in execution order, up to the configured capacity.
...
In addition to opcodes which match 1-1 with the RV64IM opcodes, the following RV64 IO opcodes
handle hint input and write-once public output.
...
| REVEAL_RV64      | `a`             | Appends `[a:8]_1` to the public-output stream. The stream must remain within its configured capacity. |
