...
| OUTPUT | `a,b,def_idx,1,2` | Retrieves the output of a previous deferred call. Reads an `OutputKey` from `[r32{0}(b):40]_2`. Writes `output_len` bytes to `[r32{0}(a):output_len]_2`, where `output_len` is the length from the `OutputKey`. The instruction is only valid for deferred outputs whose byte length is divisible by `DIGEST_SIZE`. |
