...
The address space `e` is `2` for load instructions, and can be `2` or `3` for store instructions. Load/store
instructions cannot access the [deferral address space](#address-spaces).
