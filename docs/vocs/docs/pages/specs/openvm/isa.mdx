...
We use the term `LIMBS` to refer to the number of 8 bit limbs required to cover an integer register (`XLEN/8`).
...
binary, specified [here](/specs/reference/riscv-custom-code). These consist of opcodes corresponding 1-1 with RV32IM/RV64IM opcodes, as well as
...
addition with `pc` is treated as unchecked `iXLEN` addition since that is the same as `uXLEN` addition at the bit level.
