...
- At the leaf layer, folds cached trace commits into each child's `input_i_commit` to produce a `folded_input_commit`, then compresses `(folded_input_commit, output_i_commit)` into a single `merkle_commit`. At internal layers, Merklizes child `merkle_commit`s into a single `merkle_commit`
...
Note that because we are building a Merkle tree, tail proofs are combined with "non-present" proofs. The `merkle_commit` of a non-present proof at a particular depth is the Merkle root of a zeroed tree of that depth.
