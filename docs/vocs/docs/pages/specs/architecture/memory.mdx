...
- [The `MemoryBoundaryChip`](#the-memoryboundarychip)
...
## The `MemoryBoundaryChip`

The `MemoryBoundaryChip` commits to both the initial and final memory state, making these commitments available as public values for proof aggregation.
...
- **Multiple Buses:**
  To enforce these commitments, the `MemoryBoundaryChip` interacts with three buses:
  - **Merkle Bus:**
...
The initial memory state is constrained at the AIR level by the `MemoryBoundaryChip`, which commits to it via the `initial_memory_root` Merkle root (a public value) and sends initial values on the memory bus at timestamp $0$.
