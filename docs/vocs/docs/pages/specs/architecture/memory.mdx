...
- `eval_instruction` does an `execute_and_increment_pc` (`3`), `3` register reads (`12`), and `1`
  range check (`1`), giving `16` interactions.
...
In total, there are at least `16 + 68 + 68 + 136 + 68 = 356` interactions per row when
`num_blocks = 17`. Since `timestamp_delta = 54 < 356 = num_interactions`, the condition is
satisfied.
...
The initial memory state is constrained at the AIR level by the `PersistentBoundaryChip`, which commits to it via the `initial_root` Merkle root (a public value) and sends initial values on the memory bus at timestamp $0$.
