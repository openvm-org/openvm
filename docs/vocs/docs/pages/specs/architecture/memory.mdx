...
- `1` `execute_and_increment_pc` (`3`), `1` register read (`4`), `1` pointer range check (`1`),
  `25` block writes (`100`), `50` paired byte range checks (`50`), and `2` keccakf state bus sends (`2`).

In total, there are at least `160` interactions per row. Since `timestamp_delta = 26 < 160`, the
