...
bus and `2` range-check interactions (for a block size of `8`, there are `4` range-check interactions per access). For one-row instruction executors, this implies
...
| KeccakfOpChip         | 26              | -                 | Case 2. 1 register read + 25 block writes.                 |
| Rv64HintStoreChip     | –               | –                 | Case 1.                                                    |
| Sha2MainChip          | –               | –                 | Case 1.                                                    |
...
The `XorinVmAir` timestamp change is `3 + 3 * num_blocks` where `num_blocks = len / 8` is the number
of non-padding 8-byte blocks (at most `17`, since `len` is at most `136`). The maximum timestamp
change is therefore `3 + 3 * 17 = 54`. Each XORIN instruction uses `1` trace row.
...
- `constrain_input_read` does up to `17` input reads (`68`) and `17` buffer reads (`68`).
- `constrain_xor` does up to `136` XOR lookups on the bitwise lookup bus.
- `constrain_output_write` does up to `17` buffer writes (`68`).

In total, there are at least `17 + 68 + 68 + 136 + 68 = 357` interactions per row when
`num_blocks = 17`. Since `timestamp_delta = 54 < 357 = num_interactions`, the condition is
satisfied.

#### KeccakfOpChip

The `KeccakfOpAir` timestamp delta is `26` (`1` register read + `25` block writes). Each KECCAKF
...
- `1` `execute_and_increment_pc` (`3`), `1` register read (`4`), `1` pointer range check (`1`),
  `25` block writes (`100`), `100` paired byte range checks (`100`), and `2` keccakf state bus sends (`2`).

In total, there are at least `210` interactions per row. Since `timestamp_delta = 26 < 210`, the
