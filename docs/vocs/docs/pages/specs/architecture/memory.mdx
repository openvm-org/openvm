...
The only unaccounted for interactions are the initial birth of a segment and the final memory state. We use the **boundary chip** (`BoundaryChip`) to handle them.

The `BoundaryChip` commits to both the initial and final memory state. The initial memory is provided as part of the program, and the chip produces commitments (Merkle roots) for both the initial and final states. These commitments become part of the public values used later in proof aggregation (see [Continuations](/specs/architecture/continuations)).

The boundary chip performs, for every touched memory chunk, a send interaction on birth and a receive interaction on death.
...
The `BoundaryChip` commits to both the initial and final memory state, making these commitments available as public values for proof aggregation.
...
- **Multiple Buses:**
  To enforce these commitments, the `BoundaryChip` interacts with three buses:
  - **Merkle Bus:**
...
The initial memory state is constrained at the AIR level by the `BoundaryChip`, which commits to it via the `initial_memory_root` Merkle root (a public value) and sends initial values on the memory bus at timestamp $0$.
