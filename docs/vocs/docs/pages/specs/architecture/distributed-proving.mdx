...
For each segment, we re-execute via _pure execution_ on the host CPU to recover the starting VM state at the beginning of the segment. The host then performs _preflight execution_ up to the segment end, appending a program log and a timed memory log. _Postflight_ validates these logs and derives immutable memory chronology and opcode indexes. During _trace generation_, each chip replays its indexed instructions into _trace matrices_ that match the zkVM circuit frontend. Preflight execution is serial, while postflight and trace generation can use the proving device. The STARK backend then generates a cryptographic proof from the trace matrices on device.
...
- **Preflight** execution. This is a special logging execution that records the minimal execution trace such that subsequent [trace generation](#trace-generation) may be done in parallel without reliance on the VM state.
    - The execution context maintains normal read/write guest memory while appending generic
      program and memory logs. It does not construct chip-specific witness records.
...
For a given segment, preflight history contains the program order and timed memory values needed to
replay every instruction. Postflight derives read-only memory chronology and opcode indexes from
that history. The program, history, and indexes are transferred to device memory, where trace
generation re-executes independent rows in parallel without mutable VM memory.
