...
- Hardware-agnostic execution - The same execution implementation may be used across different hardware acceleration platforms (CPU or GPU).
...
We now give more details about the components described above.
...
- **Metered** execution. This is pure execution that additionally measures weighted functions of instruction counts and guest memory usage in order to determine where to [segment](/specs/architecture/circuit-architecture#program-execution) the program execution. The output of metered execution is the sequence of segments together with instruction counters at the segment boundaries.
- **Preflight** execution. This is a special logging execution that records the minimal execution trace such that subsequent [trace generation](#trace-generation) may be done without reliance on the VM state.
...
Presently, each aggregation proof is generated as a proof of a single segment of an Agg VM program. Since aggregation proofs are necessarily of single segments, metered execution is not required.
