...
| Layer | Description |
|-------|-------------|
| app | Executes a program on the VM, then produces one `Proof` per segment and a terminal public-value sidecar |
...
| root | Wraps a single internal-recursive `Proof` and binds the terminal public-value sidecar to the final accumulator endpoint |
...
|------|--------|-------------|
| App proof | `ContinuationVmProof` | Non-aggregated segment `Proof`s plus the user public-values proof payload |
| STARK proof | `VmStarkProof` | A single aggregated `Proof` plus the user public-values proof payload |
...
- `initial_public_values_commit: Digest` — initial ordered public-output accumulator
- `final_public_values_commit: Digest` — final ordered public-output accumulator
...
1. The `final_pc`, `final_root`, and `final_public_values_commit` of each non-terminal segment equal
   the corresponding initial values of the next segment
...
The inner subcircuit takes up to $N$ child `Proof`s and propagates, but does **not** decommit, the
append-only public-output accumulator. Its output `Proof` is meant to either be recursively aggregated or used
as the `Proof` component in a `VmStarkProof`.
...
The root subcircuit takes a single internal-recursive child `Proof` and constrains the terminal
public-value sidecar against the child's final accumulator endpoint. Its output `Proof` is meant to be the
static layer input.
...
- `num_public_values: F` — authenticated number of revealed outputs
- `user_public_values: Vec<F>` — fixed-capacity cells, four little-endian `u16` limbs per output,
  zero-padded after the revealed prefix
- `app_exe_commit: Digest` — hashed combination of:
  ...
  - The initial public-output accumulator (`initial_public_values_commit`)
...
- The valid prefix of `user_public_values` is folded in order from the capacity-specific initial
  accumulator. The padded suffix is constrained to zero, and the resulting commitment and length
  equal the child's final public-output endpoint.
...
3. Exposes `DeferralPvs` containing `initial_acc_hash` and `final_acc_hash` (the initial and final deferral accumulator values as memory subtree Merkle leaves), `depth` (the Merkle subtree depth), and `node_idx = def_idx`
...
- `node_idx: F` — node index at the current depth of the deferral accumulator Merkle tree

When multiple deferral hook proofs are combined, the inner subcircuit Merklizes their `initial_acc_hash` and `final_acc_hash` values together, increments the depth, and constrains the child node indices to be canonical: adjacent present children must have adjacent indices, the left child index is even, and the parent index is the left child index divided by two. One-row wrappers pass `DeferralPvs` through and require `node_idx = 0`. This ensures the final mixed proof is rooted at the canonical deferral-address-space location.
...
- The child proof's `deferral_flag` is either `0` (no deferral proofs used) or `2` (VM and deferral public values combined)
- If `deferral_flag == 2`, the child proof's `def_hook_commit` matches a pre-generated constant
- If `deferral_flag == 0`, the child proof's deferral public values and `def_hook_commit` are unset
- The child proof's `node_idx` is `0`
- The child proof's `initial_acc_hash` was present in the **initial** memory state at the deferral address space region, or the deferral address space was unchanged when `deferral_flag == 0`
- The child proof's `final_acc_hash` was present in the **final** memory state at the deferral address space region, or the deferral address space was unchanged when `deferral_flag == 0`
