# V2 Continuations Specification
...
This forms an **aggregation tree** with multiple **layers**, where each layer aggregates child proofs from the layer below. In V2, there are seven **layer types**:
...
A **prover** is a struct that generates a proof for a specific circuit given appropriate inputs. V2 distinguishes:
...
In V2, there are three **client-facing proof** types:
...
In V2, every non-app STARK-based layer has a circuit composed of exactly two subcircuits:
