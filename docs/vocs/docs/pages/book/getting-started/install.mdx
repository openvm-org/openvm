...
This downloads the prebuilt tarball for your host triple from the fork's [Releases](https://github.com/openvm-org/rust/releases) page, extracts it under `~/.openvm/toolchains/`, and links it into rustup. The default toolchain is the stable OpenVM release `openvm-1.94.1`. To install another published release — including nightly variants for guest code that needs nightly Rust features — pass `--version <tag>` (e.g. `--version openvm-nightly-2026-01-18`).
