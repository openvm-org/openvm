...
```bash
cargo +1.91 install --locked --git https://github.com/openvm-org/openvm.git --tag v2.1.0 cargo-openvm
```
...
```bash
git clone --branch v2.1.0 --single-branch https://github.com/openvm-org/openvm.git
cd openvm
...
The OpenVM CLI has been tested with CUDA toolkit versions 12.9, 13.0, and 13.1. OpenVM's default CI runners use CUDA toolkit version 12.9, so we regard 12.9 as the most stable target.
...
```bash
cargo +1.91 install --locked --git https://github.com/openvm-org/openvm.git --tag v2.1.0 --features cuda cargo-openvm
```
...
:::code-group
```bash [Without GPU]
cargo +nightly-2026-01-18 install --locked --git https://github.com/openvm-org/openvm.git --tag v2.1.0 --features tco cargo-openvm
```
```bash [Nvidia GPU]
cargo +nightly-2026-01-18 install --locked --git https://github.com/openvm-org/openvm.git --tag v2.1.0 --features tco,cuda cargo-openvm
```
:::
...
:::code-group
```bash [Without GPU]
cargo +nightly-2026-01-18 install --locked --force --path crates/cli --features tco
```
```bash [Nvidia GPU]
cargo +nightly-2026-01-18 install --locked --force --path crates/cli --features tco,cuda
```
:::
...
:::code-group
```bash [Without GPU]
cargo +1.91 install --locked --git https://github.com/openvm-org/openvm.git --tag v2.1.0 --features aot cargo-openvm
```
```bash [Nvidia GPU]
cargo +1.91 install --locked --git https://github.com/openvm-org/openvm.git --tag v2.1.0 --features aot,cuda cargo-openvm
```
:::
...
:::code-group
```bash [Without GPU]
cargo +1.91 install --locked --force --path crates/cli --features aot
```
```bash [Nvidia GPU]
cargo +1.91 install --locked --force --path crates/cli --features aot,cuda
```
:::
...
`cargo openvm build` cross-compiles guest programs with a custom Rust toolchain built from the [openvm-org/rust](https://github.com/openvm-org/rust) fork, which adds the `riscv64im-unknown-openvm-elf` tier-3 target with prebuilt `std`/`core`/`alloc`/`panic_abort` rlibs. Install it once after installing `cargo-openvm`:

```bash
cargo openvm toolchain install
```

This downloads the prebuilt tarball for your host triple from the fork's [Releases](https://github.com/openvm-org/rust/releases) page, extracts it under `~/.openvm/toolchains/`, and links it into rustup. The default toolchain is the stable OpenVM release `openvm-1.94.0`. To install another published release — including nightly variants for guest code that needs nightly Rust features — pass `--version <tag>` (e.g. `--version openvm-nightly-2026-01-18`).

You can list and remove installed openvm toolchains with `cargo openvm toolchain list` and `cargo openvm toolchain uninstall`.
