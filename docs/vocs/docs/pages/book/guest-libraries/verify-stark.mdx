...
`DEF_IDX` is the index of the corresponding `DeferralFn` in the `DeferralExtension.fns` list — for verify-stark, this is the index at which the `DeferredVerifyCircuit`'s function was registered.
