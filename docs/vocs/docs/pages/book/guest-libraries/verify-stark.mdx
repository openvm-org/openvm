...
To use `DeferredVerifyCircuit` in an app, you must wire it into your VM config via `DeferralExtension`. A VM config can support up to 512 different deferral circuits at one time. `DeferralExtension` stores:
