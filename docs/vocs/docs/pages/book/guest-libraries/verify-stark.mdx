...
- `num_public_values: u32` — number of `u64` values revealed by the verified app
- `user_public_values: Vec<u8>` — densely packed little-endian bytes of those revealed values
...
The deferral circuit output stores a fixed-capacity, zero-padded cell array, but the guest helper validates the explicit count and returns only the dense published prefix. `ProofOutput.user_public_values` therefore matches the bytes revealed by the verified app.
...
2. Constrains the revealed user public values against the app's final public-values commitment
3. Produces the output commitment containing `app_exe_commit`, `app_vm_commit`, the explicit output count, and the fixed-capacity public-value cells
