...
For verify-stark recursion, the simplest setup is `DeferralPathProver::verify_stark`, which derives the deferral-enabled internal-recursive verifying key and self-referential `def_hook_commit` needed to verify proofs from the same SDK, including proofs that themselves carry deferrals:
