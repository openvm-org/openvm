...
To use `DeferredVerifyCircuit` in an app, you must wire it into your VM config via `DeferralConfig`. A VM config can support up to `MAX_DEF_CIRCUITS = 512` different deferral circuits at one time. `DeferralConfig` has a single field:
