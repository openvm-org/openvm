...
A guest library for recursive verification of OpenVM STARK proofs. The library uses the [deferral framework](/book/acceleration-using-extensions/deferral) to offload proof verification to the aggregation phase, making recursive verification efficient within guest programs.
...
To use `DeferredVerifyCircuit` in an app, you must wire it into your VM config via `DeferralConfig`. A VM config can support up to `MAX_DEF_CIRCUITS = 512` different deferral circuits at one time. `DeferralConfig` stores:
...
These entries are aligned by index. The SDK converts `DeferralConfig` into the lower-level `DeferralExtension` when building execution, transpiler, and prover components.
