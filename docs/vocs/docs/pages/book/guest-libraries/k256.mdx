...
:::warning
`Secp256k1Point` stores raw projective `(X, Y, Z)` coordinates. Deserializing a `Secp256k1Point` does not check that the coordinates are canonical or represent a point on the secp256k1 curve. Validate points from untrusted input before using point addition, scalar multiplication, or MSM. For public keys, use `VerifyingKey::from_sec1_bytes`.
:::
