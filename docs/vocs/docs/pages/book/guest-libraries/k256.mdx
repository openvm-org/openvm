...
:::warning
`VerifyingKey::from_affine` does not check that raw coordinates are on the secp256k1 curve. Use it only with points that have already been checked. For public keys from untrusted bytes, use `VerifyingKey::from_sec1_bytes` or `VerifyingKey::from_encoded_point`.
:::
...
```toml
openvm-algebra-guest = { git = "https://github.com/openvm-org/openvm.git", tag = "v2.0.2" }
openvm-ecc-guest = { git = "https://github.com/openvm-org/openvm.git", tag = "v2.0.2" }
openvm-k256 = { git = "https://github.com/openvm-org/openvm.git", tag = "v2.0.2", package = "k256" }
```
...
:::warning
`Secp256k1Point` stores raw affine coordinates. Deserializing a `Secp256k1Point` does not check that the point is on the secp256k1 curve. Validate points from untrusted input before using point addition, scalar multiplication, or MSM. For public keys, use `VerifyingKey::from_sec1_bytes`.
:::
...
In this example, the `Secp256k1Point` struct is provided by the `openvm-k256` crate.
