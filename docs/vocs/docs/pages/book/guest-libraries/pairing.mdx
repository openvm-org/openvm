...
We'll be working with an example using the BLS12-381 elliptic curve. This is in addition to the setup that needs to be done in the [Writing a Program](../writing-apps/writing-a-program) section.
...
Additionally, we'll need to initialize our moduli and `Fp2` struct via the following macros. For a more in-depth description of these macros, please see the [OpenVM Algebra](../acceleration-using-extensions/algebra) section.
...
```rust,no_run,noplayground
// [!include ~/snippets/examples/pairing/src/main.rs:pre]
// [!include ~/snippets/examples/pairing/src/main.rs:imports]

// [!include ~/snippets/examples/pairing/src/main.rs:init]

// [!include ~/snippets/examples/pairing/src/main.rs:main]
```
