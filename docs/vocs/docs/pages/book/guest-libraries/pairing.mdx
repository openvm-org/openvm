# Elliptic Curve Pairing

We'll be working with an example using the BLS12-381 elliptic curve. This is in addition to the setup that needs to be done in the [Writing a Program](/book/writing-apps/writing-a-program) section.

In the guest program, we will import the `PairingCheck` and `IntMod` traits, along with the BLS12-381 curve structs (**IMPORTANT:** this requires the `bls12_381` feature enabled in Cargo.toml for the `openvm-pairing` dependency), and a few other values that we will need:

```rust title="guest program"
// [!include ~/snippets/examples/pairing/src/main.rs:imports]
```

Additionally, we'll need to initialize our moduli and `Fp2` struct via the following macros. For a more in-depth description of these macros, please see [Algebra (Modular Arithmetic)
](/book/acceleration-using-extensions/algebra).

```rust
// [!include ~/snippets/examples/pairing/src/main.rs:init]
```

## Input values

The inputs to the pairing check are `AffinePoint`s in $\mathbb{F}_p$ and $\mathbb{F}_{p^2}$. They can be constructed via the `AffinePoint::new` function, with the inner `Fp` and `Fp2` values constructed via various `from_...` functions.

:::warning
`AffinePoint` does not validate that coordinates are canonical, on-curve, or in the correct prime-order subgroup. If pairing inputs are supplied by an untrusted source, validate those properties before calling `pairing_check`, `pairing_check_hint`, or `multi_miller_loop`. Callers should also enforce any protocol-specific policy about whether identity points are allowed.
:::

## Pairing check

Most users that use the pairing extension will want to assert that a pairing is valid (the final exponentiation equals one). With the `PairingCheck` trait imported from the previous section, we have access to the `pairing_check` function on the `Bls12_381` struct. Once we have constructed the input `AffinePoint`s, we can pass them to `pairing_check`:

:::warning
The `pairing_check` function assumes its inputs have already been validated by the caller. It does not perform point decompression, curve-membership checks, subgroup checks, or protocol-specific identity checks internally.
:::

```rust
// [!include ~/snippets/examples/pairing/src/main.rs:pairing_check]
```

## Additional functionality

We also have access to each of the specific functions that the pairing check utilizes for either the BN254 or BLS12-381 elliptic curves.

### Multi-Miller loop

The multi-Miller loop can also be run separately by importing the `MultiMillerLoop` trait. This trait currently comes from the `openvm-pairing-guest` crate, so add `openvm-pairing-guest` as a dependency alongside `openvm-pairing`.

```rust
use openvm_pairing::bls12_381::Bls12_381;
use openvm_pairing_guest::pairing::MultiMillerLoop;

struct MillerLoopInput {
    p: Vec<AffinePoint<Fp>>,   // G1 points
    q: Vec<AffinePoint<Fp2>>,  // G2 points
}

let input: MillerLoopInput = /* ... */;
let f = Bls12_381::multi_miller_loop(&input.p, &input.q);
```

## Running via CLI

### Config parameters

For the guest program to build successfully, we'll need to create an `openvm.toml` configuration file somewhere. It contains all of the necessary configuration information for enabling the OpenVM components that are used in the pairing check.

```toml
# openvm.toml
[app_vm_config.riscv_i]
[app_vm_config.riscv_m]
[app_vm_config.io]
[app_vm_config.pairing]
supported_curves = ["Bls12_381"]

[app_vm_config.modular]
supported_moduli = [
    "4002409555221667393417789825735904156556882819939007885332058136124031650490837864442687629129015664037894272559787",
]

[app_vm_config.fp2]
supported_moduli = [
    ["Bls12_381Fp2", "4002409555221667393417789825735904156556882819939007885332058136124031650490837864442687629129015664037894272559787"],
]
```

Also note that since this is a complicated computation, the `keygen` step requires quite a lot of memory. Run it with `RUST_MIN_STACK` set to a large value, e.g.

```bash
RUST_MIN_STACK=8388608 cargo openvm keygen
```

### Full example program

This [example code](https://github.com/openvm-org/openvm/blob/main/examples/pairing/src/main.rs) contains hardcoded values and no inputs as an example that can be run via the CLI.

```rust
// [!include ~/snippets/examples/pairing/src/main.rs]
```
