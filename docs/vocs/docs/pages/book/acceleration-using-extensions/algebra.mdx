...
To [leverage](./overview) compile-time known moduli for performance, you declare and initialize the arithmetic structures:
...
2. **Init**: Use the [`openvm::init!` macro](./overview#automating-the-init-step) exactly once in the final binary:
...
2. **Init**: After calling `complex_declare!`, the [`openvm::init!` macro](./overview#automating-the-init-step) will now expand to the appropriate call to `complex_init!`.
...
// [!include ~/docs/snippets/examples/algebra/src/main.rs]
```
