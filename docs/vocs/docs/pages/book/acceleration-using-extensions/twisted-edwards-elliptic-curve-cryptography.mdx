...
The OpenVM Twisted Edwards Elliptic Curve Cryptography Extension provides support for twisted Edwards elliptic curve operations through the `openvm-ecc-guest` crate.
...
For twisted Edwards elliptic curve cryptography, the `openvm-te-guest` crate provides macros similar to those in [`openvm-algebra-guest`](/book/acceleration-using-extensions/algebra):
