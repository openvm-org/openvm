...
The deferral extension requires programmatic setup through the [SDK](/book/advanced-usage/sdk) because each deferral circuit needs a `DeferralFn` (the function that computes outputs from inputs) and corresponding proving infrastructure. Once that setup has produced a `DeferralConfig`, supported deferral circuits such as verify-stark can be serialized into `openvm.toml`.
