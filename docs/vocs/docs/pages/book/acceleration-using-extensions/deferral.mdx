...
The guest program only passes an **input commitment** to `deferred_compute`; the raw input never enters the VM, and the VM writes the output back rather than computing it. So the host must supply the deferral data for two phases: **execution** and **proving**.
