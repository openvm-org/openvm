...
2. The guest calls `get_deferred_output` with the output key to retrieve the raw output bytes.
3. During aggregation, each deferred computation is independently proven by its corresponding deferral circuit, and the accumulators are reconciled with the VM's memory state.
