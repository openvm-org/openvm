...
`DEFERRAL_IDX` is a compile-time constant that identifies which deferral circuit to invoke. A VM configuration can support up to 512 deferral circuits. Each index corresponds to a specific `DeferralFn` registered in the `DeferralExtension`.
