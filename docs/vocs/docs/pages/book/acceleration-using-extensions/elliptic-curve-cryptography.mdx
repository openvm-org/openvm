...
- `WeierstrassPoint` trait:
  It represents a point on a Weierstrass elliptic curve and it extends `Group`. Internally, points use projective coordinates `(X, Y, Z)` for efficient computation, but the API provides affine coordinate access.

  - `Coordinate` type is the type of the coordinates of the point, and it implements `IntMod`.
  - `x()`, `y()`, `z()` are used to get the projective coordinates
  - `normalize()` converts from projective to affine coordinates `(X/Z, Y/Z, 1)` (requires one field inversion). Returns identity for `Z=0`.
  - `from_xy` is a constructor for the point, which checks if the point is either identity or on the curve.
  - The point supports elliptic curve operations through intrinsic functions that use complete formulas (handling all edge cases including identity and equal points).
...
During `cargo openvm build`, OpenVM generates an `openvm_init.rs` file, and `openvm::init!()` includes that file in the final guest binary.
...
To verify an ECDSA signature, first call the `VerifyingKey::recover_from_prehash_noverify` associated function to recover the verifying key, then call the `VerifyingKey::verify_prehash` method on the recovered verifying key.
