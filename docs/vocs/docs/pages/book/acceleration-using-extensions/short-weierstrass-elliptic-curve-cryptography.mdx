...
The OpenVM Short Weierstrass Elliptic Curve Cryptography Extension provides support for short Weierstrass elliptic curve operations through the `openvm-weierstrass-guest` crate.
...
For elliptic curve cryptography, the `openvm-weierstrass-guest` crate provides macros similar to those in [`openvm-algebra-guest`](/book/acceleration-using-extensions/algebra):
