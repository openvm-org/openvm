...
The OpenVM Short Weierstrass Elliptic Curve Cryptography Extension provides support for short Weierstrass elliptic curve operations through the `weierstrass` module in the `openvm-ecc-guest` crate.

Developers can enable arbitrary short Weierstrass curves by configuring this extension with the modulus for the coordinate field and the coefficients in the curve equation. Preset configurations for the secp256k1 and secp256r1 curves are provided through the [K256](/book/guest-libraries/k256) and [P256](/book/guest-libraries/p256) guest libraries.
...
For elliptic curve cryptography, the `openvm-weierstrass-guest::weierstrass` module provides macros similar to those in [`openvm-algebra-guest`](/book/acceleration-using-extensions/algebra):
