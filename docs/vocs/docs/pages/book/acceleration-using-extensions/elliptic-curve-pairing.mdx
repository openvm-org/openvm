...
:::warning
This function assumes the caller has already validated externally supplied points. In particular, callers must check canonical coordinate encoding, curve membership, membership in the correct prime-order subgroup, and any protocol-specific identity-point policy before passing points to `pairing_check`.
:::
