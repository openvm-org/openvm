...
- [`openvm-keccak-guest`](./keccak) - Keccak256 hash function. See the [Keccak256 guest library](../guest-libraries/keccak256) for usage details.
- [`openvm-sha256-guest`](./sha-256) - SHA-256 hash function. See the [SHA-2 guest library](../guest-libraries/sha2) for usage details.
- [`openvm-bigint-guest`](./big-integer) - Big integer arithmetic for 256-bit signed and unsigned integers. See the [ruint guest library](../guest-libraries/ruint) for using accelerated 256-bit integer ops in rust.
- [`openvm-algebra-guest`](./algebra) - Modular arithmetic and complex field extensions.
- [`openvm-ecc-guest`](./elliptic-curve-cryptography) - Elliptic curve cryptography. See the [k256](../guest-libraries/k256) and [p256](../guest-libraries/p256) guest libraries for using this extension over the respective curves.
- [`openvm-pairing-guest`](./elliptic-curve-pairing) - Elliptic curve optimal Ate pairings. See the [pairing guest library](../guest-libraries/pairing) for usage details.
...
2. When [compiling the program](../writing-apps/compiling), `cargo openvm build` will read the [configuration file](#configuration) to automatically generate the correct init code and write it to `<INIT_FILE_NAME>`, which defaults to `openvm_init.rs` in the manifest directory.
