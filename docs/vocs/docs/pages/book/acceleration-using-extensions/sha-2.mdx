...
fn sha256(input: &[u8]) -> [u8; 32] {
    ...
    #[cfg(not(openvm_intrinsics))] {
        unimplemented!("native sha256 is only available with openvm intrinsics")
    }
}
```

Here, `add_padding` applies the standard SHA-2 padding (append a `1` bit, pad with `0` bits, then append the message length) so the input is a multiple of the block size, since `zkvm_shaXXX_impl` only operates on full blocks.
