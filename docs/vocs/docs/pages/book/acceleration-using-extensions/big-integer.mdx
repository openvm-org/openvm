...
The extension also supports signed 256-bit arithmetic. The checked-in signed example uses `alloy_primitives::I256`, with `ruint` patched to the OpenVM fork underneath. See the full example [here](https://github.com/openvm-org/openvm/blob/main/examples/i256/src/main.rs).
