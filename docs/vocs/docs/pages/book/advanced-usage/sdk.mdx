...
- `Sdk::riscv64(app_params, agg_params)` returns a `GenericSdk` supporting the RV64IM extension.
- `Sdk::standard(app_params, agg_params)` returns a `GenericSdk` supporting all default OpenVM extensions, including RV64IM.

Note that to use `Sdk::riscv64(app_params, agg_params)` or `Sdk::standard(app_params, agg_params)` the `app_vm_config` field of your `openvm.toml` must exactly match a specific set configuration.

`Sdk::riscv64(app_params, agg_params)` supports normal Rust compiled to the `rv64im` target (for more information see [OpenVM Rust Frontend](/specs/reference/rust-frontend)) and `openvm::io` functions. To use it, the `app_vm_config` field of your `openvm.toml` must be:

```toml [openvm_riscv64.toml]
// [!include ~/snippets/examples_sdk_toml/openvm_riscv64.toml]
```
...
<details>
<summary> Click here to view the `Sdk::standard(app_params, agg_params)` `openvm.toml`</summary>
```toml [openvm_standard.toml]
// [!include ~/snippets/examples_sdk_toml/openvm_standard.toml]
```
</details>

Observe that this standard `openvm.toml` also enables normal Rust and `openvm::io` functions (via the `rv64i`, `rv64m`, and `io` extensions). `keccak` and `sha2` enable intrinsic instructions for the [Keccak](/book/acceleration-using-extensions/keccak) and [SHA-2](/book/acceleration-using-extensions/sha-2) hashes respectively, and `bigint` supports [Big Integer](/book/acceleration-using-extensions/big-integer) operations.
...
`SdkVmConfig::to_toml()` serializes a config back to TOML, which is useful after programmatic setup fills generated fields such as supported [deferral](/book/acceleration-using-extensions/deferral) circuit commits.
...
use openvm_stark_sdk::config::{app_params_with_128_bits_field_security, MAX_APP_LOG_STACKED_HEIGHT};
```
...
### Caching SDK Proving Keys

Generated SDK proving keys can be saved and reused with `Sdk::cached_proving_key()`. This returns an `SdkCachedProvingKey` containing the app proving key, aggregation proving keys, and any already-generated deferral proving keys. Halo2 proving keys are intentionally not included; use the Halo2-specific read/write helpers for those.

```rust
let _ = sdk.app_keygen();
let _ = sdk.agg_pk();
let cached_pk = sdk.cached_proving_key()?;

let sdk = Sdk::from_cached_proving_key(cached_pk)?;
```

For SDKs that include supported deferral proving keys, use `Sdk::from_deferral_cached_proving_key(cached_pk)` instead. That constructor reads the serialized `DeferralConfig` from the cached app VM config and reconstructs the supported deferral provers. Custom deferral circuits still need to be supplied manually through `Sdk::builder().deferral_agg_prover(...)` or `Sdk::builder().multi_deferral_circuit_prover(...)`.
