...
You can also use the `SdkVmConfig::standard()` or `SdkVmConfig::riscv32()` methods to get commonly used default
configurations supporting all default OpenVM extensions or just RV32IM, respectively. To use a custom OpenVM configuration
specified in an `openvm.toml` file, you can use the `SdkVmConfig::from_toml()` method.
