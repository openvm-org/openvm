...
- [STARK Proof](#stark-proof): Generates a STARK proof of the guest program
...
After building and transpiling a program, you can then generate a proof. To do so, you need to commit your `VmExe`, format your input into `StdIn`, and then generate a proof.
...
After generating a proof, you can verify it. To do so, you need your verifying key and the output of your `generate_app_proof` call.
...
**Note**: you **do not** need to generate the STARK proof with the `prove` function, as the EVM proof function will handle this automatically.
