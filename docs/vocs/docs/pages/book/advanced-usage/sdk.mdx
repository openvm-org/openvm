...
For more information on the basic CLI flow, see [Overview of Basic Usage](../writing-apps/overview). Writing a guest program is the same as in the CLI.
...
// [!include ~/snippets/examples/sdk_app.rs:dependencies]
```
...
```rust,no_run,noplayground
// [!include ~/snippets/examples/sdk_app.rs:build]
// [!include ~/snippets/examples/sdk_app.rs:read_elf]

// [!include ~/snippets/examples/sdk_app.rs:transpilation]
```
...
```rust,no_run,noplayground
// [!include ~/snippets/examples/sdk_app.rs:vm_config]
```
...
```rust,no_run,noplayground
// [!include ~/snippets/examples/sdk_app.rs:execution]
```
...
```rust,no_run,noplayground
// [!include ~/snippets/examples/sdk_app.rs:proof_generation]
```
...
```rust,no_run,noplayground
// [!include ~/snippets/examples/sdk_app.rs:verification]
```
...
To generate an EVM proof, you'll first need to ensure that you have followed the [CLI installation steps](../getting-started/install). get the appropriate KZG params by running the following command.
...
```rust,no_run,noplayground
// [!include ~/snippets/examples/sdk_evm.rs:keygen]
```
...
```rust,no_run,noplayground
// [!include ~/snippets/examples/sdk_evm.rs:evm_verification]
```

> ⚠️ **WARNING**
> The aggregation proving key `agg_pk` above is large. Avoid cloning it if possible.

Note that `DEFAULT_PARAMS_DIR` is the directory where Halo2 parameters are stored by the `cargo openvm setup` CLI command. For more information on the setup process, see the `EVM Level` section of the [verify](../writing-apps/verifying-proofs) doc.
