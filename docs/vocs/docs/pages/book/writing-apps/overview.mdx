...
:::warning
This command requires a large amount of compute and memory (~16 GB). Use `--download` to download pre-built verifier artifacts from S3 instead of generating them locally.
:::
