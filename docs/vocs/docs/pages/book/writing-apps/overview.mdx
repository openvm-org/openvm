...
:::note
The hex string must be of an even length. The reason for this is to avoid ambiguity: for example, if `12c` is passed, it is unclear whether it is supposed to be `012c`, `120c` or anything else. For `0x02` inputs, the remaining hex string length after the `0x02` prefix must be a multiple of `8`, as each native field element is represented as a little-endian `u32`.
:::
...
This writes version-dependent STARK aggregation and EVM proving artifacts to
`~/.openvm/v[OPENVM_VERSION]/`, where `OPENVM_VERSION` is the OpenVM major and minor version.
Version-independent KZG parameters remain in `~/.openvm/params/`.

:::warning
`cargo openvm setup --evm` requires a large amount of compute and memory (~16 GB). Use `--download` to download the pre-built Halo2 proving key and verifier artifacts from S3 instead of generating them locally.
:::
