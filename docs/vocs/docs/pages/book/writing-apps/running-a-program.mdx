...
- `--mode <MODE>`
  ...
  **Description**: Execution mode. Available options are `pure`, `meter`, and `segment`.
  ...
  - **segment**: Runs the program and calculates the number of segments that the execution will be split into for proving (see [continuations](/specs/architecture/continuations))
