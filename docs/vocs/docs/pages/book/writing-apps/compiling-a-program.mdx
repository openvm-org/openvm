...
The presently supported guest target is `riscv64im-unknown-openvm-elf` — a tier-3 built-in target in the [openvm-org/rust](https://github.com/openvm-org/rust) fork, installed via `cargo openvm toolchain install` (see [Install](/book/getting-started/install#install-the-openvm-rust-toolchain)). More details about how cross-compilation works in Rust can be found in the [Rust docs](https://rust-lang.github.io/rustup/cross-compilation.html).
...
OpenVM-specific artifacts will be placed in `${target-dir}/../openvm/` (by default `./openvm/`, a sibling of the target directory), but if `--output-dir` is specified they will be copied to `${output-dir}/` as well.
