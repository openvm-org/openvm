...
For a guest program example, see this [Fibonacci program](https://github.com/openvm-org/openvm-examples/tree/main/fibonacci). More examples can be found in the [benchmarks/guest](https://github.com/openvm-org/openvm/tree/main/benchmarks/guest) directory.
...
- `std::time` is deterministic and does not expose host wall-clock time. `Instant::now()` starts at zero, and `SystemTime::now()` returns `UNIX_EPOCH`; these APIs do not panic, but they do not report elapsed VM time or real time.
...
- Reading of environment variables will always return an empty string.
...
A crate with this header only builds for the openvm guest target; see below for the `cfg_attr`-gated variant (what `cargo openvm init` generates) that also builds under plain `cargo check` on the host.
...
This lets the same source build as a standard binary on the host and as an OpenVM guest, while `"std"` independently toggles the standard library.
