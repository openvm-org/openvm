...
This lets the same source build as a standard binary on the host (with `main()`) and as a `no_main` openvm guest, while `"std"` independently toggles the standard library.
