...
:::warning
If the `$HOME` environment variable is not set, this command may fail. This command requires very large amounts of computation and memory (~200 GB) and can take ~20mins on a `m6a.16xlarge` instance due to the keygen time.
:::
