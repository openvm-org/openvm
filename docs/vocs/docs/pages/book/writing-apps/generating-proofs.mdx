...
If `--app-pk` or `--agg-prefix-pk` are not provided, the command will search for them at `${target-dir}/../openvm/app.pk` and `${target-dir}/../openvm/agg_prefix.pk` respectively. If `--exe` is not provided, the command will call `build` before generating the commits. Many of the Cargo options available to `cargo openvm run` are available to `commit` too, for more information on the available options see [Run Flags](/book/writing-apps/running-a-program#run-flags).
