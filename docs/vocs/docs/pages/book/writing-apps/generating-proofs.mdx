...
:::warning
`cargo openvm setup --evm` requires a large amount of computation and memory (~16 GB).
:::
...
:::warning
If the `$HOME` environment variable is not set, this command may fail. This command requires a large amount of compute and memory (~16 GB) and can take ~7mins on a `m6a.16xlarge` instance.
:::

:::info
Users who wish to pre-download the published verifier artifacts and Halo2 parameters may use the following script:
:::
...
`cargo openvm setup --evm`, which has a large compute and memory requirement (~16 GB).
:::
