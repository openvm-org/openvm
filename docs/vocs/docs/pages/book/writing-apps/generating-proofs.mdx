...
The proving key, verification key, and aggregation prefix proving key will be written to `${target-dir}/../openvm/` (by default `./openvm/`), and also to `--output-dir` if specified. Use `--app-only` to skip generating the aggregation prefix key, which is used in STARK and EVM proving.
