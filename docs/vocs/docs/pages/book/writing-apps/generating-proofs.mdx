...
:::warning
If the `$HOME` environment variable is not set, this command may fail. This command requires large amounts of compute and memory (~200 GB) and can take ~6mins on a `m6a.16xlarge` instance.
:::
