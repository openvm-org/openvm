...
:::info
`cargo openvm keygen` requires that `num_public_value_cells` and `memory_dimensions` (derived from `memory_config`) match the defaults. These can be customized via `[app_vm_config.system.config]` in your `openvm.toml`, but non-default values will cause `keygen` to fail. Use the [SDK](/book/advanced-usage/sdk) directly for custom configurations.
:::
...
- `root.pk`
- `halo2.pk`
- `halo2/src/[OPENVM_VERSION]-base/Halo2Verifier.sol`
- `halo2/src/[OPENVM_VERSION]-base/OpenVmHalo2Verifier.sol`
- `halo2/src/[OPENVM_VERSION]-base/interfaces/IOpenVmHalo2Verifier.sol`
- `halo2/src/[OPENVM_VERSION]-base/verifier.bytecode.json`
- `params/`

to `~/.openvm/`, where `~` is the directory specified by environment variable `$HOME` and
`OPENVM_VERSION` is the version of OpenVM. The `-base` suffix matches the
...
The proving keys cached under `~/.openvm/` can be overridden with: `--agg-pk` for the internal-recursive key (`stark` and `evm`), and `--root-pk` and `--halo2-pk` for the root and Halo2 keys (`evm` only).
