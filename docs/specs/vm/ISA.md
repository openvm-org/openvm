...
| **KECCAK256**                                                                                                                                                                                                                      | `a, b, c`                                     | Let `input` be the **variable length** byte array of length `len <- proj(word[c]_d)` loaded from memory via `input[i] <- [proj(word[b]_d) + i]_e` for `i = 0..len`. Let `output = keccak256(input)` as `[u8; 32]`. This opcode sets `[proj(word[a]_d) + i]_e <- output[2*i] + output[2*i + 1] << 8` for `i = 0..16` (note this is **little-endian**). This opcode assumes that the input memory cells are **bytes** but does not range check them. The output is written to memory as **`u16` limbs**. |
