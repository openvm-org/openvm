...
| sw_setup\<C\>     | SETUP_EC_ADD_NE_RV32\<C\> `ind(rd), ind(rs1), x0, 1, 2` if `ind(rs2) != 0`, SETUP_EC_DOUBLE_RV32\<C\> `ind(rd), ind(rs1), x0, 1, 2` if `ind(rs2) = 0` |
| sw_hint_decompress| PHANTOM `ind(rd), ind(rs1), phantom_c(curve_idx, HintDecompress)` |
