...
We note that the definitions of the curve arithmetic operations for short Weierstrass curves do not depend on `C::B`. The VM configuration will specify a list of supported curves. For
...
| TeHintDecompress | 0x41         | `a,b,c_upper` | Uses `c_upper = C::IDX` to determine the index of the curve `C`, from the list of enabled curves. Read from memory `y = [r32{0}(a): C::COORD_SIZE]_2` for an element in the coordinate field of `C`. Let `rec_id = [r32{0}(b)]_2` be a byte in memory for the recovery id, where the lowest bit is 1 if and only if the `x` coordinate of the corresponding point is odd. The sub-instruction resets the hint stream to equal the unique `x: [_; C::COORD_SIZE]` such that `(x, y)` is a point on `C` with parity matching `rec_id`, if it exists, or to undefined `C::COORD_SIZE` elements otherwise.                                                                                                                                     |
