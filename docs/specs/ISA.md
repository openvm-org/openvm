...
| FRI_REDUCED_OPENING                                                                                                                                                                                                      | `a,b,c,d,e,f,g` | Let `a_ptr = [a]_4`, `b_ptr = [b]_4`, `length = [b+1]_4`, `alpha = [d:EXT_DEG]_4`, `hint_id = [e]_4`, `hint_offset = [f]_4`, `ood_point_idx = [g]_4`. `a_ptr` is the address of Felt array `a_arr` and `b_ptr` is the address of Ext array `b_arr`. Compute `sum((b_arr[i] - a_arr[i]) * alpha ^ i)` for `i=0..length` and write the value into `[c:EXT_DEG]_4`. If `ood_point_idx = 0`, read content of `a_arr` from the hint space and write into `a_arr`. Otherwise, read `a_arr` from memory.                                                                                                                                                                                                                                                                                                                                                                                                |
