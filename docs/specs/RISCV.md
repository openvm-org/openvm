...
| hint_final_exp             | R   | 0101011     | 011    | `idx*16 + 3` | Read `p: Fp` from `[rs1..]_2` and `q: Fp2` from `[rs2..]_2`, then compute `f: Fp12 = multi_miller_loop(p, q)`. Reset the hint stream to equal `hint_final_exp(f) = (residue_witness, scaling_factor): (Fp12, Fp12)` flattened into bytes. `rd` should be `x0`. |
