...
| 433 | `univariate_round_coeffs.len() == s_0_deg + 1` | UnivariateSumcheckAir: `coeff_idx` starts at `univariate_deg` (`batch_constraint/sumcheck/univariate/air.rs:120`, in `UnivariateSumcheckAir::eval`), decrements by 1 (`batch_constraint/sumcheck/univariate/air.rs:124`, in `UnivariateSumcheckAir::eval`), and must reach 0 on the last row (`batch_constraint/sumcheck/univariate/air.rs:126`, in `UnivariateSumcheckAir::eval`). `univariate_deg = (max_constraint_degree + 1) * (2^l_skip - 1)` is VK-determined when the AIR is constructed (`batch_constraint/mod.rs:366`, in `BatchConstraintModule::airs`). Forces exactly `univariate_deg + 1` rows. |
