...
graph TD
    ...
    Sumcheck -->|"receive (observe/sample)"| TranscriptBus
...
6. **Constraint randomness (ConstraintSumcheckRandomnessBus — receives):** Receives `r_round` from the batch constraint module when `has_r` is true (i.e., when `round <= n_max`).
7. **Tidx coordination (StackingModuleTidxBus — receives/sends):** Receives/sends tidx for phase coordination.
8. **Transcript (TranscriptBus — receives):** Observes `s_eval_at_1` and `s_eval_at_2`, and samples `u_round` for each round.
...
6. **Mu proof-of-work (ExpBitsLenBus — lookup):** When `mu_pow_bits > 0`, verifies the mu PoW witness via exponentiation lookup, with associated transcript interactions to observe the witness and sample the challenge.
