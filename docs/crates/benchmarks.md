...
build with target set to your **host** machine. If you want to directly build for `openvm` (more specifically a RISC-V target), run `cargo openvm build` and it will output a RISC-V ELF file to `target/riscv64im-unknown-openvm-elf/release/*`. You can install [cargo-binutils](https://github.com/rust-embedded/cargo-binutils) to be able to disassemble the ELF file:

```bash
rust-objdump -d target/riscv64im-unknown-openvm-elf/release/openvm-fibonacci-program
```
