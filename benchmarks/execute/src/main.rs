...
use openvm_circuit::{
    arch::{instructions::exe::VmExe, InitFileGenerator, SystemConfig, VirtualMachine, VmExecutor},
    derive::VmConfig,
};
...
fn main() -> Result<()> {
    ...
    run_with_metric_collection(&cli.output, || -> Result<()> {
        for program in &programs_to_run {
            ...
            //     let mut result = executor.execute_and_generate::<BabyBearPoseidon2Config>(
            //         exe.clone(),
            //         state,
            //         num_insns,
            //     )?;
