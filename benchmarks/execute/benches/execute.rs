...
use openvm_stark_sdk::{
    ...
    openvm_stark_backend::{self, keygen::types::MultiStarkVerifyingKey, p3_field::PrimeField32},
...
static SHARED_VK: OnceLock<MultiStarkVerifyingKey<BabyBearPoseidon2Config>> = OnceLock::new();
...
fn shared_vk(
    vm: &VirtualMachine<BabyBearPoseidon2Config, BabyBearPoseidon2Engine, ExecuteConfig>,
) -> &'static MultiStarkVerifyingKey<BabyBearPoseidon2Config> {
    SHARED_VK.get_or_init(|| {
        let pk = vm.keygen();
        pk.get_vk()
    })
}
...
fn benchmark_execute_metered(bencher: Bencher, program: &str) {
    bencher
        .with_inputs(|| {
            ...
            (vm.executor, exe, state, vk)
        })
        .bench_values(|(executor, exe, state, vk)| {
            executor
                .execute_metered_from_state(exe, state, vk.total_widths(), vk.num_interactions())
                .expect("Failed to execute program");
        });
}
