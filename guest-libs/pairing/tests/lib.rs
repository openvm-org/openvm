...
mod bls12_381 {
    ...
    /// PoC for `G1Affine::mul_scalar`'s prime-order assumption on a cofactor curve.
    ///
    /// Execute-only on purpose: the point is to isolate the guest-library bug from the EC_MUL
    /// chip's own totality argument, which also assumes prime order.
    #[test]
