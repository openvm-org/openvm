...
/*pub struct ModularMultiplicationBigIntCols<T> {
    pub io: ModularMultiplicationBigIntIoCols<T>,
    pub aux: ModularMultiplicationBigIntAuxCols<T>,
}

pub struct ModularMultiplicationBigIntIoCols<T> {
    pub a_elems: Vec<T>,
    pub b_elems: Vec<T>,
    pub r_elems: Vec<T>,
}

pub struct ModularMultiplicationBigIntAuxCols<T> {
    pub a_limbs: Vec<T>,
    pub b_limbs: Vec<T>,
    pub r_limbs: Vec<T>,
    pub q_limbs: Vec<T>,
...
impl<T: Clone> ModularMultiplicationBigIntAuxCols<T> {
...
impl<T: Clone> ModularMultiplicationBigIntIoCols<T> {
    pub fn from_slice(slc: &[T], air: &ModularMultiplicationAir) -> Self {
        let mut start = 0;
        let mut end = 0;
...
impl<T: Clone> ModularMultiplicationBigIntCols<T> {
    pub fn from_slice(slc: &[T], air: &ModularMultiplicationAir) -> Self {
        let io_width = ModularMultiplicationBigIntIoCols::<T>::get_width(air);
        let aux_width = ModularMultiplicationBigIntAuxCols::<T>::get_width(air);
        Self {
            io: ModularMultiplicationBigIntIoCols::from_slice(&slc[0..io_width], air),
            aux: ModularMultiplicationBigIntAuxCols::from_slice(
    ...
    pub fn get_width(air: &ModularMultiplicationAir) -> usize {
        ModularMultiplicationBigIntIoCols::<T>::get_width(air)
            + ModularMultiplicationBigIntAuxCols::<T>::get_width(air)
    }
}
*/
