...
fn test_flatten_fromslice_roundtrip() {
    let lt_air = AssertLessThanAir::new(RangeCheckBus::new(0, 1 << 8), 16, 8);

    let num_cols = AssertLessThanCols::<usize>::width(&lt_air);
    let all_cols = (0..num_cols).collect::<Vec<usize>>();

    let cols_numbered = AssertLessThanCols::<usize>::from_slice(&all_cols);
...
fn test_assert_less_than_chip_lt() {
    ...
    let chip = AssertLessThanChip::new(bus, max_bits, decomp, range_checker);
