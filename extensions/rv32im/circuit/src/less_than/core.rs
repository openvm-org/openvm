...
pub struct LessThanStep<F, AdapterAir, AdapterStep, const NUM_LIMBS: usize, const LIMB_BITS: usize>
{
    pub air: VmAirWrapper<AdapterAir, LessThanCoreAir<NUM_LIMBS, LIMB_BITS>>,
    adapter: AdapterStep,
...
impl<F, AdapterAir, AdapterStep, const NUM_LIMBS: usize, const LIMB_BITS: usize>
    LessThanStep<F, AdapterAir, AdapterStep, NUM_LIMBS, LIMB_BITS>
{
    pub fn new(
        ...
        Self {
            air: VmAirWrapper::new(
                adapter_air,
                LessThanCoreAir::new(bitwise_lookup_chip.bus(), offset),
            ),
...
impl<F, AdapterAir, AdapterStep, const NUM_LIMBS: usize, const LIMB_BITS: usize>
    InstructionExecutor<F> for LessThanStep<F, AdapterAir, AdapterStep, NUM_LIMBS, LIMB_BITS>
{
    fn get_opcode_name(&self, opcode: usize) -> String {
        format!(
            "{:?}",
            LessThanOpcode::from_usize(opcode - self.air.core.offset)
        )
    }
...
impl<F, AdapterAir, AdapterStep, RA, const NUM_LIMBS: usize, const LIMB_BITS: usize>
    InsExecutor<F, RA> for LessThanStep<F, AdapterAir, AdapterStep, NUM_LIMBS, LIMB_BITS>
where
    AdapterStep: 'static
...
impl<F, AdapterAir, AdapterStep, const NUM_LIMBS: usize, const LIMB_BITS: usize> InsExecutorE1<F>
    for LessThanStep<F, AdapterAir, AdapterStep, NUM_LIMBS, LIMB_BITS>
where
    F: PrimeField32,
    AdapterStep: 'static
        ...
        >,
{
        ...
        Ctx: E1E2ExecutionCtx,
    {
        ...
        // Run the comparison
        let (cmp_result, _, _, _) = run_less_than::<NUM_LIMBS, LIMB_BITS>(
            opcode.local_opcode_idx(self.air.core.offset) == LessThanOpcode::SLT as usize,
            &rs1,
            &rs2,
        );
...
impl<SC, AdapterAir, AdapterStep, const NUM_LIMBS: usize, const LIMB_BITS: usize> Chip<SC>
    for LessThanStep<Val<SC>, AdapterAir, AdapterStep, NUM_LIMBS, LIMB_BITS>
where
    SC: StarkGenericConfig,
    Val<SC>: PrimeField32,
    AdapterAir: BaseAir<Val<SC>>,
    VmAirWrapper<AdapterAir, LessThanCoreAir<NUM_LIMBS, LIMB_BITS>>: Clone + AnyRap<SC> + 'static,
