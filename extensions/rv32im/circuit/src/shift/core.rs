...
pub struct ShiftStep<F, AdapterAir, AdapterStep, const NUM_LIMBS: usize, const LIMB_BITS: usize> {
    pub air: VmAirWrapper<AdapterAir, ShiftCoreAir<NUM_LIMBS, LIMB_BITS>>,
...
impl<F, AdapterAir, AdapterStep, const NUM_LIMBS: usize, const LIMB_BITS: usize>
    ShiftStep<F, AdapterAir, AdapterStep, NUM_LIMBS, LIMB_BITS>
{
    pub fn new(
        ...
        Self {
            air: VmAirWrapper::new(
                adapter_air,
                ShiftCoreAir::new(bitwise_lookup_chip.bus(), range_checker_chip.bus(), offset),
            ),
...
impl<F, AdapterAir, AdapterStep, const NUM_LIMBS: usize, const LIMB_BITS: usize>
    InstructionExecutor<F> for ShiftStep<F, AdapterAir, AdapterStep, NUM_LIMBS, LIMB_BITS>
{
    fn get_opcode_name(&self, opcode: usize) -> String {
        format!(
            "{:?}",
            ShiftOpcode::from_usize(opcode - self.air.core.offset)
        )
    }
...
impl<F, AdapterAir, AdapterStep, RA, const NUM_LIMBS: usize, const LIMB_BITS: usize>
    InsExecutor<F, RA> for ShiftStep<F, AdapterAir, AdapterStep, NUM_LIMBS, LIMB_BITS>
where
    AdapterStep: 'static
    ...
    >,
{
        ...
        F: PrimeField32,
    {
        ...
        let local_opcode = ShiftOpcode::from_usize(opcode.local_opcode_idx(self.air.core.offset));
        ...
        let shift_opcode = ShiftOpcode::from_usize(local_opcode as usize);
        ...
        let (output, _, _) =
            run_shift::<NUM_LIMBS, LIMB_BITS>(shift_opcode, &core_record.b, &core_record.c);
...
impl<F, AdapterAir, AdapterStep, const NUM_LIMBS: usize, const LIMB_BITS: usize> InsExecutorE1<F>
    for ShiftStep<F, AdapterAir, AdapterStep, NUM_LIMBS, LIMB_BITS>
where
    F: PrimeField32,
    AdapterStep: 'static
...
impl<SC, AdapterAir, AdapterStep, const NUM_LIMBS: usize, const LIMB_BITS: usize> Chip<SC>
    for ShiftStep<Val<SC>, AdapterAir, AdapterStep, NUM_LIMBS, LIMB_BITS>
where
    SC: StarkGenericConfig,
    Val<SC>: PrimeField32,
    AdapterAir: BaseAir<Val<SC>>,
    VmAirWrapper<AdapterAir, ShiftCoreAir<NUM_LIMBS, LIMB_BITS>>: Clone + AnyRap<SC> + 'static,
