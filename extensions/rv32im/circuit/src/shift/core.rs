...
unsafe fn execute_e1_impl<F: PrimeField32, CTX: E1E2ExecutionCtx>(
    ...
    let rs1 = state.vm_read::<u8, 4>(RV32_REGISTER_AS, pre_compute.b);
    let rs2 = if pre_compute.e == RV32_REGISTER_AS {
    ...
    state.vm_write(RV32_REGISTER_AS, pre_compute.a, &rd);
