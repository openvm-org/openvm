...
pub struct DivRemStep<F, AdapterAir, AdapterStep, const NUM_LIMBS: usize, const LIMB_BITS: usize> {
    pub air: VmAirWrapper<AdapterAir, DivRemCoreAir<NUM_LIMBS, LIMB_BITS>>,
    adapter: AdapterStep,
...
impl<F, AdapterAir, AdapterStep, const NUM_LIMBS: usize, const LIMB_BITS: usize>
    DivRemStep<F, AdapterAir, AdapterStep, NUM_LIMBS, LIMB_BITS>
{
    pub fn new(
        ...
        Self {
            air: VmAirWrapper::new(
                adapter_air,
                DivRemCoreAir::new(bitwise_lookup_chip.bus(), *range_tuple_chip.bus(), offset),
            ),
...
impl<F, AdapterAir, AdapterStep, const NUM_LIMBS: usize, const LIMB_BITS: usize>
    InstructionExecutor<F> for DivRemStep<F, AdapterAir, AdapterStep, NUM_LIMBS, LIMB_BITS>
{
    fn get_opcode_name(&self, opcode: usize) -> String {
        format!(
            "{:?}",
            DivRemOpcode::from_usize(opcode - self.air.core.offset)
        )
    }
...
impl<F, AdapterAir, AdapterStep, RA, const NUM_LIMBS: usize, const LIMB_BITS: usize>
    InsExecutor<F, RA> for DivRemStep<F, AdapterAir, AdapterStep, NUM_LIMBS, LIMB_BITS>
where
    AdapterStep: 'static
    ...
    for<'a> RA: RecordArena<
        ...
        (
            ...
            &'a mut DivRemCoreRecords<NUM_LIMBS>,
        ),
    >,
{
    fn execute_tracegen(
...
impl<F, AdapterAir, AdapterStep, const NUM_LIMBS: usize, const LIMB_BITS: usize> InsExecutorE1<F>
    for DivRemStep<F, AdapterAir, AdapterStep, NUM_LIMBS, LIMB_BITS>
where
    F: PrimeField32,
    AdapterStep: 'static
        ...
        >,
{
        ...
        Ctx: E1E2ExecutionCtx,
    {
        ...
        let divrem_opcode = DivRemOpcode::from_usize(opcode.local_opcode_idx(self.air.core.offset));
...
impl<SC, AdapterAir, AdapterStep, const NUM_LIMBS: usize, const LIMB_BITS: usize> Chip<SC>
    for DivRemStep<Val<SC>, AdapterAir, AdapterStep, NUM_LIMBS, LIMB_BITS>
where
    SC: StarkGenericConfig,
    Val<SC>: PrimeField32,
    AdapterAir: BaseAir<Val<SC>>,
    VmAirWrapper<AdapterAir, DivRemCoreAir<NUM_LIMBS, LIMB_BITS>>: Clone + AnyRap<SC> + 'static,
