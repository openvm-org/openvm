...
#[cfg(feature = "cuda")]
use {
    crate::{adapters::Rv32BaseAluAdapterRecord, AddSubCoreRecord, Rv32AddSubChipGpu},
    openvm_circuit::arch::{
...
use super::{core::run_add_sub, AddSubCoreAir, Rv32AddSubChip, Rv32AddSubExecutor};
use crate::{
    ...
    AddSubFiller, Rv32AddSubAir,
};

const MAX_INS_CAPACITY: usize = 128;
type F = BabyBear;
type Harness = TestChipHarness<F, Rv32AddSubExecutor, Rv32AddSubAir, Rv32AddSubChip<F>>;
...
fn create_harness_fields(
...
) -> (Rv32AddSubAir, Rv32AddSubExecutor, Rv32AddSubChip<F>) {
    let air = Rv32AddSubAir::new(
    ...
    let executor =
        Rv32AddSubExecutor::new(Rv32BaseAluAdapterExecutor::new(), BaseAluOpcode::CLASS_OFFSET);
    let chip = Rv32AddSubChip::new(
        AddSubFiller::new(
...
fn set_and_execute<RA: Arena, E: PreflightExecutor<F, RA>>(
    ...
    let a = run_add_sub::<RV32_REGISTER_NUM_LIMBS, RV32_CELL_BITS>(opcode, &b, &c).map(F::from_u8);
...
#[test_case(ADD, 100)]
#[test_case(SUB, 100)]
fn rand_rv32_add_sub_test(opcode: BaseAluOpcode, num_ops: usize) {
...
#[test_case(ADD, 100)]
#[test_case(SUB, 100)]
fn rand_rv32_add_sub_test_persistent(opcode: BaseAluOpcode, num_ops: usize) {
    let mut rng = create_seeded_rng();
...
fn run_negative_alu_test(
    ...
    let modify_trace = |trace: &mut DenseMatrix<BabyBear>| {
        ...
        let cols: &mut AddSubCoreCols<F, RV32_REGISTER_NUM_LIMBS, RV32_CELL_BITS> =
...
fn run_add_sanity_test() {
    ...
    let result = run_add_sub::<RV32_REGISTER_NUM_LIMBS, RV32_CELL_BITS>(ADD, &x, &y);
...
fn run_sub_sanity_test() {
    ...
    let result = run_add_sub::<RV32_REGISTER_NUM_LIMBS, RV32_CELL_BITS>(SUB, &x, &y);
    for i in 0..RV32_REGISTER_NUM_LIMBS {
        assert_eq!(z[i], result[i])
    }
}
...
#[cfg(feature = "cuda")]
type GpuHarness =
    GpuTestChipHarness<F, Rv32AddSubExecutor, Rv32AddSubAir, Rv32AddSubChipGpu, Rv32AddSubChip<F>>;
...
fn create_cuda_harness(tester: &GpuChipTestBuilder) -> GpuHarness {
    ...
    let gpu_chip = Rv32AddSubChipGpu::new(
...
fn test_cuda_rand_add_sub_tracegen(opcode: BaseAluOpcode, num_ops: usize) {
    ...
    type Record<'a> = (
        ...
        &'a mut AddSubCoreRecord<RV32_REGISTER_NUM_LIMBS>,
    );
