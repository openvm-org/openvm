...
__attribute__((preserve_most)) void rvr_ext_sha256(
  ...
  if (likely(main_chip_idx != UINT32_MAX)) {
    ...
    rd_mem_u64_range(state, input_ptr, block_words, RVR_SHA256_BLOCK_WORDS);
    rd_mem_u64_range(state, state_ptr, state_words, RVR_SHA256_STATE_WORDS);
    ...
    wr_mem_u64_range(state, dst_ptr, state_words, RVR_SHA256_STATE_WORDS);
    trace_chip(state, block_hasher_chip_idx, RVR_SHA256_ROWS_PER_BLOCK);
    return;
  }
#endif
  rvr_ext_sha256_fallback(state, dst_ptr, state_ptr, input_ptr,
                          block_hasher_chip_idx);
}
