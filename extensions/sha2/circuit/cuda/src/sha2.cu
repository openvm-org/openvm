...
#include "block_hasher/tracegen.cuh"
...
#pragma unroll
    ...
    if (digest_row < trace_height) {
        ...
        for (uint32_t j = 0; j < V::ROUNDS_PER_ROW - 1; j++) {
            ...
            for (uint32_t limb = 0; limb < V::WORD_U16S; limb++) {
                SHA2INNER_WRITE_DIGEST(V, inner_row, schedule_helper.w_3[j][limb], Fp(word_to_u16_limb<V>(val, limb)));
            }
        }

        typename V::Word final_hash[V::HASH_WORDS];
        for (int i = 0; i < static_cast<int>(V::HASH_WORDS); i++) {
            final_hash[i] = prev_hash[i] + (i == 0 ? a : (i == 1 ? b : (i == 2 ? c : (i == 3 ? d : (i == 4 ? e : (i == 5 ? f : (i == 6 ? g : h)))))));
    ...
    for (uint32_t row_in_block = 0; row_in_block < V::ROUND_ROWS; row_in_block++) {
        ...
        for (uint32_t j = 0; j < V::ROUNDS_PER_ROW; j++) {
            for (uint32_t limb = 0; limb < V::WORD_U16S; limb++) {
                SHA2INNER_WRITE_ROUND(V, inner_row, schedule_helper.intermed_4[j][limb], Fp(intermed_4[row_in_block][j][limb]));
                SHA2INNER_WRITE_ROUND(V, inner_row, schedule_helper.intermed_8[j][limb], Fp(intermed_8[row_in_block][j][limb]));
                SHA2INNER_WRITE_ROUND(V, inner_row, schedule_helper.intermed_12[j][limb], Fp(intermed_12[row_in_block][j][limb]));
            }
            if (j < V::ROUNDS_PER_ROW - 1 && row_in_block > 0) {
                for (uint32_t limb = 0; limb < V::WORD_U16S; limb++) {
                    SHA2INNER_WRITE_ROUND(V, inner_row, schedule_helper.w_3[j][limb], Fp(w_3[row_in_block][j][limb]));
                }
            }
        }
    }
...
__global__ void sha2_second_pass_dependencies(Fp *trace, size_t trace_height, size_t total_blocks) {
    ...
    if (last_round_row >= trace_height || digest_row >= trace_height || next_block_row_base >= trace_height) {
        return;
    }
    ...
    auto read_carry = [](RowSlice inner, uint32_t i, uint32_t limb) {
        size_t base = SHA2_COL_INDEX(V, Sha2RoundCols, message_schedule.carry_or_buffer) + i * V::WORD_U8S * 2;
    ...
    for (uint32_t i = 0; i < V::ROUNDS_PER_ROW; i++) {
        ...
        for (uint32_t limb = 0; limb < V::WORD_U16S; limb++) {
            ...
            SHA2INNER_WRITE_ROUND(V, last_round_inner, schedule_helper.intermed_12[i][limb], intermed);
        }
    }
    ...
    for (uint32_t i = 0; i < V::ROUNDS_PER_ROW; i++) {
        ...
        for (uint32_t limb = 0; limb < V::WORD_U16S; limb++) {
            uint32_t val = word_to_u16_limb<V>(w_vals_next[i], limb) + word_to_u16_limb<V>(sig_w, limb);
...
int launch_sha2_second_pass_dependencies(
    ...
    sha2_second_pass_dependencies<V><<<grid_size, block_size>>>(d_trace, trace_height, total_blocks);
    return CHECK_KERNEL();
}
