...
use crate::{
    MessageType, Sha2BlockHasherVmConfig, Sha2BlockHasherVmDigestColsRef,
    Sha2BlockHasherVmRoundColsRef, INNER_OFFSET,
};
...
impl<C: Sha2BlockHasherVmConfig> Sha2BlockHasherVmAir<C> {
    pub fn new(bitwise_lookup_bus: BitwiseOperationLookupBus, sha2_bus_idx: BusIndex) -> Self {
        Self {
            inner: Sha2BlockHasherSubAir::new(bitwise_lookup_bus),
...
impl<C: Sha2BlockHasherVmConfig> Sha2BlockHasherVmAir<C> {
    fn eval_interactions<AB: InteractionBuilder>(&self, builder: &mut AB) {
        ...
        let local = Sha2BlockHasherVmDigestColsRef::<AB::Var>::from::<C>(
        ...
        let local = Sha2BlockHasherVmRoundColsRef::<AB::Var>::from::<C>(
            &local_slice[..C::BLOCK_HASHER_ROUND_WIDTH],
        );
        let next = Sha2BlockHasherVmRoundColsRef::<AB::Var>::from::<C>(
        ...
        let get_ith_byte = |i: usize, cols: &Sha2BlockHasherVmRoundColsRef<AB::Var>| {
        ...
        self.sha2_bus.receive(
            ...
            is_local_first_row * local.inner.flags.is_enabled(),
        );
        ...
        self.sha2_bus.receive(
            ...
            .chain(local_message)
            .chain(next_message),
            is_local_third_row * local.inner.flags.is_enabled(),
        );
    }
    ...
    fn eval_request_id<AB: InteractionBuilder>(&self, builder: &mut AB) {
        ...
        // field
        let local =
            Sha2BlockHasherVmRoundColsRef::<AB::Var>::from::<C>(&local[..C::BLOCK_HASHER_WIDTH]);
        let next =
            Sha2BlockHasherVmRoundColsRef::<AB::Var>::from::<C>(&next[..C::BLOCK_HASHER_WIDTH]);

        builder
            .when_transition()
            .when(not(*local.inner.flags.is_digest_row))
