...
use crate::{
    ...
    Sha2BlockHasherVmAir, Sha2BlockHasherVmConfig, Sha2BlockHasherVmDigestColsRefMut, Sha2Config,
...
fn create_test_harness<RA: Arena, C: Sha2Config>(
    ...
    (Sha2BlockHasherVmAir<C>, Sha2BlockHasherChip<F, C>),
) {
    ...
    let block_hasher_air = Sha2BlockHasherVmAir::new(bitwise_chip.bus(), SHA2_BUS_IDX);
...
#[test]
fn debug() {
    println!("main chip width: {}", Sha256Config::MAIN_CHIP_WIDTH);
    println!(
        "block hasher chip width: {}",
        Sha256Config::BLOCK_HASHER_WIDTH
    );
    println!("main chip width: {}", Sha512Config::MAIN_CHIP_WIDTH);
    println!(
        "block hasher chip width: {}",
        Sha512Config::BLOCK_HASHER_WIDTH
    );
    println!("main chip width: {}", Sha384Config::MAIN_CHIP_WIDTH);
    println!(
        "block hasher chip width: {}",
        Sha384Config::BLOCK_HASHER_WIDTH
    );
    println!();
    println!(
        "Sha2RecordMut::size(Sha256): {}",
        Sha2RecordMut::size(&Sha2RecordLayout {
    ...
    println!(
        "Sha256: Sha2BlockHasherVmWidth: {}",
        Sha256Config::BLOCK_HASHER_WIDTH
    );
    println!(
        "Sha512: Sha2BlockHasherVmWidth: {}",
        Sha512Config::BLOCK_HASHER_WIDTH
    );
    println!(
        "Sha384: Sha2BlockHasherVmWidth: {}",
        Sha384Config::BLOCK_HASHER_WIDTH
    );
    println!(
        "Sha256: Sha2BlockHasherSubairWidth: {}",
        Sha256Config::SUBAIR_WIDTH
    );
    println!(
        "Sha512: Sha2BlockHasherSubairWidth: {}",
        Sha512Config::SUBAIR_WIDTH
    );
    println!(
        "Sha384: Sha2BlockHasherSubairWidth: {}",
        Sha384Config::SUBAIR_WIDTH
    );
}
...
// Note that this test is actually the same as rand_sha512_single_block_test, but we include it here
// for completeness.
#[test]
...
fn set_and_execute_full_message<RA: Arena, C: Sha2Config + 'static, E: PreflightExecutor<F, RA>>(
    ...
    let output = read_slice_from_memory(tester, dst_ptr, C::STATE_BYTES)
        .into_iter()
...
// Note that this test is different from rand_sha512_multi_block_test because it uses the initial
// hash state for SHA384 instead of SHA512.
#[test]
...
fn execute_roundtrip_sanity_test<C: Sha2Config + 'static>() {
    let mut rng = create_seeded_rng();
    let mut tester = VmChipTestBuilder::default();
    let (mut harness, _, _) = create_test_harness::<MatrixRecordArena<F>, C>(&mut tester);
