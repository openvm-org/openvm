...
pub struct Sha2InstructionCols<T> {
    ...
    pub input_reg_ptr: T,
    // Register values
    /// dst_ptr_limbs <- \[dst_reg_ptr:8\]_1 (upper 4 bytes constrained to zero)
    pub dst_ptr_limbs: [T; RV64_REGISTER_NUM_LIMBS],
    /// state_ptr_limbs <- \[state_reg_ptr:8\]_1 (upper 4 bytes constrained to zero)
    pub state_ptr_limbs: [T; RV64_REGISTER_NUM_LIMBS],
    /// input_ptr_limbs <- \[input_reg_ptr:8\]_1 (upper 4 bytes constrained to zero)
