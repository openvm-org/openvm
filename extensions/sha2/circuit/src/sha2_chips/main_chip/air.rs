...
use openvm_riscv_circuit::adapters::{
    compose_rv64_u16_limbs, expand_to_rv64_block_from_slice, rv64_ptr_max_bits_shift,
...
impl<C: Sha2MainChipConfig + Sha2BlockHasherSubairConfig> Sha2MainAir<C> {
    pub fn eval_block<AB: InteractionBuilder>(
        ...
        // The message is already packed as u16 cells. Split the payload in half for the
        // two sha2-bus message rows.
    ...
    pub fn eval_instruction<AB: InteractionBuilder>(
        ...
        let shift = AB::F::from_usize(1 << rv64_ptr_max_bits_shift(self.ptr_max_bits));
        for limbs in [
            local.instruction.dst_ptr_limbs,
            local.instruction.state_ptr_limbs,
            local.instruction.input_ptr_limbs,
            ...
            self.range_bus
                .range_check(limbs[RV64_PTR_U16_LIMBS - 1] * shift, RV64_U16_LIMB_BITS)
                .eval(builder, *local.instruction.is_enabled);
        }
