...
pub fn add_padding_to_message<C: Sha2Config + 'static>(mut message: Vec<u8>) -> Vec<u8> {
    ...
    let padding_len_bits = (C::BLOCK_BITS
        - ((message_len_bits + 1 + C::MESSAGE_LENGTH_BITS) % C::BLOCK_BITS))
        % C::BLOCK_BITS;
    message.push(0x80);
    ...
    match C::VARIANT {
        Sha2Variant::Sha256 => {
            message.extend_from_slice(&((message_len_bits as u64).to_be_bytes()));
        }
        Sha2Variant::Sha512 => {
            message.extend_from_slice(&((message_len_bits as u128).to_be_bytes()));
        }
        Sha2Variant::Sha384 => {
            message.extend_from_slice(&((message_len_bits as u128).to_be_bytes()));
        }
