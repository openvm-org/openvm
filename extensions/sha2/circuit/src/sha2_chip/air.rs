...
impl<C: Sha2ChipConfig> Sha2VmAir<C> {
    pub fn new(
        ...
        Self {
            ...
            // optimization opportunity: we use fewer encoder cells for sha256 than sha512 or sha384
...
pub(super) enum PaddingFlags {
    ...
    /// NOTE: if the Last row has padding it has to be at least:
    ///   - 9 cells since the last 8 cells are padded with the message length (for SHA-256)
    ///   - 17 cells since the last 16 cells are padded with the message length (for SHA-512)
...
impl<C: Sha2ChipConfig> Sha2VmAir<C> {
    ...
    fn eval_padding_row<AB: InteractionBuilder>(
        ...
        // We can't support messages longer than 2^29 bytes because the length has to fit in a
        // field element. So, constrain that the first few bytes of the length are 0 (so only the
        // last 4 bytes of the length can be nonzero). Thus, the bit-length is < 2^32 so the message
        // is < 2^29 bytes.
        // For SHA-256, assert bytes 8..12 are 0, because the message length is 8 bytes, and each
        // row has 16 bytes.
        // For SHA-512 and SHA-384, assert bytes 16..28 are 0, because the
        // message length is 16 bytes and each row has 32 bytes.
        for i in C::CELLS_PER_ROW - C::MESSAGE_LENGTH_BITS / 8..C::CELLS_PER_ROW - 4 {
            builder
