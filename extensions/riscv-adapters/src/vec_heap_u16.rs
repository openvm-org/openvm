...
use openvm_riscv_circuit::adapters::{
    tracing_read_reg_ptr, tracing_read_u16, tracing_write_u16, RV64_LOW32_BITS,
    RV64_LOW32_U16_LIMBS, RV64_U16_LIMB_BITS,
};
...
> {
    ...
    pub rs_val: [[T; RV64_LOW32_U16_LIMBS]; NUM_READS],
    /// Low 32 bits of rd register as u16 limbs.
    pub rd_val: [T; RV64_LOW32_U16_LIMBS],
    ...
    > VmAdapterAir<AB> for Rv64VecHeapU16AdapterAir<NUM_READS, BLOCKS_PER_READ, BLOCKS_PER_WRITE>
{
    ...
    fn eval(
        ...
        let limb_shift = AB::F::from_u32(1 << (RV64_LOW32_BITS - self.address_bits));
        for val in cols.rs_val.iter().chain(once(&cols.rd_val)) {
            self.range_bus
                .range_check(val[1] * limb_shift, RV64_U16_LIMB_BITS)
                .eval(builder, ctx.instruction.is_valid.clone());
        }

        // Compose the materialized u16 cells of each register value into a memory address.
        let compose = |val: [AB::Var; RV64_LOW32_U16_LIMBS]| -> AB::Expr {
            val[0] + val[1] * AB::F::from_u32(1 << RV64_U16_LIMB_BITS)
        };
    ...
    for Rv64VecHeapU16AdapterFiller<NUM_READS, BLOCKS_PER_READ, BLOCKS_PER_WRITE>
{
    ...
    fn fill_trace_row(&self, mem_helper: &MemoryAuxColsFactory<F>, mut adapter_row: &mut [F]) {
        ...
        debug_assert!(self.pointer_max_bits <= RV64_LOW32_BITS);
        let limb_shift_bits = RV64_LOW32_BITS - self.pointer_max_bits;
        for &v in record.rs_vals.iter().chain(once(&record.rd_val)) {
            self.range_checker_chip.add_count(
                (v >> RV64_U16_LIMB_BITS) << limb_shift_bits,
                RV64_U16_LIMB_BITS,
            );
        }
