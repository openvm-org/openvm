...
#pragma unroll
        for (size_t i = 1; i < NUM_LIMBS; i++) {
            c[i] = imm_sign * 0xFFFFu;
        }
        c[0] = imm_low11 + imm_sign * 0xF800u;
        ...
        if (diff_idx == NUM_LIMBS) {
            ...
            diff_val = uint32_t(uint16_t(uint32_t(c[diff_idx]) - uint32_t(b[diff_idx])));
        } else {
            diff_val = uint32_t(uint16_t(uint32_t(b[diff_idx]) - uint32_t(c[diff_idx])));
        }
    ...
    fill_trace_row(RowSlice row, LessThanImmCoreRecord<NUM_LIMBS, LIMB_BITS> record) {
        fill_trace_row(
            row, record.b, record.imm_low11, record.imm_sign, record.local_opcode
        );
    }
};
