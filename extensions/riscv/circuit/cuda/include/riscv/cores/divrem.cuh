...
#pragma unroll
        ...
        // Mirror the AIR's non-MSB read-side `send_range(b[i], c[i])`. For the
        // unsigned path the MSB also needs an explicit u8 check; the signed path's
        // MSB is covered by the sign-shifted send above.
#pragma unroll
