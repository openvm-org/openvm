...
#pragma unroll
    for (size_t i = 0; i < NUM_LIMBS; i++) {
        val |= (uint64_t)limbs[i] << (i * BYTE_BITS);
    }
    return val;
}
...
#pragma unroll
    for (size_t i = 0; i < NUM_LIMBS; i++) {
        limbs[i] = (uint8_t)(val >> (i * BYTE_BITS));
    }
}
...
template <typename T, size_t NUM_LIMBS> struct DivRemCoreCols {
    ...
    // field inverse of r_prime[i] - 2^BYTE_BITS, ensures each r_prime[i] is in range.
    T r_prime[NUM_LIMBS];
    T r_inv[NUM_LIMBS];
...
template <size_t NUM_LIMBS> struct DivRemCore {
    ...
    static constexpr size_t TOTAL_BITS = NUM_LIMBS * BYTE_BITS;
    ...
    __device__ void fill_trace_row(RowSlice row, DivRemCoreRecords<NUM_LIMBS> const &record) {
        ...
        bool b_sign = is_signed && (record.b[NUM_LIMBS - 1] >> (BYTE_BITS - 1));
        bool c_sign = is_signed && (record.c[NUM_LIMBS - 1] >> (BYTE_BITS - 1));
        bool q_sign = false;
...
#pragma unroll
        ...
        Fp r_sum_f = Fp(r_sum);
        bool use_r_sum_inv = case_none && !r_zero;
        if (use_r_sum_inv) {
            product *= r_sum_f;
        }

        // Batch all enabled per-row inverses. Prefixes for the limb inverses live temporarily in
        // their final trace cells, avoiding two register-resident arrays. AIR-disabled witnesses
        // are written as zero below and excluded from the product.
        Fp product_inv = inv(product);
        COL_WRITE_VALUE(row, Cols, r_sum_inv,
                        use_r_sum_inv ? product_inv * r_sum_prefix : Fp(0));
        if (use_r_sum_inv) {
            product_inv *= r_sum_f;
        }
...
#pragma unroll
        for (int i = NUM_LIMBS - 1; i >= 0; i--) {
            ...
            COL_WRITE_VALUE(row, Cols, r_inv[i], sign_xor ? product_inv * prefix : Fp(0));
            if (sign_xor) {
...
#pragma unroll
            ...
            carry = carry >> BYTE_BITS;
            range_tuple_checker.add_count((uint32_t[2]){(uint32_t)q[i], carry});
        }
        bool r_sign = is_signed && (r[NUM_LIMBS - 1] >> (BYTE_BITS - 1));

        uint32_t q_ext = (q_sign && is_signed) * ((1 << BYTE_BITS) - 1);
        uint32_t c_ext = (c_sign << BYTE_BITS) - c_sign;
        uint32_t r_ext = (r_sign << BYTE_BITS) - r_sign;
