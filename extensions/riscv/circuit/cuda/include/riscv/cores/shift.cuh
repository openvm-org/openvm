...
template <typename T, size_t NUM_LIMBS> struct ShiftLeftCoreCols {
    T a[NUM_LIMBS];
    T b[NUM_LIMBS];
    T c[NUM_LIMBS];
...
template <typename T, size_t NUM_LIMBS> struct ShiftRightCoreCols {
    T a[NUM_LIMBS];
    T b[NUM_LIMBS];
    T c[NUM_LIMBS];
...
template <size_t NUM_LIMBS> struct ShiftLeftCore {
    BitwiseOperationLookup bitwise_lookup;
    VariableRangeChecker range_checker;

    template <typename T> using Cols = ShiftLeftCoreCols<T, NUM_LIMBS>;

    __device__ ShiftLeftCore(BitwiseOperationLookup lookup, VariableRangeChecker range)
...
#pragma unroll
            for (size_t i = 0; i < NUM_LIMBS; i++) {
                uint8_t carry = record.b[i] >> (RV64_BYTE_BITS - bit_shift);
...
template <size_t NUM_LIMBS> struct ShiftRightCore {
    BitwiseOperationLookup bitwise_lookup;
    VariableRangeChecker range_checker;

    template <typename T> using Cols = ShiftRightCoreCols<T, NUM_LIMBS>;

    __device__ ShiftRightCore(BitwiseOperationLookup lookup, VariableRangeChecker range)
        : bitwise_lookup(lookup), range_checker(range) {}
...
#pragma unroll
        ...
        uint8_t b_sign_val = is_sra ? (record.b[NUM_LIMBS - 1] >> (RV64_BYTE_BITS - 1)) : 0u;
