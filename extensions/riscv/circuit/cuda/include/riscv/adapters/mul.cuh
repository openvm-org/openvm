#pragma once

#include "primitives/execution.h"
#include "primitives/trace_access.h"
#include "system/memory/controller.cuh"
#include "system/memory/offline_checker.cuh"

using namespace riscv;

template <typename T> struct MultAdapterCols {
    ExecutionState<T> from_state;
    T rd_ptr;
    T rs1_ptr;
    T rs2_ptr;
    MemoryReadAuxCols<T> reads_aux[2];
    MemoryWriteAuxCols<T, BLOCK_FE_WIDTH> writes_aux;
};

struct MultAdapterRecord {
    uint32_t from_pc;
    uint32_t from_timestamp;
    uint32_t rd_ptr;
    uint32_t rs1_ptr;
    uint32_t rs2_ptr;
    MemoryReadAuxRecord reads_aux[2];
    MemoryWriteBytesAuxRecord<REGISTER_NUM_LIMBS> writes_aux;
};

struct MultAdapter {
    MemoryAuxColsFactory mem_helper;

    __device__ MultAdapter(VariableRangeChecker range_checker, uint32_t timestamp_max_bits)
        : mem_helper(range_checker, timestamp_max_bits) {}

    __device__ void fill_trace_row(RowSlice row, MultAdapterRecord record);
};

__device__ inline void MultAdapter::fill_trace_row(RowSlice row, MultAdapterRecord record) {
    uint32_t ts = record.from_timestamp;

    Fp packed_prev[BLOCK_FE_WIDTH];
    pack_u8_block_bytes(packed_prev, record.writes_aux.prev_data);
    COL_WRITE_ARRAY(row, MultAdapterCols, writes_aux.prev_data, packed_prev);
    mem_helper.fill(
        row.slice_from(COL_INDEX(MultAdapterCols, writes_aux)),
        record.writes_aux.prev_timestamp,
        ts + 2
    );

    for (int i = 0; i < 2; i++) {
        mem_helper.fill(
            row.slice_from(COL_INDEX(MultAdapterCols, reads_aux[i])),
            record.reads_aux[i].prev_timestamp,
            ts + i
        );
    }

    COL_WRITE_VALUE(row, MultAdapterCols, rs2_ptr, record.rs2_ptr);
    COL_WRITE_VALUE(row, MultAdapterCols, rs1_ptr, record.rs1_ptr);
    COL_WRITE_VALUE(row, MultAdapterCols, rd_ptr, record.rd_ptr);
    COL_WRITE_VALUE(row, MultAdapterCols, from_state.pc, ::program::pc_to_idx(record.from_pc));
    COL_WRITE_VALUE(row, MultAdapterCols, from_state.timestamp, record.from_timestamp);
}
