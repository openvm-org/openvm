...
template <typename T> struct BaseAluRegAdapterCols {
...
struct BaseAluRegAdapterRecord {
    uint32_t from_pc;
...
static_assert(sizeof(BaseAluRegAdapterRecord) == 40);
static_assert(offsetof(BaseAluRegAdapterRecord, reads_aux) == 20);
static_assert(offsetof(BaseAluRegAdapterRecord, writes_aux) == 28);

struct BaseAluRegAdapter {
    MemoryAuxColsFactory mem_helper;

    __device__ BaseAluRegAdapter(
    ...
    __device__ void fill_trace_row(
        ...
        COL_WRITE_VALUE(row, BaseAluRegAdapterCols, rs2_ptr, rs2_ptr);
        mem_helper.fill(
            row.slice_from(COL_INDEX(BaseAluRegAdapterCols, reads_aux[0])),
            rs1_prev_timestamp,
            from_timestamp
        );
        mem_helper.fill(
            row.slice_from(COL_INDEX(BaseAluRegAdapterCols, reads_aux[1])),
            rs2_prev_timestamp,
            from_timestamp + 1
        );
        Fp packed_prev[BLOCK_FE_WIDTH];
        copy_u16_cells(packed_prev, write_prev_data);
        COL_WRITE_ARRAY(row, BaseAluRegAdapterCols, writes_aux.prev_data, packed_prev);
        mem_helper.fill(
            row.slice_from(COL_INDEX(BaseAluRegAdapterCols, writes_aux)),
    ...
    __device__ void fill_trace_row(RowSlice row, BaseAluRegAdapterRecord record) {
        ...
        COL_WRITE_VALUE(row, BaseAluRegAdapterCols, rd_ptr, record.rd_ptr);
        COL_WRITE_VALUE(row, BaseAluRegAdapterCols, rs1_ptr, record.rs1_ptr);
        COL_WRITE_VALUE(row, BaseAluRegAdapterCols, rs2_ptr, record.rs2_ptr);
