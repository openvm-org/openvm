...
template <typename T> struct MultWAdapterCols {
...
struct MultWAdapterRecord {
    ...
    uint8_t rs1_high[REGISTER_NUM_LIMBS - WORD_NUM_LIMBS];
    // Upper 4 bytes of rs2 register read, kept to satisfy the full-width memory read.
    uint8_t rs2_high[REGISTER_NUM_LIMBS - WORD_NUM_LIMBS];
    uint8_t result_sign;
...
struct MultWAdapter {
    ...
    __device__ void fill_trace_row(RowSlice row, MultWAdapterRecord record) {
        Fp packed_prev[BLOCK_FE_WIDTH];
        pack_u8_block_bytes(packed_prev, record.writes_aux.prev_data);
        COL_WRITE_ARRAY(row, MultWAdapterCols, writes_aux.prev_data, packed_prev);
        mem_helper.fill(
            row.slice_from(COL_INDEX(MultWAdapterCols, writes_aux.base)),
...
#pragma unroll
        for (int i = 0; i < 2; i++) {
            mem_helper.fill(
                row.slice_from(COL_INDEX(MultWAdapterCols, reads_aux[i])),
        ...
        bitwise_lookup.add_xor(
            static_cast<uint32_t>(record.result_word_msl), 1u << (BYTE_BITS - 1)
        );

        Fp rs2_high[PTR_U16_LIMBS];
        Fp rs1_high[PTR_U16_LIMBS];
        bytes_to_u16_limbs(rs2_high, record.rs2_high);
        bytes_to_u16_limbs(rs1_high, record.rs1_high);

        COL_WRITE_VALUE(row, MultWAdapterCols, result_sign, record.result_sign);
        COL_WRITE_ARRAY(row, MultWAdapterCols, rs2_high, rs2_high);
        COL_WRITE_ARRAY(row, MultWAdapterCols, rs1_high, rs1_high);
        COL_WRITE_VALUE(row, MultWAdapterCols, rs2_ptr, record.rs2_ptr);
        ...
        COL_WRITE_PC(row, MultWAdapterCols, from_state.pc, record.from_pc);
    }
};
