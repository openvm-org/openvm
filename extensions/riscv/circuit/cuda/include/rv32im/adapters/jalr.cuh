...
struct Rv64JalrAdapter {
    ...
    __device__ void fill_trace_row(RowSlice row, Rv64JalrAdapterRecord record) {
        ...
        if (do_write) {
            RowSlice aux_row = row.slice_from(COL_INDEX(Rv64JalrAdapterCols, rd_aux_cols));
            // NOTE: COL_WRITE_ARRAY uses the default NUM_LIMBS = RV32_REGISTER_NUM_LIMBS in MemoryWriteAuxCols template definition for size calculations, which is correct in this case for Rv64JalrAdapterCols
