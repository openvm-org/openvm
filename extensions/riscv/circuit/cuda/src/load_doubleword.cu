...
__global__ void rv64_load_doubleword_tracegen_compact(
    Fp *trace,
    size_t height,
...
#pragma unroll
        ...
        row.fill_zero(0, sizeof(Rv64LoadDoublewordCols<uint8_t>));
    }
}

extern "C" int _rv64_load_doubleword_tracegen_compact(
    Fp *trace,
    size_t height,
    size_t width,
    DeviceBufferConstView<RvrAlu3Compact> records,
    ...
    assert(width == sizeof(Rv64LoadDoublewordCols<uint8_t>));
    auto [grid, block] = kernel_launch_params(height, 512);
    rv64_load_doubleword_tracegen_compact<<<grid, block, 0, stream>>>(
        trace,
        height,
        records,
        operand_table,
...
DEFINE_RVR_G2_TRACEGEN_LAUNCHER_WITH_WIDTH(
    _rv64_load_doubleword_tracegen_g2, Rv64LoadDoublewordCols,
    rv64_load_doubleword_tracegen, Rv64LoadRecord, 512, pointer_max_bits,
