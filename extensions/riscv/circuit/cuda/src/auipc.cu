...
template <typename T> struct Rv64AuipcCoreCols {
    T is_valid;
    T is_sign_extend;
    T imm_low_8;   // imm & ((1 << RV64_CELL_BITS) - 1)
    T imm_high_16; // (imm >> RV64_CELL_BITS) & RV64_U16_LIMB_MASK
...
struct Rv64AuipcCore {
    ...
    __device__ void fill_trace_row(RowSlice row, Rv64AuipcCoreRecord record) {
        uint32_t imm_low_8 = record.imm & ((1u << RV64_CELL_BITS) - 1u);
        uint32_t imm_high_16 = (record.imm >> RV64_CELL_BITS) & RV64_U16_LIMB_MASK;
        auto auipc = run_auipc(record.from_pc, record.imm);
        uint16_t rd_lo = (uint16_t)(auipc & RV64_U16_LIMB_MASK);
        uint16_t rd_hi = (uint16_t)(auipc >> RV64_U16_LIMB_BITS);
        uint32_t is_sign_ext = (rd_hi >> RV64_U16_SIGN_BIT) & 1;

        // Range-check the immediate split and low-32 rd cells used by the relation.
        range_checker.add_count(imm_low_8, RV64_CELL_BITS);
        range_checker.add_count(imm_high_16, RV64_U16_LIMB_BITS);
        ...
        range_checker.add_count(
            2u * rd_hi - (is_sign_ext << RV64_U16_LIMB_BITS), RV64_U16_LIMB_BITS
        );
