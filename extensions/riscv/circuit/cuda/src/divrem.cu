...
__global__ void rv64_div_rem_replay_tracegen(
    ...
    if (program_index + 1 >= program_log.len()) { preflight_set_error(error, 661); return; }
    auto const &from = program_log[program_index];
    auto const &to = program_log[program_index + 1];
    if (from.pc < pc_base || (from.pc - pc_base) % 4 != 0) {
        preflight_set_error(error, 662); return;
    }
    size_t instruction_index = (from.pc - pc_base) / 4;
    if (instruction_index >= instructions.len()) { preflight_set_error(error, 663); return; }
