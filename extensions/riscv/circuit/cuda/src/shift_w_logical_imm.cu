...
__global__ void shift_w_logical_imm_replay_tracegen(
    Fp *trace,
    size_t height,
    ...
    size_t slliw_step_start,
    size_t num_slliw_steps,
    size_t srliw_step_start,
    size_t num_srliw_steps,
    uint32_t *error,
    uint32_t slliw_opcode,
    uint32_t srliw_opcode,
    ...
    bool is_slliw = idx < num_slliw_steps;
    size_t group_index = is_slliw ? idx : idx - num_slliw_steps;
    size_t step_index = (is_slliw ? slliw_step_start : srliw_step_start) + group_index;
    uint32_t expected_opcode = is_slliw ? slliw_opcode : srliw_opcode;
    uint8_t local_opcode = is_slliw ? 0 : 1;
    auto const &step = steps[step_index];
    size_t program_index = step.program_index;
    if (program_index + 1 >= program_log.len()) {
        preflight_set_error(error, 71);
        return;
    }
    ...
    if (from.pc < pc_base || (from.pc - pc_base) % 4 != 0 ||
        ...
        preflight_set_error(error, 72);
        return;
    }
    size_t instruction_index = (from.pc - pc_base) / 4;
    if (instruction_index >= instructions.len()) {
        preflight_set_error(error, 73);
        return;
    }
...
extern "C" int _shift_w_logical_imm_replay_tracegen(
    Fp *trace,
    size_t height,
    size_t width,
    DeviceBufferConstView<RvrReplayInstruction> instructions,
    ...
    assert(srliw_step_start <= steps.len());
