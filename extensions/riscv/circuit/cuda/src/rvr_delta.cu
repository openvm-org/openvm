...
extern "C" int _rvr_delta_predecode(
    ...
    if (!event_count_overflow) {
        ...
        event_count_overflow = delta_count > (SIZE_MAX - event_count) / 3;
        if (!event_count_overflow) event_count += delta_count * 3;
    }
    bool touched_bytes_overflow =
        event_count_overflow || event_count > UINT32_MAX ||
        event_count > SIZE_MAX / sizeof(TouchedMemoryRecord);
    if (delta_count > UINT32_MAX / 3 || event_count_overflow || touched_bytes_overflow ||
