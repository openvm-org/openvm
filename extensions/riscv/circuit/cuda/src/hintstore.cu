...
template <typename T> struct Rv64HintStoreCols {
    ...
    T mem_ptr_limbs[RV64_LOW32_U16_LIMBS];
...
struct Rv64HintStore {
    ...
    __device__ void fill_trace_row(
        ...
        uint32_t mem_ptr_limbs[RV64_LOW32_U16_LIMBS];
        pack_u32_to_u16_cells_le(mem_ptr_limbs, mem_ptr);
        ...
        if (local_idx == 0) {
            ...
            uint32_t mem_ptr_msl_lshift = RV64_LOW32_BITS - (uint32_t)pointer_max_bits;
            uint32_t mem_ptr_high_u16 = record.mem_ptr >> RV64_U16_LIMB_BITS;
            range_checker.add_count(
                mem_ptr_high_u16 << mem_ptr_msl_lshift, RV64_U16_LIMB_BITS
            );

            // Constrain rem_words < 2^MAX_HINT_BUFFER_DWORDS_BITS.
            uint32_t rem_words_lshift =
                RV64_U16_LIMB_BITS - (uint32_t)MAX_HINT_BUFFER_DWORDS_BITS;
            range_checker.add_count(record.num_words << rem_words_lshift, RV64_U16_LIMB_BITS);
