...
#pragma unroll
    for (int i = 0; i < NUM_LIMBS; i++) {
        ...
        out_carry[i] = out_mul[i] >> BYTE_BITS;
        out_mul[i] %= (1u << BYTE_BITS);
    }

    out_x_ext = (x[NUM_LIMBS - 1] >> (BYTE_BITS - 1)) *
                (opcode == MULHU ? 0 : ((1u << BYTE_BITS) - 1));
    out_y_ext = (y[NUM_LIMBS - 1] >> (BYTE_BITS - 1)) *
                (opcode == MULH ? ((1u << BYTE_BITS) - 1) : 0);
...
#pragma unroll
        ...
        out_carry[NUM_LIMBS + i] = out_mulh[i] >> BYTE_BITS;
        out_mulh[i] %= (1u << BYTE_BITS);
    }
}
...
#pragma unroll
        ...
        if (opcode != MULHU) {
            uint32_t b_sign_mask = (b_ext == 0) ? 0 : (1u << (BYTE_BITS - 1));
            uint32_t c_sign_mask = (c_ext == 0) ? 0 : (1u << (BYTE_BITS - 1));
...
template <typename T> struct MulHCols {
    ...
    MulHCoreCols<T, REGISTER_NUM_LIMBS> core;
};
