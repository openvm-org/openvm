...
__global__ void mulh_tracegen_compact(
    Fp *d_trace,
    size_t height,
...
#pragma unroll
        ...
        core.fill_trace_row(row.slice_from(COL_INDEX(MulHCols, core)), full.core);
    } else {
        row.fill_zero(0, sizeof(MulHCols<uint8_t>));
    }
}

extern "C" int _mulh_tracegen_compact(
    Fp *d_trace,
    size_t height,
    size_t width,
    ...
    mulh_tracegen_compact<<<grid, block, 0, stream>>>(
        d_trace,
        height,
        d_records,
        operand_table,
...
DEFINE_RVR_G2_TRACEGEN_LAUNCHER(
    _mulh_tracegen_g2, MulHCols, mulh_tracegen_compact, RvrAlu3Compact, 512,
