...
__global__ void rv64_store_halfword_tracegen_compact(
    Fp *trace,
    size_t height,
...
extern "C" int _rv64_store_halfword_tracegen_compact(
    Fp *trace,
    size_t height,
    size_t width,
    DeviceBufferConstView<RvrAlu3Compact> records,
    ...
    assert(width == sizeof(Rv64StoreHalfwordCols<uint8_t>));
    auto [grid, block] = kernel_launch_params(height, 512);
    rv64_store_halfword_tracegen_compact<<<grid, block, 0, stream>>>(
        trace,
        height,
        records,
        operand_table,
