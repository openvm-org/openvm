...
template <typename T> struct Rv64JalrCoreCols {
    T imm;
    T rs1_data[RV64_LOW32_U16_LIMBS];
    T rd_data[RV64_LOW32_HIGH_U16_LIMBS];
    T is_valid;
...
struct Rv64JalrCore {
    ...
    __device__ void fill_trace_row(RowSlice row, Rv64JalrCoreRecord record) {
        uint32_t offset =
            record.imm + (record.imm_sign ? (RV64_U16_LIMB_MASK << RV64_U16_LIMB_BITS) : 0);
        uint32_t to_pc = record.rs1_val + offset;
        assert(to_pc < (1u << PC_BITS));

        uint32_t to_pc_limbs[2] = {
            (to_pc & RV64_U16_LIMB_MASK) >> 1, to_pc >> RV64_U16_LIMB_BITS
        };
        // to_pc_limbs[0] is 15 bits because it is doubled to reconstruct
        // the aligned JALR target.
        rc.add_count(to_pc_limbs[0], RV64_U16_LIMB_BITS - 1);
        rc.add_count(to_pc_limbs[1], PC_BITS - RV64_U16_LIMB_BITS);

        uint32_t rd_low_u32 = record.from_pc + DEFAULT_PC_STEP;
        uint32_t rd_low_u16_lo = rd_low_u32 & RV64_U16_LIMB_MASK;
        uint32_t rd_low_u16_hi = (rd_low_u32 >> RV64_U16_LIMB_BITS) & RV64_U16_LIMB_MASK;
        ...
        rc.add_count(rd_low_u16_lo, RV64_U16_LIMB_BITS);
        rc.add_count(rd_low_u16_hi, PC_BITS - RV64_U16_LIMB_BITS);

        uint32_t rs1_u16_lo = record.rs1_val & RV64_U16_LIMB_MASK;
        uint32_t rs1_u16_hi = (record.rs1_val >> RV64_U16_LIMB_BITS) & RV64_U16_LIMB_MASK;
        // rs1 is read as two u16 cells.
        rc.add_count(rs1_u16_lo, RV64_U16_LIMB_BITS);
        rc.add_count(rs1_u16_hi, RV64_U16_LIMB_BITS);
        ...
        uint32_t rs1_limbs[RV64_LOW32_U16_LIMBS] = {rs1_u16_lo, rs1_u16_hi};
        COL_WRITE_ARRAY(row, Rv64JalrCoreCols, rs1_data, rs1_limbs);
        uint32_t rd_limbs[RV64_LOW32_HIGH_U16_LIMBS] = {rd_low_u16_hi};
