...
__global__ void rv64_load_word_replay_tracegen(
    Fp *trace,
    size_t height,
    ...
    core.fill_trace_row(
        row.slice_from(COL_INDEX(Rv64LoadWordCols, core)), input.read_data, input.shift
    );
}
...
extern "C" int _rv64_load_word_replay_tracegen(
    Fp *d_trace,
    size_t height,
    size_t width,
    ...
    auto [grid, block] = kernel_launch_params(height, 512);
    rv64_load_word_replay_tracegen<<<grid, block, 0, stream>>>(
        d_trace,
        height,
        d_instructions,
        pc_base,
