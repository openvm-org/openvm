...
__global__ void load_word_replay_tracegen(
    Fp *trace,
    size_t height,
    ...
    row.fill_zero(0, sizeof(LoadWordCols<uint8_t>));
    if (idx >= num_steps) return;

    ReplayLoadMultiByteInput input = {};
    if (!replay_load_multibyte<WORD_ACCESS_WIDTH, false>(
            instructions,
            pc_base,
            program,
            memory,
    ...
    auto core = LoadWordCore(BitwiseOperationLookup(bitwise_lookup));
    core.fill_trace_row(
        row.slice_from(COL_INDEX(LoadWordCols, core)), input.read_data, input.shift
    );
}



extern "C" int _load_word_replay_tracegen(
    Fp *d_trace,
    size_t height,
    size_t width,
    ...
    assert(width == sizeof(LoadWordCols<uint8_t>));
    ...
    load_word_replay_tracegen<<<grid, block, 0, stream>>>(
        d_trace,
        height,
        d_instructions,
        pc_base,
