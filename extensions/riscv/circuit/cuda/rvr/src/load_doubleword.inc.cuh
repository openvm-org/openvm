...
__global__ void load_doubleword_replay_tracegen(
    Fp *trace,
    size_t height,
    ...
    row.fill_zero(0, sizeof(LoadDoublewordCols<uint8_t>));
    if (idx >= num_steps) return;

    ReplayLoadMultiByteInput input = {};
    if (!replay_load_multibyte<DOUBLEWORD_ACCESS_WIDTH, false>(
            instructions,
            pc_base,
            program,
            memory,
    ...
    auto core = LoadDoublewordCore(BitwiseOperationLookup(bitwise_lookup));
    core.fill_trace_row(
        row.slice_from(COL_INDEX(LoadDoublewordCols, core)),
        input.read_data,
        input.shift
    );
}
...
extern "C" int _load_doubleword_replay_tracegen(
    Fp *d_trace,
    size_t height,
    size_t width,
    ...
    assert(width == sizeof(LoadDoublewordCols<uint8_t>));
    ...
    load_doubleword_replay_tracegen<<<grid, block, 0, stream>>>(
        d_trace,
        height,
        d_instructions,
        pc_base,
