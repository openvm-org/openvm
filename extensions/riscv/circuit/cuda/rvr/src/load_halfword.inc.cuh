...
__global__ void load_halfword_replay_tracegen(
    Fp *trace,
    size_t height,
    ...
    row.fill_zero(0, sizeof(LoadHalfwordCols<uint8_t>));
    if (idx >= num_steps) return;

    ReplayLoadMultiByteInput input = {};
    if (!replay_load_multibyte<HALFWORD_ACCESS_WIDTH, false, 231>(
            instructions,
            pc_base,
            program,
            memory,
    ...
    auto core = LoadHalfwordCore(BitwiseOperationLookup(bitwise_lookup));
    core.fill_trace_row(
        row.slice_from(COL_INDEX(LoadHalfwordCols, core)),
        input.read_data,
        input.shift
    );
}
...
extern "C" int _load_halfword_replay_tracegen(
    Fp *d_trace,
    size_t height,
    size_t width,
    ...
    assert(width == sizeof(LoadHalfwordCols<uint8_t>));
    ...
    load_halfword_replay_tracegen<<<grid, block, 0, stream>>>(
        d_trace,
        height,
        d_instructions,
        pc_base,
