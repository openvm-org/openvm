...
__global__ void store_word_replay_tracegen(
    Fp *trace,
    size_t height,
    ...
    core.fill_trace_row(
        row.slice_from(COL_INDEX(StoreWordCols, core)),
        input.read_data,
        input.prev_data,
        input.shift
    );
...
extern "C" int _store_word_replay_tracegen(
    Fp *d_trace,
    size_t height,
    size_t width,
    ...
    assert(width == sizeof(StoreWordCols<uint8_t>));
    ...
    store_word_replay_tracegen<<<grid, block, 0, stream>>>(
        d_trace,
        height,
        d_instructions,
        pc_base,
