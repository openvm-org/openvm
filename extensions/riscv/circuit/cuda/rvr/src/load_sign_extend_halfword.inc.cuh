...
__global__ void load_sign_extend_halfword_replay_tracegen(
    Fp *trace,
    size_t height,
    ...
    row.fill_zero(0, sizeof(LoadSignExtendHalfwordCols<uint8_t>));
    ...
    auto adapter = LoadAdapter(pointer_max_bits, checker, timestamp_max_bits);
    ...
    auto core = LoadSignExtendHalfwordCore(checker, BitwiseOperationLookup(bitwise_lookup));
    core.fill_trace_row(
        row.slice_from(COL_INDEX(LoadSignExtendHalfwordCols, core)),
        input.read_data,
        input.shift
    );
}
...
extern "C" int _load_sign_extend_halfword_replay_tracegen(
    Fp *d_trace,
    size_t height,
    size_t width,
    ...
    assert(width == sizeof(LoadSignExtendHalfwordCols<uint8_t>));
    ...
    load_sign_extend_halfword_replay_tracegen<<<grid, block, 0, stream>>>(
        d_trace,
        height,
        d_instructions,
        pc_base,
