...
__global__ void load_sign_extend_word_replay_tracegen(
    Fp *trace,
    size_t height,
    ...
    row.fill_zero(0, sizeof(LoadSignExtendWordCols<uint8_t>));
    ...
    core.fill_trace_row(
        row.slice_from(COL_INDEX(LoadSignExtendWordCols, core)),
        input.read_data,
        input.shift
    );
}
...
extern "C" int _load_sign_extend_word_replay_tracegen(
    Fp *d_trace,
    size_t height,
    size_t width,
    ...
    assert(width == sizeof(LoadSignExtendWordCols<uint8_t>));
    ...
    load_sign_extend_word_replay_tracegen<<<grid, block, 0, stream>>>(
        d_trace,
        height,
        d_instructions,
        pc_base,
