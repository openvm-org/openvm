...
use crate::{
    adapters::{
        ...
        RV64_REGISTER_NUM_LIMBS, RV64_U16_LIMB_BITS, RV_B_TYPE_IMM_BITS,
    },
...
fn set_and_execute<RA: Arena, E: PreflightExecutor<F, RA>>(
    ...
    let (cmp_result, _, _, _) =
        run_cmp::<BLOCK_FE_WIDTH, RV64_U16_LIMB_BITS>(opcode.local_usize() as u8, &a, &b);
...
fn run_negative_branch_lt_test(
    ...
    let modify_trace = |trace: &mut DenseMatrix<BabyBear>| {
        ...
        let cols: &mut BranchLessThanCoreCols<F, BLOCK_FE_WIDTH, RV64_U16_LIMB_BITS> =
...
const A_LT_B_LOWER: [u16; BLOCK_FE_WIDTH] = [0x4900, 0x5638, 0x6459, 0xcd05];
const A_LT_B_HIGHER: [u16; BLOCK_FE_WIDTH] = [0x9100, 0x5638, 0x6459, 0xcd05];
...
fn run_cmp_unsigned_sanity_test() {
    let x: [u16; BLOCK_FE_WIDTH] = [0x9100, 0x5638, 0x6459, 0xcd05];
    let y: [u16; BLOCK_FE_WIDTH] = [0x4900, 0x5638, 0x6459, 0xcd05];
    let (cmp_result, diff_idx, x_sign, y_sign) =
        run_cmp::<BLOCK_FE_WIDTH, RV64_U16_LIMB_BITS>(BranchLessThanOpcode::BLTU as u8, &x, &y);
    assert!(!cmp_result);
    ...
    let (cmp_result, diff_idx, x_sign, y_sign) =
        run_cmp::<BLOCK_FE_WIDTH, RV64_U16_LIMB_BITS>(BranchLessThanOpcode::BGEU as u8, &x, &y);
    assert!(cmp_result);
    assert_eq!(diff_idx, 0);
    assert!(!x_sign); // unsigned
    assert!(!y_sign); // unsigned
...
fn run_cmp_same_sign_sanity_test() {
    ...
    let y: [u16; BLOCK_FE_WIDTH] = [0x4900, 0x5638, 0x6459, 0xcd05];
    let (cmp_result, diff_idx, x_sign, y_sign) =
        run_cmp::<BLOCK_FE_WIDTH, RV64_U16_LIMB_BITS>(BranchLessThanOpcode::BLT as u8, &x, &y);
    assert!(!cmp_result);
    assert_eq!(diff_idx, 0);
    ...
    let (cmp_result, diff_idx, x_sign, y_sign) =
        run_cmp::<BLOCK_FE_WIDTH, RV64_U16_LIMB_BITS>(BranchLessThanOpcode::BGE as u8, &x, &y);
    assert!(cmp_result);
    assert_eq!(diff_idx, 0);
    assert!(x_sign); // negative
    assert!(y_sign); // negative
...
fn run_cmp_diff_sign_sanity_test() {
    let x: [u16; BLOCK_FE_WIDTH] = [0x232d, 0x3719, 0x0000, 0x3700];
    let y: [u16; BLOCK_FE_WIDTH] = [0x22ad, 0xcd19, 0xffff, 0xcd00];
    let (cmp_result, diff_idx, x_sign, y_sign) =
        run_cmp::<BLOCK_FE_WIDTH, RV64_U16_LIMB_BITS>(BranchLessThanOpcode::BLT as u8, &x, &y);
    assert!(!cmp_result);
...
fn run_cmp_eq_sanity_test() {
    let x: [u16; BLOCK_FE_WIDTH] = [0x232d, 0x3719, 0x0000, 0x3700];
    let (cmp_result, diff_idx, x_sign, y_sign) =
        run_cmp::<BLOCK_FE_WIDTH, RV64_U16_LIMB_BITS>(BranchLessThanOpcode::BLT as u8, &x, &x);
    assert!(!cmp_result);
    assert_eq!(diff_idx, BLOCK_FE_WIDTH);
    assert_eq!(x_sign, y_sign);

    let (cmp_result, diff_idx, x_sign, y_sign) =
        run_cmp::<BLOCK_FE_WIDTH, RV64_U16_LIMB_BITS>(BranchLessThanOpcode::BLTU as u8, &x, &x);
    assert!(!cmp_result);
    assert_eq!(diff_idx, BLOCK_FE_WIDTH);
    assert_eq!(x_sign, y_sign);

    let (cmp_result, diff_idx, x_sign, y_sign) =
        run_cmp::<BLOCK_FE_WIDTH, RV64_U16_LIMB_BITS>(BranchLessThanOpcode::BGE as u8, &x, &x);
    assert!(cmp_result);
    assert_eq!(diff_idx, BLOCK_FE_WIDTH);
    assert_eq!(x_sign, y_sign);

    let (cmp_result, diff_idx, x_sign, y_sign) =
        run_cmp::<BLOCK_FE_WIDTH, RV64_U16_LIMB_BITS>(BranchLessThanOpcode::BGEU as u8, &x, &x);
    assert!(cmp_result);
    assert_eq!(diff_idx, BLOCK_FE_WIDTH);
    assert_eq!(x_sign, y_sign);
}
...
fn test_cuda_rand_branch_lt_tracegen(opcode: BranchLessThanOpcode, num_ops: usize) {
    ...
    type Record<'a> = (
        ...
        &'a mut BranchLessThanCoreRecord<BLOCK_FE_WIDTH, RV64_U16_LIMB_BITS>,
    );
    harness
        .dense_arena
