...
use super::adapters::{bytes_to_u16_block, REGISTER_NUM_LIMBS, RV_IS_TYPE_IMM_BITS};
...
fn gen_register_pointer_except(rng: &mut StdRng, excluded: usize) -> usize {
    loop {
        let pointer = gen_register_pointer(rng, REGISTER_NUM_LIMBS);
        if pointer != excluded {
            return pointer;
        }
    }
...
pub fn rand_write_register_or_imm(
    tester: &mut impl TestBuilder<BabyBear>,
    rs1_writes: [u8; REGISTER_NUM_LIMBS],
    rs2_writes: [u8; REGISTER_NUM_LIMBS],
    imm: Option<usize>,
    ...
    tester.write_bytes::<REGISTER_NUM_LIMBS>(1, rs1, rs1_writes.map(BabyBear::from_u8));
    if !rs2_is_imm {
        tester.write_bytes::<REGISTER_NUM_LIMBS>(1, rs2, rs2_writes.map(BabyBear::from_u8));
    }
...
#[cfg_attr(all(feature = "test-utils", not(test)), allow(dead_code))]
pub fn generate_is_type_immediate(rng: &mut StdRng) -> (usize, [u8; REGISTER_NUM_LIMBS]) {
...
#[cfg_attr(all(feature = "test-utils", not(test)), allow(dead_code))]
pub fn marker_bytes_to_u16_marker(marker: [u8; REGISTER_NUM_LIMBS]) -> [u32; BLOCK_FE_WIDTH] {
    bytes_to_u16_block(marker).map(|marker| u32::from(marker != 0))
}

#[cfg_attr(all(feature = "test-utils", not(test)), allow(dead_code))]
pub fn msb_byte_prank_to_u16_limb(bytes: [u8; REGISTER_NUM_LIMBS], msb: i32) -> i32 {
    let low_byte = bytes[REGISTER_NUM_LIMBS - 2];
