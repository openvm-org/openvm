...
fn preflight_gpu_tracegen_proves_system_and_rv64i_airs_without_record_arenas() {
    ...
    let execution = preflight
        .execute_from_state(
            state,
            PreflightLimits::new(instructions.len(), instructions.len() * 2, 8),
        )
        .unwrap();
...
#[test]
fn preflight_gpu_replay_proves_a_suspended_segment() {
    let instructions = [
...
#[test]
fn preflight_gpu_replay_expands_more_than_one_launch_block() {
...
#[test]
fn preflight_gpu_replay_launches_high_register_m_kernels() {
...
#[test]
fn preflight_gpu_replay_carries_a_register_across_segments() {
    let instructions = [
        checkpoint_ri(BaseAluImmOpcode::ADDI, 1, 0, 7),
        Instruction::from_usize(
            Rv64JalLuiOpcode::JAL.global_opcode(),
...
#[test]
fn preflight_gpu_replay_proves_an_empty_suspended_segment() {
...
fn preflight_gpu_replay_proves_bounded_rv64i_slice() {
    ...
    let cached_program = vm.commit_program_on_device(&program);
    vm.load_program(cached_program);
    vm.transport_init_memory_to_device(&preflight_state.memory);
    let execution = preflight
        .execute_from_state(
            preflight_state,
...
fn preflight_gpu_tracegen_proves_rv64m_airs_without_record_arenas() {
    ...
    vm.transport_init_memory_to_device(&state.memory);
    let execution = preflight
        .execute_from_state(state, PreflightLimits::new(instructions.len(), 0, 8))
        .unwrap();
...
#[test]
fn preflight_mul_replay_rejects_corrupt_results_and_predecessors_before_lookups() {
    let instructions = [
        instruction(
            MulOpcode::MUL,
            [
...
#[test]
fn preflight_postflight_rejects_raw_x0_destination() {
    let instructions = [
        instruction(
            MulOpcode::MUL,
            [
    ...
    let error = match Rv64ImPreflightGpuTracegen::postflight(
        ...
        Ok(_) => panic!("raw x0 destination should fail postflight"),
        Err(error) => error,
    };
    assert!(error.to_string().contains("code 304"));
}
