...
use crate::{adapters::imm_to_u64, common::*, OrExecutor};

#[derive(AlignedBytesBorrow, Clone)]
#[repr(C)]
pub(super) struct OrPreCompute {
    c: u64,
    a: u8,
    b: u8,
}

impl<A, const LIMB_BITS: usize> OrExecutor<A, { RV64_REGISTER_NUM_LIMBS }, LIMB_BITS> {
    ...
    ($execute_impl:ident, $is_imm:ident, $opcode:expr, $offset:expr) => {
        Ok(
            match (
                ...
                _ => unreachable!("OrExecutor received non-OR opcode"),
            },
        )
    };
}
...
impl<F, A, const LIMB_BITS: usize> InterpreterExecutor<F>
    for OrExecutor<A, { RV64_REGISTER_NUM_LIMBS }, LIMB_BITS>
where
    F: PrimeField32,
{
    #[inline(always)]
    fn pre_compute_size(&self) -> usize {
        size_of::<OrPreCompute>()
    }
    ...
    {
        let data: &mut OrPreCompute = data.borrow_mut();
    ...
    F: PrimeField32,
{
    ...
    fn metered_pre_compute_size(&self) -> usize {
        size_of::<E2PreCompute<OrPreCompute>>()
    }
    ...
    {
        let data: &mut E2PreCompute<OrPreCompute> = data.borrow_mut();
...
>(
    ...
    let pre_compute: &OrPreCompute =
        std::slice::from_raw_parts(pre_compute, size_of::<OrPreCompute>()).borrow();
...
>(
    ...
    let pre_compute: &E2PreCompute<OrPreCompute> =
        std::slice::from_raw_parts(pre_compute, size_of::<E2PreCompute<OrPreCompute>>()).borrow();
    exec_state
        .ctx
