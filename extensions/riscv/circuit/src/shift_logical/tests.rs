...
#[cfg(feature = "cuda")]
use {
    crate::{adapters::Rv64BaseAluAdapterRecord, Rv64ShiftLogicalChipGpu, ShiftLogicalCoreRecord},
    openvm_circuit::arch::{
        testing::{
...
fn set_and_execute<RA: Arena, E: PreflightExecutor<F, RA>>(
    ...
    let (a, _, _) = run_shift_logical::<RV64_REGISTER_NUM_LIMBS, RV64_BYTE_BITS>(opcode, &b, &c);
    assert_eq!(
        a.map(F::from_u8),
...
fn run_negative_shift_test(
    ...
    let modify_trace = |trace: &mut DenseMatrix<BabyBear>| {
        ...
        let cols: &mut ShiftLogicalCoreCols<F, RV64_REGISTER_NUM_LIMBS, RV64_BYTE_BITS> =
...
#[test]
fn rv64_shift_logical_wrong_negative_test() {
...
fn rv64_srl_wrong_bit_mult_side_negative_test() {
    // Prank bit_multiplier_left=1, bit_multiplier_right=0 for the right shift.
...
fn run_sll_sanity_test() {
    ...
    let (result, limb_shift, bit_shift) =
        run_shift_logical::<RV64_REGISTER_NUM_LIMBS, RV64_BYTE_BITS>(SLL, &x, &y);
...
fn run_srl_sanity_test() {
    ...
    let (result, limb_shift, bit_shift) =
        run_shift_logical::<RV64_REGISTER_NUM_LIMBS, RV64_BYTE_BITS>(SRL, &x, &y);
    for i in 0..RV64_REGISTER_NUM_LIMBS {
        assert_eq!(z[i], result[i])
    }
    let shift = (y[0] as usize) % (RV64_REGISTER_NUM_LIMBS * RV64_BYTE_BITS);
...
fn test_cuda_rand_shift_logical_tracegen(opcode: ShiftOpcode, num_ops: usize) {
    ...
    type Record<'a> = (
        ...
        &'a mut ShiftLogicalCoreRecord<RV64_REGISTER_NUM_LIMBS, RV64_BYTE_BITS>,
    );
    harness
        .dense_arena
