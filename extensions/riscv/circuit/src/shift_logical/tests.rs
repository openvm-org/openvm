...
use super::{
    core::run_shift_logical, trace::generate_trace_from_postflight, Rv64ShiftLogicalChip,
...
fn postflight_trace_matches_record_arena_trace() {
    let mut tester = VmChipTestBuilder::default();
    let mut harness = create_test_chip(&tester);
    let sll = Instruction::from_usize(
        SLL.global_opcode(),
        [
            24,
            8,
    ...
    let srl = Instruction::from_usize(
        SRL.global_opcode(),
        [
            32,
            24,
    ...
    tester.execute_with_pc(&mut harness.executor, &mut harness.arena, &sll, 0);
    tester.execute_with_pc(&mut harness.executor, &mut harness.arena, &srl, 4);
    ...
    let program = Program::new_without_debug_infos(&[sll, srl, sentinel], 0);
