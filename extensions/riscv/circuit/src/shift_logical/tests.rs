...
use super::{
    core::run_shift_logical, trace::generate_trace_from_postflight, ShiftLogicalChip,
    ShiftLogicalCoreAir, ShiftLogicalCoreCols,
};
use crate::{
    adapters::{
        bytes_to_u16_block, u16_block_to_bytes, BaseAluRegU16AdapterAir, REGISTER_NUM_LIMBS,
        U16_BITS,
    },
    test_utils::rand_write_register_or_imm,
    ShiftLogicalAir, ShiftLogicalExecutor, ShiftLogicalFiller,
};

type F = BabyBear;
const MAX_INS_CAPACITY: usize = 128;
const REGISTER_SHIFT_AMOUNTS: [u8; 8] = [0, 1, 15, 16, 31, 32, 63, 64];
type Harness = TestChipHarness<F, ShiftLogicalExecutor, ShiftLogicalAir, ShiftLogicalChip<F>>;
...
fn create_harness_fields(
...
) -> (ShiftLogicalAir, ShiftLogicalExecutor, ShiftLogicalChip<F>) {
    let air = ShiftLogicalAir::new(
    ...
    let executor = ShiftLogicalExecutor::new(ShiftOpcode::CLASS_OFFSET);
    let chip =
        ShiftLogicalChip::<F>::new(ShiftLogicalFiller::new(range_checker_chip), memory_helper);
    (air, executor, chip)
}
...
fn set_and_execute<E: openvm_circuit::arch::Executor<F> + Clone>(
    ...
    let a_bytes = u16_block_to_bytes(a_u16);
    assert_eq!(
...
#[test_case(SLL, 100)]
#[test_case(SRL, 100)]
fn run_shift_logical_rand_test(opcode: ShiftOpcode, num_ops: usize) {
...
#[test]
fn shift_logical_wrong_a_negative_test() {
...
#[test]
fn sll_wrong_bit_carry_negative_test() {
...
#[test]
fn sll_wrong_bit_aux_negative_test() {
...
#[test]
fn sll_wrong_limb_shift_negative_test() {
...
#[test]
fn sll_wrong_bit_multiplier_negative_test() {
...
#[test]
fn srl_wrong_bit_carry_negative_test() {
    let b = [255; REGISTER_NUM_LIMBS];
    let c = [9, 0, 0, 0, 0, 0, 0, 0];
    let prank_vals = ShiftPrankValues {
        bit_shift_carry: Some([0; BLOCK_FE_WIDTH]),
...
#[test]
fn srl_wrong_bit_multiplier_negative_test() {
...
fn run_sll_sanity_test() {
    let x = bytes_to_u16_block([45, 7, 61, 186, 31, 190, 221, 200]);
    let y = bytes_to_u16_block([91, 0, 100, 0, 49, 190, 190, 113]);
    ...
    assert_eq!(u16_block_to_bytes(result), expected);
...
fn run_srl_sanity_test() {
    let x = bytes_to_u16_block([31, 190, 221, 200, 45, 7, 61, 186]);
    let y = bytes_to_u16_block([81, 190, 190, 190, 113, 20, 50, 80]);
    let (result, limb_shift, bit_shift) =
...
fn create_cuda_harness(tester: &GpuChipTestBuilder) -> GpuHarness {
    ...
    let gpu_chip = ShiftLogicalChipGpu::new(tester.range_checker(), tester.timestamp_max_bits());
