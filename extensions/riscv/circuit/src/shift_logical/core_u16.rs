...
pub struct ShiftLogicalU16CoreCols<T, const NUM_LIMBS: usize, const LIMB_BITS: usize> {
    pub a: [T; NUM_LIMBS],
    pub b: [T; NUM_LIMBS],
    pub c: [T; NUM_LIMBS],
...
///
/// Unlike the byte-limb [`super::ShiftLogicalCoreAir`], every arithmetic relation here stays below
/// 2^LIMB_BITS, so it is sound for `LIMB_BITS = 16` where a `b * 2^bit_shift` product would
/// otherwise exceed the BabyBear modulus. Each `b` limb is decomposed into `carry`/`aux` parts and
/// recombined additively, so no product of two witnessed values is formed.
///
...
#[columns_via(ShiftLogicalU16CoreCols<u8, NUM_LIMBS, LIMB_BITS>)]
pub struct ShiftLogicalU16CoreAir<const NUM_LIMBS: usize, const LIMB_BITS: usize> {
    pub range_bus: VariableRangeCheckerBus,
    pub offset: usize,
}

impl<F: Field, const NUM_LIMBS: usize, const LIMB_BITS: usize> BaseAir<F>
    for ShiftLogicalU16CoreAir<NUM_LIMBS, LIMB_BITS>
{
    fn width(&self) -> usize {
        ShiftLogicalU16CoreCols::<F, NUM_LIMBS, LIMB_BITS>::width()
    }
}
    ...
    I::ProcessedInstruction: From<MinimalInstruction<AB::Expr>>,
{
    fn eval(
        ...
        let cols: &ShiftLogicalU16CoreCols<_, NUM_LIMBS, LIMB_BITS> = local_core.borrow();
...
pub struct ShiftLogicalU16CoreRecord<const NUM_LIMBS: usize, const LIMB_BITS: usize> {
    pub b: [u16; NUM_LIMBS],
    pub c: [u16; NUM_LIMBS],
    pub local_opcode: u8,
}
...
pub struct ShiftLogicalU16Executor<A, const NUM_LIMBS: usize, const LIMB_BITS: usize> {
    adapter: A,
    pub offset: usize,
}

#[derive(Clone, derive_new::new)]
pub struct ShiftLogicalU16Filler<A, const NUM_LIMBS: usize, const LIMB_BITS: usize> {
    adapter: A,
...
where
    ...
    for<'buf> RA: RecordArena<
        ...
        (
            A::RecordMut<'buf>,
            &'buf mut ShiftLogicalU16CoreRecord<NUM_LIMBS, LIMB_BITS>,
        ),
    >,
{
    ...
    fn execute(
        ...
        let (output, _, _) =
            run_shift_logical_u16::<NUM_LIMBS, LIMB_BITS>(local_opcode, &rs1, &rs2);
...
impl<F, A, const NUM_LIMBS: usize, const LIMB_BITS: usize> TraceFiller<F>
    for ShiftLogicalU16Filler<A, NUM_LIMBS, LIMB_BITS>
where
    F: PrimeField32,
    A: 'static + AdapterTraceFiller<F>,
{
    fn fill_trace_row(&self, mem_helper: &MemoryAuxColsFactory<F>, row_slice: &mut [F]) {
        ...
        // ShiftLogicalU16CoreCols::width() elements
        let (adapter_row, mut core_row) = unsafe { row_slice.split_at_mut_unchecked(A::WIDTH) };
        self.adapter.fill_trace_row(mem_helper, adapter_row);
        // SAFETY: core_row contains a valid ShiftLogicalU16CoreRecord written by the executor
        // during trace generation
        let record: &ShiftLogicalU16CoreRecord<NUM_LIMBS, LIMB_BITS> =
        ...
        let (a, limb_shift, bit_shift) =
            run_shift_logical_u16::<NUM_LIMBS, LIMB_BITS>(opcode, &record.b, &record.c);
        ...
        let core_row: &mut ShiftLogicalU16CoreCols<F, NUM_LIMBS, LIMB_BITS> = core_row.borrow_mut();
...
pub(crate) fn run_shift_logical_u16<const NUM_LIMBS: usize, const LIMB_BITS: usize>(
    ...
    match opcode {
        ShiftOpcode::SLL => run_shift_left_u16::<NUM_LIMBS, LIMB_BITS>(x, y),
        // SRL (SRA is not handled by the logical shift chip).
        _ => run_shift_right_logical_u16::<NUM_LIMBS, LIMB_BITS>(x, y),
    }
}

#[inline(always)]
fn run_shift_left_u16<const NUM_LIMBS: usize, const LIMB_BITS: usize>(
    x: &[u16; NUM_LIMBS],
    y: &[u16; NUM_LIMBS],
) -> ([u16; NUM_LIMBS], usize, usize) {
...
#[inline(always)]
fn run_shift_right_logical_u16<const NUM_LIMBS: usize, const LIMB_BITS: usize>(
    x: &[u16; NUM_LIMBS],
    y: &[u16; NUM_LIMBS],
) -> ([u16; NUM_LIMBS], usize, usize) {
    let mut result = [0u16; NUM_LIMBS];
