...
#[derive(new)]
pub struct Rv64LoadHalfwordChipGpu {
...
impl Rv64LoadHalfwordChipGpu {
    pub fn generate_proving_ctx_from_rvr(
        ...
        let step_range = replay_plan.opcode_range(Rv64LoadStoreOpcode::LOADHU.global_opcode());
        if step_range.is_empty() {
            return Ok(AirProvingContext::simple_no_pis(DeviceMatrix::dummy()));
        }

        let trace_width = Rv64LoadMultiByteAdapterCols::<F>::width()
            + LoadCoreCols::<F, LOAD_HALFWORD_OVERLAP_CELLS>::width();
        let trace_height = next_power_of_two_or_zero(step_range.len());
        let d_trace = DeviceMatrix::<F>::with_capacity_on(trace_height, trace_width, device_ctx);
        unsafe {
            load_halfword_cuda::replay_tracegen(
                d_trace.buffer(),
                trace_height,
                ...
                Rv64LoadStoreOpcode::LOADHU.global_opcode().as_usize() as u32,
                RV64_REGISTER_AS,
                RV64_MEMORY_AS,
...
impl Chip<DenseRecordArena, GpuBackend> for Rv64LoadHalfwordChipGpu {
