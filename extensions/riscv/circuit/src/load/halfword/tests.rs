...
use crate::{
    ...
    load::{
        common::load_write_data, core::LoadCoreCols, LoadHalfwordAir, LoadHalfwordChip,
        LoadHalfwordCoreAir, LoadHalfwordExecutor, LoadHalfwordFiller, LOAD_HALFWORD_OVERLAP_CELLS,
    },
    test_utils::memory::{set_and_execute_load, F, MAX_INS_CAPACITY},
};
#[cfg(all(feature = "cuda", feature = "rvr"))]
use crate::{load::LoadHalfwordChipGpu, test_utils::memory::dummy_range_checker};

type HalfwordHarness =
    TestChipHarness<F, LoadHalfwordExecutor, LoadHalfwordAir, LoadHalfwordChip<F>>;
...
fn create_halfword_harness(
    ...
    let air = LoadHalfwordAir::new(
        ...
        LoadHalfwordCoreAir::new(LoadStoreOpcode::CLASS_OFFSET, bitwise_chip.bus()),
    );
    let executor = LoadHalfwordExecutor::new(LoadStoreOpcode::CLASS_OFFSET);
    let chip = LoadHalfwordChip::<F>::new(
...
fn create_cuda_halfword_harness(tester: &GpuChipTestBuilder) -> GpuHalfwordHarness {
    ...
    let executor = LoadHalfwordExecutor::new(LoadStoreOpcode::CLASS_OFFSET);
    let cpu_chip = LoadHalfwordChip::<F>::new(
        LoadHalfwordFiller::new(
            LoadMultiByteAdapterFiller::new(tester.address_bits(), range_checker.clone()),
            LoadStoreOpcode::CLASS_OFFSET,
            bitwise_chip,
    ...
    let gpu_chip = LoadHalfwordChipGpu::new(
