...
impl LoadByteChipGpu {
    pub fn generate_proving_ctx_from_postflight(
        ...
        let step_range = replay_plan.opcode_range(LoadStoreOpcode::LOADBU.global_opcode());
        if step_range.is_empty() {
            return Ok(AirProvingContext::simple_no_pis(DeviceMatrix::dummy()));
        }

        let trace_width = LoadByteAdapterCols::<F>::width() + LoadByteCoreCols::<F>::width();
        let trace_height = next_power_of_two_or_zero(step_range.len());
        let d_trace = DeviceMatrix::<F>::with_capacity_on(trace_height, trace_width, device_ctx);
        unsafe {
            load_byte_cuda::replay_tracegen(
                d_trace.buffer(),
                trace_height,
                ...
                LoadStoreOpcode::LOADBU.global_opcode().as_usize() as u32,
                REGISTER_AS,
                MEMORY_AS,
