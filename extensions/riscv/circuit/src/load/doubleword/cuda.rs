...
use super::LOAD_DOUBLEWORD_SELECTOR_WIDTH;
use crate::{
...
impl Chip<DenseRecordArena, GpuBackend> for Rv64LoadDoublewordChipGpu {
    fn generate_proving_ctx(&self, arena: DenseRecordArena) -> AirProvingContext<GpuBackend> {
        ...
        let delta_records = self.rvr_decode.device_delta_records(
            crate::rvr_gpu_decode::DeltaAirKind::LoadDoubleword,
        ...
        let trace_width = Rv64LoadAdapterCols::<F>::width()
            + LoadCoreCols::<F, LOAD_DOUBLEWORD_SELECTOR_WIDTH>::width();
        ...
        if rvr_wire || delta_records.is_some() {
            ...
            unsafe {
                load_doubleword_cuda::tracegen_compact(
                    trace.buffer(),
                    compact_height,
                    &compact_records,
                    &table,
