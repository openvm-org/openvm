...
use crate::{adapters::imm_to_u64, common::*, AndExecutor};

#[derive(AlignedBytesBorrow, Clone)]
#[repr(C)]
pub(super) struct AndPreCompute {
    c: u64,
    a: u8,
    b: u8,
}

impl<A, const LIMB_BITS: usize> AndExecutor<A, { RV64_REGISTER_NUM_LIMBS }, LIMB_BITS> {
    ...
    ($execute_impl:ident, $is_imm:ident, $opcode:expr, $offset:expr) => {
        Ok(
            match (
                ...
                _ => unreachable!("AndExecutor received non-AND opcode"),
            },
        )
    };
}
...
impl<F, A, const LIMB_BITS: usize> InterpreterExecutor<F>
    for AndExecutor<A, { RV64_REGISTER_NUM_LIMBS }, LIMB_BITS>
where
    F: PrimeField32,
{
    #[inline(always)]
    fn pre_compute_size(&self) -> usize {
        size_of::<AndPreCompute>()
    }
    ...
    {
        let data: &mut AndPreCompute = data.borrow_mut();
    ...
    F: PrimeField32,
{
    ...
    fn metered_pre_compute_size(&self) -> usize {
        size_of::<E2PreCompute<AndPreCompute>>()
    }
    ...
    {
        let data: &mut E2PreCompute<AndPreCompute> = data.borrow_mut();
...
>(
    ...
    let pre_compute: &AndPreCompute =
        std::slice::from_raw_parts(pre_compute, size_of::<AndPreCompute>()).borrow();
...
>(
    ...
    let pre_compute: &E2PreCompute<AndPreCompute> =
        std::slice::from_raw_parts(pre_compute, size_of::<E2PreCompute<AndPreCompute>>()).borrow();
    exec_state
        .ctx
