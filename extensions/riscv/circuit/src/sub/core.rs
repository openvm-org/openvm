...
pub struct SubCoreCols<T, const NUM_LIMBS: usize, const LIMB_BITS: usize> {
    pub is_valid: T,
    pub a: [T; NUM_LIMBS],
    pub b: [T; NUM_LIMBS],
    pub c: [T; NUM_LIMBS],
...
#[columns_via(SubCoreCols<u8, NUM_LIMBS, LIMB_BITS>)]
pub struct SubCoreAir<const NUM_LIMBS: usize, const LIMB_BITS: usize> {
    pub bus: BitwiseOperationLookupBus,
    pub offset: usize,
}

impl<F: Field, const NUM_LIMBS: usize, const LIMB_BITS: usize> BaseAir<F>
    for SubCoreAir<NUM_LIMBS, LIMB_BITS>
{
    fn width(&self) -> usize {
        SubCoreCols::<F, NUM_LIMBS, LIMB_BITS>::width()
    }
}
    ...
    I::ProcessedInstruction: From<MinimalInstruction<AB::Expr>>,
{
    fn eval(
        ...
        let cols: &SubCoreCols<_, NUM_LIMBS, LIMB_BITS> = local_core.borrow();
        ...
        // For SUB, define carry[i] = (a[i] + c[i] - b[i] + carry[i - 1]) / 2^LIMB_BITS.
        // If each carry[i] is boolean and 0 <= a[i] < 2^LIMB_BITS, then
        // a[i] = (b[i] - c[i]) % 2^LIMB_BITS as necessary.
        ...
        let expected_opcode = VmCoreAir::<AB, I>::opcode_to_global_expr(self, BaseAluOpcode::SUB);
...
pub struct SubCoreRecord<const NUM_LIMBS: usize> {
    pub b: [u8; NUM_LIMBS],
    pub c: [u8; NUM_LIMBS],
...
pub struct SubExecutor<A, const NUM_LIMBS: usize, const LIMB_BITS: usize> {
    adapter: A,
    pub offset: usize,
}

#[derive(derive_new::new)]
pub struct SubFiller<A, const NUM_LIMBS: usize, const LIMB_BITS: usize> {
    adapter: A,
...
impl<F, A, RA, const NUM_LIMBS: usize, const LIMB_BITS: usize> PreflightExecutor<F, RA>
    for SubExecutor<A, NUM_LIMBS, LIMB_BITS>
where
    F: PrimeField32,
    A: 'static
    ...
    for<'buf> RA: RecordArena<
        ...
        (A::RecordMut<'buf>, &'buf mut SubCoreRecord<NUM_LIMBS>),
    >,
{
    ...
    fn execute(
        ...
        debug_assert!(matches!(local_opcode, BaseAluOpcode::SUB));
        ...
        let rd = run_sub::<NUM_LIMBS, LIMB_BITS>(&core_record.b, &core_record.c);
...
impl<F, A, const NUM_LIMBS: usize, const LIMB_BITS: usize> TraceFiller<F>
    for SubFiller<A, NUM_LIMBS, LIMB_BITS>
where
    F: PrimeField32,
    A: 'static + AdapterTraceFiller<F>,
{
    fn fill_trace_row(&self, mem_helper: &MemoryAuxColsFactory<F>, row_slice: &mut [F]) {
        ...
        // SubCoreCols::width() elements
        let (adapter_row, mut core_row) = unsafe { row_slice.split_at_mut_unchecked(A::WIDTH) };
        self.adapter.fill_trace_row(mem_helper, adapter_row);
        // SAFETY: core_row contains a valid SubCoreRecord written by the executor
        // during trace generation
        let record: &SubCoreRecord<NUM_LIMBS> = unsafe { get_record_from_slice(&mut core_row, ()) };
        let core_row: &mut SubCoreCols<F, NUM_LIMBS, LIMB_BITS> = core_row.borrow_mut();
        ...
        let a = run_sub::<NUM_LIMBS, LIMB_BITS>(&record.b, &record.c);
...
#[inline(always)]
pub(super) fn run_sub<const NUM_LIMBS: usize, const LIMB_BITS: usize>(
    x: &[u8; NUM_LIMBS],
    y: &[u8; NUM_LIMBS],
) -> [u8; NUM_LIMBS] {
