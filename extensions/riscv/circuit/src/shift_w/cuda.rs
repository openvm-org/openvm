...
impl Chip<DenseRecordArena, GpuBackend> for Rv64ShiftWLogicalChipGpu {
    fn generate_proving_ctx(&self, arena: DenseRecordArena) -> AirProvingContext<GpuBackend> {
        ...
        let delta_records = self.rvr_decode.device_delta_records(
            crate::rvr_gpu_decode::DeltaAirKind::ShiftWLogical,
        ...
        if rvr_wire || delta_records.is_some() {
            ...
            unsafe {
                crate::cuda_abi::shift_w_cuda::tracegen_logical_compact(
                    d_trace.buffer(),
                    trace_height,
                    &d_records,
                    &d_table,
...
impl Chip<DenseRecordArena, GpuBackend> for Rv64ShiftWRightArithmeticChipGpu {
    fn generate_proving_ctx(&self, arena: DenseRecordArena) -> AirProvingContext<GpuBackend> {
        ...
        #[cfg(feature = "rvr")]
        let delta_records = self.rvr_decode.device_delta_records(
            crate::rvr_gpu_decode::DeltaAirKind::ShiftWRightArithmetic,
            &self.range_checker.device_ctx,
        );
        #[cfg(feature = "rvr")]
        let g2_records = self.rvr_decode.device_g2_trace_input(
        ...
        if rvr_wire || delta_records.is_some() {
            ...
            let d_trace =
                DeviceMatrix::<F>::with_capacity_on(trace_height, trace_width, device_ctx);
            unsafe {
                crate::cuda_abi::shift_w_cuda::tracegen_right_arithmetic_compact(
                    d_trace.buffer(),
                    trace_height,
                    &d_records,
                    &d_table,
