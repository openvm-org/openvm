...
use super::{run_addi, AddICoreCols, Rv64AddIChip, Rv64AddIWChip};
use crate::adapters::{
    Rv64BaseAluImmU16AdapterCols, Rv64BaseAluImmU16AdapterFiller, Rv64BaseAluWImmU16AdapterCols,
...
pub fn generate_trace_from_postflight<F: PrimeField32>(
    ...
    fill_trace_rows(&mut trace, 0, steps, |row, step| {
        ...
        let (rs1, rd) = Rv64BaseAluImmU16AdapterFiller::replay(
            postflight,
            step,
...
pub fn generate_w_trace_from_postflight<F: PrimeField32>(
    ...
    fill_trace_rows(&mut trace, 0, steps, |row, step| {
        ...
        let (rs1, rd) = adapter.replay(
            ...
            &chip.mem_helper.as_borrowed(),
            adapter_row.borrow_mut(),
            |rs1, _| run_addi::<RV64_WORD_U16_LIMBS, U16_BITS>(&rs1, imm_low11, imm_sign),
        )?;

        let core_row: &mut AddICoreCols<F, RV64_WORD_U16_LIMBS, U16_BITS> = core_row.borrow_mut();
        core_row.is_valid = F::ONE;
        core_row.imm_sign = F::from_u16(imm_sign);
        core_row.imm_low11 = F::from_u16(imm_low11);
        chip.inner
        ...
        core_row.rs1 = rs1.map(F::from_u16);
        core_row.rd = rd.map(F::from_u16);
        for &value in &rd[..RV64_WORD_U16_LIMBS - 1] {
            chip.inner
                .range_checker_chip
                .add_count(value as u32, U16_BITS);
        }
