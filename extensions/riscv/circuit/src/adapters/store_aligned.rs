...
use super::{
    ...
    Rv64StoreAdapterAirInterface, Rv64StoreAdapterRecord, RV64_PTR_BITS, RV64_PTR_U16_LIMBS,
    RV64_REGISTER_NUM_LIMBS, U16_BITS,
};
use crate::adapters::{memory_read_u16, timed_write_u16, tracing_read, tracing_read_u16};

// Lean store adapter for `sb`, a single-byte access that can never cross a block boundary. It
// drops the crossing machinery (`write1_base_aux`, the second block write and its timestamp slot,
// `mem_ptr_carry`, and the crossing range checks) while sharing the interface, instruction, and
// record types with the misaligned (width) adapter — the byte core emits a zero second block and
// `store_cross = 0`, both of which this adapter ignores.
...
impl<F: PrimeField32> AdapterTraceFiller<F> for Rv64StoreByteAdapterFiller {
    ...
    fn fill_trace_row(&self, mem_helper: &MemoryAuxColsFactory<F>, mut adapter_row: &mut [F]) {
        ...
        // SAFETY: as in `Rv64StoreAdapterFiller`; the byte adapter reuses `Rv64StoreAdapterRecord`,
