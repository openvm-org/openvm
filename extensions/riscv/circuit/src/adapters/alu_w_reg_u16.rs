...
use super::{
    byte_ptr_to_u16_ptr, checked_register_u16_pointer, concat_u16_block, U16_BITS, WORD_U16_LIMBS,
};

#[repr(C)]
#[derive(AlignedBorrow, StructReflection)]
pub struct BaseAluWRegU16AdapterCols<T> {
...
#[columns_via(BaseAluWRegU16AdapterCols<u8>)]
pub struct BaseAluWRegU16AdapterAir {
    pub(super) execution_bridge: ExecutionBridge,
    pub(super) memory_bridge: MemoryBridge,
    pub range_bus: VariableRangeCheckerBus,
}

impl<F: Field> BaseAir<F> for BaseAluWRegU16AdapterAir {
    fn width(&self) -> usize {
        BaseAluWRegU16AdapterCols::<F>::width()
    }
}

impl<AB: InteractionBuilder> VmAdapterAir<AB> for BaseAluWRegU16AdapterAir {
    ...
    fn eval(
        ...
        let local: &BaseAluWRegU16AdapterCols<_> = local.borrow();
        ...
        let rs2_data: [AB::Expr; BLOCK_FE_WIDTH] = concat_u16_block(&ctx.reads[1], &local.rs2_high);
        self.memory_bridge
            .read(
                MemoryAddress::new(
                    AB::F::from_u32(REGISTER_AS),
...
pub struct BaseAluWRegU16AdapterFiller {
    pub range_checker_chip: SharedVariableRangeCheckerChip,
}

impl BaseAluWRegU16AdapterFiller {
    pub(crate) fn replay<F: PrimeField32>(
        &self,
        postflight: &Postflight<'_, F>,
        step: PostflightStep,
        mem_helper: &MemoryAuxColsFactory<F>,
        adapter_row: &mut BaseAluWRegU16AdapterCols<F>,
        compute: impl FnOnce([[u16; WORD_U16_LIMBS]; 2]) -> [u16; WORD_U16_LIMBS],
    ) -> Result<([[u16; WORD_U16_LIMBS]; 2], [u16; WORD_U16_LIMBS]), PostflightError> {
        ...
        adapter_row.rs2_high = array::from_fn(|i| F::from_u16(rs2.value[WORD_U16_LIMBS + i]));
