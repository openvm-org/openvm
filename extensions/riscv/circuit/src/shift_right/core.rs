...
pub struct ShiftRightCoreCols<T, const NUM_LIMBS: usize, const LIMB_BITS: usize> {
    pub a: [T; NUM_LIMBS],
    pub b: [T; NUM_LIMBS],
    pub c: [T; NUM_LIMBS],
...
/// `shift_amount % num_bits` bits. This matches the RISC-V specs for SRL/SRA.
#[derive(Copy, Clone, Debug, derive_new::new, ColumnsAir)]
#[columns_via(ShiftRightCoreCols<u8, NUM_LIMBS, LIMB_BITS>)]
pub struct ShiftRightCoreAir<const NUM_LIMBS: usize, const LIMB_BITS: usize> {
...
impl<F: Field, const NUM_LIMBS: usize, const LIMB_BITS: usize> BaseAir<F>
    for ShiftRightCoreAir<NUM_LIMBS, LIMB_BITS>
{
    fn width(&self) -> usize {
        ShiftRightCoreCols::<F, NUM_LIMBS, LIMB_BITS>::width()
    }
}
    ...
    I::ProcessedInstruction: From<MinimalInstruction<AB::Expr>>,
{
    fn eval(
        ...
        let cols: &ShiftRightCoreCols<_, NUM_LIMBS, LIMB_BITS> = local_core.borrow();
        let flags = [cols.opcode_srl_flag, cols.opcode_sra_flag];
        ...
        let expected_opcode = VmCoreAir::<AB, I>::expr_to_global_expr(
            ...
            [
                ...
                (cols.opcode_sra_flag, ShiftOpcode::SRA),
            ]
            .iter()
...
pub struct ShiftRightCoreRecord<const NUM_LIMBS: usize, const LIMB_BITS: usize> {
    pub b: [u8; NUM_LIMBS],
    pub c: [u8; NUM_LIMBS],
    pub local_opcode: u8,
}
...
#[derive(Clone, Copy)]
pub struct ShiftRightExecutor<A, const NUM_LIMBS: usize, const LIMB_BITS: usize> {
    adapter: A,
    pub offset: usize,
}

#[derive(Clone)]
pub struct ShiftRightFiller<A, const NUM_LIMBS: usize, const LIMB_BITS: usize> {
    adapter: A,
    pub offset: usize,
...
impl<A, const NUM_LIMBS: usize, const LIMB_BITS: usize>
    ShiftRightExecutor<A, NUM_LIMBS, LIMB_BITS>
{
...
impl<A, const NUM_LIMBS: usize, const LIMB_BITS: usize> ShiftRightFiller<A, NUM_LIMBS, LIMB_BITS> {
    pub fn new(
        adapter: A,
...
impl<F, A, RA, const NUM_LIMBS: usize, const LIMB_BITS: usize> PreflightExecutor<F, RA>
    for ShiftRightExecutor<A, NUM_LIMBS, LIMB_BITS>
where
    F: PrimeField32,
    A: 'static
    ...
    for<'buf> RA: RecordArena<
        ...
        (
            A::RecordMut<'buf>,
            &'buf mut ShiftRightCoreRecord<NUM_LIMBS, LIMB_BITS>,
        ),
    >,
{
    ...
    fn execute(
        ...
        debug_assert_ne!(local_opcode, ShiftOpcode::SLL);
        ...
        let (output, _, _) =
            run_shift_right::<NUM_LIMBS, LIMB_BITS>(&rs1, &rs2, local_opcode == ShiftOpcode::SRL);
...
impl<F, A, const NUM_LIMBS: usize, const LIMB_BITS: usize> TraceFiller<F>
    for ShiftRightFiller<A, NUM_LIMBS, LIMB_BITS>
where
    F: PrimeField32,
    A: 'static + AdapterTraceFiller<F>,
{
    fn fill_trace_row(&self, mem_helper: &MemoryAuxColsFactory<F>, row_slice: &mut [F]) {
        ...
        // ShiftRightCoreCols::width() elements
        let (adapter_row, mut core_row) = unsafe { row_slice.split_at_mut_unchecked(A::WIDTH) };
        self.adapter.fill_trace_row(mem_helper, adapter_row);
        // SAFETY: core_row contains a valid ShiftRightCoreRecord written by the executor
        // during trace generation
        let record: &ShiftRightCoreRecord<NUM_LIMBS, LIMB_BITS> =
            unsafe { get_record_from_slice(&mut core_row, ()) };

        let opcode = ShiftOpcode::from_usize(record.local_opcode as usize);
        let (a, limb_shift, bit_shift) = run_shift_right::<NUM_LIMBS, LIMB_BITS>(
            &record.b,
            &record.c,
            opcode == ShiftOpcode::SRL,
        );
        ...
        let mut b_sign = F::ZERO;
        if opcode == ShiftOpcode::SRA {
            b_sign = F::from_u8(record.b[NUM_LIMBS - 1] >> (LIMB_BITS - 1));
            self.bitwise_lookup_chip
                .request_xor(record.b[NUM_LIMBS - 1] as u32, 1 << (LIMB_BITS - 1));
        }

        let core_row: &mut ShiftRightCoreCols<F, NUM_LIMBS, LIMB_BITS> = core_row.borrow_mut();
...
#[inline(always)]
pub(super) fn run_shift_right<const NUM_LIMBS: usize, const LIMB_BITS: usize>(
    x: &[u8; NUM_LIMBS],
    y: &[u8; NUM_LIMBS],
    logical: bool,
