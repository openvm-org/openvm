...
#[cfg(feature = "cuda")]
use {
    crate::{adapters::Rv64BaseAluAdapterRecord, Rv64ShiftRightChipGpu, ShiftRightCoreRecord},
    openvm_circuit::arch::{
        testing::{
...
use super::{
    core::run_shift_right, Rv64ShiftRightAir, Rv64ShiftRightChip, Rv64ShiftRightExecutor,
    ShiftRightCoreAir, ShiftRightCoreCols, ShiftRightFiller,
};
use crate::{
    adapters::{
...
type RightHarness =
    TestChipHarness<F, Rv64ShiftRightExecutor, Rv64ShiftRightAir, Rv64ShiftRightChip<F>>;
...
fn create_right_harness_fields(
    ...
    Rv64ShiftRightAir,
    Rv64ShiftRightExecutor,
    Rv64ShiftRightChip<F>,
) {
    let air = Rv64ShiftRightAir::new(
        Rv64BaseAluAdapterAir::new(execution_bridge, memory_bridge, bitwise_chip.bus()),
        ShiftRightCoreAir::new(
            bitwise_chip.bus(),
            range_checker.bus(),
    ...
    let chip = Rv64ShiftRightChip::<F>::new(
...
fn set_and_execute<RA: Arena, E: PreflightExecutor<F, RA>>(
    ...
    let (a, _, _) =
        run_shift_right::<RV64_REGISTER_NUM_LIMBS, RV64_BYTE_BITS>(&b, &c, opcode == SRL);
    assert_eq!(
        a.map(F::from_u8),
...
#[test_case(SRL, 100)]
#[test_case(SRA, 100)]
fn run_rv64_shift_right_rand_test(opcode: ShiftOpcode, num_ops: usize) {
...
struct ShiftRightPrankValues<const NUM_LIMBS: usize, const LIMB_BITS: usize> {
...
fn run_negative_shift_right_test(
    ...
    prank_vals: ShiftRightPrankValues<RV64_REGISTER_NUM_LIMBS, RV64_BYTE_BITS>,
) {
    let mut rng = create_seeded_rng();
    ...
    let modify_trace = |trace: &mut DenseMatrix<BabyBear>| {
        ...
        let cols: &mut ShiftRightCoreCols<F, RV64_REGISTER_NUM_LIMBS, RV64_BYTE_BITS> =
...
#[test]
fn rv64_shift_right_wrong_negative_test() {
    let a = [1, 0, 0, 0, 0, 0, 0, 0];
    let b = [1, 0, 0, 0, 0, 0, 0, 0];
    let c = [1, 0, 0, 0, 0, 0, 0, 0];
    let prank_vals = Default::default();
    run_negative_shift_right_test(SRL, a, b, c, prank_vals);
    run_negative_shift_right_test(SRA, a, b, c, prank_vals);
}
...
fn rv64_srl_wrong_bit_shift_negative_test() {
    ...
    // Constraint on bit_multiplier catches mismatch.
    let a = [0, 0, 32, 0, 0, 0, 0, 0];
    let b = [0, 0, 0, 128, 0, 0, 0, 0];
    let c = [9, 0, 0, 0, 0, 0, 0, 0];
    let prank_vals = ShiftRightPrankValues {
...
fn rv64_srx_wrong_bit_mult_side_negative_test() {
    // Prank bit_multiplier=0 for right shifts.
...
fn run_srl_sanity_test() {
    ...
    let (result, limb_shift, bit_shift) =
        run_shift_right::<RV64_REGISTER_NUM_LIMBS, RV64_BYTE_BITS>(&x, &y, true);
...
fn run_sra_sanity_test() {
    ...
    let (result, limb_shift, bit_shift) =
        run_shift_right::<RV64_REGISTER_NUM_LIMBS, RV64_BYTE_BITS>(&x, &y, false);
    for i in 0..RV64_REGISTER_NUM_LIMBS {
        assert_eq!(z[i], result[i])
    }
    let shift = (y[0] as usize) % (RV64_REGISTER_NUM_LIMBS * RV64_BYTE_BITS);
...
fn create_cuda_right_harness(tester: &GpuChipTestBuilder) -> GpuRightHarness {
    ...
    let gpu_chip = Rv64ShiftRightChipGpu::new(
...
fn test_cuda_rand_shift_right_tracegen(opcode: ShiftOpcode, num_ops: usize) {
    let mut rng = create_seeded_rng();
    let mut tester =
    ...
    type Record<'a> = (
        ...
        &'a mut ShiftRightCoreRecord<RV64_REGISTER_NUM_LIMBS, RV64_BYTE_BITS>,
    );
    harness
        .dense_arena
