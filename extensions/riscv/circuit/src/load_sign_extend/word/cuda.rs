...
impl Chip<DenseRecordArena, GpuBackend> for Rv64LoadSignExtendWordChipGpu {
    fn generate_proving_ctx(&self, arena: DenseRecordArena) -> AirProvingContext<GpuBackend> {
        ...
        let delta_records = self.rvr_decode.device_delta_records(
            crate::rvr_gpu_decode::DeltaAirKind::LoadSignExtendWord,
        ...
        let trace_width = Rv64LoadAdapterCols::<F>::width()
            + LoadSignExtendCoreCols::<F, LOAD_SIGN_EXTEND_WORD_SELECTOR_WIDTH>::width();
        ...
        if rvr_wire || delta_records.is_some() {
            ...
            unsafe {
                load_sign_extend_word_cuda::tracegen_compact(
                    trace.buffer(),
                    compact_height,
                    &compact_records,
                    &table,
