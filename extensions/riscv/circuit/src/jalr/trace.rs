...
use super::{try_run_jalr, Rv64JalrChip, Rv64JalrCoreCols};
use crate::adapters::{Rv64JalrAdapterCols, Rv64JalrAdapterFiller, RV64_PTR_U16_LIMBS, U16_BITS};
...
pub fn generate_trace_from_postflight<F: PrimeField32>(
    ...
    fill_trace_rows(&mut trace, 0, steps, |row, step| {
        ...
        let (rs1, to_pc, rd_data) = Rv64JalrAdapterFiller::replay(
            postflight,
            step,
            &chip.mem_helper.as_borrowed(),
            adapter_row.borrow_mut(),
            |from_pc, rs1, immediate, imm_sign| {
                if rs1[RV64_PTR_U16_LIMBS..].iter().any(|&limb| limb != 0) {
