...
/// Immediate-only fork of `ShiftLogicalCoreCols`: the shift amount comes from the instruction,
/// so there are no `c` limbs at all — the bit/limb shift markers already encode the amount, and
/// the immediate operand is reconstructed from them as an expression on the execution bus.
#[repr(C)]
...
///
/// Identical to `ShiftLogicalCoreAir` except the shift amount is the instruction immediate:
/// the marker columns (whose sums are constrained to 1) uniquely encode `shamt`, and the
/// execution bridge binds `limb_shift * LIMB_BITS + bit_shift` to the immediate operand — so
/// the reconstruction needs no columns and no range check. Completeness relies on the
/// transpiler emitting `shamt < NUM_LIMBS * LIMB_BITS`, which the RISC-V encoding guarantees.
