...
impl Chip<DenseRecordArena, GpuBackend> for Rv64ShiftLogicalImmChipGpu {
    fn generate_proving_ctx(&self, arena: DenseRecordArena) -> AirProvingContext<GpuBackend> {
        ...
        let delta_records = self.rvr_decode.device_delta_records(
            crate::rvr_gpu_decode::DeltaAirKind::ShiftLogicalImm,
        ...
        if rvr_wire || delta_records.is_some() {
            ...
            let (table, pc_base) = self
                ...
                .expect("compact ShiftLogicalImm segment without operand table");
            ...
            unsafe {
                shift_logical_imm_cuda::tracegen_compact(
                    trace.buffer(),
                    trace_height,
                    &compact,
                    &table,
...
impl Chip<DenseRecordArena, GpuBackend> for Rv64ShiftWLogicalImmChipGpu {
    fn generate_proving_ctx(&self, arena: DenseRecordArena) -> AirProvingContext<GpuBackend> {
        ...
        #[cfg(feature = "rvr")]
        let delta_records = self.rvr_decode.device_delta_records(
            crate::rvr_gpu_decode::DeltaAirKind::ShiftWLogicalImm,
            &self.range_checker.device_ctx,
        );
        #[cfg(feature = "rvr")]
        let g2_records = self.rvr_decode.device_g2_trace_input(
        ...
        if rvr_wire || delta_records.is_some() {
            ...
            let (table, pc_base) = self
                .rvr_decode
                .device_operand_table(device_ctx)
                .expect("compact ShiftWLogicalImm segment without operand table");
            let compact = delta_records
                .unwrap_or_else(|| Arc::new(records.to_device_on(device_ctx).unwrap()));
            let trace = DeviceMatrix::<F>::with_capacity_on(trace_height, trace_width, device_ctx);
            unsafe {
                shift_w_logical_imm_cuda::tracegen_compact(
                    trace.buffer(),
                    trace_height,
                    &compact,
                    &table,
