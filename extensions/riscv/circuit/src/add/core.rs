...
pub struct AddCoreCols<T, const NUM_LIMBS: usize, const LIMB_BITS: usize> {
    pub is_valid: T,
    pub a: [T; NUM_LIMBS],
    pub b: [T; NUM_LIMBS],
    pub c: [T; NUM_LIMBS],
...
#[columns_via(AddCoreCols<u8, NUM_LIMBS, LIMB_BITS>)]
pub struct AddCoreAir<const NUM_LIMBS: usize, const LIMB_BITS: usize> {
    pub bus: BitwiseOperationLookupBus,
    pub offset: usize,
}

impl<F: Field, const NUM_LIMBS: usize, const LIMB_BITS: usize> BaseAir<F>
    for AddCoreAir<NUM_LIMBS, LIMB_BITS>
{
    fn width(&self) -> usize {
        AddCoreCols::<F, NUM_LIMBS, LIMB_BITS>::width()
    }
}
    ...
    I::ProcessedInstruction: From<MinimalInstruction<AB::Expr>>,
{
    fn eval(
        ...
        let cols: &AddCoreCols<_, NUM_LIMBS, LIMB_BITS> = local_core.borrow();
        ...
        // For ADD, define carry[i] = (b[i] + c[i] + carry[i - 1] - a[i]) / 2^LIMB_BITS.
        ...
        let expected_opcode = VmCoreAir::<AB, I>::opcode_to_global_expr(self, BaseAluOpcode::ADD);
...
pub struct AddCoreRecord<const NUM_LIMBS: usize> {
    pub b: [u8; NUM_LIMBS],
    pub c: [u8; NUM_LIMBS],
...
pub struct AddExecutor<A, const NUM_LIMBS: usize, const LIMB_BITS: usize> {
    adapter: A,
    pub offset: usize,
}

#[derive(derive_new::new)]
pub struct AddFiller<A, const NUM_LIMBS: usize, const LIMB_BITS: usize> {
    adapter: A,
...
impl<F, A, RA, const NUM_LIMBS: usize, const LIMB_BITS: usize> PreflightExecutor<F, RA>
    for AddExecutor<A, NUM_LIMBS, LIMB_BITS>
where
    F: PrimeField32,
    A: 'static
    ...
    for<'buf> RA: RecordArena<
        ...
        (A::RecordMut<'buf>, &'buf mut AddCoreRecord<NUM_LIMBS>),
    >,
{
    ...
    fn execute(
        ...
        debug_assert!(matches!(local_opcode, BaseAluOpcode::ADD));
...
impl<F, A, const NUM_LIMBS: usize, const LIMB_BITS: usize> TraceFiller<F>
    for AddFiller<A, NUM_LIMBS, LIMB_BITS>
where
    F: PrimeField32,
    A: 'static + AdapterTraceFiller<F>,
{
    fn fill_trace_row(&self, mem_helper: &MemoryAuxColsFactory<F>, row_slice: &mut [F]) {
        ...
        // AddCoreCols::width() elements
        let (adapter_row, mut core_row) = unsafe { row_slice.split_at_mut_unchecked(A::WIDTH) };
        self.adapter.fill_trace_row(mem_helper, adapter_row);
        // SAFETY: core_row contains a valid AddCoreRecord written by the executor
        // during trace generation
        let record: &AddCoreRecord<NUM_LIMBS> = unsafe { get_record_from_slice(&mut core_row, ()) };
        let core_row: &mut AddCoreCols<F, NUM_LIMBS, LIMB_BITS> = core_row.borrow_mut();
...
#[inline(always)]
pub(super) fn run_add<const NUM_LIMBS: usize, const LIMB_BITS: usize>(
    x: &[u8; NUM_LIMBS],
    y: &[u8; NUM_LIMBS],
) -> [u8; NUM_LIMBS] {
