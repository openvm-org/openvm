...
use crate::{adapters::imm_to_u64, common::*, AddExecutor};

#[derive(AlignedBytesBorrow, Clone)]
#[repr(C)]
pub(super) struct AddPreCompute {
    c: u64,
    a: u8,
    b: u8,
}

impl<A, const LIMB_BITS: usize> AddExecutor<A, { RV64_REGISTER_NUM_LIMBS }, LIMB_BITS> {
    ...
    ($execute_impl:ident, $is_imm:ident, $opcode:expr, $offset:expr) => {
        Ok(
            match (
                ...
                _ => unreachable!("AddExecutor received non-ADD opcode"),
            },
        )
    };
}
...
impl<F, A, const LIMB_BITS: usize> InterpreterExecutor<F>
    for AddExecutor<A, { RV64_REGISTER_NUM_LIMBS }, LIMB_BITS>
where
    F: PrimeField32,
{
    #[inline(always)]
    fn pre_compute_size(&self) -> usize {
        size_of::<AddPreCompute>()
    }
    ...
    {
        let data: &mut AddPreCompute = data.borrow_mut();
    ...
    F: PrimeField32,
{
    ...
    fn metered_pre_compute_size(&self) -> usize {
        size_of::<E2PreCompute<AddPreCompute>>()
    }
    ...
    {
        let data: &mut E2PreCompute<AddPreCompute> = data.borrow_mut();
...
>(
    ...
    let pre_compute: &AddPreCompute =
        std::slice::from_raw_parts(pre_compute, size_of::<AddPreCompute>()).borrow();
...
>(
    ...
    let pre_compute: &E2PreCompute<AddPreCompute> =
        std::slice::from_raw_parts(pre_compute, size_of::<E2PreCompute<AddPreCompute>>()).borrow();
    exec_state
        .ctx
