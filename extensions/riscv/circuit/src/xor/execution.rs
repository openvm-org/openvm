...
use crate::{adapters::imm_to_u64, common::*, XorExecutor};

#[derive(AlignedBytesBorrow, Clone)]
#[repr(C)]
pub(super) struct XorPreCompute {
    c: u64,
    a: u8,
    b: u8,
}

impl<A, const LIMB_BITS: usize> XorExecutor<A, { RV64_REGISTER_NUM_LIMBS }, LIMB_BITS> {
    ...
    ($execute_impl:ident, $is_imm:ident, $opcode:expr, $offset:expr) => {
        Ok(
            match (
                ...
                _ => unreachable!("XorExecutor received non-XOR opcode"),
            },
        )
    };
}
...
impl<F, A, const LIMB_BITS: usize> InterpreterExecutor<F>
    for XorExecutor<A, { RV64_REGISTER_NUM_LIMBS }, LIMB_BITS>
where
    F: PrimeField32,
{
    #[inline(always)]
    fn pre_compute_size(&self) -> usize {
        size_of::<XorPreCompute>()
    }
    ...
    {
        let data: &mut XorPreCompute = data.borrow_mut();
    ...
    F: PrimeField32,
{
    ...
    fn metered_pre_compute_size(&self) -> usize {
        size_of::<E2PreCompute<XorPreCompute>>()
    }
    ...
    {
        let data: &mut E2PreCompute<XorPreCompute> = data.borrow_mut();
...
>(
    ...
    let pre_compute: &XorPreCompute =
        std::slice::from_raw_parts(pre_compute, size_of::<XorPreCompute>()).borrow();
...
>(
    ...
    let pre_compute: &E2PreCompute<XorPreCompute> =
        std::slice::from_raw_parts(pre_compute, size_of::<E2PreCompute<XorPreCompute>>()).borrow();
    exec_state
        .ctx
