...
type P256Fp = halo2curves_axiom::secp256r1::Fp;

/// `3b` for P256, computed once from the standard `b` constant.
static P256_B3: LazyLock<P256Fp> = LazyLock::new(|| {
    ...
    le_bytes.reverse();
    let b = P256Fp::from_repr(le_bytes).unwrap();
    b + b + b
});
...
pub(crate) unsafe fn ec_add_proj_p256(state: *mut c_void, rd_ptr: u64, rs1_ptr: u64, rs2_ptr: u64) {
    let (x1, y1, z1) = read_proj_256::<P256Fp>(state, rs1_ptr);
    let (x2, y2, z2) = read_proj_256::<P256Fp>(state, rs2_ptr);
    let a = -P256Fp::from(P256_A_ABS);
    let out = ec_add_proj_impl_general(x1, y1, z1, x2, y2, z2, a, *P256_B3);
...
pub(crate) unsafe fn ec_double_proj_p256(state: *mut c_void, rd_ptr: u64, rs1_ptr: u64) {
    let (x1, y1, z1) = read_proj_256::<P256Fp>(state, rs1_ptr);
    let a = -P256Fp::from(P256_A_ABS);
    let out = ec_double_proj_impl_general(x1, y1, z1, a, *P256_B3);
    write_proj_256(state, rd_ptr, out);
}
