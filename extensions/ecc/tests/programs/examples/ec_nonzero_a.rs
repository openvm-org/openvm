...
pub fn main() {
    ...
    let p1 = unsafe { P256Point::from_xy(x1.clone(), y1.clone()).unwrap() };
    ...
    let p2 = unsafe { P256Point::from_xy(x2.clone(), y2.clone()).unwrap() };
    ...
    let _generator = unsafe { P256Point::from_xy(gen_x, gen_y).unwrap() };
    let (neg_x, neg_y) = P256Point::NEG_GENERATOR.into_coords();
    let _neg_generator = unsafe { P256Point::from_xy(neg_x, neg_y).unwrap() };
}
