...
pub trait WeierstrassPoint: Clone + Sized {
    ...
    ///
    /// # Safety
    /// - Caller must guarantee `(x, y)` is a valid affine point on the curve and in any required
    ///   subgroup.
    unsafe fn from_xy_unchecked(x: Self::Coordinate, y: Self::Coordinate) -> Self;
    /// Raw constructor from projective coordinates.
    ///
    /// # Safety
    /// - Caller must guarantee `(x, y, z)` is a valid projective point on the curve and in any
    ///   required subgroup. The identity point must be represented with `z = 0`.
    unsafe fn from_xyz_unchecked(
        x: Self::Coordinate,
        y: Self::Coordinate,
        z: Self::Coordinate,
    ) -> Self;
    /// Consumes the point and returns its raw projective coordinates `(X, Y, Z)`.
    fn into_coords(self) -> (Self::Coordinate, Self::Coordinate, Self::Coordinate);

    /// Consumes the point and returns affine coordinates `(X/Z, Y/Z)`.
    ///
    /// Returns `None` for the identity. Implementations should avoid division when `Z = 1`.
...
pub trait IntrinsicCurve {
    ...
    /// Computes `generator_scalar * G + point_scalar * point`.
    ...
    where
        Self::Point: CyclicGroup,
    {
        Self::msm(
            &[generator_scalar.clone(), point_scalar.clone()],
            &[Self::Point::GENERATOR, point.clone()],
        )
    }
        ...
        Self::Point: CyclicGroup,
    {
        Self::msm(
            &[generator_scalar.clone(), point_scalar.clone()],
            &[Self::Point::NEG_GENERATOR, point.clone()],
        )
    }
}
    ...
    C::Scalar: IntMod,
{
    ...
    pub fn new_with_prime_order(bases: &'a [C::Point], window_bits: usize) -> Self {
        Self::new_with_prime_order_and_precomputed(bases, window_bits, &[])
    }

    /// Constructor that reuses precomputed multiples for selected bases.
    ///
    /// `precomputed_tables[i]`, when present, must contain `2P..(2^window_bits - 1)P` for
    /// `bases[i]`. Missing entries are computed normally.
    #[inline]
    pub fn new_with_prime_order_and_precomputed(
        bases: &'a [C::Point],
        window_bits: usize,
        precomputed_tables: &[Option<&[C::Point]>],
    ) -> Self {
        C::Point::set_up_once();
        assert!(window_bits > 0);
        assert!(precomputed_tables.is_empty() || precomputed_tables.len() == bases.len());
        ...
        let table = bases
            ...
            .map(|(base_idx, base)| {
                ...
                } else if let Some(table) = precomputed_tables
                    .get(base_idx)
                    .and_then(|table| table.as_ref())
                {
                    assert_eq!(table.len(), window_size - 2);
                    table.to_vec()
                } else {
                    let mut multiples = Vec::with_capacity(window_size - 2);
                    for scalar in 2..window_size {
                        let multiple = if scalar.is_multiple_of(2) {
                            let half = if scalar == 2 {
                                base
                            } else {
                                // `multiples[i] = (i + 2) * base`.
                                &multiples[scalar / 2 - 2]
                            };
                            half.double_impl::<false>()
                        } else {
                            multiples
                                .last()
                                .expect("the preceding even multiple is initialized")
                                .add_impl::<false>(base)
                        };
    ...
    ($struct_name:ident, $field:ty, $b:expr) => {
        ...
        impl core::cmp::PartialEq for $struct_name {
            fn eq(&self, other: &Self) -> bool {
                let self_is_identity = self.z == <$field>::ZERO;
                let other_is_identity = other.z == <$field>::ZERO;
        ...
        impl WeierstrassPoint for $struct_name {
            ...
            fn into_affine_coords(self) -> Option<(Self::Coordinate, Self::Coordinate)> {
                ...
                if self.z == <$field>::ZERO {
                    None
                } else if self.z == <$field>::ONE {
                    Some((self.x, self.y))
                } else {
                    Some(((&self.x).div_unsafe(&self.z), (&self.y).div_unsafe(&self.z)))
                }
            }
            fn x(&self) -> &Self::Coordinate {
                &self.x
