...
__global__ void ec_add_ne_compute_tracegen_kernel(
    ...
    BigUintGpu lambda(tmp0);
    lambda = lambda.mod_div(tmp1, prime, barrett_mu);

    BigUintGpu lambda_sq(lambda);
    BigUintGpu lambda_rhs(lambda);
    lambda_sq = lambda_sq.mul(lambda_rhs);
    tmp0 = lambda_sq;
    tmp0 = tmp0.mod_reduce(prime, barrett_mu);
    BigUintGpu x3(tmp0);
    x3 = x3.mod_sub(x1, prime);
    x3 = x3.mod_sub(x2, prime);
    tmp0 = x1.mod_sub(x3, prime);
    BigUintGpu y3(lambda);
    y3 = y3.mul(tmp0);
...
template <size_t BLOCKS, size_t BLOCK_SIZE, size_t NUM_LIMBS>
__global__ void ec_add_ne_constraint0_tracegen_kernel(
    Fp *trace,
    size_t height,
    const uint8_t *records,
    size_t num_records,
    ...
    OverflowInt q_ov(quotient, q_limbs);
    tmp_ov = q_ov;
...
template <size_t BLOCKS, size_t BLOCK_SIZE, size_t NUM_LIMBS>
__global__ void ec_add_ne_constraint1_tracegen_kernel(
    Fp *trace,
    size_t height,
    const uint8_t *records,
    size_t num_records,
...
template <size_t BLOCKS, size_t BLOCK_SIZE, size_t NUM_LIMBS>
__global__ void ec_add_ne_constraint2_tracegen_kernel(
    Fp *trace,
    size_t height,
    const uint8_t *records,
    size_t num_records,
...
extern "C" int launch_ec_add_ne_tracegen(
    ...
    if (prime_limb_count <= 32) {
        ...
        const size_t q0_col = 1 + 7 * NUM_LIMBS;
        const size_t q1_col = q0_col + q0_limbs;
        const size_t q2_col = q1_col + q1_limbs;
        const size_t c0_col = q2_col + q2_limbs;
        const size_t c1_col = c0_col + c0_limbs;
        const size_t c2_col = c1_col + c1_limbs;
        ...
        ec_add_ne_constraint0_tracegen_kernel<BLOCKS, BLOCK_SIZE, NUM_LIMBS>
        ...
        ec_add_ne_constraint1_tracegen_kernel<BLOCKS, BLOCK_SIZE, NUM_LIMBS>
            <<<constraint_grid, constraint_block>>>(
                d_trace,
                trace_height,
                d_records,
        ...
        ec_add_ne_constraint2_tracegen_kernel<BLOCKS, BLOCK_SIZE, NUM_LIMBS>
            <<<constraint_grid, constraint_block>>>(
                d_trace,
                trace_height,
                d_records,
