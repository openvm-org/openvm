...
use crate::weierstrass_chip::curves::ec_double_proj;
...
impl<'a, const BLOCKS: usize> EcDoubleExecutor<BLOCKS> {
    fn pre_compute_impl<F: PrimeField32>(
        ...
        *data = EcDoublePreCompute {
        ...
        let local_opcode = opcode.local_opcode_idx(self.offset);
        let is_setup = local_opcode == WeierstrassOpcode::SETUP_SW_EC_DOUBLE_PROJ as usize;
...
macro_rules! dispatch {
    ($execute_impl:ident, $pre_compute:ident, $is_setup:ident, $field_type:expr) => {
...
>(
    ...
    let output_data = if FIELD_TYPE == u8::MAX || IS_SETUP {
        ...
        ec_double_proj::<FIELD_TYPE, BLOCKS>(read_data)
    };
