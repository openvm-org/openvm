...
pub fn ec_add_proj_expr(
    ...
    FieldExpr::new(
        ec_add_proj_program(config, range_bus.range_max_bits, a, b),
        range_bus,
    )
}

pub fn ec_add_proj_program(
    ...
    let builder = if a.is_zero() {
        build_ec_add_proj_a0_expr(config, range_max_bits, b3)
    } else {
        build_ec_add_proj_general_expr(config, range_max_bits, a.clone(), b3)
    };
    FieldExpressionProgram::new_with_setup_values(builder, true, vec![a, b])
}

fn build_ec_add_proj_a0_expr(
    config: ExprBuilderConfig,
    range_max_bits: usize,
    b3: BigUint,
) -> ExprBuilder {
...
fn build_ec_add_proj_general_expr(
    config: ExprBuilderConfig,
    range_max_bits: usize,
    a_val: BigUint,
    b3: BigUint,
...
// Pure and metered execution use the specialized native implementation in `execution.rs`.
#[derive(Clone)]
pub struct EcAddExecutor<const BLOCKS: usize> {
    pub(crate) inner: FieldExpressionExecutor,
    pub(crate) cached_field_type: Option<FieldType>,
}

impl<const BLOCKS: usize> EcAddExecutor<BLOCKS> {
...
impl<const BLOCKS: usize> Deref for EcAddExecutor<BLOCKS> {
...
impl<const BLOCKS: usize> DerefMut for EcAddExecutor<BLOCKS> {
...
fn gen_base_program(
    ...
    let program = ec_add_proj_program(config, range_max_bits, a, b);
...
pub fn get_ec_add_air<const BLOCKS: usize>(
...
pub fn get_ec_add_executor<const BLOCKS: usize>(
    config: ExprBuilderConfig,
    range_max_bits: usize,
    offset: usize,
    a: BigUint,
    ...
    EcAddExecutor::new(FieldExpressionExecutor::new(
        program,
        offset,
        local_opcode_idx,
        vec![],
...
pub fn get_ec_add_chip<F, const BLOCKS: usize>(
    config: ExprBuilderConfig,
