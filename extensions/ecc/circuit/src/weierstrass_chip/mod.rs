...
struct WeierstrassPreCompute<const NUM_READS: usize> {
    a: u32,
    ...
    StepExecutorE1<F> for WeierstrassStep<NUM_READS, BLOCKS, BLOCK_SIZE>
{
    fn pre_compute_size(&self) -> usize {
        std::mem::size_of::<WeierstrassPreCompute<NUM_READS>>()
    }
    ...
    {
        let data: &mut WeierstrassPreCompute<NUM_READS> = data.borrow_mut();
        let Instruction {
            opcode,
            a,
            b,
        ...
        let mut rs_addrs = [0u32; NUM_READS];
        for (i, addr) in rs_addrs.iter_mut().enumerate() {
            *addr = if i == 0 { b } else { c };
        }
...
>(
    ...
    let pre_compute: &WeierstrassPreCompute<NUM_READS> = pre_compute.borrow();
