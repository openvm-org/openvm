    ...
    > EcMulExecutor<BLOCKS_PER_SCALAR, BLOCKS_PER_POINT, SCALAR_SIZE, POINT_SIZE>
{
    fn pre_compute_impl<F: PrimeField32>(
        ...
        // TODO: Pre-compute flag_idx
        // let needs_setup = self.expr.needs_setup();
        let flag_idx = self.expr.num_flags() as u8;
        // if needs_setup {
...
>(
    ...
    // if IS_SETUP {
    //     let input_scalar_prime = BigUint::from_bytes_le(scalar_data.as_flattened());
    //     if input_scalar_prime != pre_compute.expr.builder.prime {
    //         let err = ExecutionError::Fail {
    //             pc,
    //             msg: "EcMul: mismatched scalar prime",
    //         };
    //         return Err(err);
    //     }

    //     let input_point_prime =
    //         BigUint::from_bytes_le(point_data[..BLOCKS_PER_POINT / 2].as_flattened());
    //     if input_point_prime != pre_compute.expr.builder.prime {
    //         let err = ExecutionError::Fail {
    //             pc,
    //             msg: "EcMul: mismatched point prime",
    //         };
    //         return Err(err);
    //     }
    ...
    for (i, block) in output_data.into_iter().enumerate() {
        exec_state.vm_write(RV32_MEMORY_AS, rd_val + (i * POINT_SIZE) as u32, &block);
    }
