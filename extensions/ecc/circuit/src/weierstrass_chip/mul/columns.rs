...
use super::{
    EC_MUL_COMPUTE_ROWS, SCALAR_ACC_LIMBS, SCALAR_BLOCKS, SCALAR_CARRY_LIMBS, SCALAR_MEMORY_LIMBS,
};
...
pub struct EcMulHeaderCols<T> {
    ...
    /// `FieldExpr::is_valid AND NOT setup AND NOT is_first_compute`, stored so the data links can
    /// be gated at degree 1. Setup is derived from the expression flags.
    pub is_ladder: T,
...
pub struct EcMulIoCols<T, const NUM_LIMBS: usize, const BLOCKS: usize> {
    ...
    /// Scalar u16 limbs read from rs2, checked against `2B + 1` for the completed accumulator.
    pub scalar_data: [T; SCALAR_MEMORY_LIMBS],
    pub scalar_read_aux: [MemoryReadAuxCols<T>; SCALAR_BLOCKS],
    /// Carries into u16 limbs 1 through 15. The top limb has no outgoing carry column because its
    /// equation constrains that carry to zero.
    pub scalar_carry: [T; SCALAR_CARRY_LIMBS],
