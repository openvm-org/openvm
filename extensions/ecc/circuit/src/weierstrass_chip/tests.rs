...
fn test_add_ne() {
    ...
    assert_eq!(chip.0.step.0 .0.expr.builder.num_variables, 3); // lambda, x3, y3
    ...
    let prime_limbs: [BabyBear; NUM_LIMBS] =
        prime_limbs(&chip.0.step.0 .0.expr).try_into().unwrap();
    ...
    let setup_instruction = rv32_write_heap_default(
        ...
        chip.0.step.0 .0.offset + Rv32WeierstrassOpcode::SETUP_EC_ADD_NE as usize,
    );
    ...
    let instruction = rv32_write_heap_default(
        ...
        chip.0.step.0 .0.offset + Rv32WeierstrassOpcode::EC_ADD_NE as usize,
    );
...
fn test_double() {
    ...
    let r = chip.0.step.0 .0.expr.execute(vec![p1_x, p1_y], vec![true]);
    assert_eq!(r.len(), 3); // lambda, x3, y3
    ...
    let setup_instruction = rv32_write_heap_default(
        ...
        vec![prime_limbs, a_limbs], /* inputs[0] = prime, inputs[1] = a coeff of weierstrass
                                     * equation */
        vec![],
        chip.0.step.0 .0.offset + Rv32WeierstrassOpcode::SETUP_EC_DOUBLE as usize,
    );
    tester.execute(&mut chip, &setup_instruction);
    ...
    let instruction = rv32_write_heap_default(
        ...
        vec![p1_x_limbs, p1_y_limbs],
        vec![],
        chip.0.step.0 .0.offset + Rv32WeierstrassOpcode::EC_DOUBLE as usize,
    );
