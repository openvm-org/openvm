...
pub fn get_modulus_as_bigint<F: PrimeField>() -> BigUint {
    BigUint::from_str_radix(F::MODULUS.trim_start_matches("0x"), 16).unwrap()
}

pub(super) fn get_sw_curve_type(modulus: &BigUint, a_coeff: &BigUint) -> Option<SwCurveType> {
    if modulus == &get_modulus_as_bigint::<halo2curves_axiom::secq256k1::Fq>()
        && a_coeff == &BigUint::ZERO
    {
        return Some(SwCurveType::K256);
    }
...
#[inline(always)]
pub fn sw_add_ne<const FIELD_TYPE: u8, const BLOCKS: usize, const BLOCK_SIZE: usize>(
    input_data: [[[u8; BLOCK_SIZE]; BLOCKS]; 2],
) -> [[u8; BLOCK_SIZE]; BLOCKS] {
    match FIELD_TYPE {
        x if x == FieldType::K256Coordinate as u8 => {
            sw_add_ne_256bit::<halo2curves_axiom::secq256k1::Fq, BLOCKS, BLOCK_SIZE>(input_data)
        }
        x if x == FieldType::P256Coordinate as u8 => {
            sw_add_ne_256bit::<halo2curves_axiom::secp256r1::Fp, BLOCKS, BLOCK_SIZE>(input_data)
        }
        x if x == FieldType::BN254Coordinate as u8 => {
            sw_add_ne_256bit::<halo2curves_axiom::bn256::Fq, BLOCKS, BLOCK_SIZE>(input_data)
        }
        x if x == FieldType::BLS12_381Coordinate as u8 => {
            sw_add_ne_bls12_381::<BLOCKS, BLOCK_SIZE>(input_data)
        }
...
#[inline(always)]
pub fn sw_double<const CURVE_TYPE: u8, const BLOCKS: usize, const BLOCK_SIZE: usize>(
    input_data: [[u8; BLOCK_SIZE]; BLOCKS],
) -> [[u8; BLOCK_SIZE]; BLOCKS] {
    match CURVE_TYPE {
        x if x == SwCurveType::K256 as u8 => {
            sw_double_256bit::<halo2curves_axiom::secq256k1::Fq, 0, BLOCKS, BLOCK_SIZE>(input_data)
        }
        x if x == SwCurveType::P256 as u8 => {
            sw_double_256bit::<halo2curves_axiom::secp256r1::Fp, P256_NEG_A, BLOCKS, BLOCK_SIZE>(
                input_data,
            )
        }
        x if x == SwCurveType::BN254 as u8 => {
            sw_double_256bit::<halo2curves_axiom::bn256::Fq, 0, BLOCKS, BLOCK_SIZE>(input_data)
        }
        x if x == SwCurveType::BLS12_381 as u8 => {
            sw_double_bls12_381::<BLOCKS, BLOCK_SIZE>(input_data)
        }
...
>(
    ...
    let (x3, y3) = sw_add_ne_impl::<F>(x1, y1, x2, y2);
...
>(
    ...
    let (x3, y3) = sw_double_impl::<F, NEG_A>(x1, y1);
...
#[inline(always)]
fn sw_add_ne_bls12_381<const BLOCKS: usize, const BLOCK_SIZE: usize>(
    input_data: [[[u8; BLOCK_SIZE]; BLOCKS]; 2],
) -> [[u8; BLOCK_SIZE]; BLOCKS] {
    // Extract coordinates
    let x1 =
    ...
    let (x3, y3) = sw_add_ne_impl::<halo2curves_axiom::bls12_381::Fq>(x1, y1, x2, y2);
...
#[inline(always)]
fn sw_double_bls12_381<const BLOCKS: usize, const BLOCK_SIZE: usize>(
    input_data: [[u8; BLOCK_SIZE]; BLOCKS],
) -> [[u8; BLOCK_SIZE]; BLOCKS] {
    // Extract coordinates
    ...
    let (x3, y3) = sw_double_impl::<halo2curves_axiom::bls12_381::Fq, 0>(x1, y1);
...
#[inline(always)]
pub fn sw_add_ne_impl<F: PrimeField>(x1: F, y1: F, x2: F, y2: F) -> (F, F) {
...
#[inline(always)]
pub fn sw_double_impl<F: PrimeField, const NEG_A: u64>(x1: F, y1: F) -> (F, F) {
