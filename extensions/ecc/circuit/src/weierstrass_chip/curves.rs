...
/// name; native fast-path dispatch is keyed on the coordinate [`FieldType`], which is in 1:1
/// correspondence with these curves.
...
/// Returns the coordinate [`FieldType`] only when `(modulus, a, b)` exactly match the standard
/// curve defined over that field. The native fast paths hardcode each standard curve's `a`/`b3`
/// constants, so dispatching on the modulus alone would silently compute with the wrong curve
/// constants for a custom curve defined over the same base field.
pub(super) fn get_verified_field_type(
    ...
    use num_bigint::BigUint;
    let field_type = openvm_algebra_circuit::fields::get_field_type(modulus)?;
    let matches = match field_type {
        // secp256k1: y^2 = x^3 + 7
        FieldType::K256Coordinate => a == &BigUint::ZERO && b == &BigUint::from(7u32),
        // secp256r1: y^2 = x^3 - 3x + b
        FieldType::P256Coordinate => {
            let p256_b = BigUint::parse_bytes(
                b"5AC635D8AA3A93E7B3EBBD55769886BC651D06B0CC53B0F63BCE3C3E27D2604B",
                16,
            )
            .unwrap();
            a == &(modulus - BigUint::from(P256_NEG_A)) && b == &p256_b
        }
        // BN254 G1: y^2 = x^3 + 3
        FieldType::BN254Coordinate => a == &BigUint::ZERO && b == &BigUint::from(3u32),
        // BLS12-381 G1: y^2 = x^3 + 4
        FieldType::BLS12_381Coordinate => a == &BigUint::ZERO && b == &BigUint::from(4u32),
        // Scalar fields are not used for ECC point coordinates
        _ => false,
    };
    matches.then_some(field_type)
}
