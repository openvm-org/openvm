...
/// Identifies a supported short Weierstrass curve. Re-exported for callers that describe curves by
/// name. Native fast-path dispatch requires the modulus and both curve coefficients to match.
...
static K256_B_COEFF: Lazy<BigUint> = Lazy::new(|| BigUint::from(7u8));
static P256_B_COEFF: Lazy<BigUint> = Lazy::new(|| {
...
static BN254_B_COEFF: Lazy<BigUint> = Lazy::new(|| BigUint::from(3u8));
static BLS12_381_B_COEFF: Lazy<BigUint> = Lazy::new(|| BigUint::from(4u8));
...
impl CurveType {
    ...
    pub fn b_coefficient(self) -> &'static BigUint {
        match self {
            Self::K256 => &K256_B_COEFF,
            Self::P256 => &P256_B_COEFF,
            Self::BN254 => &BN254_B_COEFF,
            Self::BLS12_381 => &BLS12_381_B_COEFF,
        }
    }

    fn field_type(self) -> FieldType {
        match self {
            Self::K256 => FieldType::K256Coordinate,
            Self::P256 => FieldType::P256Coordinate,
            Self::BN254 => FieldType::BN254Coordinate,
            Self::BLS12_381 => FieldType::BLS12_381Coordinate,
        }
    }
...
pub(crate) fn get_curve_type(
    ...
    .into_iter()
    .find(|curve| {
        modulus == curve.coordinate_modulus()
            && a_coeff == curve.a_coefficient()
            && b_coeff == curve.b_coefficient()
    })
}

pub(super) fn get_verified_field_type(
    modulus: &BigUint,
    a_coeff: &BigUint,
    b_coeff: &BigUint,
) -> Option<FieldType> {
    get_curve_type(modulus, a_coeff, b_coeff).map(CurveType::field_type)
}

/// Dispatch complete projective point addition based on the const generic coordinate field type.
/// A point is stored as three contiguous coordinates (X, Y, Z), so `BLOCKS` covers all three.
#[inline(always)]
pub fn ec_add_proj<const FIELD_TYPE: u8, const BLOCKS: usize>(
    input_data: [[[u8; MEMORY_BLOCK_BYTES]; BLOCKS]; 2],
) -> [[u8; MEMORY_BLOCK_BYTES]; BLOCKS] {
    match FIELD_TYPE {
        x if x == FieldType::K256Coordinate as u8 => ec_add_proj_k256_bytes::<BLOCKS>(input_data),
        x if x == FieldType::P256Coordinate as u8 => {
            ec_add_proj_256bit_general::<halo2curves_axiom::secp256r1::Fp, P256_NEG_A, BLOCKS>(
                input_data,
            )
        }
...
/// Dispatch complete projective point doubling based on the const generic coordinate field type.
#[inline(always)]
pub fn ec_double_proj<const FIELD_TYPE: u8, const BLOCKS: usize>(
    ...
    match FIELD_TYPE {
        ...
        x if x == FieldType::BN254Coordinate as u8 => {
            ec_double_proj_256bit_a0::<halo2curves_axiom::bn256::Fq, BN254_B3, BLOCKS>(input_data)
        }
        x if x == FieldType::BLS12_381Coordinate as u8 => {
            ec_double_proj_bls12_381::<BLOCKS>(input_data)
        }
        _ => panic!("Unsupported field type: {FIELD_TYPE}"),
    }
}
...
#[inline(always)]
fn k256_field_to_bytes_le(field: &k256::FieldElement, output: &mut [[u8; MEMORY_BLOCK_BYTES]]) {
    let be_bytes = field.to_bytes();
    let mut le_bytes = [0u8; 32];
    le_bytes.copy_from_slice(&be_bytes);
    le_bytes.reverse();
    for (i, byte) in le_bytes.iter().enumerate() {
        let block_idx = i / MEMORY_BLOCK_BYTES;
        let byte_idx = i % MEMORY_BLOCK_BYTES;
...
#[inline(always)]
fn ec_add_proj_k256_bytes<const BLOCKS: usize>(
    input_data: [[[u8; MEMORY_BLOCK_BYTES]; BLOCKS]; 2],
) -> [[u8; MEMORY_BLOCK_BYTES]; BLOCKS] {
...
#[inline(always)]
fn ec_double_proj_k256_bytes<const BLOCKS: usize>(
    input_data: [[u8; MEMORY_BLOCK_BYTES]; BLOCKS],
) -> [[u8; MEMORY_BLOCK_BYTES]; BLOCKS] {
    let coord_blocks = BLOCKS / 3;
...
#[inline(always)]
fn ec_add_proj_256bit_a0<F: PrimeField<Repr = [u8; 32]>, const B3: u64, const BLOCKS: usize>(
    input_data: [[[u8; MEMORY_BLOCK_BYTES]; BLOCKS]; 2],
) -> [[u8; MEMORY_BLOCK_BYTES]; BLOCKS] {
    let coord_blocks = BLOCKS / 3;
    ...
    let mut output = [[0u8; MEMORY_BLOCK_BYTES]; BLOCKS];
    field_element_to_blocks::<F>(&x3, &mut output[..coord_blocks]);
    field_element_to_blocks::<F>(&y3, &mut output[coord_blocks..2 * coord_blocks]);
    field_element_to_blocks::<F>(&z3, &mut output[2 * coord_blocks..]);

    output
}

#[inline(always)]
fn ec_double_proj_256bit_a0<F: PrimeField<Repr = [u8; 32]>, const B3: u64, const BLOCKS: usize>(
    input_data: [[u8; MEMORY_BLOCK_BYTES]; BLOCKS],
) -> [[u8; MEMORY_BLOCK_BYTES]; BLOCKS] {
    let coord_blocks = BLOCKS / 3;
...
#[inline(always)]
fn ec_add_proj_bls12_381<const BLOCKS: usize>(
    input_data: [[[u8; MEMORY_BLOCK_BYTES]; BLOCKS]; 2],
) -> [[u8; MEMORY_BLOCK_BYTES]; BLOCKS] {
    let coord_blocks = BLOCKS / 3;
...
#[inline(always)]
fn ec_double_proj_bls12_381<const BLOCKS: usize>(
    input_data: [[u8; MEMORY_BLOCK_BYTES]; BLOCKS],
) -> [[u8; MEMORY_BLOCK_BYTES]; BLOCKS] {
    let coord_blocks = BLOCKS / 3;
...
mod tests {
    ...
    #[test]
    fn native_dispatch_requires_exact_curve_parameters() {
        assert_eq!(
            get_verified_field_type(&K256_COORD_MODULUS, &ZERO_A_COEFF, &K256_B_COEFF),
            Some(FieldType::K256Coordinate)
        );
        assert_eq!(
            get_verified_field_type(&K256_COORD_MODULUS, &ZERO_A_COEFF, &(&*K256_B_COEFF + 1u8)),
            None
        );
        assert_eq!(
            get_verified_field_type(&K256_COORD_MODULUS, &(&*ZERO_A_COEFF + 1u8), &K256_B_COEFF),
            None
        );
    }
