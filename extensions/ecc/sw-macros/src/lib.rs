...
/// }
/// ```
/// This creates a struct `Secp256k1Point` representing an affine point on the short Weierstrass
/// curve `y^2 = x^3 + a*x + b` with coordinate type `mod_type` (a type generated by
...
pub fn sw_declare(input: TokenStream) -> TokenStream {
    ...
    for item in items.into_iter() {
        ...
        create_extern_func!(sw_mul_extern_func);
        ...
        let result = TokenStream::from(quote::quote_spanned! { span.into() =>
            extern "C" {
                ...
                fn #sw_mul_extern_func(rd: usize, rs1: usize, rs2: usize);
                fn #sw_setup_mul_extern_func(uninit: *mut core::ffi::c_void, p1: *const u8, p2: *const u8);
            }
            ...
            impl #struct_name {
                ...
                /// # Safety
                ///
                /// `scalar` must be odd and less than the order of the subgroup generated by
                /// `self`, and `self` must not be the identity. On a curve with a cofactor, `self`
                /// must also lie in the prime-order subgroup.
                ///
                /// The chip consumes the scalar as signed binary digits, whose value `sum +-2^i`
                /// is odd for any choice of signs, so an even scalar has no digit assignment and
                /// no provable trace. The order bound keeps the ladder's incomplete affine
                /// formulas total; at or above it, an intermediate doubling can share an
                /// x-coordinate with `±self`, which the circuit does not constrain. This is the
                /// same class of precondition as `add_ne_nonidentity`'s requirement that the two
                /// x-coordinates differ. For an identity base the accumulator seeds as `(0, 0)`
                /// and the first doubling divides by `2·y = 0`, which has no witness: such a call
                /// cannot be proven.
                ///
                /// The curve guest libraries discharge all of these in their `mul_scalar`
                /// wrappers, which reduce the scalar and restore oddness via `(n - k)·P = -(k·P)`.
                ///
                /// Multi-base `IntrinsicCurve::msm` does not use this: the windowed method shares
                /// doublings across bases, which a single-base intrinsic cannot. Single-pair calls
                /// route here through `mul_scalar`.
                pub unsafe fn mul_scalar_le_unchecked(&self, scalar: &[u8; 32]) -> Self {
                    #[cfg(not(any(openvm_intrinsics, target_os = "openvm")))]
                    {
                        // Plain MSB-first double-and-add; the chip's signed-digit ladder computes
                        // the same product. `double_assign_impl` and `add_assign_impl` handle the
                        // identity, so the leading zero bits need no special case.
                ...
                fn set_up_once() {
                    ...
                    is_setup.get_or_init(|| {
                        ...
                        // EC_MUL has a separate chip instance and so needs its own setup row.
                        // Same operands: p1 is (p, a), which is what the field expression checks.
                        unsafe { #sw_setup_mul_extern_func(uninit.as_mut_ptr() as *mut core::ffi::c_void, p1.as_ptr(), p2.as_ptr()); }
...
pub fn sw_init(input: TokenStream) -> TokenStream {
    ...
    for (ec_idx, struct_id) in items.into_iter().enumerate() {
        ...
        let mul_extern_func =
            syn::Ident::new(&format!("sw_mul_extern_func_{struct_id}"), span.into());
        let setup_mul_extern_func = syn::Ident::new(
            &format!("sw_setup_mul_extern_func_{struct_id}"),
            span.into(),
        );
        ...
        externs.push(quote::quote_spanned! { span.into() =>
            ...
            /// `rd = [rs2] * [rs1]`: rs1 is the base point pointer, rs2 the scalar pointer.
            #[no_mangle]
            extern "C" fn #mul_extern_func(rd: usize, rs1: usize, rs2: usize) {
                openvm::platform::custom_insn_r!(
                    opcode = OPCODE,
                    funct3 = SW_FUNCT3 as usize,
                    funct7 = SwBaseFunct7::SwMul as usize + #ec_idx
                        * (SwBaseFunct7::SHORT_WEIERSTRASS_MAX_KINDS as usize),
                    rd = In rd,
                    rs1 = In rs1,
                    rs2 = In rs2
            ...
            /// EC_MUL needs its own setup kind: `SwSetup` distinguishes double from add via `rs2`,
            /// leaving no encoding room for a third operation.
            #[no_mangle]
            extern "C" fn #setup_mul_extern_func(uninit: *mut core::ffi::c_void, p1: *const u8, p2: *const u8) {
                #[cfg(any(openvm_intrinsics, target_os = "openvm"))]
                {
                    openvm::platform::custom_insn_r!(
