...
pub fn sw_declare(input: TokenStream) -> TokenStream {
    ...
    for item in items.into_iter() {
        ...
        let result = TokenStream::from(quote::quote_spanned! { span.into() =>
            ...
            impl #struct_name {
                /// Multiplies an affine point by a scalar with the `EC_MUL` intrinsic.
                ///
                /// # Safety
                ///
                /// - `self` must not be the identity.
                /// - `self` must be in the configured prime-order subgroup.
                /// - `scalar` must be odd and less than the subgroup order.
                /// - The subgroup order must be equal to 1 modulo 4.
                /// - `CHECK_SETUP` can be `false` only after EC_MUL setup has run for this curve.
                ///
                /// Execution or proof generation can fail if a requirement is not met.
                ...
                // Calls the add/double setup instruction on first use.
                #[inline(always)]
                #[cfg(any(openvm_intrinsics, target_os = "openvm"))]
                fn set_up_once() {
                ...
                // EC_MUL has a separate setup. Run it only when EC_MUL is used.
                #[inline(always)]
                #[cfg(any(openvm_intrinsics, target_os = "openvm"))]
                fn set_up_mul_once() {
                    static is_setup: ::openvm_ecc_guest::once_cell::race::OnceBool = ::openvm_ecc_guest::once_cell::race::OnceBool::new();
