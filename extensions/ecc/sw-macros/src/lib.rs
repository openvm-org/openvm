...
/// ```
///
/// The generated point uses projective `(X, Y, Z)` coordinates. The `a` coefficient is optional
/// and defaults to zero; `b` is required. The `elliptic_curve` crate and
/// `openvm_ecc_guest::weierstrass` module must be in scope for the generated implementations.
#[proc_macro]
pub fn sw_declare(input: TokenStream) -> TokenStream {
    ...
    for item in items.into_iter() {
        ...
        let serde_repr_name = format_ident!("{}SerdeRepr", struct_name);
        ...
        let result = TokenStream::from(quote::quote_spanned! { span.into() =>
            ...
            #[serde(rename = #struct_name_str)]
            struct #serde_repr_name {
                x: #intmod_type,
                y: #intmod_type,
                z: #intmod_type,
            }
            ...
            impl<'de> serde::Deserialize<'de> for #struct_name {
                ...
                {
                    let repr = <#serde_repr_name as serde::Deserialize>::deserialize(deserializer)?;
                    if openvm_algebra_guest::IntMod::as_le_bytes(&repr.z)
                        .iter()
                        .all(|&byte| byte == 0)
                    {
                        Ok(Self::identity())
                    } else {
                        Ok(Self { x: repr.x, y: repr.y, z: repr.z })
                    }
                }
            }
            ...
            impl ::openvm_ecc_guest::weierstrass::WeierstrassPoint for #struct_name {
                ...
                #[inline(always)]
                unsafe fn from_xyz_unchecked(x: Self::Coordinate, y: Self::Coordinate, z: Self::Coordinate) -> Self {
                    Self { x, y, z }
                }
