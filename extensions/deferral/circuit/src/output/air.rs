...
use crate::{
    ...
    utils::{
        combine_output_cells, split_cell_memory_ops, u16_commit_to_f, u16s_to_f, COMMIT_NUM_U16S,
        F_NUM_U16S, OUTPUT_TOTAL_MEMORY_OPS, OUTPUT_TOTAL_NUM_U16S, SPONGE_BYTES_PER_ROW,
...
pub struct DeferralOutputCols<T> {
    ...
    pub rs_val: [T; RV64_PTR_U16_LIMBS],
    ...
    AB::F: PrimeField32,
{
    fn eval(&self, builder: &mut AB) {
        ...
        self.range_bus
            .range_check(
                ptr_bound_from_high_u16_expr(local.rd_val[1], self.address_bits),
                U16_BITS,
            )
            .eval(builder, local.is_first);
        self.range_bus
            .range_check(
                ptr_bound_from_high_u16_expr(local.rs_val[1], self.address_bits),
                U16_BITS,
            )
            .eval(builder, local.is_first);
        ...
        self.range_bus
            .range_check(
                ptr_bound_from_high_u16_expr(local.output_len[1], self.address_bits),
                U16_BITS,
            )
            .eval(builder, local.is_first);
        ...
        let input_ptr = u16_limbs_to_ptr(&local.rs_val);
        let output_ptr = u16_limbs_to_ptr(&local.rd_val);
