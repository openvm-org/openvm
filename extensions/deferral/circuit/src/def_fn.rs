...
pub struct RawDeferralResult<F> {
...
pub fn generate_deferral_results<F: VmField>(
    raw_results: Vec<RawDeferralResult<F>>,
    deferral_idx: u32,
    hasher: &DeferralPoseidon2Chip<F>,
) -> Vec<DeferralResult<F>> {
    raw_results
        .into_iter()
        .map(|r| {
