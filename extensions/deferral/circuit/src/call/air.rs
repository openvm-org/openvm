...
use crate::{
    ...
    utils::{
        combine_output_cells, split_cell_memory_ops, u16_commit_to_f, COMMIT_MEMORY_OPS,
        COMMIT_NUM_U16S, DIGEST_F_MEMORY_OPS, F_NUM_U16S, OUTPUT_TOTAL_MEMORY_OPS,
        OUTPUT_TOTAL_NUM_U16S, U16_BITS,
    },
};
...
impl<AB: InteractionBuilder> VmAdapterAir<AB> for DeferralCallAdapterAir {
    ...
    fn eval(
        ...
        self.range_bus
            .range_check(
                ptr_bound_from_high_u16_expr(cols.rd_val[1], self.address_bits),
                U16_BITS,
            )
            .eval(builder, ctx.instruction.is_valid.clone());
        self.range_bus
            .range_check(
                ptr_bound_from_high_u16_expr(cols.rs_val[1], self.address_bits),
                U16_BITS,
            )
            .eval(builder, ctx.instruction.is_valid.clone());
        ...
        let input_ptr = u16_limbs_to_ptr(&cols.rs_val);
        let output_ptr = u16_limbs_to_ptr(&cols.rd_val);
