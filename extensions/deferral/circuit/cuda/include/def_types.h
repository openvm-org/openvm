...
// Guest bytes absorbed per deferral output Poseidon2 row (two bytes packed
// per sponge rate cell) and the matching memory-op count for the row's
// payload write back to byte-addressed AS.
inline constexpr size_t SPONGE_BYTES_PER_ROW = 2 * DIGEST_SIZE;
...
// BabyBear modulus. Used by the canonicity sub-AIR's lexicographic comparison
// against `F::ORDER_U32` in big-endian limb order.
