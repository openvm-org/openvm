...
macro_rules! dispatch {
    ($execute_impl:ident, $local_opcode:ident, $enabled:ident, $is_native_store:ident) => {
        Ok(match ($local_opcode, $enabled, $is_native_store) {
            (LOADD, true, _) => $execute_impl::<_, _, U8, LoadDOp, true>,
            (LOADD, false, _) => $execute_impl::<_, _, U8, LoadDOp, false>,
            (LOADWU, true, _) => $execute_impl::<_, _, U8, LoadWUOp, true>,
            (LOADWU, false, _) => $execute_impl::<_, _, U8, LoadWUOp, false>,
            (LOADHU, true, _) => $execute_impl::<_, _, U8, LoadHUOp, true>,
            (LOADHU, false, _) => $execute_impl::<_, _, U8, LoadHUOp, false>,
            (LOADBU, true, _) => $execute_impl::<_, _, U8, LoadBUOp, true>,
            (LOADBU, false, _) => $execute_impl::<_, _, U8, LoadBUOp, false>,
            (STORED, true, false) => $execute_impl::<_, _, U8, StoreDOp, true>,
            (STORED, false, false) => $execute_impl::<_, _, U8, StoreDOp, false>,
            (STORED, true, true) => $execute_impl::<_, _, F, StoreDOp, true>,
            (STORED, false, true) => $execute_impl::<_, _, F, StoreDOp, false>,
            (STOREW, true, false) => $execute_impl::<_, _, U8, StoreWOp, true>,
            (STOREW, false, false) => $execute_impl::<_, _, U8, StoreWOp, false>,
            (STOREW, true, true) => $execute_impl::<_, _, F, StoreWOp, true>,
            (STOREW, false, true) => $execute_impl::<_, _, F, StoreWOp, false>,
            (STOREH, true, false) => $execute_impl::<_, _, U8, StoreHOp, true>,
            (STOREH, false, false) => $execute_impl::<_, _, U8, StoreHOp, false>,
            (STOREH, true, true) => $execute_impl::<_, _, F, StoreHOp, true>,
            (STOREH, false, true) => $execute_impl::<_, _, F, StoreHOp, false>,
            (STOREB, true, false) => $execute_impl::<_, _, U8, StoreBOp, true>,
            (STOREB, false, false) => $execute_impl::<_, _, U8, StoreBOp, false>,
            (STOREB, true, true) => $execute_impl::<_, _, F, StoreBOp, true>,
            (STOREB, false, true) => $execute_impl::<_, _, F, StoreBOp, false>,
            (_, _, _) => unreachable!(),
        })
    };
}
...
>(
    ...
    let mut write_data: [T; RV64_NUM_LIMBS] = if OP::HOST_READ {
        exec_state.host_read(pre_compute.e as u32, aligned_ptr as u32)
    } else {
    ...
    if ENABLED {
        if OP::IS_LOAD {
            ...
            exec_state.vm_write(pre_compute.e as u32, aligned_ptr as u32, &write_data);
        }
    }
