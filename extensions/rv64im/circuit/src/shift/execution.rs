...
#[repr(C)]
pub(super) struct Rv64ShiftPreCompute {
    c: u64,
    a: u8,
    b: u8,
}
...
pub struct Rv64ShiftExecutor {
    pub offset: usize,
}

impl Rv64ShiftExecutor {
    #[inline(always)]
...
macro_rules! dispatch {
    ($execute_impl:ident, $is_imm:ident, $shift_opcode:ident) => {
        match ($is_imm, $shift_opcode) {
            (true, Rv64ShiftOpcode::SLL) => Ok($execute_impl::<_, _, true, SllOp64>),
            (false, Rv64ShiftOpcode::SLL) => Ok($execute_impl::<_, _, false, SllOp64>),
            (true, Rv64ShiftOpcode::SRL) => Ok($execute_impl::<_, _, true, SrlOp64>),
            (false, Rv64ShiftOpcode::SRL) => Ok($execute_impl::<_, _, false, SrlOp64>),
            (true, Rv64ShiftOpcode::SRA) => Ok($execute_impl::<_, _, true, SraOp64>),
            (false, Rv64ShiftOpcode::SRA) => Ok($execute_impl::<_, _, false, SraOp64>),
        }
    };
}

impl<F: PrimeField32> InterpreterExecutor<F> for Rv64ShiftExecutor {
    #[inline(always)]
    fn pre_compute_size(&self) -> usize {
        size_of::<Rv64ShiftPreCompute>()
    }
...
impl<F: PrimeField32> InterpreterMeteredExecutor<F> for Rv64ShiftExecutor {
    #[inline(always)]
    fn metered_pre_compute_size(&self) -> usize {
        size_of::<E2PreCompute<Rv64ShiftPreCompute>>()
    }
...
impl<F: PrimeField32, RA> PreflightExecutor<F, RA> for Rv64ShiftExecutor {
...
#[cfg(feature = "aot")]
impl<F: PrimeField32> AotExecutor<F> for Rv64ShiftExecutor {
...
#[cfg(feature = "aot")]
impl<F: PrimeField32> AotMeteredExecutor<F> for Rv64ShiftExecutor {
...
>(
    ...
    let pre_compute: &E2PreCompute<Rv64ShiftPreCompute> =
        std::slice::from_raw_parts(pre_compute, size_of::<E2PreCompute<Rv64ShiftPreCompute>>())
            .borrow();
    exec_state
        .ctx
