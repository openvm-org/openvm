...
macro_rules! hint_store_u64 {
    ($x:expr) => {
        openvm_custom_insn::custom_insn_i!(
            opcode = openvm_rv64im_guest::SYSTEM_OPCODE,
            funct3 = openvm_rv64im_guest::HINT_FUNCT3,
            rd = In $x,
            rs1 = Const "x0",
            imm = Const 0,
        )
...
/// Reset the hint stream with `len` random dwords
#[inline(always)]
...
#[inline(always)]
pub fn hint_load_by_key(ptr: *const u8, len: u64) {
    openvm_custom_insn::custom_insn_i!(
        opcode = SYSTEM_OPCODE,
        funct3 = PHANTOM_FUNCT3,
        rd = In ptr,
...
macro_rules! reveal {
    ($rd:ident, $rs1:ident, $imm:expr) => {
        openvm_custom_insn::custom_insn_i!(
            opcode = openvm_rv64im_guest::SYSTEM_OPCODE,
            funct3 = openvm_rv64im_guest::REVEAL_FUNCT3,
            rd = In $rd,
            rs1 = In $rs1,
            imm = Const $imm
        )
...
macro_rules! store_to_native {
    ($rd:ident, $rs1:ident) => {
        openvm_custom_insn::custom_insn_r!(
            opcode = openvm_rv64im_guest::SYSTEM_OPCODE,
            funct3 = openvm_rv64im_guest::NATIVE_STORED_FUNCT3,
            funct7 = openvm_rv64im_guest::NATIVE_STORED_FUNCT7,
            rd = In $rd,
            rs1 = In $rs1,
            rs2 = In $rs1,
        )
