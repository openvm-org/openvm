...
use openvm_stark_backend::{
    p3_field::{
        extension::BinomiallyExtendable, field_to_array, Field, FieldAlgebra,
        FieldExtensionAlgebra, Packable,
    },
    p3_util::convert_vec,
};
...
pub type PackedBabyBear = <openvm_stark_sdk::p3_baby_bear::BabyBear as Field>::Packing;
pub type PackedBabyBearExt4 = BinomialExtensionField<PackedBabyBear, 4>;
...
impl FieldAlgebra for PackedBabyBearExt4 {
    type F = BabyBearExt4;

    const ZERO: Self = Self {
        value: [PackedBabyBear::ZERO; 4],
    };

    const ONE: Self = Self {
        value: field_to_array::<PackedBabyBear, 4>(PackedBabyBear::ONE),
    };

    const TWO: Self = Self {
        value: field_to_array::<PackedBabyBear, 4>(PackedBabyBear::TWO),
    };

    const NEG_ONE: Self = Self {
        value: field_to_array::<PackedBabyBear, 4>(PackedBabyBear::NEG_ONE),
    };

    #[inline]
    fn from_f(f: Self::F) -> Self {
        Self {
            value: f.value.map(PackedBabyBear::from_f),
        }
    }

    #[inline]
    fn from_bool(b: bool) -> Self {
        PackedBabyBear::from_bool(b).into()
    }

    #[inline]
    fn from_canonical_u8(n: u8) -> Self {
        PackedBabyBear::from_canonical_u8(n).into()
    }

    #[inline]
    fn from_canonical_u16(n: u16) -> Self {
        PackedBabyBear::from_canonical_u16(n).into()
    }

    #[inline]
    fn from_canonical_u32(n: u32) -> Self {
        PackedBabyBear::from_canonical_u32(n).into()
    }

    #[inline]
    fn from_canonical_u64(n: u64) -> Self {
        PackedBabyBear::from_canonical_u64(n).into()
    }

    #[inline]
    fn from_canonical_usize(n: usize) -> Self {
        PackedBabyBear::from_canonical_usize(n).into()
    }

    #[inline]
    fn from_wrapped_u32(n: u32) -> Self {
        PackedBabyBear::from_wrapped_u32(n).into()
    }

    #[inline]
    fn from_wrapped_u64(n: u64) -> Self {
        PackedBabyBear::from_wrapped_u64(n).into()
    }
    ...
    fn zero_vec(len: usize) -> Vec<Self> {
        ...
        unsafe { convert_vec(PackedBabyBear::zero_vec(len * 4)) }
    }
}
...
impl Sum for PackedBabyBearExt4 {
...
impl Product for PackedBabyBearExt4 {
    fn product<I: Iterator<Item = Self>>(iter: I) -> Self {
        iter.fold(Self::ONE, |acc, x| acc * x)
    }
}

// impl Div for PackedBabyBearExt4 {
//     type Output = Self;
...
// impl DivAssign for PackedBabyBearExt4 {
//     #[inline]
...
impl<FA> FieldExtensionAlgebra<FA> for BinomialExtensionField<FA, 4>
where
    FA: FieldAlgebra,
    FA::F: BinomiallyExtendable<4>,
    BinomialExtensionField<FA, 4>: FieldAlgebra,
{
    const D: usize = 4;
