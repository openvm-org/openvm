...
pub type OuterHash =
    MultiField32PaddingFreeSponge<OuterVal, Bn254, OuterPerm, WIDTH, RATE, DIGEST_WIDTH>;
pub type OuterDigest = [Bn254; 1];
pub type OuterCompress = TruncatedPermutation<OuterPerm, 2, 1, WIDTH>;
pub type OuterValMmcs = MerkleTreeMmcs<BabyBear, Bn254, OuterHash, OuterCompress, 1>;
...
pub type OuterChallenger = MultiField32Challenger<OuterVal, Bn254, OuterPerm, WIDTH, 2>;
...
pub(crate) fn new_from_outer_vkv2(
    ...
    StarkVerificationAdvice {
        preprocessed_data: preprocessed_data.map(|data| {
            let commit: [Bn254; DIGEST_WIDTH] = data.commit.into();
            VerifierSinglePreprocessedDataInProgram {
...
pub fn new_from_outer_multi_vk(
    ...
    let pre_hash: [Bn254; DIGEST_WIDTH] = vk.pre_hash.into();
