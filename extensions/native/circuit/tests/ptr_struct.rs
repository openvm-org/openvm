...
fn test_compiler_array() {
    ...
    let dyn_len: Var<_> = builder.eval(F::from_usize(len));
    ...
    // Put values statically
    let var_vals = (0..len).map(|_| rng.random::<F>()).collect::<Vec<_>>();
    let felt_vals = (0..len).map(|_| rng.random::<F>()).collect::<Vec<_>>();
    let ext_vals = (0..len).map(|_| rng.random::<EF>()).collect::<Vec<_>>();
    for i in 0..len {
    ...
    builder.range(0, dyn_len).for_each(|i_vec, builder| {
        let i = i_vec[0];
        builder.set(&var_array, i, i * RVar::from_field(F::from_u32(2)));
        builder.set(&felt_array, i, F::from_u32(3));
        builder.set(&ext_array, i, EF::from_u32(4).cons());
    });
    ...
    builder.range(0, dyn_len).for_each(|i_vec, builder| {
        let i = i_vec[0];
        let var_value = builder.get(&var_array, i);
        builder.assert_var_eq(var_value, i * RVar::from_field(F::from_u32(2)));
        let felt_value = builder.get(&felt_array, i);
        builder.assert_felt_eq(felt_value, F::from_u32(3));
        let ext_value = builder.get(&ext_array, i);
        builder.assert_ext_eq(ext_value, EF::from_u32(4).cons());
    });
