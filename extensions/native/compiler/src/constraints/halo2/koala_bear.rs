...
pub(crate) const KOALABEAR_MAX_BITS: usize = 31;
...
impl AssignedKoalaBear {
    pub fn to_koala_bear(&self) -> KoalaBear {
        let mut b_int = fe_to_bigint(self.value.value()) % KoalaBear::ORDER_U32;
        if b_int < BigInt::from(0) {
            b_int += KoalaBear::ORDER_U32;
        }
        KoalaBear::from_canonical_u32(b_int.try_into().unwrap())
    }
}

pub struct KoalaBearChip {
    pub range: Arc<RangeChip<Fr>>,
}

impl KoalaBearChip {
    pub fn new(range_chip: Arc<RangeChip<Fr>>) -> Self {
        KoalaBearChip { range: range_chip }
    }
    ...
    pub fn load_witness(&self, ctx: &mut Context<Fr>, value: KoalaBear) -> AssignedKoalaBear {
        let value = ctx.load_witness(Fr::from(PrimeField64::as_canonical_u64(&value)));
        self.range.range_check(ctx, value, KOALABEAR_MAX_BITS);
        AssignedKoalaBear {
            value,
            max_bits: KOALABEAR_MAX_BITS,
        }
    }

    pub fn load_constant(&self, ctx: &mut Context<Fr>, value: KoalaBear) -> AssignedKoalaBear {
        let max_bits = bit_length(value.as_canonical_u64());
        let value = ctx.load_constant(Fr::from(PrimeField64::as_canonical_u64(&value)));
        AssignedKoalaBear { value, max_bits }
    }

    pub fn reduce(&self, ctx: &mut Context<Fr>, a: AssignedKoalaBear) -> AssignedKoalaBear {
        debug_assert!(fe_to_bigint(a.value.value()).bits() as usize <= a.max_bits);
        let (_, r) = signed_div_mod(&self.range, ctx, a.value, a.max_bits);
        let r = AssignedKoalaBear {
            value: r,
            max_bits: KOALABEAR_MAX_BITS,
        };
        debug_assert_eq!(a.to_koala_bear(), r.to_koala_bear());
        r
    }

    /// Reduce max_bits if possible. This function doesn't guarantee that the actual value is within KoalaBear.
    pub fn reduce_max_bits(
        &self,
        ctx: &mut Context<Fr>,
        a: AssignedKoalaBear,
    ) -> AssignedKoalaBear {
        if a.max_bits > KOALABEAR_MAX_BITS {
            self.reduce(ctx, a)
        } else {
            a
        }
    ...
    pub fn add(
        &self,
        ctx: &mut Context<Fr>,
        mut a: AssignedKoalaBear,
        mut b: AssignedKoalaBear,
    ) -> AssignedKoalaBear {
        ...
        let mut c = AssignedKoalaBear { value, max_bits };
        debug_assert_eq!(c.to_koala_bear(), a.to_koala_bear() + b.to_koala_bear());
    ...
    pub fn neg(&self, ctx: &mut Context<Fr>, a: AssignedKoalaBear) -> AssignedKoalaBear {
        let value = self.gate().neg(ctx, a.value);
        let b = AssignedKoalaBear {
            value,
            max_bits: a.max_bits,
        };
        debug_assert_eq!(b.to_koala_bear(), -a.to_koala_bear());
        b
    }
    ...
    pub fn mul_add(
        ...
        let mut d = AssignedKoalaBear { value, max_bits };
        if d.max_bits > Fr::CAPACITY as usize - RESERVED_HIGH_BITS {
            d = self.reduce(ctx, d);
        }
        debug_assert_eq!(
            d.to_koala_bear(),
            a.to_koala_bear() * b.to_koala_bear() + c.to_koala_bear()
        );
        d
    }
    ...
    pub fn div(
        ...
        let b_val = b.to_koala_bear();
        let b_inv = b_val.try_inverse().unwrap();

        let mut c = self.load_witness(ctx, a.to_koala_bear() * b_inv);
    ...
    fn special_inner_product(
        &self,
        ctx: &mut Context<Fr>,
        a: &mut [AssignedKoalaBear],
        b: &mut [AssignedKoalaBear],
        s: usize,
    ) -> AssignedKoalaBear {
        assert!(a.len() == b.len());
        assert!(a.len() == 4);
    ...
    pub fn assert_equal(&self, ctx: &mut Context<Fr>, a: AssignedKoalaBear, b: AssignedKoalaBear) {
        debug_assert_eq!(a.to_koala_bear(), b.to_koala_bear());
        let diff = self.sub(ctx, a, b);
        self.assert_zero(ctx, diff);
    }
}

/// Constrains and returns `(c, r)` such that `a = KoalaBear::ORDER_U32 * c + r`.
///
    ...
    F: BigPrimeField,
{
    ...
    // Let `b` be the order of `KoalaBear` and `p` be the order of `Fr`.
    ...
    let a = a.into();
    let b = BigUint::from(KoalaBear::ORDER_U32);
...
pub struct KoalaBearExt4Chip {
    pub base: Arc<KoalaBearChip>,
}

#[derive(Copy, Clone, Debug)]
pub struct AssignedKoalaBearExt4(pub [AssignedKoalaBear; 4]);
pub type KoalaBearExt4 = BinomialExtensionField<KoalaBear, 4>;

impl AssignedKoalaBearExt4 {
    pub fn to_extension_field(&self) -> KoalaBearExt4 {
        let b_val = (0..4).map(|i| self.0[i].to_koala_bear()).collect_vec();
        KoalaBearExt4::from_base_slice(&b_val)
    }
}

impl KoalaBearExt4Chip {
    pub fn new(base_chip: Arc<KoalaBearChip>) -> Self {
        KoalaBearExt4Chip { base: base_chip }
    }
    pub fn load_witness(
        &self,
        ctx: &mut Context<Fr>,
        value: KoalaBearExt4,
    ) -> AssignedKoalaBearExt4 {
        AssignedKoalaBearExt4(
            value
                .as_base_slice()
                .iter()
    ...
    pub fn add(
        &self,
        ctx: &mut Context<Fr>,
        a: AssignedKoalaBearExt4,
        b: AssignedKoalaBearExt4,
    ) -> AssignedKoalaBearExt4 {
        AssignedKoalaBearExt4(
            a.0.iter()
                .zip(b.0.iter())
    ...
    pub fn neg(&self, ctx: &mut Context<Fr>, a: AssignedKoalaBearExt4) -> AssignedKoalaBearExt4 {
        AssignedKoalaBearExt4(
            a.0.iter()
    ...
    pub fn assert_zero(&self, ctx: &mut Context<Fr>, a: AssignedKoalaBearExt4) {
        for x in a.0.iter() {
    ...
    pub fn mul(
        &self,
        ctx: &mut Context<Fr>,
        mut a: AssignedKoalaBearExt4,
        mut b: AssignedKoalaBearExt4,
    ) -> AssignedKoalaBearExt4 {
        ...
        let w = self
            .base
            .load_constant(ctx, <KoalaBear as BinomiallyExtendable<4>>::W);
        for i in 4..7 {
            coeffs[i - 4] = self.base.mul_add(ctx, coeffs[i], w, coeffs[i - 4]);
        }
        coeffs.truncate(4);
        let c = AssignedKoalaBearExt4(coeffs.try_into().unwrap());
        debug_assert_eq!(
