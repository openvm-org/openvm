...
use derive_new::new;
use openvm_circuit::{
    arch::{DenseRecordArena, BLOCK_FE_WIDTH, MEMORY_BLOCK_BYTES},
...
use openvm_riscv_adapters::{
    Rv64VecHeapAdapterCols, Rv64VecHeapAdapterRecord, Rv64VecHeapBranchAdapterCols,
...
use crate::{INT256_NUM_BLOCKS, INT256_NUM_U16_LIMBS, INT256_U16_LIMB_BITS, NUM_READS};
...
pub type BranchEqual256AdapterRecord =
    Rv64VecHeapBranchAdapterRecord<NUM_READS, INT256_NUM_BLOCKS, BLOCK_FE_WIDTH>;
...
impl Chip<DenseRecordArena, GpuBackend> for BranchEqual256ChipGpu {
    fn generate_proving_ctx(&self, arena: DenseRecordArena) -> AirProvingContext<GpuBackend> {
        ...
        let trace_width = BranchEqualCoreCols::<F, INT256_NUM_U16_LIMBS>::width()
            + Rv64VecHeapBranchAdapterCols::<F, NUM_READS, INT256_NUM_BLOCKS, BLOCK_FE_WIDTH>::width();
        let trace_height = next_power_of_two_or_zero(records.len() / RECORD_SIZE);
        let device_ctx = &self.range_checker.device_ctx;
...
impl Chip<DenseRecordArena, GpuBackend> for BranchLessThan256ChipGpu {
    fn generate_proving_ctx(&self, arena: DenseRecordArena) -> AirProvingContext<GpuBackend> {
        ...
        let trace_width = BranchLessThanCoreCols::<F, INT256_NUM_U16_LIMBS, INT256_U16_LIMB_BITS>::width()
            + Rv64VecHeapBranchAdapterCols::<F, NUM_READS, INT256_NUM_BLOCKS, BLOCK_FE_WIDTH>::width();
        let trace_height = next_power_of_two_or_zero(records.len() / RECORD_SIZE);
        let device_ctx = &self.range_checker.device_ctx;
