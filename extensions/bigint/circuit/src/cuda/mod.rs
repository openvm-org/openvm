...
use derive_new::new;
use openvm_circuit::{
    arch::{DenseRecordArena, MEMORY_BLOCK_BYTES},
...
pub type BranchEqual256AdapterRecord =
    Rv64VecHeapBranchAdapterRecord<NUM_READS, INT256_NUM_BLOCKS, MEMORY_BLOCK_BYTES>;
...
impl Chip<DenseRecordArena, GpuBackend> for BranchEqual256ChipGpu {
    fn generate_proving_ctx(&self, arena: DenseRecordArena) -> AirProvingContext<GpuBackend> {
        ...
        let trace_width = BranchEqualCoreCols::<F, INT256_NUM_LIMBS>::width()
            + Rv64VecHeapBranchAdapterCols::<F, NUM_READS, INT256_NUM_BLOCKS, MEMORY_BLOCK_BYTES>::width();
        let trace_height = next_power_of_two_or_zero(records.len() / RECORD_SIZE);
        let device_ctx = &self.range_checker.device_ctx;
