...
const RANGE_TUPLE_SIZES: [u32; 2] = [
    1 << RV64_CELL_BITS,
    (INT256_NUM_U8_LIMBS * (1 << RV64_CELL_BITS)) as u32,
];
...
fn blt_branch_fn(
    ...
    let x_sign = x[INT256_NUM_U8_LIMBS - 1] >> (RV64_CELL_BITS - 1) != 0 && is_signed;
    let y_sign = y[INT256_NUM_U8_LIMBS - 1] >> (RV64_CELL_BITS - 1) != 0 && is_signed;
...
fn set_and_execute_rand<RA: Arena, E: PreflightExecutor<F, RA>>(
    ...
    let b = generate_long_number::<INT256_NUM_U8_LIMBS, RV64_CELL_BITS>(rng);
    let c = generate_long_number::<INT256_NUM_U8_LIMBS, RV64_CELL_BITS>(rng);
    if branch {
