...
use crate::{
    ...
    AluAdapterExecutor, Rv64ShiftLeft256Executor, Rv64ShiftRight256Executor, INT256_NUM_U64_LIMBS,
    INT256_NUM_U8_LIMBS,
};

impl Rv64ShiftLeft256Executor {
    pub fn new(adapter: AluAdapterExecutor, offset: usize) -> Self {
        Self(openvm_riscv_circuit::ShiftLeftExecutor::new(
            adapter, offset,
        ))
    }
}

impl Rv64ShiftRight256Executor {
    pub fn new(adapter: AluAdapterExecutor, offset: usize) -> Self {
        Self(openvm_riscv_circuit::ShiftRightExecutor::new(
            adapter, offset,
        ))
    }
}
        ...
        impl $executor {
            fn pre_compute_impl<F: PrimeField32>(
                ...
                *data = ShiftPreCompute {
                ...
                if (local_opcode == ShiftOpcode::SLL) == $right_shift {
                    return Err(StaticProgramError::InvalidInstruction(pc));
                }
                Ok(local_opcode)
            }
...
impl_shift256_executor!(Rv64ShiftLeft256Executor, false);
impl_shift256_executor!(Rv64ShiftRight256Executor, true);
