...
use openvm_riscv_circuit::adapters::{
    read_rv64_register_as_u32, rv64_bytes_to_u32, rv64_ptr_max_bits_shift, tracing_read,
    tracing_write, u32_to_le_u16_cells, u32_to_le_u16_limbs, RV64_PTR_U16_LIMBS,
    RV64_U16_LIMB_BITS,
};
...
impl<F: PrimeField32> TraceFiller<F> for XorinVmFiller {
    fn fill_trace_row(&self, mem_helper: &MemoryAuxColsFactory<F>, mut row_slice: &mut [F]) {
        ...
        let ptr_msl_lshift = rv64_ptr_max_bits_shift(self.pointer_max_bits) as u32;
        for ptr in [record.buffer, record.input] {
            let high_u16 = u32_to_le_u16_cells(ptr)[RV64_PTR_U16_LIMBS - 1] as u32;
            self.range_checker_chip
                .add_count(high_u16 << ptr_msl_lshift, RV64_U16_LIMB_BITS);
        }
    }
}
