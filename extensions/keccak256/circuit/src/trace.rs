...
/// followed by a slice of `u8`'s of length `num_reads * KECCAK_WORD_SIZE` where `num_reads` is provided at runtime,
/// followed by a slice of `MemoryReadAuxRecord`'s of length `num_reads`.
        ...
        RA: RecordArena<'buf, Self::RecordLayout, Self::RecordMut<'buf>>,
    {
        ...
        for idx in 0..num_reads {
            if idx % KECCAK_ABSORB_READS == 0 && idx != 0 {
                // Need to increment the timestamp according at the start of each block due to the AIR constraints
                state
                    .memory
...
impl<F: PrimeField32, CTX> TraceFiller<F, CTX> for KeccakVmStep {
    fn fill_trace(
        ...
        // Then, compute some additional values for each block and parallelize over blocks within an instruction
        // Finally, compute some additional values for each row and parallelize over rows within a block
        chunks
            .par_iter_mut()
            .zip(sizes.par_iter())
            .for_each(|(slice, (num_blocks, len))| {
                if *num_blocks == 0 {
                    ...
                    // Note: a 'block' of dummy rows is generated by `generate_trace_rows` from the zero state
                    //       dummy rows are repeated every `NUM_ROUNDS` rows
                ...
                *input.last_mut().unwrap() += 0x80;
                ...
                slice
                    ...
                    .for_each(|(block_idx, block_slice)| {
                        ...
                        // Note: we can call `generate_trace_rows` for each block separately because its trace only depends on the current `state`
                        // `generate_trace_rows` will generate additional dummy rows to make the height into power of 2, but we can safely discard them
