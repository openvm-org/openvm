...
#pragma unroll
    for (size_t i = 0; i < KECCAK_WIDTH_U16S; i++) {
        preimage_u16s[i] = u16_from_bytes_le(&rec.preimage_buffer_bytes[2 * i]);
        postimage_u16s[i] = u16_from_bytes_le(&state.bytes[2 * i]);
    }
    ...
    uint32_t buffer_ptr_msl_lshift = RV64_PTR_BITS - pointer_max_bits;
    uint32_t buffer_ptr_high_u16 = rec.buffer_ptr >> RV64_U16_LIMB_BITS;
    range_checker.add_count(
        buffer_ptr_high_u16 << buffer_ptr_msl_lshift, RV64_U16_LIMB_BITS
    );
}
