...
pub enum WeierstrassExtensionExecutor {
    // 32 limbs prime
    EcAddNeRv32_32(EcAddNeExecutor<3, 32>),
    EcDoubleRv32_32(EcDoubleExecutor<3, 32>),
    // 48 limbs prime
    EcAddNeRv32_48(EcAddNeExecutor<9, 16>),
    ...
    fn extend_circuit(&self, inventory: &mut AirInventory<SC>) -> Result<(), AirInventoryError> {
        ...
        for (i, curve) in self.supported_curves.iter().enumerate() {
            ...
            if bytes <= 32 {
                ...
                let addne = get_ec_addne_air::<3, 32>(
                    exec_bridge,
                    memory_bridge,
                    config.clone(),
                    range_checker_bus,
                ...
                let addne = get_ec_addne_air::<9, 16>(
                    exec_bridge,
                    memory_bridge,
                    config.clone(),
                    range_checker_bus,
    ...
    fn extend_prover(
        ...
        for curve in extension.supported_curves.iter() {
            ...
            if bytes <= 32 {
                ...
                let addne = get_ec_addne_chip::<Val<SC>, 3, 32>(
                    config.clone(),
                    mem_helper.clone(),
                    range_checker.clone(),
                    bitwise_lu.clone(),
                ...
                let addne = get_ec_addne_chip::<Val<SC>, 9, 16>(
                    config.clone(),
                    mem_helper.clone(),
                    range_checker.clone(),
                    bitwise_lu.clone(),
