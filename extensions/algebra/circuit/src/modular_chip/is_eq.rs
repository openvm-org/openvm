...
impl<const BLOCKS_PER_READ: usize, const BLOCK_SIZE: usize, const TOTAL_READ_SIZE: usize>
    VmModularIsEqualStep<BLOCKS_PER_READ, BLOCK_SIZE, TOTAL_READ_SIZE>
{
    pub fn new(
        adapter: Rv32IsEqualModeAdapterStep<2, BLOCKS_PER_READ, BLOCK_SIZE, TOTAL_READ_SIZE>,
...
impl<F, const BLOCKS_PER_READ: usize, const BLOCK_SIZE: usize, const TOTAL_READ_SIZE: usize>
    StepExecutorE1<F> for VmModularIsEqualStep<BLOCKS_PER_READ, BLOCK_SIZE, TOTAL_READ_SIZE>
where
    F: PrimeField32,
{
    #[inline(always)]
        ...
        Ctx: E1E2ExecutionCtx,
    {
        ...
        *data = ModularIsEqualPreCompute {
        ...
        let fn_ptr = if is_setup {
            execute_e1_impl::<_, _, BLOCKS_PER_READ, BLOCK_SIZE, TOTAL_READ_SIZE, true>
        } else {
            execute_e1_impl::<_, _, BLOCKS_PER_READ, BLOCK_SIZE, TOTAL_READ_SIZE, false>
        };
...
>(
    ...
    // Read register values to get memory addresses
    let rs_vals = [
    ...
    // Read memory values
    let [b, c] = rs_vals.map(|address| {
        // TODO(ayush): add back
        // assert!(address as usize + TOTAL_READ_SIZE - 1 < (1 << self.pointer_max_bits));
        from_fn::<_, BLOCKS_PER_READ, _>(|j| {
            vm_state.vm_read::<u8, BLOCK_SIZE>(RV32_MEMORY_AS, address + (j * BLOCK_SIZE) as u32)
        })
        .concat()
        .try_into()
        .unwrap()
    ...
    vm_state.vm_write(RV32_MEMORY_AS, pre_compute.a, &write_data);
