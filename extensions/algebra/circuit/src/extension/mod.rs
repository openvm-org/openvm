use std::result::Result;

use num_bigint::BigUint;
use openvm_circuit::{
    arch::{
        AirInventory, ChipInventoryError, InitFileGenerator, SystemConfig, VmBuilder,
        VmChipComplex, VmField, VmProverExtension,
    },
    system::{SystemChipInventory, SystemCpuBuilder, SystemExecutor},
};
use openvm_circuit_derive::VmConfig;
use openvm_cpu_backend::{CpuBackend, CpuDevice};
use openvm_riscv_circuit::{
    Rv64I, Rv64IExecutor, Rv64ImCpuProverExt, Rv64Io, Rv64IoExecutor, Rv64M, Rv64MExecutor,
};
use openvm_stark_backend::{StarkEngine, StarkProtocolConfig, Val};
use serde::{Deserialize, Serialize};
#[cfg(all(feature = "rvr", any(feature = "cuda", test)))]
use {
    openvm_algebra_transpiler::ModularArithmeticOpcode,
    openvm_instructions::{program::Program, LocalOpcode},
    openvm_stark_backend::p3_field::PrimeField32,
    strum::EnumCount,
};

mod modular;
pub use modular::*;
mod fp2;
pub use fp2::*;

cfg_if::cfg_if! {
    if #[cfg(feature = "cuda")] {
        mod hybrid;
        pub use hybrid::*;
        pub use {
            AlgebraHybridProverExt as AlgebraProverExt,
            ModularHybridBuilder as ModularBuilder,
            ModularWithFp2HybridBuilder as ModularWithFp2Builder,
        };
    } else {
        pub use self::{
            AlgebraCpuProverExt as AlgebraProverExt,
            ModularCpuBuilder as ModularBuilder,
            ModularWithFp2CpuBuilder as ModularWithFp2Builder,
        };
    }
}

#[cfg(all(test, feature = "rvr"))]
mod rvr_tests;

/// Returns the first configured modular IS_EQ/SETUP_ISEQ program slot whose
/// destination is x0.
#[cfg(all(feature = "rvr", any(feature = "cuda", test)))]
pub(crate) fn modular_is_eq_x0_destination<F: PrimeField32>(
    program: &Program<F>,
    num_moduli: usize,
) -> Option<usize> {
    let opcode_base = ModularArithmeticOpcode::CLASS_OFFSET;
    let opcode_count = ModularArithmeticOpcode::COUNT;
    program
        .instructions_and_debug_infos
        .iter()
        .enumerate()
        .find_map(|(slot, entry)| {
            let (instruction, _) = entry.as_ref()?;
            let relative = instruction.opcode.as_usize().checked_sub(opcode_base)?;
            let local = relative % opcode_count;
            (relative / opcode_count < num_moduli
                && matches!(
                    ModularArithmeticOpcode::from_usize(local),
                    ModularArithmeticOpcode::IS_EQ | ModularArithmeticOpcode::SETUP_ISEQ
                )
                && instruction.a.as_canonical_u32() == 0)
                .then_some(slot)
        })
}

pub struct AlgebraCpuProverExt;

#[derive(Clone, Debug, VmConfig, Serialize, Deserialize)]
pub struct ModularConfig {
    #[config(executor = "SystemExecutor")]
    pub system: SystemConfig,
    #[extension]
    pub base: Rv64I,
    #[extension]
    pub mul: Rv64M,
    #[extension]
    pub io: Rv64Io,
    #[extension]
    pub modular: ModularExtension,
}

impl InitFileGenerator for ModularConfig {
    fn generate_init_file_contents(&self) -> Option<String> {
        Some(format!(
            "// This file is automatically generated by cargo openvm. Do not rename or edit.\n{}\n",
            self.modular.generate_moduli_init()
        ))
    }
}

impl ModularConfig {
    pub fn new(moduli: Vec<BigUint>) -> Self {
        Self {
            system: SystemConfig::default(),
            base: Default::default(),
            mul: Default::default(),
            io: Default::default(),
            modular: ModularExtension::new(moduli),
        }
    }
}

#[derive(Clone, Debug, VmConfig, Serialize, Deserialize)]
pub struct ModularWithFp2Config {
    #[config]
    pub modular: ModularConfig,
    #[extension]
    pub fp2: Fp2Extension,
}

impl ModularWithFp2Config {
    pub fn new(moduli_with_names: Vec<(String, BigUint)>) -> Self {
        let moduli = moduli_with_names
            .iter()
            .map(|(_, modulus)| modulus.clone())
            .collect();
        Self {
            modular: ModularConfig::new(moduli),
            fp2: Fp2Extension::new(moduli_with_names),
        }
    }
}

impl InitFileGenerator for ModularWithFp2Config {
    fn generate_init_file_contents(&self) -> Option<String> {
        Some(format!(
            "// This file is automatically generated by cargo openvm. Do not rename or edit.\n{}\n{}\n",
            self.modular.modular.generate_moduli_init(),
            self.fp2.generate_complex_init(&self.modular.modular)
        ))
    }
}

#[derive(Clone)]
pub struct ModularCpuBuilder;

impl<SC, E> VmBuilder<E> for ModularCpuBuilder
where
    SC: StarkProtocolConfig,
    E: StarkEngine<SC = SC, PB = CpuBackend<SC>, PD = CpuDevice<SC>>,
    Val<SC>: VmField,
    SC::EF: Ord,
{
    type VmConfig = ModularConfig;
    type SystemChipInventory = SystemChipInventory<SC>;

    fn create_chip_complex(
        &self,
        config: &ModularConfig,
        circuit: AirInventory<SC>,
        device_ctx: &openvm_stark_backend::EngineDeviceCtx<E>,
    ) -> Result<VmChipComplex<SC, E::PB, Self::SystemChipInventory>, ChipInventoryError> {
        let mut chip_complex = VmBuilder::<E>::create_chip_complex(
            &SystemCpuBuilder,
            &config.system,
            circuit,
            device_ctx,
        )?;
        let inventory = &mut chip_complex.inventory;
        VmProverExtension::<E, _>::extend_prover(&Rv64ImCpuProverExt, &config.base, inventory)?;
        VmProverExtension::<E, _>::extend_prover(&Rv64ImCpuProverExt, &config.mul, inventory)?;
        VmProverExtension::<E, _>::extend_prover(&Rv64ImCpuProverExt, &config.io, inventory)?;
        VmProverExtension::<E, _>::extend_prover(&AlgebraCpuProverExt, &config.modular, inventory)?;
        Ok(chip_complex)
    }
}

#[derive(Clone)]
pub struct ModularWithFp2CpuBuilder;

impl<SC, E> VmBuilder<E> for ModularWithFp2CpuBuilder
where
    SC: StarkProtocolConfig,
    E: StarkEngine<SC = SC, PB = CpuBackend<SC>, PD = CpuDevice<SC>>,
    Val<SC>: VmField,
    SC::EF: Ord,
{
    type VmConfig = ModularWithFp2Config;
    type SystemChipInventory = SystemChipInventory<SC>;

    fn create_chip_complex(
        &self,
        config: &ModularWithFp2Config,
        circuit: AirInventory<SC>,
        device_ctx: &openvm_stark_backend::EngineDeviceCtx<E>,
    ) -> Result<VmChipComplex<SC, E::PB, Self::SystemChipInventory>, ChipInventoryError> {
        let mut chip_complex = VmBuilder::<E>::create_chip_complex(
            &ModularCpuBuilder,
            &config.modular,
            circuit,
            device_ctx,
        )?;
        let inventory = &mut chip_complex.inventory;
        VmProverExtension::<E, _>::extend_prover(&AlgebraCpuProverExt, &config.fp2, inventory)?;
        Ok(chip_complex)
    }
}
