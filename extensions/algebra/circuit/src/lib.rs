...
pub(crate) struct FieldExpressionStep<const BLOCKS: usize, const BLOCK_SIZE: usize>(
    openvm_mod_circuit_builder::FieldExpressionStep<
        Rv32VecHeapAdapterStep<2, BLOCKS, BLOCKS, BLOCK_SIZE, BLOCK_SIZE>,
    >,
);

impl<const BLOCKS: usize, const BLOCK_SIZE: usize> FieldExpressionStep<BLOCKS, BLOCK_SIZE> {
    #[allow(clippy::too_many_arguments)]
    pub fn new(
        adapter: Rv32VecHeapAdapterStep<2, BLOCKS, BLOCKS, BLOCK_SIZE, BLOCK_SIZE>,
        expr: FieldExpr,
        offset: usize,
...
struct FieldExpressionPreCompute {
    a: u32,
    b: u32,
    c: u32,
    expr: FieldExpr,
...
impl<F: PrimeField32, const BLOCKS: usize, const BLOCK_SIZE: usize> StepExecutorE1<F>
    for FieldExpressionStep<BLOCKS, BLOCK_SIZE>
{
    ...
    {
        let data: &mut FieldExpressionPreCompute = data.borrow_mut();
        let Instruction {
            opcode,
            a,
            b,
        ...
        *data = FieldExpressionPreCompute {
        ...
        let fn_ptr = if needs_setup {
            execute_e1_impl::<_, _, BLOCKS, BLOCK_SIZE, true>
        } else {
            execute_e1_impl::<_, _, BLOCKS, BLOCK_SIZE, false>
        };
