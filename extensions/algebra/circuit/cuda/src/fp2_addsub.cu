...
template <size_t NUM_LIMBS> struct Fp2AddSubCoreRecord {
    uint8_t opcode;
    uint8_t input_limbs[4 * NUM_LIMBS];
};

template <size_t BLOCKS, size_t BLOCK_SIZE, size_t NUM_LIMBS> struct Fp2AddSubRecord {
    Rv32VecHeapAdapterRecord<2, BLOCKS, BLOCKS, BLOCK_SIZE, BLOCK_SIZE> adapter;
    Fp2AddSubCoreRecord<NUM_LIMBS> core;
};
...
__global__ void fp2_addsub_adapter_tracegen_kernel(
    Fp *trace,
    size_t height,
...
template <size_t BLOCKS, size_t BLOCK_SIZE, size_t NUM_LIMBS>
__global__ void fp2_addsub_compute_tracegen_kernel(
    Fp *trace,
    size_t height,
    size_t core_width,
    const uint8_t *records,
    ...
    if (is_add) {
        ...
        z0 = x0.mod_sub(y0, prime);
        z1 = x1.mod_sub(y1, prime);
    }
...
__global__ void fp2_addsub_constraint_tracegen_kernel(
    ...
    for (uint32_t coord = 0; coord < 2; coord++) {
        ...
        result -= tmp_ov;
        OverflowInt carries = result.carry_limbs(c_counts[coord]);
