...
__global__ void modular_addsub_constraint_tracegen_kernel(
    ...
    for (uint32_t limb = 0; limb < NUM_LIMBS; limb++) {
        int64_t limb_value = (limb < x1.num_limbs ? x1.limbs[limb] : 0) -
                             (limb < x6.num_limbs ? x6.limbs[limb] : 0);
        if (is_add) {
            limb_value += limb < x2.num_limbs ? x2.limbs[limb] : 0;
        } else if (is_sub) {
            limb_value -= limb < x2.num_limbs ? x2.limbs[limb] : 0;
        }
        result_limbs[limb] = limb_value;
    }
    for (uint32_t q_limb = 0; q_limb < q_limbs; q_limb++) {
        int64_t q_signed = q_limb < quotient.mag.num_limbs ? (int64_t)quotient.mag.limbs[q_limb] : 0;
        if (quotient.is_negative) {
            q_signed = -q_signed;
        }
