[init] modulus #0 = 115792089237316195423570985008687907853269984665640564039457584007908834671663
...
extern "C" {
    ...
    fn hint_sqrt_extern_func_fffffffffffffffffffffffffffffffffffffffffffffffffffffffefffffc2f(
        rs1: usize,
    );
    fn hint_non_qr_extern_func_fffffffffffffffffffffffffffffffffffffffffffffffffffffffefffffc2f();
    fn moduli_setup_extern_func_fffffffffffffffffffffffffffffffffffffffffffffffffffffffefffffc2f();
}
impl Secp256k1Coord {
    #[inline(always)]
    ...
    fn eq_impl(&self, other: &Self) -> bool {
        #[cfg(not(target_os = "zkvm"))] { self.as_le_bytes() == other.as_le_bytes() }
    }
    fn assert_is_setup() {
        ...
        is_setup
            .get_or_init(|| {
                unsafe {
                    moduli_setup_extern_func_fffffffffffffffffffffffffffffffffffffffffffffffffffffffefffffc2f();
                }
                true
            });
    }
...
impl openvm_algebra_guest::Reduce for Secp256k1Coord {
    fn reduce_le_bytes(bytes: &[u8]) -> Self {
        ...
        let mut base = <Self as openvm_algebra_guest::IntMod>::from_le_bytes(
            &[255u8; 32usize],
        );
        base += <Self as openvm_algebra_guest::IntMod>::ONE;
        for chunk in bytes.chunks(32usize).rev() {
            res = res * &base
                + <Self as openvm_algebra_guest::IntMod>::from_le_bytes(chunk);
        }
        res
    }
}
