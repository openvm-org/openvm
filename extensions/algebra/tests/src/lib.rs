...
mod tests {
    ...
    use openvm_algebra_circuit::{
        Fp2Extension, ModularBuilder, ModularConfig, ModularWithFp2Builder, ModularWithFp2Config,
    };
    ...
    #[cfg(test)]
    fn test_modular_config(moduli: Vec<BigUint>) -> ModularConfig {
        let mut config = ModularConfig::new(moduli);
        config.system = test_system_config();
        config
    }

    #[cfg(test)]
    fn test_modularwithfp2_config(
        moduli_with_names: Vec<(String, BigUint)>,
    ) -> ModularWithFp2Config {
        let mut config = ModularWithFp2Config::new(moduli_with_names);
    ...
    fn test_moduli_setup() -> Result<()> {
        ...
        let config = test_modular_config(moduli.to_vec());
    ...
    fn test_modular() -> Result<()> {
        let config = test_modular_config(vec![SECP256K1_CONFIG.modulus.clone()]);
    ...
    fn test_complex_two_moduli() -> Result<()> {
        let config = test_modularwithfp2_config(vec![
            (
                "Complex1".to_string(),
        ...
        air_test(ModularWithFp2Builder, config, openvm_exe);
        Ok(())
    }

    #[test]
    fn test_complex_redundant_modulus() -> Result<()> {
        let config = ModularWithFp2Config {
            modular: test_modular_config(vec![
                BigUint::from_str("998244353").unwrap(),
                BigUint::from_str("1000000007").unwrap(),
    ...
    fn test_bls12_381_rvr_equivalence() -> Result<()> {
        let mut modular =
            ModularConfig::new(vec![BLS12_381_MODULUS.clone(), BLS12_381_ORDER.clone()]);
    ...
    fn test_invalid_setup() {
        let config = test_modular_config(vec![
            BigUint::from_str("998244353").unwrap(),
            BigUint::from_str("1000000007").unwrap(),
        ]);
        let elf = build_example_program_at_path(
