...
Here `add_extern_func_de0b6b3a7640003` is the name of the function that will be generated by the `moduli_init!` macro in the abbreviated examples. Note that `de0b6b3a7640003` is the hexadecimal representation of the modulus value `1000000000000000003`.

For readability, the examples below omit the leading zero padding in generated symbol names. In the actual macro output, the modulus suffix is a zero-padded 64-character hexadecimal string, so the real symbol corresponding to `add_extern_func_de0b6b3a7640003` is `add_extern_func_0000000000000000000000000000000000000000000000000de0b6b3a7640003`. Therefore, the names of the functions corresponding to equal moduli would be the same.
...
6. For convenience, running `cargo openvm build` will automatically generate an appropriate call to `moduli_init!` based on the order of the moduli in `openvm.toml`.
More specifically, `cargo openvm build` will read `openvm.toml`, then generate a file named `openvm_init.rs` in the project's manifest directory (where `Cargo.toml` is located) containing a call to `moduli_init!`.
