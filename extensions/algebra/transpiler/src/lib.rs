...
#[derive(Default)]
pub struct ModTranspilerExtension;
...
impl<F: PrimeField32> TranspilerExtension<F> for ModTranspilerExtension {
    fn process_custom(&self, instruction_stream: &[u32]) -> Option<(Instruction<F>, usize)> {
        ...
        if funct3 != MODULAR_ARITHMETIC_FUNCT3 && funct3 != COMPLEX_EXT_FIELD_FUNCT3 {
            return None;
        }
