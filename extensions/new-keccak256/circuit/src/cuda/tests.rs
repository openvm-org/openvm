...
mod tests {
    ...
    fn set_and_execute(
        ...
        tester.write(1, buffer_reg, buffer_ptr.to_le_bytes().map(F::from_canonical_u8));

        // Write input pointer to register
        tester.write(1, input_reg, input_ptr.to_le_bytes().map(F::from_canonical_u8));

        // Write len VALUE directly to register (not a pointer)
        tester.write(1, len_reg, (len as u32).to_le_bytes().map(F::from_canonical_u8));
