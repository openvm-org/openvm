...
The CLI reflects this compatibility boundary by caching version-dependent setup artifacts under `~/.openvm/v[OPENVM_VERSION]/`. Patch releases share this cache, while upgrading to a new minor version requires running `cargo openvm setup` (or `cargo openvm setup --evm`) again.
