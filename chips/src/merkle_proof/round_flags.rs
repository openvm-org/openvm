...
use super::{columns::MerkleProofCols, MERKLE_PROOF_DEPTH};
...
pub(crate) fn eval_round_flags<AB: AirBuilder>(builder: &mut AB) {
    ...
    for i in 1..MERKLE_PROOF_DEPTH {
        builder.when_first_row().assert_zero(local.step_flags[i]);
    }

    for i in 0..MERKLE_PROOF_DEPTH {
        let current_round_flag = local.step_flags[i];
        let next_round_flag = next.step_flags[(i + 1) % MERKLE_PROOF_DEPTH];
        builder
            .when_transition()
