...
impl GroupByTest {
    ...
    pub fn data_len(&self) -> usize {
        self.page_width - self.idx_len() - 1
    }
    ...
    fn load_page_test(
        ...
        //     &pis,
        // )
        page_controller.verify(engine, partial_vk, proof)
    }
}
...
fn static_test() {
    let page_vec = vec![
        ...
        vec![1, 0, 0, 2, 5, 0, 0, 0, 9],
        vec![1, 0, 0, 3, 9, 0, 0, 1, 5],
        vec![1, 0, 0, 4, 1, 0, 0, 9, 1],
        vec![1, 0, 1, 6, 6, 0, 0, 0, 2],
        vec![1, 1, 1, 6, 8, 0, 1, 0, 1],
...
fn group_by_test() {
    ...
    // let log_page_height = rng.gen_range(3..8);
    let test = GroupByTest::new(9, 2, 4, 10, 4);
    ...
    // ];
    // let page = Page::from_2d_vec(&page_vec, 4, 4);
    let page = test.generate_page(&mut rng, 15);

    for row in page.clone().rows {
        let flat_data = [vec![row.is_alloc], row.idx, row.data]
            .iter()
            .flat_map(|x| x.iter().cloned())
            .collect::<Vec<u32>>();
        println!("vec!{:?}", flat_data);
    }

    let page_width = 1 + page.idx_len() + page.data_len();
    // let page_height = test.page_height();
    let page_height = page.rows.len();
    println!("page_height: {}", page_height);
    let log_page_height = log2_strict_usize(page_height);

    let mut page_controller = PageController::new(page_width, vec![1, 2], 3, 0, 1, 2, 10, 4);
    // println!("page_width: {}, group_by_cols: {:?}, aggregated_col: {}, internal_bus_index: {}, output_bus_index: {}, range_bus_index: {}, idx_limb_bits: {}, idx_decomp: {}", test.page_width, test.group_by_cols, test.aggregated_col, test.internal_bus_index, test.output_bus_index, test.range_bus_index, test.idx_limb_bits, test.idx_decomp);
    ...
    page_controller.set_up_keygen_builder(&mut keygen_builder, page_height, 1 << 4);
