    ...
    PageLevelJoin<COMMIT_LEN, SC, E>
{
    ...
    pub fn generate_trace(&self, output_df: &mut DataFrame<COMMIT_LEN>, pairs_list_index: u32) {
        ...
        pis.output_page_commit = output_page_commit;
        ...
        PcsProof<SC>: Send + Sync,
    {
        ...
        self.verify(engine, proof);
    }

    pub fn verify(&self, engine: &E, proof: Proof<SC>)
    where
        Val<SC>: PrimeField,
        Domain<SC>: Send + Sync,
        SC::Pcs: Sync,
        ...
        PcsProof<SC>: Send + Sync,
    {
        ...
        // TODO: implement the following logic
        // TODO: basically, need to decommit pis.init_running_df_commit and pis.final_running_df_commit
        // and make sure they only differ by the commit of the output join page
