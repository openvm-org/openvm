...
__device__ BigIntGpu evaluate_bigint(
    ...
    switch (node.kind) {
        ...
        case EXPR_CONST: {
            ...
            result = BigIntGpu(const_limbs + offset, expr_meta->const_limb_counts[const_idx], limb_bits);
        ...
        case EXPR_ADD: {
            ...
            BigIntGpu left = evaluate_bigint(expr_ops, left_idx, expr_meta, 
                                               inputs, vars, flags, n, limb_bits);
            BigIntGpu right = evaluate_bigint(expr_ops, right_idx, expr_meta, 
                                                inputs, vars, flags, n, limb_bits);
        ...
        case EXPR_INT_ADD: {
            ...
            BigIntGpu child = evaluate_bigint(expr_ops, child_idx, expr_meta, 
                                                 inputs, vars, flags, n, limb_bits);
        ...
        case EXPR_SELECT: {
            ...
            if (flags[flag_idx]) {
                result = evaluate_bigint(expr_ops, true_idx, expr_meta, 
                                           inputs, vars, flags, n, limb_bits);
            } else {
                result = evaluate_bigint(expr_ops, false_idx, expr_meta, 
                                           inputs, vars, flags, n, limb_bits);
            }
            break;
        }
...
__device__ void compute(
    ...
    switch (e.kind) {
        ...
        case EXPR_SUB: {
            uint32_t* a = temp_buf;
            uint32_t* b = temp_buf + num_limbs;
            compute(a, expr_ops, e.data0, meta, inputs, vars, flags, num_limbs, limb_bits, temp_buf + 2 * num_limbs);
            compute(b, expr_ops, e.data1, meta, inputs, vars, flags, num_limbs, limb_bits, temp_buf + 2 * num_limbs);
        ...
        case EXPR_MUL: {
            ...
            compute(a, expr_ops, e.data0, meta, inputs, vars, flags, num_limbs, limb_bits, temp_buf + 4 * num_limbs);
            compute(b, expr_ops, e.data1, meta, inputs, vars, flags, num_limbs, limb_bits, temp_buf + 4 * num_limbs);
        ...
        case EXPR_DIV: {
            ...
            compute(a, expr_ops, e.data0, meta, inputs, vars, flags, num_limbs, limb_bits, temp_buf + 4 * num_limbs);
            compute(b, expr_ops, e.data1, meta, inputs, vars, flags, num_limbs, limb_bits, temp_buf + 4 * num_limbs);
            limb_mod_div(result, a, b, meta->prime_limbs, num_limbs, limb_bits, meta->barrett_mu, temp_buf + 2 * num_limbs);
            break;
        }
        case EXPR_INT_ADD: {
            uint32_t* c = temp_buf;
            compute(c, expr_ops, e.data0, meta, inputs, vars, flags, num_limbs, limb_bits, temp_buf + num_limbs);
        ...
        case EXPR_INT_MUL: {
            uint32_t* c = temp_buf;
            compute(c, expr_ops, e.data0, meta, inputs, vars, flags, num_limbs, limb_bits, temp_buf + 2 * num_limbs);
...
__device__ void evaluate_overflow(
    ...
    switch (d.kind) {
        ...
        case EXPR_ADD:
            evaluate_overflow(c1_res, expr_ops, d.data0, meta, inputs, vars, flags, num_limbs, limb_bits, temp_storage + 4 * num_limbs);
            evaluate_overflow(c2_res, expr_ops, d.data1, meta, inputs, vars, flags, num_limbs, limb_bits, temp_storage + 4 * num_limbs);
        ...
        case EXPR_MUL:
            ...
            break;
        case EXPR_SELECT: {
            bool f = flags[d.data0];
            uint32_t idx = f ? d.data1 : d.data2;
            evaluate_overflow(result, expr_ops, idx, meta, inputs, vars, flags, num_limbs, limb_bits, temp_storage);
            break;
        }
        case EXPR_INT_ADD: {
            evaluate_overflow(c1_res, expr_ops, d.data0, meta, inputs, vars, flags, num_limbs, limb_bits, temp_storage + 4 * num_limbs);
