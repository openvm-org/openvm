...
struct Rv32VecHeapConfig {
    ...
    __device__ Rv32VecHeapConfig(size_t nr, size_t bpr, size_t bpw, size_t rs, size_t ws)
        : num_reads(nr), blocks_per_read(bpr), blocks_per_write(bpw), 
          read_size(rs), write_size(ws) {}
};
...
template<size_t NUM_READS, size_t BLOCKS_PER_READ, size_t BLOCKS_PER_WRITE, 
...
__device__ inline void instantiate_rv32_vec_heap_adapter(
    ...
    AdapterStep adapter_step(pointer_max_bits, range_checker, bitwise_lookup);
...
__device__ inline void route_rv32_vec_heap_adapter(
    ...
    if (config.num_reads == 1 && config.blocks_per_read == 2 && 
        config.blocks_per_write == 2 && config.read_size == 32 && config.write_size == 32) {
        instantiate_rv32_vec_heap_adapter<1, 2, 2, 32, 32>(
            row, rec_bytes, pointer_max_bits, range_checker, bitwise_lookup, adapter_size);
    }
