...
__device__ inline void biguint_divrem(
    BigUintGpu *quotient,
    BigUintGpu *remainder,
    const BigUintGpu *dividend,
    ...
    *remainder = *dividend;
    ...
    if (biguint_compare(divisor, &zero) == 0)
        return;

    if (biguint_compare(dividend, divisor) < 0)
        return;
...
__device__ inline void bigint_div_biguint(
    BigIntGpu *quotient,
    const BigIntGpu *dividend,
    const BigUintGpu *divisor
) {
...
__device__ inline const BigUintGpu *bigint_mag(const BigIntGpu *num) { return &num->mag; }
...
__device__ inline void bigint_abs(BigUintGpu *result, const BigIntGpu *num) { *result = num->mag; }

__device__ inline void biguint_mod_reduce(
    BigUintGpu *result,
    const BigUintGpu *value,
    const BigUintGpu *modulus,
