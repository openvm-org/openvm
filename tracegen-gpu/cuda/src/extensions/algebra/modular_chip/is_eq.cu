...
struct ModularIsEqualCoreRecord {
    uint8_t is_setup;
    uint8_t b[READ_LIMBS];
    uint8_t c[READ_LIMBS];
};

template <typename T, size_t READ_LIMBS>
struct ModularIsEqualCoreCols {
    T is_valid;
    T is_setup;
    T b[READ_LIMBS];
    T c[READ_LIMBS];
...
struct ModularIsEqualCore {
    ...
    __device__ void fill_trace_row(RowSlice row, ModularIsEqualCoreRecord<READ_LIMBS> record) {
        ...
        IsEqualArray::generate_subrow(READ_LIMBS, b_slice, c_slice, eq_marker_slice, cmp_result_ptr);
        
        int b_diff = READ_LIMBS; // essentially means "no differing limb"
        int c_diff = READ_LIMBS;
        for (int i = READ_LIMBS - 1; i >= 0; i--) {
            if (record.b[i] != modulus[i]) { b_diff = i; break; }
        }
        for (int i = READ_LIMBS - 1; i >= 0; i--) {
            if (record.c[i] != modulus[i]) { c_diff = i; break; }
        }
        ...
        for (uint32_t i = 0; i < READ_LIMBS; i++) {
            Fp v = Fp::zero();
            if (b_diff < READ_LIMBS && (int)i == b_diff) v = Fp::one();
            else if (c_diff < READ_LIMBS && (int)i == c_diff) v = mark;
