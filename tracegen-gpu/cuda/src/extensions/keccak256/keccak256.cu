...
#pragma unroll 4
                for (auto limb = 0; limb < U64_LIMBS; limb++) {
                    ...
                    COL_WRITE_VALUE(round_row, KeccakPermCols, a[y][x][limb], 
                        ((x == 0) && (y == 0)) ? 
                        prev_round_row[COL_INDEX(KeccakPermCols, a_prime_prime_prime_0_0_limbs[limb])] :
    ...
    uint32_t bitwise_num_bits) 
{
    ...
    if (row_idx < rows_used) {
        MemoryAuxColsFactory mem_helper(VariableRangeChecker(range_checker_ptr, range_checker_num_bins));
        ...
        auto start_timestamp = vm_record.timestamp + (block_idx * (KECCAK_REGISTER_READS + KECCAK_ABSORB_READS));
...
#pragma unroll 8
                for (auto i = 0; i < KECCAK_RATE_U16S; i++) {
                    KECCAK_WRITE(sponge.state_hi[i], static_cast<uint8_t>(state[i / U64_LIMBS] >> ((i % U64_LIMBS) * 16 + 8)));
                }
            } else if (round_idx == NUM_ROUNDS - 1) {
                auto next_state = state + total_num_blocks * KECCAK_STATE_SIZE;
#pragma unroll 8
                for (auto i = 0; i < KECCAK_RATE_U16S; i++) {
                    KECCAK_WRITE(sponge.state_hi[i], static_cast<uint8_t>(next_state[i / U64_LIMBS] >> ((i % U64_LIMBS) * 16 + 8)));
                }
                if (block_idx == num_blocks - 1) {
...
#pragma unroll 8
        ...
        {   // Fill the absorb reads
            if (round_idx == 0) {
                ...
                auto num_reads = min(
                    d_div_ceil(size_t(rem_len), KECCAK_WORD_SIZE),
                    KECCAK_ABSORB_READS
                );
                ...
                for (auto i = num_reads; i < KECCAK_ABSORB_READS; i++) {
                    KECCAK_WRITE(mem_oc.absorb_reads[i], 0);
                }
            } else {
        ...
        { // Fill the digest writes
            if (block_idx == num_blocks - 1 && round_idx == NUM_ROUNDS - 1) {
                ...
                for (auto i = 0; i < KECCAK_DIGEST_WRITES; i++) {
                    KECCAK_WRITE_ARRAY(mem_oc.digest_writes[i].prev_data, vm_record.write_aux[i].prev_data);
                    mem_helper.fill(
        ...
        // inner rows are filled in p3_tracegen kernel
        row.fill_zero(COL_INDEX(KeccakVmCols, sponge), NUM_KECCAK_SPONGE_COLS + NUM_KECCAK_INSTRUCTION_COLS + NUM_KECCAK_MEMORY_COLS);
