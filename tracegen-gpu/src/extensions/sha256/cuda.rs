...
pub mod sha256 {
    ...
    unsafe extern "C" {
        unsafe fn launch_sha256_hash_computation(
            d_records: *const u8,
            num_records: usize,
        ...
        unsafe fn launch_sha256_first_pass_tracegen(
        ...
        unsafe fn launch_sha256_second_pass_dependencies(
            d_trace: *mut std::ffi::c_void,
            trace_height: usize,
            rows_used: u32,
        ) -> i32;

        unsafe fn launch_sha256_fill_invalid_rows(
            d_trace: *mut std::ffi::c_void,
            trace_height: usize,
            rows_used: u32,
        ) -> i32;
    ...
    pub unsafe fn sha256_first_pass_tracegen<T>(
    ...
    pub unsafe fn sha256_second_pass_dependencies<T>(
        d_trace: &DeviceBuffer<T>,
        height: usize,
        rows_used: u32,
    ) -> Result<(), CudaError> {
        let result =
            launch_sha256_second_pass_dependencies(d_trace.as_mut_raw_ptr(), height, rows_used);
        CudaError::from_result(result)
    }

    pub unsafe fn sha256_fill_invalid_rows<T>(
        d_trace: &DeviceBuffer<T>,
        height: usize,
        rows_used: u32,
    ) -> Result<(), CudaError> {
        let result = launch_sha256_fill_invalid_rows(d_trace.as_mut_raw_ptr(), height, rows_used);
        CudaError::from_result(result)
    }
}
