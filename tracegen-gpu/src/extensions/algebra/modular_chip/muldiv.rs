...
#[derive(new)]
pub struct ModularMulDivChipGpu<'a, const BLOCKS: usize, const BLOCK_SIZE: usize> {
...
impl<const BLOCKS: usize, const BLOCK_SIZE: usize> ModularMulDivChipGpu<'_, BLOCKS, BLOCK_SIZE> {
...
impl<const BLOCKS: usize, const BLOCK_SIZE: usize> ChipUsageGetter
    for ModularMulDivChipGpu<'_, BLOCKS, BLOCK_SIZE>
{
...
mod tests {
    ...
    fn create_dense_muldiv_chip(
        ...
        let tmp_chip =
            openvm_algebra_circuit::modular_chip::ModularMulDivChip::<F, 1, NUM_LIMBS>::new(
    ...
    ) -> NewVmChipWrapper<F, ModAir<1, NUM_LIMBS>, ModStep<1, NUM_LIMBS>, MatrixRecordArena<F>>
    {
        ...
        let chip = openvm_algebra_circuit::modular_chip::ModularMulDivChip::<F, 1, NUM_LIMBS>::new(
            tester.execution_bridge(),
            tester.memory_bridge(),
            tester.cpu_memory_helper(),
            tester.address_bits(),
    ...
    #[test]
    fn test_modular_muldiv_tracegen_mod_builder() {
        let modulus = secp256k1_coord_prime();
        let offset = Rv32ModularArithmeticOpcode::CLASS_OFFSET + 100; // Different offset for muldiv
        ...
        let mut dense_chip =
            create_dense_muldiv_chip(&tester, &modulus, offset, shared_bitwise.clone());
        // Build sparse CPU chip for expected trace
        let mut sparse_chip =
            create_sparse_muldiv_chip(&tester, &modulus, offset, shared_bitwise.clone());
        // GPU chip wrapper (arena set later)
        let mut gpu_chip = ModularMulDivChipGpu::new(
        ...
        for i in 0..50 {
            ...
            let (a, b, op_local) = if is_setup {
                ...
                a %= &modulus;
                b %= &modulus;
                // For DIV operation, make sure b is not zero
                if op == 1 && b == num_bigint::BigUint::from(0u32) {
                    b = num_bigint::BigUint::from(1u32);
                }
            ...
            let data_as = 2;
            let addr_ptr1 = 0;
            let addr_ptr2 = 3 * openvm_rv32im_circuit::adapters::RV32_REGISTER_NUM_LIMBS;
            let addr_ptr3 = 6 * openvm_rv32im_circuit::adapters::RV32_REGISTER_NUM_LIMBS;
