...
#[derive(new)]
pub struct Fp2AddSubChipGpu<'a, const BLOCKS: usize, const BLOCK_SIZE: usize> {
...
impl<const BLOCKS: usize, const BLOCK_SIZE: usize> Fp2AddSubChipGpu<'_, BLOCKS, BLOCK_SIZE> {
...
impl<const BLOCKS: usize, const BLOCK_SIZE: usize> ChipUsageGetter
    for Fp2AddSubChipGpu<'_, BLOCKS, BLOCK_SIZE>
{
...
mod tests {
    ...
    fn create_dense_fp2_addsub_chip(
        ...
        let tmp_chip = openvm_algebra_circuit::fp2_chip::Fp2AddSubChip::<F, 2, NUM_LIMBS>::new(
    ...
    fn create_sparse_fp2_addsub_chip(
        tester: &GpuChipTestBuilder,
        modulus: &num_bigint::BigUint,
        offset: usize,
        bitwise_chip: SharedBitwiseOperationLookupChip<RV32_CELL_BITS>,
    ) -> NewVmChipWrapper<F, Fp2Air<2, NUM_LIMBS>, Fp2Step<2, NUM_LIMBS>, MatrixRecordArena<F>>
    {
        ...
        let chip = openvm_algebra_circuit::fp2_chip::Fp2AddSubChip::<F, 2, NUM_LIMBS>::new(
            tester.execution_bridge(),
            tester.memory_bridge(),
            tester.cpu_memory_helper(),
            tester.address_bits(),
    ...
    #[test]
    fn test_fp2_addsub_tracegen_mod_builder() {
        ...
        let mut dense_chip =
            create_dense_fp2_addsub_chip(&tester, &modulus, offset, shared_bitwise.clone());
        // Build sparse CPU chip for expected trace
        let mut sparse_chip =
            create_sparse_fp2_addsub_chip(&tester, &modulus, offset, shared_bitwise.clone());
        // GPU chip wrapper (arena set later)
        let mut gpu_chip = Fp2AddSubChipGpu::new(
