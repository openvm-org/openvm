...
use openvm_rv32im_circuit::{
    adapters::{Rv32JalrAdapterRecord, RV32_CELL_BITS},
    Rv32JalrAir, Rv32JalrCoreRecord,
};
...
impl ChipUsageGetter for Rv32JalrChipGpu<'_> {
...
impl DeviceChip<SC, GpuBackend> for Rv32JalrChipGpu<'_> {
...
mod tests {
    ...
    use openvm_rv32im_circuit::{
        adapters::{
            Rv32JalrAdapterAir, Rv32JalrAdapterRecord, Rv32JalrAdapterStep, RV32_CELL_BITS,
        },
    ...
    type DenseChip<F> = NewVmChipWrapper<F, Rv32JalrAir, Rv32JalrStepWithAdapter, DenseRecordArena>;
    type SparseChip<F> =
    ...
    fn rand_jalr_tracegen_test() {
        ...
        let mut gpu_chip = Rv32JalrChipGpu::new(
        ...
        for _ in 0..100 {
            ...
            let a = rng.gen_range(0..32) << 2; // rs1 register
            let b = rng.gen_range(1..32) << 2; // rd register (avoid x0)
            ...
            tester.execute_with_pc(
                &mut dense_chip,
                &Instruction::from_usize(JALR.global_opcode(), [a, b, imm, 1, 0, 1, imm_sign]),
                initial_pc,
            );
        }
    ...
    fn create_dense_chip(
        ...
        let mut chip = DenseChip::<F>::new(
            VmAirWrapper::new(
                ...
                Rv32JalrCoreAir::new(bitwise.bus(), tester.range_checker().air.bus),
            ),
            Rv32JalrStep::new(
