    ...
    I::ProcessedInstruction: From<MinimalInstruction<AB::Expr>>,
{
    fn eval(
        ...
        for i in 0..NUM_LIMBS {
            ...
            carry[i] = AB::Expr::from(carry_divide) * (expected_limb - a_mul[i]);
        }

        for (a_mul, carry) in a_mul.iter().zip(carry.iter()) {
            self.range_tuple_bus
                .send(vec![(*a_mul).into(), carry.clone()])
                .eval(builder, is_valid.clone());
        }

        // We can now constrain that a is correct using carry[NUM_LIMBS - 1]. We reuse
        // the carry array to save space.
        let a = &cols.a;
