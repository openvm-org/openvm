...
impl MemoryBus {
    pub fn send_interaction<const WORD_SIZE: usize, AB: InteractionBuilder>(&self, builder: &mut AB, mem_access: MemoryAccess<WORD_SIZE, AB::Expr>, count: impl Into<AB::Expr>) {
        let fields = [
            ...
            mem_access.address,
        ].into_iter().chain(mem_access.data.into_iter());
