...
impl<AB: AirBuilder, const WIDTH: usize, F: Field> Air<AB> for Poseidon2Air<WIDTH, F>
where
    Poseidon2Air<WIDTH, F>: BaseAir<AB::F>,
{
    fn eval(&self, builder: &mut AB) {
        let main = builder.main();
        let local = main.row_slice(0);
        let local: &[AB::Expr] = (*local).borrow();
...
fn ext_lin_layer<AB: AirBuilder, const WIDTH: usize>(state: Vec<AB::Expr>) -> Vec<AB::Expr> {
    ...
    for i in (0..WIDTH).step_by(4) {
        ...
        for j in 0..4 {
            new_state[i + j] = sum + state[i + j] + AB::Expr::two() * state[i + ((j + 1) % 4)];
        }
    }
    let sums = (0..4).map(|j| {
        (0..WIDTH)
