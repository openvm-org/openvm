...
use afs_test_utils::{
    config::{
        baby_bear_poseidon2::{engine_from_perm, random_perm, BabyBearPoseidon2Config},
...
pub fn execute_program_and_generate_traces(
    program: Program<BabyBear>,
    input_stream: Vec<Vec<BabyBear>>,
) {
    let vm = VirtualMachine::new(
    ...
    vm.execute_and_generate::<BabyBearPoseidon2Config>()
        .unwrap();
}
...
pub fn end_to_end_test<EF: ExtensionField<BabyBear> + TwoAdicField>(
